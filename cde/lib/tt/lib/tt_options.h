...
/*%%  $XConsortium: tt_options.h /main/cde1_maint/4 1995/10/07 19:04:04 pascale $ 			 				 */
/*
 * @(#)tt_options.h	1.49 95/09/26
 *
 ...
 *
 * OPT_INCLUDE_XTHREADS_H - Defined if the system needs to include its Xthreads
 *			interfaces because the ones in the X11 release aren't
 *			up to date.
 *
 ...
 *			thread (or process).
 *
 * OPT_TT_GSS_API	  - If TRUE, enables GSS-API support (and changes
 *			    version number of  ttsession) for  libtt->ttsession
 *			    RPC.  (New for Solaris 2.6)
 *
 * OPT_SYSTEM_MTSAFE	  - If TRUE, the UNIX libc call system(3S) is MT-safe.
 *			    by default it is not MT save because it uses vfork,
 *			    so we provide our internal version _tt_system().
 *
 */
...
/* Enable mt-safe libtt */
#define OPT_XTHREADS			1

/* Hack to get correct Xthreads stuff, until X11 releases catch up to us */
#define OPT_INCLUDE_XTHREADS_H		1

/* Solaris 2.5 and above, supports clnt_create_timed(3N) */
#if (OS_MINOR_VERSION >= 5)
#define OPT_LOCKF_MNTTAB 
#define OPT_HAS_CLNT_CREATE_TIMED	1
#define OPT_CLNT_CREATE_TIMEOUT		30
#endif

/* Solaris 2.6 and above, supports GSS API RPC security model */
#if (OS_MINOR_VERSION >= 6)
#define OPT_TT_GSS_API
#undef OPT_SYSTEM_MTSAFE
#endif
