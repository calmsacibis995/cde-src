...
mid(int o, int l) const
{
	...
	_Tt_string result(l);
	_Tt_string current = *this;	// new

	for (int i=0;i<l;++i) {
//		result[i] = (*this)[i+o];
		result[i] = current[i+o];	// new
	}
	return result;
}
...
split(int i, _Tt_string &prefix) const
{
	if (i < 0) {
		...
		// It is critical to not assign prefix before the returned result,
		// otherwise lines of the form "str.split(i, str);" will fail by
		// overwriting "this" before result is assigned.
		
		_Tt_string tmp_prefix = mid(0,i);
		_Tt_string result = mid(i+1,len()-i-1);
		prefix = tmp_prefix;
		return result;
	}
}
...
split(const char *s, _Tt_string &prefix) const
{
	...
	// It is critical to not assign prefix before the returned result,
	// otherwise lines of the form "str.split(c, str);" will fail by
	// overwriting "this" before result is assigned.

	_Tt_string tmp_prefix = left(i);
	_Tt_string result = right(len()-i-strlen(s));
	prefix = tmp_prefix;
	return result;
}
