...
//%%  $XConsortium: mp_c_mp.C /main/4 1995/11/21 19:25:36 cde-sun $ 			 				
/*
 *
 * mp_c_mp.cc
 *
...
_Tt_c_mp() : _Tt_mp()
{
	...
	initial_session = initial_c_session =
		_default_c_session = new _Tt_c_session;
}
...
c_init()
{
	...
	if (status == TT_ERR_NOMP) {
		if (_default_c_session.is_eq(initial_c_session)) {
			_default_c_session = 0;
		}
#ifdef OPT_XTHREADS
		...
		if (!tss) {
			...
			tss = new _Tt_threadspecific(_default_c_session,
						     _default_c_procid);
...
_Tt_session_ptr & _Tt_c_mp::
default_session()
{
	return(default_c_session());
}

_Tt_c_session_ptr & _Tt_c_mp::
default_c_session()
{
	...
	if (_tt_global->multithreaded()) {
		...
		if (tss->thread_c_session().is_null()) {
			tss->set_thread_c_session(_default_c_session);
		}
		
		return tss->thread_c_session();
	}
#endif
	return(_default_c_session);
}

Tt_status _Tt_c_mp::
set_default_procid(_Tt_string id, int thread_only)
{
	_Tt_procid_ptr	p;
	...
	if (thread_only) {
		...
		if (!tss) {
			// thread-specific storage not yet initialized -- do it
			tss = new _Tt_threadspecific(_default_c_session, pp);
			xthread_set_specific(_tt_global->threadkey, tss);
		} else {
...
c_remove_procid(_Tt_procid_ptr &proc)
{
	...
	// should stay around.
	_Tt_string temp = (char *) 0;
	_default_c_procid->set_default_session(temp);
...
create_new_procid()
{
	...
	int			commit = 0;
	_Tt_string		session;
	_Tt_c_procid_ptr	cp = new _Tt_c_procid();
	...
	if (_tt_global->multithreaded()){
		...
		if (!tss) {
			// thread-specific storage not yet initialized -- do it
			tss = new _Tt_threadspecific(_default_c_session, cp);
			xthread_set_specific(_tt_global->threadkey, tss);
		} else {
			tss->set_thread_c_procid(cp);
		}
	}
#endif
...
set_multithreaded()
{
	if (_tt_global->multithreaded()) {
		_Tt_threadspecific* tss =
			new _Tt_threadspecific(_default_c_session,
					       (_Tt_c_procid *) 0);
		xthread_set_specific(_tt_global->threadkey, tss);
	}
}
