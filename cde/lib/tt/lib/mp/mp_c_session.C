...
//%%  $XConsortium: mp_c_session.C /main/cde1_maint/2 1995/10/04 13:28:48 gtsang $ 			 				
/*
...
// Returns:
//	TT_OK
//	TT_ERR_SESSION	Could not init desktop, or, connnect to specified X server.
...
startup_ttsession(_Tt_string &start_ttcmd)
{
	...
	if (pc_env_value) {
		...
		return _ttsystem((char *)start_ttcmd);
	}
...
// We used to do vfork here, but now that libtt clients could be
// multithreaded that's a bad idea as vfork is not MT safe.
