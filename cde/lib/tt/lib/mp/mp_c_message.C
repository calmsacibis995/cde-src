	...
	while (rc.next()) {
		...
		if (_session->has_id(sid)) {
			...
			continue;
                ...
                // This code is the same as in find_session, which was formerly
                // called here.  The call to find_session() was removed because
                // it cached the RPC client connections for TT_FILE observers as
                // an "optimization".  It was removed because the caching never
                // closes the connections, and they probably rapidly loose their
                // value over time.
                rs = new _Tt_c_session();
 
                if (rs->set_id(sid) != TT_OK || rs->client_session_init() != TT_OK) {
			//
