...
//%%  $XConsortium: mp_stream_socket.C /main/cde1_maint/4 1995/10/07 19:08:00 pascale $ 			 				
/*
 *
 * mp_socket.cc
 *
...
init(int init_as_source)
{
	...
	if (init_as_source) { /* 'from' end of socket */
		...
		if ( ( _tt_global->get_local_host(local_host) ) &&
			...
			_hostaddr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
		} else {
