...
//%%  $XConsortium: mp_session.C /main/3 1995/10/23 10:28:47 rswiston $ 			 				
/*
 * @(#)mp_session.C	1.91 97/01/15
 *
...
// Returns:
//	TT_OK
//	TT_ERR_SESSION	Could not init desktop, or not connect to specified X server
...
// ToolTalk session. Currently this is done by calling rpc procedure
// TT_RPC_VRFY_SESSION which
...
ping()
{
	...
	// different.  Just check the UNIX pid, We already know the
	...
	if (is_live == TT_OK) {
		if (0 == sid.len()) {
			...
			_Tt_session_ptr ts = new _Tt_session();
			ts->parsed_address(sid.right(sid.len()-2));
			if (ts->_pid != _pid) {
				is_live = TT_ERR_NOMP;
			}
		}
...
//	TT_ERR_NO_MATCH	RPC VERSMISMATCH
     ...
     int timeout, int rebind)
{
	...
	_Tt_c_session_ptr & d_session = _tt_c_mp->default_c_session();
	if (status == TT_ERR_NOMP &&
	    !_tt_mp->in_server() &&
	    !d_session.is_null() &&
	    has_id(d_session->address_string())) {
		d_session = NULL;
	}
...
{
	char		session_host[16];
	char		strid[255];
	int		junk_version = 1;
	const char 	*fcs1_addr_format = "%d %d %d %d %u %s";
	const char 	*addr_format = "%d %d %d %d %u %s %d";
	...
	if (addr_string.len() == 0) {
		...
		// XXX This is only done in a true client program, i.e
		// not in ttsession.  This is not a change since in
		// the server (which is a client of itself) parsed_address() 
		// is always  called  with a zero-length addr_string, so this
		// code  has never been executed in the  server, but I
		// just wanted  to spell it out as we move to more technical
		// support of security mechanisms, for any future developers
		// that might be  through  here.  Ideally there would  be
		// seperate client and server  set_address() and parse_address()
		// routines,  but that would require re-architecting
		// _Tt_session::client_session_init(), which looks like it should
		// really live in the _Tt_c_session class instead of the _Tt_session
		// class.
		if (!_tt_mp->in_server()) {
			auth->client_set_auth_level(al);
		}
...
set_id(char *id)
{
	...
	if (id != (char *)0) {
		...
		switch (_id[0]) {
		      case 'X':
			  // rfm 24 Apr 96:  I believe this is now dead code.
