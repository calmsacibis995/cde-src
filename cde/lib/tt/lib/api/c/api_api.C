...
//%%  $XConsortium: api_api.C /main/6 1995/11/28 19:18:13 cde-sun $ 			 				
...
_tt_status_message(Tt_status ttrc)
{
	...
	switch (ttrc) {
		...
		result = catgets( _ttcatd, 11, 25,
				  "TT_ERR_PROCID\tThe process id used refers to no valid ToolTalk client. The client may have crashed, exited, or closed its ToolTalk connection.");
		break;
	      case TT_ERR_PROPLEN:
		...
		result = catgets( _ttcatd, 11, 39,
				  "TT_ERR_PTYPE_START\tThe attempt to launch an application to handle a ToolTalk request failed.  This may happen if the requested application is not installed, or if it crashed or exited during startup.");
		break;
