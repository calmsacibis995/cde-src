...
//%%  $XConsortium: api_message.C /main/cde1_maint/2 1995/09/06 02:27:59 lehors CHECKEDOUT $ 			 				
...
_tt_message_destroy(Tt_message m)
{
	...
	if (   mclass == TT_OFFER && mstate != TT_RETURNED
	    ...
	    && mstate != TT_ABSTAINED && mstate != TT_CREATED)
	{
		return TT_WRN_STOPPED;
	}
	//
...
{
	_Tt_string key;
	if (typename.len()>0 && opnum!=-1) {
		_Tt_typecb_ptr tcb;
		if (!tcbt.is_null()) {
			key = _tt_typecb_key(typename, opnum);
			tcb = tcbt->lookup(key);
			if (!tcb.is_null()) {
				if (TT_CALLBACK_PROCESSED ==
...
_tt_message_session_set(Tt_message m, const char *sessid)
{
	...
	if (! _tt_c_mp->find_session(strsessid, sptr, 1)) {
