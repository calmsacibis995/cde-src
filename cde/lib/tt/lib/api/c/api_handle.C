...
//%%  $XConsortium: api_handle.C /main/cde1_maint/2 1995/09/06 02:27:43 lehors CHECKEDOUT $ 			 				
/*
 *
 * api_handle.cc
 *
...
clear(Tt_pattern p)
{
	...
	while (c.next()) {
		if ((void *)((*c).c_pointer()) == (void *)p) {
			pc = c->pptr();
			if ((!pc.is_null()) && (pc->id() == last_pat_id)) {
				last_pat_id = 0;
			}
			c.remove();
		}
...
print(FILE *f) const
{
	(void)fprintf(f,"_Tt_api_handle_table at %x <\n",(int)this);
...
print(FILE *f) const
{
	(void)fprintf(f,"_Tt_api_handle at %x\n",(int)this);
}
...
print(FILE *f) const
{
	(void)fprintf(f,"_Tt_api_userdata at %#x < Key: %d, Value %#x>\n",
		      (int)this, key, (int)userdata);
}
...
print(FILE *f) const
{
	(void)fprintf(f,"_Tt_api_callback at %#x < Callback at: %#x>\n",
		      (int)this, (int)callback);
}
