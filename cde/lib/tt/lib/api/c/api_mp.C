...
//%%  $XConsortium: api_mp.C /main/cde1_maint/1 1995/07/14 16:49:49 drk CHECKEDOUT $ 			 				
...
_tt_open()
{
	...
	// If we don't already have a default session set, create one.
	//
	id = _tt_c_mp->default_c_session()->id();
	if (id.len() <= 0) {
...
{
	_Tt_string s;
	s = _tt_c_mp->initial_session->id();
	// if tt_default_session_set has been called, it's possible
	// initial_session points to a uninitialized session now.
	// call c_init so we can get the id.
	if (s.len()==0) {
		_tt_c_mp->initial_c_session->c_init();
		s = _tt_c_mp->initial_session->id();
	}
...
// Bug 1194367: We used to create session ids like "X 192.111.222.33 0"
// to represent the ttsession associated with the X server at 192.111.222.33:0.
// We couldn't use the "normal" process tree ID "P 01 ..." since the
// session might not yet have been started.  Having two possible names for
// a session has turned out to be a continuing source of grief.  Now,
// we simply go ahead and start the ttsession for the X server if we need
// it -- in the normal OpenWindows and CDE startup scripts, the ttsession
...
_tt_X_session(const char *display_name)
{
	_Tt_string	id;
	Tt_status	status;
	_Tt_c_session_ptr tsess;
	

	_tt_internal_init();

	tsess = new _Tt_c_session;
	
	tsess->set_env(_TT_ENV_X11, display_name);
	status = tsess->c_init();  // autostarts if necessary.
	if (status != TT_OK) {
		return((char *)error_pointer(status));
	}
