...
)
{
	...
	// Help autostarted clients find the right display.
	const char *dollarDisplay = "$DISPLAY";
	const char *display = &(dollarDisplay[1]);
	_Tt_string dpyString = _ttdt_DisplayString2Name( getenv( display));
	tt_message_context_set( msg, dollarDisplay, dpyString );
	_TttkItem2Free fuse = msg;
	Tt_mode mode;
	switch (op) {
	    case TTME_DISPLAY:
