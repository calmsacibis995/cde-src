...
)
{
	...
	if (   (displayString[0] != ':')
	    && (strncmp( displayString, "unix:", 5 ) != 0)
	    && (strncmp( displayString, "local:", 6 ) != 0))
	{
		return displayString;
	}
	char *suffix = strrchr( displayString, ':' );
	_Tt_string displayName = _tt_gethostname();
	return displayName.cat( suffix );
}
...
)
{
	...
	Display *dpy = (Display *)CALLXT(XtDisplay)( widget );
	*display = _tt_strdup( _ttdt_DisplayString2Name( DisplayString( dpy)));
	if (*display == 0) {
