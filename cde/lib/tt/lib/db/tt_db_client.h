...
private:
  ...
  // This is the hostname passed in from the calling object,
  // and which is used as the key to the cache.
  _Tt_string         dbHostname;

  // If dbHostname is equivalent to localhost/127.0.0.1,
  // then this will be set  to 127.0.0.1, otherwise it will
  // be set to dbHostname.  This is what is passed to
  // client_create() for making RPC calls.  It should NOT
  // be used in caches, or for any other reason than calling
  // client_create() or similar RPC calls.
  _Tt_string         _host_or_localhost;
