...
//%%  $XConsortium: tt_db_client.C /main/cde1_maint/2 1995/09/06 02:28:49 lehors CHECKEDOUT $ 			 				
/*
...
_Tt_db_results _Tt_db_client::connectToDB (const _Tt_string &hostname)
{
	...
	if (hostname == _tt_gethostname()) {
		_host_or_localhost = "127.0.0.1";
	} else {
		_host_or_localhost = hostname;
	}
	...
	dbServer = _tt_clnt_create_timed((char *)_host_or_localhost,
					 TT_DBSERVER_PROG,
					 TT_DBSERVER_VERS,
	...
	if (0!= (h = gethostbyname((char *)_host_or_localhost))) {
		_socket = RPC_ANYSOCK;
	...
	if (!dbServer) {
		...
		// Only uncomment this for debugging.  This message can
		// be generated in a variety of cases when a hostname is
		// misspelled, or currently  (XXX) due to the  failover
		// mnttab syntax.
		//
		//_tt_syslog(0, LOG_ERR, catgets(_ttcatd, 1, 3,
		//	 "clnt_create for rpc.ttdbserverd on %s failed%s"),
		//	 (char *)_host_or_localhost, clnt_spcreateerror(""));
		...
		if ((auth_level_results) && (auth_level_results->results == TT_DB_OK)) {
			...
			switch (dbAuthLevel) {
				...
				case AUTH_DES: {
					...
					if (host2netname(server_net_name, _host_or_localhost, 0) &&
					    (hp = gethostbyname((char *)_host_or_localhost))) {
#ifdef OPT_TLI
		     ...
		     int                            &cache_level)
{
	...
	// If not in DB, create and retry...
	if (results->results == TT_DB_ERR_NO_SUCH_FILE) {

		results->results = createFile(file, properties,
					      dbAccess, cache_level);
