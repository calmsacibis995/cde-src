...
//%%  $XConsortium: mp_types_table.C /main/cde1_maint/2 1995/10/04 13:04:34 gtsang $ 			 				
/*
...
parse_path(_Tt_string path)
{
	...
	if (use_cpp) {
		...
		if (pipe(pipefd) == -1) {
			_tt_syslog(stderr, LOG_ERR, "pipe(): %m");
			return _TT_PARSE_ERROR;
		}
...
#define MSIZE	1024
		...
		switch (fork1()) {
			case 0: {// child
				...
				// what we're exec'ing
				args[i] = (char *) malloc(MSIZE);
				sprintf(args[i++], "%s", OPT_CPP_PATH);

				// Add any user supplied cpp_options
				while (cpp_options.len()) {

					cpp_options = cpp_options.split(' ', prefix);

					if ((char *)prefix) {
						args[i] = (char *) malloc(MSIZE);
						sprintf(args[i], "%s", (char *) prefix);
						i++;
					} else  {
						break;
					}
				...
				// types file  we're operating on
				args[i] = (char *) malloc(MSIZE);
				strncpy(args[i++], (char *) _file_name, MSIZE);

				// null terminate vector
				args[i] = (char *) NULL;

				// execute the command.
				(void) execv(OPT_CPP_PATH, args);
			...
			case -1: { // failure
				_tt_syslog(stderr, LOG_ERR, "fork1(): %m");
				return _TT_PARSE_ERROR;
			}
			break;
		}
		...
		yyin = fdopen(pipefd[0], "r");
		if (!yyin) {
			_tt_syslog(stderr, LOG_ERR, "fdopen(): %m");
			return _TT_PARSE_ERROR;
		}
...
parse_yyin()
{
	...
	if ((use_cpp && fclose(yyin) != 0) || (!use_cpp && fclose(yyin) != 0)) {
		return _TT_PARSE_ERROR;
	}

	close(pipefd[0]);
	// pipefd[1] was closed in the parent
	// of the fork.
