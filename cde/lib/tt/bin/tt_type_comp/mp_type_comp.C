...
//%%  $XConsortium: mp_type_comp.C /main/cde1_maint/3 1995/10/04 13:03:50 gtsang $ 			 				
/*
 *
 * mp_type_comp.cc
 *
...
_Tt_typedb 		*db_being_written = NULL;

void
sig_handler(int sig)
{
	if (NULL != db_being_written) {
		db_being_written->abort_write();
		db_being_written = NULL;
	}
	exit (3);
}
...
merge(int overwrite)
{
	...
	db_being_written = xdb.c_pointer();
...
process_args(int argc, char **argv)
{
	...
	// Bug 1215530: Try to remove the lock file if interrupted

	_tt_sigset(SIGHUP, &sig_handler);
