...
extern char		**environ;
extern int		signal_exit;	// SIGTERM received
...
// "des", or "gss". The default is "none".
char			*option_auth_level = (char *)0;
char			*types_auth_level = (char *)0;
int			auth_mask = 0;
...
//  _Tt_s_mp::_Tt_s_mp) 
_Tt_ptype_children failed_procs[MAXPIDS];

//
// Returns diagnosed child exit status, or -1 if we are not convinced
// the child exited.
//
// WARNING: THIS ROUTINE IS CALLED FROM THE SIGNAL HANDLER!!  Do not
// invoke any class constructors in it that control  global memory
// or which allocate or free memory.  Failure to heed  this  warning
// can cause ttsession to hang in certain constructors in certain
// unusual (but I've seen it happen) circumstances.
//
int
_tt_child_exit_status(pid_t child, _Tt_wait_status status)
{
	if (WIFEXITED(status)) {
		return WEXITSTATUS(status);
	} else if (WIFSIGNALED(status)) {
		syslog(LOG_ERR, catgets( _ttcatd, 3, 2,
				     "child (%d) exited due to signal %d%s" ),
				     child, WTERMSIG(status),
				     WCOREDUMP(status) ?
				     catgets( _ttcatd, 3, 3, " (core dumped)" )
				: "");
		return 1;
	} else {
		syslog(LOG_ERR, catgets( _ttcatd, 3, 4,
...
main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "o:A:pa:htvcd:sSXEN")) != -1) {
		switch (c) {
		      case 'A':
			...
			break;
		      case 'a':
			// set authorization level based on the command line args 
			// (or lack thereof)
			int result;

			option_auth_level = optarg;
			if (option_auth_level == (char *)0) {
				result = _tt_mp->initial_session->auth->parse_auth("unix");
				_tt_s_mp->unix_cred_chk_flag = 0;
			} else {
				result = _tt_mp->initial_session->auth->parse_auth(option_auth_level);
			}
			...
			// display.
			if (getenv("DISPLAY") == 0) {
				char *cp = _tt_putenv( "DISPLAY", optarg );
				free(cp);
			}
			...
			break;
		      ...
		      case 'o': {
				// Set compatability options based on command line args.
				// Currently there is only one, and that's to allow or
				// disallow tt_session_types_load().

				if (_tt_mp->initial_session->auth->parse_compat_from_cmdline(optarg) == 1) {
					print_usage_and_exit();
				}
			}
			break;
	...
	// resolve clashes between command-line option compat and
	// auth options and the (possible) contents of /etc/default/ttsession.
	// Note:  if OPT_TT_GSS_API isn't defended then _tt_mp->initial_session->auth
	// contains all auth information already.

	_Tt_rpc_auth_ptr	file;
	_Tt_rpc_auth_ptr	cmdline = _tt_mp->initial_session->auth;

	if (access(_AUTH_DEFAULT_CONFIG, F_OK|R_OK) == 0) {
		...
		// Open the file...
		if ((fp = fopen(_AUTH_DEFAULT_CONFIG, "r")) == NULL) {
			_tt_syslog(errstr, LOG_ERR,
				   catgets(_ttcatd, 3, 21,
					   "Error: %s exists but is unreadable by ttsession."),
					   _AUTH_DEFAULT_CONFIG);
			exit(1);
		}

		file = new _Tt_rpc_auth;

		// Parse lines  from the file one at a time...
		while (fgets((char *) buf1, 1024, fp) != NULL) {

			buf1[strlen(buf1)-1] = '\0'; // get rid  of '\n'

			// Get rid of tabs & spaces.  I wish _Tt_string did this easily...
			for (i=0, p1=buf1, p2=buf2; i < strlen(buf1); i++, p1++) {
				if (*p1 != ' ' && *p1 != '	') {
					*p2++ = *p1;
				}
			}
			*p2 = '\0';
			...
			if (file->parse_file_line(buf2) == 1) {
				// XXX - log a more useful diagnostic
				_tt_syslog(errstr, LOG_ERR,
					   catgets(_ttcatd, 3, 22,
					   "Error: %s exists but has syntax errors. See ttsession_file(4)."),
					   _AUTH_DEFAULT_CONFIG);
				exit(1);
			}
		}
	...
	// Now check for command line auth args that might conflict 
	// We have to compare what's passed on the command line with what's
	// in the file, and come up with a subset that obeys the  rules:
	// 1) The /etc/default/ttsession file has precedence over command
	//	line args.
	// 2) "lock=yes/no" in the file determines if command line args
	//	overturn the file args.
	// 3) Default: allow_types_load() == 0, types_load_locked() == 1,
	//	auth_level() = UNIX


	if (! file.is_null()) {

		if (file->compat_present()) {
			if (cmdline->compat_present()) {
				if (file->types_load_locked()) {

					//warn user of overrride.
					_tt_syslog(errstr, LOG_ERR,
						   catgets(_ttcatd, 3, 23,
						   "Warning: using COMPAT option in %s."),
						   _AUTH_DEFAULT_CONFIG);

					// XXX - should be able to just assign obj.
					cmdline->parse_compat_from_file(file->compat_init_string());
				}
			} else {
				// XXX - should be able to just assign obj.
				cmdline->parse_compat_from_file(file->compat_init_string());
		...
		if (file->auth_present()) {
			if (cmdline->auth_present()) {
				if (file->auth_locked()) {

					//warn user of overrride.
					_tt_syslog(errstr, LOG_ERR,
						   catgets(_ttcatd, 3, 24,
						   "Warning: using AUTH option in %s."),
						   _AUTH_DEFAULT_CONFIG);

					// XXX - should be able to just assign obj.
					cmdline->parse_auth(file->auth_init_string());
				}
			} else {
				// XXX - should be able to just assign obj.
				cmdline->parse_auth(file->auth_init_string());
	...
	if (!background_mode || (forked_pid=fork())==0) {
		...
		// if this is  truly a child process set it to be the
		// process group leader so that our parent's signals
		// won't propagate down to us.
		...
		if (waitpid(forked_pid, &ch_status, 0) == -1) {
			//
	...
	if (cmd != (char *)0 && _tt_mp->initial_session->env() == _TT_ENV_PROCESS_TREE) {
		int i;
...
void notify_start_failure()
{
	...
	while (_tt_s_mp->fout != _tt_s_mp->fin) {
		...
		while (!ptype_found && ptypes.next()) {
			if (failed_procs[_tt_s_mp->fout].child_pid==ptypes->start_pid()) {
				if (ptypes->launching()) {
					ptypes->launch_failed(&failed_procs[_tt_s_mp->fout]);
					ptype_found = 1;
				}
			}
		}
...
#if defined(OPT_TT_GSS_API)
"\nUsage: ttsession [-a unix|des|gss,args...][-d display][-o allow_unauth_types_load][-spStvhNX" ),
#else
"\nUsage: ttsession [-a unix|des,args...][-d display][-spStvhNX" ),
#endif
...
"\n"
#if defined(OPT_TT_GSS_API)
" -a unix|des|gss,...	set server authentication level\n"
#else
" -a unix|des,...	set server authentication level\n"
#endif
...
sig_handler(int sig)
{
	...
	switch (sig) {
...
#else
		// XXX - it's a bad plan to have a loop like  this in
		// a signal handler, but this  code isn't used anymore
		// in Solaris 2.x
		...
		while (((child_pid = wait3(&status, WNOHANG, 0)) == -1)
		       && errno == EINTR) {
			if (! intrs--) {
				syslog(LOG_ERR, "ttsession[%d]: wait3(): %m", (int) getpid());
				break;
			}
		}
#endif // OPT_BSD_WAIT
		...
		switch (child_pid) {
			case  -1:
				syslog(LOG_ERR, "ttsession[%d]: waitpid(): %m", (int) getpid());
			break;

			case  0:
				// XXX - this shouldn't happen if  we  get a SIGCHILD,
				// but if it does happen it's bad.
			break;

			default: {
				int status2exit = _tt_child_exit_status(child_pid, status);
...
#ifdef OPT_XTHREADS
						if(child_pid != _tt_s_mp->garbage_collector_pid)
#endif
					{
						failed_procs[_tt_s_mp->fin].child_pid = child_pid;
						failed_procs[_tt_s_mp->fin].child_wait_status = status;
						failed_procs[_tt_s_mp->fin].child_exit_status = status2exit;
						_tt_s_mp->fin++;
		...
		syslog(LOG_WARNING, "ttsession[%d]: waitpid(): %m", (int) getpid());
		break;
	      case SIGTERM:
		...
		// The child ttsession falls through to clean itself up
	      case SIGINT:
		syslog(LOG_ERR, catgets( _ttcatd, 3, 20, "exiting"));
		...
		if (forked_pid > 0 && 0 == kill(forked_pid, SIGINT)) {
		...
		// Calling free in a sig handler is a no-no.
		// Exit from  main when we return from signal handler.
		signal_exit = 1;
		break;
	      case SIGUSR1:
