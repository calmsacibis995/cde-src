...
{
  // XXX This is required because of a version glitch in the DB server.
  //     Originally the new DB server had a magic string value of "2.0.0".
  //     To prevent user confusion the magic string was changed to match
  //     the ToolTalk version the DB server was initially released with,
  //     "1.1".  Since the change was made just before the 4/93alpha3.3
  //     freeze, it was decided that we should check for a "2.0.0" and change
  //     it to "1.1" so that that pre-4/93alpha3.3 users would not complain
  //     about the version mismatch and would automatically have their
  //     DB tables updated as soon as they ran a post-4/93alpha3.3 DB server.
  //     Code related to fixing the version number should probably be stripped
  //     out during the 10/93 development cycle.
  bool_t fix_version_string = FALSE;
  ...
  if (!results && fileTable->isNew()) {
    ...
    //     Code related to fixing the version number should probably be stripped
    //     out during the 10/93 development cycle.
    if (version == _Tt_string("2.0.0")) {
      fix_version_string = TRUE;
    }
    else if (version != _Tt_string(TT_DB_VERSION)) {
      dbResults = TT_DB_ERR_DB_OPEN_FAILED;
      _tt_syslog(errstr, LOG_ERR,
		 catgets(_ttcatd, 5, 6,
			 "rpc.ttdbserverd version (%s) does not match "
			 "the version (%s) of the database tables. Please "
			 "install an rpc.ttdbserverd version %s (or greater)"),
		 TT_DB_VERSION, (char *)version, (char *)version);
    }
  }
