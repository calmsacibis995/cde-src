...
//%%  $XConsortium: db_server_svc.C /main/cde1_maint/3 1995/10/07 18:59:51 pascale $ 			 				
/*
...
#endif
{
	...
	if ((uswitch(USC_SET, uswitch_val | USW_NULLP)) <0) {
	    perror("Null pointer behavior probably not set");
	    exit(1) ;
	}
#endif
...
//		int c = getopt(argc, argv, "vm:t:dna:");
	...
	for (;;) {
		// dbserver got a SIGTERM or SIGINT; exit gracefully.
		if (signal_exit) {
			svc_exit();
			break;
		}

		readmask = svc_fdset;	// Get copy of active RPC descriptors.

		switch (sel_stat = select(FD_SETSIZE, &readmask, (fd_set *) 0, (fd_set *) 0, (struct timeval *)0)) {
			case -1:
				if (errno==EAGAIN || errno==EINTR) {
					// interrupted, try again.
				} else {
					// something is wrong
					_tt_syslog(errstr, LOG_ERR, "select(: %m)");
					break;
				}
			break;
			...
			default:
				// XXX - svc_getreqset is "unsafe in MT applications"
				// according to the man page.  If  the dbserver
				// threading is ever turned on again this needs
				// to be addressed.

				svc_getreqset(&readmask);
			break;
		}
	}
...
install_signal_handler()
{
	...
	_tt_sigset(SIGINT, &sig_handler);
	_tt_sigset(SIGTERM, &sig_handler);
