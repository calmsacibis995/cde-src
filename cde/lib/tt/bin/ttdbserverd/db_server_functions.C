...
//%%  $XConsortium: db_server_functions.C /main/cde1_maint/5 1995/10/04 13:06:35 gtsang $ 			 				
/*
					    ...
					    SVCXPRT * /* transp */)
{
  ...
  if (results.results == TT_DB_OK) {
    ...
    if (!properties.is_null()) {
      ...
      // If file not in DB - put it in and retry.
      if (results.results == TT_DB_ERR_NO_SUCH_FILE) {
	results.results = db->createFile(real_file, accessPtr);
