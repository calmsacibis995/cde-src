...
#include "tt_client_isam_record_utils.h"
...
_Tt_string get_new_db(_Tt_string old_db, int index, _Tt_string new_file)
{
  ...
  if (stat((char *)new_db_index, &stat_buf) &&
    ...
    if (new_file == TT_DB_ACCESS_TABLE_FILE) {
      ...
      if (!results) {
	_Tt_client_isam_key_descriptor_ptr access_key = new _Tt_client_isam_key_descriptor;
	...
	results = access_table->getErrorStatus();
      ...
      if (!results) {
	_Tt_client_isam_record_ptr new_record = access_table->getEmptyRecord();
	_Tt_client_isam_record_ptr record;
