...
//%%  $XConsortium: mp_rpc_server.C /main/cde1_maint/2 1995/10/04 13:47:39 gtsang $ 			 				
/*
 *
...
// a SIGTERM was received by the server process
// and we want to exit main_loop so the process can  exit.
int signal_exit = 0;
 ...
 */
_Tt_rpc_server::
_Tt_rpc_server(int program, int version, int Rsocket, _Tt_rpc_auth_ptr auth)
{
	_version = version;
	_socket = Rsocket;
	_program = program;
...
init(void (*service_fn)(struct svc_req *, SVCXPRT *))
{
	...
	// If we're using GSS security then set the service  name now.
	if (auth->auth_level() == _TT_AUTH_GSS) {
		bool_t result;                                                
		result = rpc_gss_set_svc_name((char *) auth->gss_principal(),
					      (char *) auth->gss_mechanism(),
					      0,
					      _program,
					      TT_RPC_VERSION);

		if (result == FALSE) {
			_tt_syslog(0, LOG_ERR, "rpc_gss_set_svc_name(): %m");
			return(0);
		}
	}
#endif
...
run_until(int *stop, int timeout, _Tt_int_rec_list_ptr &efds)
{
	...
	do {
		// process got a SIGTERM, bounce up to main, delete the mp
		// object, and exit gracefully.
		if (signal_exit) {
			return _TT_RPCSRV_SIG;
		}
...
/* HPUX expects args to select to be (int *) instead of (fd_set *) */
			select(FD_SETSIZE,(int *)&readfds, 0, 0,
