...
u_rpc_init()
{
	...
	server_object = new _Tt_rpc_server(_rpc_program,
					   TT_RPC_VERSION,
					   msgsock,
					   _tt_mp->initial_session->auth->auth_level());
...
advertise_address()
{
	switch(env()) {
		...
		if (_desktop->set_prop(TT_CDE_XATOM_NAME, _address_string) &&
		    _desktop->set_prop(TT_XATOM_NAME, _address_string))  {
			_tt_putenv("DISPLAY", _displayname);
			return(TT_OK);
		}
		return(TT_ERR_INTERNAL);
	      case _TT_ENV_PROCESS_TREE:
		// advertise our address by exporting a special
		// envariable.  Erase mp_ptype.C autostart envariable.

		if (_tt_put_all_env_var(2, _address_string,
			TT_CDE_XATOM_NAME, TT_XATOM_NAME) != 2)
		{
			return(TT_ERR_INTERNAL);
		}
		if (_tt_put_all_env_var(2, (char *)0,
			TT_CDE_START_SID, TT_START_SID) != 2)
		{
			return(TT_ERR_INTERNAL);
		}
		return(TT_OK);
