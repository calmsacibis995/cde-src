...
//%%  $XConsortium: mp_ptype.C /main/cde1_maint/2 1995/10/04 13:46:50 gtsang $ 			 				
#include <stdlib.h>
#include <macros.h>
...
proc_started(const _Tt_s_procid_ptr &pr)
{
	...
	if (pr->add_message(_startmsg)) {
		...
		// messages. 
		launch_failed((const _Tt_ptype_children *) 0);
		return;
	}
}
...
// 
void _Tt_ptype::
launch_failed(const _Tt_ptype_children * failed_proc)
{
	_Tt_s_message_list_cursor	qmsgs;
	_Tt_string			s(1024);
        _Tt_string                      start_cmd;


        //
        // Find out what the start string for this ptype is.
        //
        (void) getprop("start", start_cmd);


	// XXX - should be added to I18N message set
	sprintf((char *)s, "Application %s could not be started, or exited, for unknown reasons.\n",
		(char *) start_cmd);
	...
	if (! _startmsg.is_null()) {
		...
		if (failed_proc) {
			...
			// Since we know somethin about why the launch failed,
			// provide it to the process  that's going  to get
			// the  TT_ERR_PTYPE_START.
			//
			if (WIFEXITED(failed_proc->child_wait_status)) {

				// XXX - should be added to message set.
				sprintf((char *)s, "Application %s exited with code %d.\n",
					 (char *) start_cmd, failed_proc->child_exit_status);
			} else if (WIFSIGNALED(failed_proc->child_wait_status)) {

				// XXX - should be added to message set.
				sprintf((char *)s, "Application %s exited due to signal %d%s.\n",
					 (char *) start_cmd, WTERMSIG(failed_proc->child_wait_status),
					 WCOREDUMP(failed_proc->child_wait_status) ? " (core dumped)"  : "");
			} else if (WIFSTOPPED(failed_proc->child_wait_status)) {
				// XXX - should be added to message set.
				sprintf((char *)s, "Application %s `stopped' due to signal %d.\n",
					 (char *) start_cmd, WSTOPSIG(failed_proc->child_wait_status));
			}

			_startmsg->set_status_string(s);
		}
