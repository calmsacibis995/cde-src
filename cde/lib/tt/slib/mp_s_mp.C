...
static int is_system_directory(const _Tt_string path);
...
_Tt_s_mp::_observe_Saved()
{
	...
	_Tt_string_list_ptr ttpath = _tt_path(1);
	if (ttpath.is_null()) {
		return TT_ERR_NOMEM;
	}
	_Tt_string_list_cursor pathC( ttpath );
	while (pathC.next()) {
		if (! is_system_directory(*pathC)) {
...
// Used to check for TTPATH directories being in "system" directories,
// i.e. /usr/dt and, on Solaris, /usr/openwin.
// Since types files in system directores are updated only by
// installation of new releases, and not by running tt_type_comp,
// there's no point in registering interest in Saved messages for
// those files.

static int
is_system_directory(const _Tt_string path)
{
	const char *sys_directories[] = {
		"/usr/dt/",
#ifdef sun
		"/usr/openwin/",
#endif		
	...
	const char **listp, *strp;

	listp = sys_directories;
	while (strp = *listp++) {
		if (0==strncmp((char *)path,
			       strp,
			       strlen(strp))) {
			return 1;
		}
