...
indoctrinate(const _Tt_msg_trace &trace)
{
	...
	if (_session->address_string() == _tt_s_mp->initial_session->address_string()) {
		...
		if (!_tt_s_mp->find_session(_session->process_tree_id(), _session, 1)){
...
hdispatch()
{
	...
	while (rc.next()) {
		init_rc = *rc;
		rc.remove();
		if (_tt_s_mp->initial_session->has_id(init_rc)) {
			continue;
		}
		if (! _tt_s_mp->find_session(init_rc,rs,1)) {
			continue;
		}
