...
// 
//  If TTPATH isn't set this function returns
//  $HOME/.tt/types.xdr for the user database, /etc/tt/types.xdr for the
//  system database, and /usr/dt/appconfig/tttypes for the network
//  database.
// 
int
_tt_map_xdr_dbpaths(_Tt_string &udb, _Tt_string &sdb, _Tt_string &ndb)
{
	_Tt_string_list_ptr path = _tt_path(1);
	if (path.is_null()) {
		return 1;
	}
...
ce2xdr()
{
	...
	status = _ttsystem("ttce2xdr -d user &");
	if (WIFEXITED(status)) {
		...
		_tt_syslog(stderr, LOG_ERR, "_ttsystem(\"ttce2xdr -d user\"): %d",
			   status );
		return TT_ERR_PTYPE_START;
	}
}
