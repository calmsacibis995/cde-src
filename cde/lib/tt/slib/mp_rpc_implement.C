...
void _tt_rpc_gss_get_info(SVCXPRT *);
...
const _Tt_rpc_stub	_tt_rpc_dispatch_table[TT_RPC_LAST] = {
	...
	_tt_rpc_gss_get_info		/* 49 - TT_RPC_GET_GSS_INFO */
#endif
};
...
#if defined(OPT_TT_GSS_API)
	// Don't allow tt_session_types_load() to be invoked
	// unless under the necessary security.
	// XXX - for the 2.5 patch unfortunately I don't have
	// time to back port this functionality w/out also
	// including GSS, which can't be done.
	if (rpc_func == TT_RPC_LOAD_TYPES &&
	    _tt_mp->initial_session->auth->allow_types_load() == 0) {
		svcerr_auth(transp, AUTH_TOOWEAK);
		return;
	}
#endif

	// These  RPC calls do not require authentication, so
	// just bypass the auth stuff below.
	if (rpc_func != NULLPROC &&
	    rpc_func != TT_RPC_ALLOC_PROCID_KEY &&
#if defined(OPT_TT_GSS_API)
	    rpc_func != TT_RPC_GET_GSS_INFO &&
#endif
	    rpc_func != TT_RPC_VRFY_SESSION) {

	    //
	    // authenticate request
	    //
	    session_auth = _tt_s_mp->initial_s_session->auth->auth_level();
		...
		case RPCSEC_GSS:
		    ...
		    result = rpc_gss_getcred(rqstp, &rcred, &ucred, NULL);
		    if  (result == FALSE) {
			    svcerr_auth(transp, AUTH_TOOWEAK);
			    return;
		    }

		    // Does the users  creds match our creds?
		    //
		    if (rcred->mechanism	!= _tt_s_mp->initial_s_session->auth->gss_mechanism() ||
			rcred->qop		!= _tt_s_mp->initial_s_session->auth->gss_qop() ||
			rcred->service	!= _tt_s_mp->initial_s_session->auth->gss_service()) {
...
#if defined(OPT_TT_GSS_API)
/* 
 * If the server is running  with "-a  gss," then provide the client with
 * information about the  mechanism, QOP and  server principal name for
 * rpc_gss_secreate().
 */
void
_tt_rpc_gss_get_info(SVCXPRT *transp)
{
	static struct _Tt_gss_info_args	args;
	XDR				xdrs;


	args.mechanism = _tt_s_mp->initial_s_session->auth->gss_mechanism();
	args.qop = _tt_s_mp->initial_s_session->auth->gss_qop();
	args.principal = _tt_s_mp->initial_s_session->auth->gss_principal();

	if (svc_sendreply(transp,(xdrproc_t)tt_xdr_gss_info,(RPC_ARG_T)&args) == 0) {
		return;
	}
}
#endif
