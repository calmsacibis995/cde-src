...
add_message(const _Tt_s_message_ptr &m)
{
	...
	if (! _flags&(1<<_TT_PROC_ACTIVE)) {
		// A ToolTalk client died before it could be signalled
		// to retrieve a message it was due.
		return(0);
	}
	...
	if (_version >= TT_ADDMSG_VERS) {
		if (signal_new_message(m) == TT_OK) {
			_tt_syslog(0, LOG_ERR, _Tt_msg_trace trace( *m, *this );
			return(1);
		} else {
...
add_pattern(const _Tt_s_pattern_ptr &p)
{
	...
	// For Push patterns, record the "time" that the pattern
	// was registered, so that delivery can be made to the most
	// recently registered pattern.	

	if (TT_HANDLE_PUSH == p->category()) {
...
init()
{
	// XXX - hack alert.  find_host() expects an ASCII string specifiing
	// an IP address in decimal-dot nototation.  Unfortunately the
	// procid class  stores it in 4 byte binary form.  It would be
	// too tedious (and dangerous, since I'm not sure if procids go
	// go out over the wire in binary form) to change this, so I'm
	// just doing the conversion here before calling find_host.  Every
	// other place that calls find host already uses the  ASCII string
	// representation.
	// Note: this code is copied from _Tt_host::init_from_hostent()

	char * cp = (char *) _proc_host_ipaddr;
	char str[40];
	_Tt_string tmpstr;

	sprintf(str, "%d.%d.%d.%d",
	       (int)((unsigned char *)cp)[0], (int)((unsigned char *)cp)[1],
	       (int)((unsigned char *)cp)[2], (int)((unsigned char *)cp)[3]);
	tmpstr = str;

	if (! _tt_global->find_host(tmpstr, _proc_host, 1)) {
...
//
//  XXX again:  somewhere along the line this function was completely
//  de-referenced.
// 
//int _Tt_s_procid::
