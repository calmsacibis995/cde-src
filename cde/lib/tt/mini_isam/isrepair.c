...
static	char *rp_readrecord_v(int, int, off_t, size_t, int);

static	char *rp_readrecord_f(int, off_t, size_t);

static void printkey();

static void cmd_error();

static int typeletter(int);

static rp_readcntlpg(int, char *);
    ...
    int		verbose;
{
  ...
  offset = ISCNTLSIZE;
  recfile_end = lseek(datfd, (off_t) 0, 2);
  recnum = 1;
  nrecords_found = 0;
