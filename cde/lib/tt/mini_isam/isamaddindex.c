...
static void _readallrecords(Fcb *, Issort *, Keydesc2 *);


static void _attach_dups_serial(Issort *, Keydesc2 *);

static Blkno _buildbtree(Fcb *, Keydesc2 *, Issort *);

static int _duplicate_exist(Issort *, int);
    ...
    Issort		*srt;
{
	...
	_isfatal_error("Too many levels in B-tree", (char *) 0);
...
checkavailfd()
{
	...
	if((isfhandle2 = _mngfcb_victim()) == NULL)
	    _isfatal_error ("_openfcb() cannot find LRU victim", (char *) 0);
