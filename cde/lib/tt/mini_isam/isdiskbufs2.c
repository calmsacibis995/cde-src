...
/*---------------------- Local data ---------------------------------------*/
static Bufhdr *_getavail(void), *_findblock();

static Bufhdr *_findblock(Fcb *, int, Blkno);

static void _commit1buffer(register Bufhdr *);

static void _disk_init(void);

static void _flush1buffer(register Bufhdr *);

static void _rollback1buffer(register Bufhdr *);

static void _makenodata(register Bufhdr *);
...
_getavail()
{
	...
	_isfatal_error("No buffer in pool available", (char *) 0);
    }
