...
static int _create_indfile(char *), _create_varfile(char *), _create_datfile(char *);

static void _remove_datfile(char *), _remove_indfile(char *), _remove_varfile(char *);

static int _open_indfile(char *, Bool);

static int _open_varfile(char *, Bool);

static int _open_datfile(char *, Bool *);
    ...
    struct errcode	*errcode;
{
    ...
    int			ind_fd = -1;
    int			var_fd = -1;
    mode_t		oldumask = umask (u_mask); /* Change umask to client's */
    ...
    Fcb		*fcb;
{
    ...
    (void)lseek(fcb->datfd, (off_t) 0, 0);
    ...
    Fcb		*fcb;
{
	...
	_isfatal_error("Cannot create .ind file", (char *) 0);
    }
