...
/* Local functions */
static void remove_from_chain2(Fcb *, Recno);	     /* used by _vlrec_wrrec() */
static void _istail_delete(Fcb *, long);
static int _istail_read(Fcb *, long, char *);
    ...
    int			reclen;
{
	 ...
	 */
	rec_position = (ulong) _vl_getpos(fcb, recnum2); /* Offset in .rec file */
	 ...
	 */
	while ((ulong) _vl_getpos(fcb, recnum2 + 1) > fcb->datsize * ISPAGESIZE) {
    ...
    int			*reclen;
{
    ...
    *reclen += _istail_read(fcb, tailoff, record + fcb->minreclen);
	...
	_isfatal_error("Corrupted file: too long variable length record", (char *) 0);
    ...
    int			reclen;
{
     ...
     * Store variable part of record (the 'tail') in .var file.
     */
    tailoff = _istail_modify(fcb,
			     tailoff,
			     record + fcb->minreclen, 
			     reclen - fcb->minreclen);
...
static int
_istail_read(fcb, offset, buffer)
    Fcb			*fcb;
    long		offset;
    char		*buffer;
{
