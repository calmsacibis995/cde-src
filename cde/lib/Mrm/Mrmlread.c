...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Mrmlread.c,v $ $Revision: 1.7 $ $Date: 94/04/19 07:49:03 $"
#endif
#endif
...
if ( result != MrmSUCCESS )
    {
    sprintf (err_msg, _MrmMsg_0042, index);
...
switch ( type )
	...
	if ( swap_needed )
	{
	    swapbytes( (*(double *)UrmRCBuffer (context_id)) );
	    swap_needed = FALSE ;
	}
	break; 
...
if ( ! UrmRCValid(context_id) )
    return Urm__UT_Error
        ("UrmGetIndexedLiteral", _MrmMsg_0043,
...
if ( hierarchy_id == NULL )
    return Urm__UT_Error
        ("Urm__HGetIndexedLiteral", _MrmMsg_0023,
        NULL, NULL, MrmBAD_HIERARCHY) ;
if ( ! MrmHierarchyValid(hierarchy_id) )
    return Urm__UT_Error
        ("Urm__HGetIndexedLiteral", _MrmMsg_0024,
        NULL, NULL, MrmBAD_HIERARCHY) ;
if ( ! UrmRCValid(context_id) )
    return Urm__UT_Error
        ("Urm__HGetIndexedLiteral", _MrmMsg_0043,
