...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Mrmcontext.c,v $ $Revision: 1.5 $ $Date: 94/04/19 07:48:00 $"
#endif
#endif
...
if ( *context_id_return == NULL )
    return Urm__UT_Error
        ("UrmGetResourceContext", _MrmMsg_0027,
...
if ( size > 0 )
    ...
    if ( (*context_id_return)->data_buffer == NULL )
        ...
        return Urm__UT_Error
            ("UrmGetResourceContext", _MrmMsg_0001,
            NULL, *context_id_return, MrmFAILURE) ;
        }
    }
...
if ( ! UrmRCValid(context_id) )
    ...
    return Urm__UT_Error
        ("UrmResizeResourceContext", _MrmMsg_0028,
        NULL, context_id, MrmBAD_CONTEXT) ;

if ( size > MrmMaxResourceSize)
    return Urm__UT_Error
        ("MrmResizeResourceContext", _MrmMsg_0029,
...
if ( context_id->alloc_func == XtMalloc )
    ...
    if ( newbuf == NULL )
        return Urm__UT_Error
            ("UrmResizeResourceContext", _MrmMsg_0001,
...
if ( ! UrmRCValid(context_id) )
    return Urm__UT_Error
        ("UrmFreeResourceContext", _MrmMsg_0028,
        NULL, context_id, MrmBAD_CONTEXT) ;
