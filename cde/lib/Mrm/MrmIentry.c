...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: MrmIentry.c,v $ $Revision: 1.5 $ $Date: 94/04/19 07:46:18 $"
#endif
#endif
...
if (! UrmRCValid (context_id))
    return Urm__UT_Error
        ("Idb__DB_GetDataEntry", _MrmMsg_0006,
...
if (datahdr->validation != IDBDataEntryValid)
    return Urm__UT_Error
        ("Idb__DB_GetDataEntry", _MrmMsg_0007,
 ...
 */
		...
		if ( file_id->byte_swapped ) SwapIDBDataEntryHdr(datahdr) ;
                if (datahdr->validation != IDBDataEntryValid)
                    return Urm__UT_Error
                        ("Idb__DB_GetDataEntry", _MrmMsg_0008,
                        NULL, context_id, MrmNOT_VALID) ;
                ofl_data = (IDBOverflowDataPtr) datahdr ;
		if ( file_id->byte_swapped ) SwapIDBOverflowData(ofl_data);
                }
    ...
    default:
        return Urm__UT_Error
            ("Idb__DB_GetDataEntry", _MrmMsg_0009,
...
if (! UrmRCValid (context_id))
    return Urm__UT_Error
        ("Idb__DB_PutDataEntry", _MrmMsg_0006,
        NULL, NULL, MrmBAD_CONTEXT) ;
