...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: MrmIrid.c,v $ $Revision: 1.5 $ $Date: 94/04/19 07:47:04 $"
#endif
#endif
...
if ( resndx >= IDBridPtrVecMax )
    return Urm__UT_Error
        ("Idb__RID_EnterItem", _MrmMsg_0013,
...
if ( recno > file_id->last_record )
return Urm__UT_Error
    ("Idb__RID_ReturnItem", _MrmMsg_0019,
    file_id, NULL, MrmOUT_OF_RANGE);
result = Idb__BM_GetRecord (file_id, recno, &bufptr);
if ( result != MrmSUCCESS ) return result;
if ( ! Idb__RID_ValidRecord(bufptr) )
return Urm__UT_Error
    ("Idb__RID_ReturnItem", _MrmMsg_0010,
    file_id, NULL, MrmBAD_RECORD);
...
if ( resndx >= IDBridPtrVecMax )
    return Urm__UT_Error
        ("Idb__RID_ReturnItem", _MrmMsg_0013,
        file_id, NULL, MrmBAD_DATA_INDEX);
if ( recptr->pointers[resndx].pointer == NULL )
    if ( signal_null )
        return Urm__UT_Error
            ("Idb__RID_ReturnItem", _MrmMsg_0014,
