...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Mrmwcrw.c,v $ $Revision: 1.9 $ $Date: 94/04/19 07:50:12 $"
#endif
#endif
    ...
    switch ( childptr->type )
        ...
        case URMrIndex:
            ...
            if ( result != MrmSUCCESS )
                sprintf (err_msg, _MrmMsg_0052, child_idx) ;
            break ;
        case URMrRID:
            result = UrmGetRIDWidget (file_id, childptr->key.id,
                child_ctx) ;
            if ( result != MrmSUCCESS )
                sprintf (err_msg, _MrmMsg_0053, childptr->key.id) ;
            break ;
        default:
            result = MrmFAILURE ;
            sprintf (err_msg, _MrmMsg_0054, childptr->type) ;
            break ;
        }
    if ( result != MrmSUCCESS )
        {
...
if ( ! UrmRCValid(context_id) )
    return Urm__UT_Error
        ("UrmCreateOrSetWidgetInstance", _MrmMsg_0043,
        NULL, NULL, MrmBAD_CONTEXT) ;
widgetrec = (RGMWidgetRecordPtr) UrmRCBuffer (context_id) ;
if ( ! UrmWRValid(widgetrec) )
    return Urm__UT_Error
        ("UrmCreateOrSetWidgetInstance", _MrmMsg_0026,
...
else return Urm__UT_Error("UrmCreateOrSetWidgetInstance", _MrmMsg_0055,
			  NULL, context_id, MrmBAD_WIDGET_REC);
}
...
if ( ! UrmRCValid(context_id) )
    return Urm__UT_Error
        ("UrmCreateWidgetInstance", _MrmMsg_0043,
        NULL, NULL, MrmBAD_CONTEXT) ;
widgetrec = (RGMWidgetRecordPtr) UrmRCBuffer (context_id) ;
if ( ! UrmWRValid(widgetrec) )
    return Urm__UT_Error
        ("UrmCreateWidgetInstance", _MrmMsg_0026,
        NULL, context_id, MrmBAD_WIDGET_REC) ;
...
/* END OSF Fix pir 2813 */
      ...
      Urm__UT_Error("UrmCreateWidgetInstance", _MrmMsg_0056,
		    NULL, NULL, MrmFAILURE) ;
    else return Urm__UT_Error("UrmCreateWidgetInstance", _MrmMsg_0057,
			      NULL, NULL, MrmFAILURE);
...
if ( ! UrmRCValid(context_id) )
    return Urm__UT_Error
        ("UrmSetWidgetInstance", _MrmMsg_0043,
        NULL, NULL, MrmBAD_CONTEXT) ;
widgetrec = (RGMWidgetRecordPtr) UrmRCBuffer (context_id) ;
if ( ! UrmWRValid(widgetrec) )
    return Urm__UT_Error
        ("UrmSetWidgetInstance", _MrmMsg_0026,
        NULL, context_id, MrmBAD_WIDGET_REC) ;
...
result = Urm__UncompressCode
    (file_id,
     ...
     &c_name) ;
if ( result != MrmSUCCESS )
    return Urm__UT_Error("UrmSetWidgetInstance", _MrmMsg_0058,
...
if (*w_return == NULL)
    return Urm__UT_Error("UrmSetWidgetInstance", _MrmMsg_0059,
...
/*
 * Fix for CR 5410 - Do not run ConvertValue on Colors because the parent's
 *                   colormap should be used when allocating the colors 
 *                   which is currently impossible in ConvertValue.  Convert
 ...
 */ 
                    if (reptype == MrmRtypeColor)
                    {
                      ...
                      cmap = parent ? parent->core.colormap : 
 ...
 */
			...
			Urm__CW_SafeCopyValue (&val, reptype, cblist, 
                ...
                default:
                    Urm__UT_Error ("Urm__CW_CreateArglist", _MrmMsg_0063,
				   NULL, NULL, MrmFAILURE) ;
                    continue ;
                }
            break ;
 ...
 */
	...
	if ( argptr->stg_or_relcode.related_code != 0)
		...
		case MrmRtypeCStringVector:
/*
 * Fix for HaL DTS 10226 - If the type is Integer Vector (used mainly in the
 *			   XmNselectionArray resource on the Text and 
 *			   TextField widgets), load in the count for the
 *			   XmNselectionArrayCount resource.
 */
...
Cardinal Urm__CW_FixupValue 
    ...
    Boolean			*swap_needed)
#else
(val, reptype, bufptr, file_id, swap_needed)
    int				val ;
    MrmType			reptype ;
    XtPointer			bufptr ;
    IDBFile			file_id ;
...
switch ( reptype )
        ...
        for ( fixndx=0 ; fixndx<vecptr->count ; fixndx++ ) 
	{
	    if ( *swap_needed ) 
	    {
		swapbytes(vecptr->item[fixndx].text_item.rep_type);
		swapbytes(vecptr->item[fixndx].text_item.offset);
	    }
 ...
 */
	        cstg = XmStringLtoRCreate (((String)*val), XmFONTLIST_DEFAULT_TAG);
		if ( cstg == NULL )
		    {
		    sprintf
			(err_msg, _MrmMsg_0064,
			 (String)(*val)) ;
 ...
 */
	...
	*val = (int) addr ;
        break ;
    case MrmRtypeIconImage:
        return Urm__UT_Error ("Urm__CW_ConvertValue", _MrmMsg_0067,
			      NULL, NULL, MrmFAILURE) ;
    case MrmRtypeXBitmapFile:
        return Urm__UT_Error ("Urm__CW_ConvertValue", _MrmMsg_0068,
			      NULL, NULL, MrmFAILURE) ;
    case MrmRtypeFont:
    case MrmRtypeFontSet:
        ...
        if ( dpyandfontstr == NULL)
            {
            return Urm__UT_Error ("Urm__CW_ConvertValue", _MrmMsg_0069,
				  NULL, NULL, MrmFAILURE) ;
            }
	switch (reptype)
	  {
	  case MrmRtypeFont:
		...
		if ( font == NULL )
		  {
		    sprintf (err_msg, _MrmMsg_0070, fontstg);
		    return Urm__UT_Error
	  ...
	  case MrmRtypeFontSet:
		...
		if (fontset == NULL)
		  {
		    sprintf(err_msg, _MrmMsg_0071, fontstg);
		    return Urm__UT_Error
		      ("Urm__CW_ConvertValue",
		       err_msg, NULL, NULL, MrmNOT_FOUND) ;
		  }
		...
		if ( result != MrmSUCCESS )
		  {
		    ...
		    if (missing_csets != NULL)
		      {
			sprintf(err_msg, _MrmMsg_0072, fontstg);
...
/* END OSF Fix CR 4859 */
	...
	*val = (int) trans ;
	...
	if ( clrec == NULL )
	  {
	    sprintf (err_msg, _MrmMsg_0075, (String)(*val)) ;
	    return Urm__UT_Error ("Urm__CW_ConvertValue",
				  err_msg, NULL, NULL, MrmNOT_FOUND) ;
	  }
	*val = (int) clrec ;
	...
	if ( xkey == NoSymbol )
	  {
	    sprintf (err_msg, _MrmMsg_0076, (String)(*val)) ;
	    return Urm__UT_Error ("Urm__CW_ConvertValue",
				  err_msg, NULL, NULL, MrmNOT_FOUND) ;
	  }
	*val = (int) xkey;
...
switch ( resptr->type )
    ...
    case URMrIndex:
        ...
        if ( result != MrmSUCCESS )
            ...
            sprintf (err_msg, _MrmMsg_0077, resptr->key.index) ;
	...
	*act_file_id = file_id ;
        if ( result != MrmSUCCESS )
            {
            UrmFreeResourceContext (context_id) ;
            sprintf (err_msg, _MrmMsg_0078, resptr->key.id) ;
            return Urm__UT_Error ("Urm__CW_ReadLiteral", err_msg,
                NULL, NULL, result) ;
            }
        break ;
    default:
        result = MrmFAILURE ;
        UrmFreeResourceContext (context_id) ;
        sprintf ( err_msg, _MrmMsg_0079, resptr->type) ;
        return Urm__UT_Error ("Urm__CW_ReadLiteral", err_msg,
            NULL, NULL, result) ;
    }
...
switch ( iconptr->ct_type )
	...
	if ( swap_needed ) Urm__SwapRGMResourceDesc(resptr);
        ...
        if ( cttype != MrmRtypeColorTable )
            {
            sprintf (err_msg, _MrmMsg_0080, cttype) ;
    ...
    default:
        sprintf (err_msg, _MrmMsg_0081, iconptr->ct_type) ;
        return Urm__UT_Error ("Urm__CW_LoadIconImage",
            err_msg, NULL, NULL, MrmNOT_VALID) ;
    }
...
for ( ndx=URMColorTableUserMin ; ndx<ctable->count ; ndx++ )
    ...
    switch ( citem->type )
		...
		(resptr, hierarchy_id, file_id,
		 ...
		 &vec_count, &act_file, &vec_size) ;
            ...
            if ( ctype != MrmRtypeColor )
                {
                sprintf (err_msg, _MrmMsg_0082, ctype) ;
                return Urm__UT_Error ("Urm__CW_LoadIconImage",
                    err_msg, NULL, NULL, MrmNOT_VALID) ;
                }        
                break ;
        ...
        default:
            sprintf ( err_msg, _MrmMsg_0083, citem->type) ;
            return Urm__UT_Error ("Urm__CW_LoadIconImage",
                err_msg, NULL, NULL, MrmNOT_VALID) ;
        }
    }
 ...
 */
    ...
    if ( result != MrmSUCCESS )
        {
        sprintf (err_msg, _MrmMsg_0084, rtn_name) ;
 ...
 */
    ...
    switch ( reptype )
        ...
        case MrmRtypeResource:
		  ...
		  if (resptr->type != URMrIndex)
		    {
		      Urm__UT_Error("Urm__CW_FixupCallback", _MrmMsg_0085,
		  ...
		  itmptr->runtime.resolved = TRUE;
                    break ;
                default:
                    return Urm__UT_Error ("Urm__CW_FixupCallback", _MrmMsg_0063,
					  NULL, NULL, MrmFAILURE) ;
                }
            break ;
        default:
    ...
    case URMrIndex:
        if ( resptr->access == URMaPublic )
            ...
            result = UrmGetIndexedWidget
                (file_id, resptr->key.index, context_id) ;
        if ( result != MrmSUCCESS )
            sprintf (err_msg, _MrmMsg_0086, resptr->key.index) ;
        break ;
    case URMrRID:
        result = UrmGetRIDWidget (file_id, resptr->key.id, context_id) ;
        if ( result != MrmSUCCESS )
            sprintf (err_msg, _MrmMsg_0087, resptr->key.id) ;
        break ;
    default:
        result = MrmFAILURE ;
        sprintf ( err_msg, _MrmMsg_0088, resptr->type) ;
    }
if ( result != MrmSUCCESS )
    {
    UrmFreeResourceContext (context_id) ;
...
if ( result != MrmSUCCESS )
    Urm__UT_Error ("Urm__CW_LoadWidgetResource", _MrmMsg_0089,
		   NULL, NULL, result) ;
UrmFreeResourceContext (context_id) ;
return result ;
