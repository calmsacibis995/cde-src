...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: MrmIindexw.c,v $ $Revision: 1.5 $ $Date: 94/04/19 07:46:56 $"
#endif
#endif
...
if ( entndx > 0 )
    ...
    if ( itemvec[prvndx].GT_record != gt_record )
        return Urm__UT_Error
            ("Idb__INX_EnterNodeIndex", _MrmMsg_0016,
...
if ( entndx < (itemcnt-1) )
    ...
    if ( itemvec[nxtndx].LT_record != gt_record )
        return Urm__UT_Error
            ("Idb__INX_EnterNodeIndex", _MrmMsg_0017,
            file_id, NULL, MrmBAD_BTREE) ;
...
if ( ! Idb__INX_ValidLeaf(gt_buffer) )
    return Urm__UT_Error
        ("Idb__INX_SplitLeafRecord", _MrmMsg_0010,
...
if ( gt_hdrptr->parent != NULL )
    ...
    if ( ! Idb__INX_ValidNode(p_buffer) )
        return Urm__UT_Error
            ("Idb__INX_SplitLeafRecord", _MrmMsg_0018,
            file_id, NULL, MrmBAD_RECORD) ;
...
if ( ! Idb__INX_ValidNode(gt_buffer) )
    return Urm__UT_Error
        ("Idb__INX_SplitNodeRecord", _MrmMsg_0010,
        file_id, NULL, MrmBAD_RECORD) ;
...
if ( p_recno != NULL )
    ...
    if ( ! Idb__INX_ValidNode(p_buffer) )
        return Urm__UT_Error
            ("Idb__INX_SplitNodeRecord", _MrmMsg_0018,
            file_id, NULL, MrmBAD_RECORD) ;
    result = Idb__INX_ConfirmNodeSpace (file_id, p_buffer) ;
    if ( result != MrmSUCCESS ) return result ;
    }
...
if ( ! Idb__INX_ValidRecord(buffer) )
    return Urm__UT_Error
        ("Idb__INX_SetParent", _MrmMsg_0010,
        file_id, NULL, MrmBAD_RECORD) ;
...
if ( ! Idb__INX_ValidNode(buffer) )
    return Urm__UT_Error
        ("Idb__INX_FixNodeChildren", _MrmMsg_0010,
        file_id, NULL, MrmBAD_RECORD) ;
