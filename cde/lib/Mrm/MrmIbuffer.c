...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: MrmIbuffer.c,v $ $Revision: 1.5 $ $Date: 94/04/19 07:46:10 $"
#endif
#endif
...
if ( idb__buffer_pool_vec == NULL )
     return Urm__UT_Error
         ("Idb__BM_InitBufferVector", _MrmMsg_0000,
...
if ( (*buffer_return)->IDB_record == NULL )
    ...
    if ( (*buffer_return)->IDB_record == NULL )
        return Urm__UT_Error ("Idb__BM_GetBuffer",
            _MrmMsg_0001, NULL, NULL, MrmFAILURE) ;
...
if ( ! Idb__BM_Valid(buffer) )
    return Urm__UT_Error
        ("Idb__BM_MarkActivity", _MrmMsg_0002,
...
if ( ! Idb__BM_Valid(buffer) )
    return Urm__UT_Error
        ("Idb__BM_MarkModified", _MrmMsg_0002,
        NULL, NULL, MrmNOT_VALID) ;
buffer->activity = idb__buffer_activity_count++ ;
...
if ( result != MrmSUCCESS )
     return Urm__UT_Error
         ("Idb__BM_GetRecord", _MrmMsg_0003,
         file_id, NULL, result) ;
file_id->get_count++ ;
...
if ( (*buffer_return)->IDB_record->header.validation != IDBRecordHeaderValid ) {
    ...
    return Urm__UT_Error("Idb__BM_GetRecord", _MrmMsg_0005,
