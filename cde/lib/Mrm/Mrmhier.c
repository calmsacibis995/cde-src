...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Mrmhier.c,v $ $Revision: 1.8 $ $Date: 94/04/19 07:48:24 $"
#endif
#endif
...
if (display == NULL)
    {
    return Urm__UT_Error ("Urm__OpenHierarchy", _MrmMsg_0030,
...
if ( hierarchy_id == NULL )
    return Urm__UT_Error
        ("Urm__CloseHierarchy", _MrmMsg_0023,
        NULL, NULL, MrmBAD_HIERARCHY) ;
if ( ! MrmHierarchyValid(hierarchy_id) )
    return Urm__UT_Error
        ("Urm__CloseHierarchy", _MrmMsg_0024,
        NULL, NULL, MrmBAD_HIERARCHY) ;
...
if ( hierarchy_id == NULL )
    return Urm__UT_Error
        ("UrmHGetIndexedResource", _MrmMsg_0023,
        NULL, NULL, MrmBAD_HIERARCHY) ;
if ( ! MrmHierarchyValid(hierarchy_id) )
    return Urm__UT_Error
        ("UrmHGetIndexedResource", _MrmMsg_0024,
...
if (resolvedname == 0)
    {
    sprintf (err_stg, _MrmMsg_0031, name) ;
...
switch ( result )
    ...
    case MrmSUCCESS:
      break;
    case MrmNOT_VALID:
      sprintf (err_stg, _MrmMsg_0032, resolvedname) ;
      break;
    case MrmNOT_FOUND:
    default:
      sprintf (err_stg, _MrmMsg_0031, resolvedname) ;
      break;
    }
