...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: MrmIheader.c,v $ $Revision: 1.7 $ $Date: 94/04/19 07:46:34 $"
#endif
#endif
...
if ( ! Idb__HDR_ValidRecord(hdrbuf) )
    return Urm__UT_Error
        ("Idb__HDR_GetHeader", _MrmMsg_0010, file_id, NULL, MrmBAD_RECORD);
...
if ( strcmp(hdrptr->db_version,idb__database_version) > 0 )
    {
    sprintf (errmsg, _MrmMsg_0011, hdrptr->db_version, idb__database_version);
    return Urm__UT_Error
...
if ( ! Idb__HDR_ValidRecord(*buffer) )
    return Urm__UT_Error("Idb__HDR_PutHeader", _MrmMsg_0010, 
...
if ( recno != IDBHeaderRecordNumber )
    return Urm__UT_Error("Idb__HDR_EnterItem", _MrmMsg_0012,
			 file_id, NULL, MrmOUT_OF_RANGE);

result = Idb__BM_GetRecord (file_id, recno, &bufptr);
if ( result != MrmSUCCESS ) return result;
if ( ! Idb__HDR_ValidRecord(bufptr) )
    return Urm__UT_Error("Idb__HDR_EnterItem", _MrmMsg_0010,
			 file_id, NULL, MrmBAD_RECORD);
...
if ( resndx >= IDBHeaderRIDMax )
    return Urm__UT_Error("Idb__HDR_EnterItem", _MrmMsg_0013,
...
resndx = resid.internal_id.res_index;
if ( recno != IDBHeaderRecordNumber )
    return Urm__UT_Error("Idb__HDR_ReturnItem", _MrmMsg_0012,
			 file_id, NULL, MrmOUT_OF_RANGE);

result = Idb__BM_GetRecord (file_id, recno, &bufptr);
if ( result != MrmSUCCESS ) return result;
if ( ! Idb__HDR_ValidRecord(bufptr) )
    return Urm__UT_Error("Idb__HDR_ReturnItem", _MrmMsg_0010,
			 file_id, NULL, MrmBAD_RECORD);
 ...
 */
if ( resndx >= IDBHeaderRIDMax )
    return Urm__UT_Error("Idb__HDR_ReturnItem", _MrmMsg_0013,
			 file_id, NULL, MrmBAD_DATA_INDEX);
if ( recptr->RID_pointers[resndx].pointer == 0)
    if ( signal_null )
        return Urm__UT_Error("Idb__HDR_ReturnItem", _MrmMsg_0014,
...
if ( file_id->next_RID.internal_id.map_rec != IDBHeaderRecordNumber )
    return Urm__UT_Error("Idb__HDR_NextRID", _MrmMsg_0012,
			 file_id, NULL, MrmBAD_RECORD);
if ( resndx >= IDBHeaderRIDMax )
    return MrmFAILURE;
...
if (! UrmRCValid (context_id))
    return Urm__UT_Error("Idb__HDR_GetDataEntry", _MrmMsg_0006,
...
if ( record_number != IDBHeaderRecordNumber )
    return Urm__UT_Error("Idb__HDR_GetDataEntry", _MrmMsg_0012,
...
if (datahdr->validation != IDBDataEntryValid)
    return Urm__UT_Error("Idb__HDR_GetDataEntry", _MrmMsg_0007,
...
switch ( datahdr->entry_type )
    ...
    case IDBdrOverflow:
	return Urm__UT_Error("Idb__HDR_GetDAtaEntry", _MrmMsg_0015,
	     file_id, NULL, MrmFAILURE);

    default:
        return Urm__UT_Error("Idb__HDR_GetDataEntry", _MrmMsg_0009,
 ...
 */
if (! UrmRCValid (context_id))
    return Urm__UT_Error("Idb__HDR_PutDataEntry", _MrmMsg_0006,
			 NULL, NULL, MrmBAD_CONTEXT);
...
if ( record_number != IDBHeaderRecordNumber )
    {
    Urm__UT_Error("Idb__HDR_MatchFilter", _MrmMsg_0012,
		  file_id, NULL, MrmBAD_RECORD);
    return FALSE;
    }
...
if (datahdr->validation != IDBDataEntryValid)
    {
    Urm__UT_Error("Idb__HDR_GetDataEntry", _MrmMsg_0007,
