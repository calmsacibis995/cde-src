/* $XConsortium: HelpUtil.c /main/cde1_maint/4 1995/10/18 14:08:00 lehors $ */
/************************************<+>*************************************
 ****************************************************************************
 **
 **   File:        HelpUtil.c
...
#define  PT_IN_CHILD(X, Y, CHILD) \
         (((X) >=  (CHILD)->core.x) && \
          ((X) <= (Position)((CHILD)->core.x + (CHILD)->core.width)) && \
          ((Y) >=  (CHILD)->core.y) && \
          ((Y) <= (Position)((CHILD)->core.y + (CHILD)->core.height)))
...
**
** ------------------------------------------------------------ */
Boolean trusted (char *hv_path)  /* Full path to the help volume */
{
   struct stat buf;
   Boolean     writable;
 ...
 ************************************************************************/ 
#ifdef _NO_PROTO
static void _DtMessageClose(w, client_data, event)
    Widget w ;
    XtPointer client_data ;
    XEvent *event ;
#else
 ...
 ************************************************************************/ 
#ifdef _NO_PROTO
static void ExecuteContextCB(w, client_data, event)
    Widget w ;
    XtPointer client_data ;
    XEvent *event ;
#else
...
#ifdef _NO_PROTO
static Widget 
CreateErrorDialog(parent, message)
    Widget                   parent;
    char *                   message;
...
{     
   ...
   ok_string = XmStringCreateLocalized((char *)_DTGETMESSAGE(HUSET, 2,"Close"));
...
#ifdef _NO_PROTO
static Widget
CreateExecErrorDlg(helpWidget, cmdStr, invalidAlias, pHelpStuff, condition, current_hd)
      Widget   helpWidget;
      const char *  cmdStr;
...
"  execute \"su - root\"  rather than \"su root\".\n");
   ...
   fullmsg = (char *) malloc(strlen(msg)+strlen(cmdStr)+strlen(current_hd)+2);
   if (fullmsg) 
   {
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpErrorDialog(parent, message)
    Widget                   parent;
    char *                   message;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
Boolean _DtHelpFilterExecCmdStr(helpWidget, executionPolicy, 
                                commandStr, ret_cmdStr, ret_invalidAlias,
                                ret_execPermitted, ret_queryNeeded, hv_path)
    Widget                   helpWidget;
...
#define ExecAliasCmd "DtHelpExecAlias"
   ...
   ** ------------------------------------------------------------- */
   ...
   (void) _DtHelpCeStrspn((char *)commandStr, " \t", MB_CUR_MAX, &rtn_len);
   token = (char *) commandStr + rtn_len;
   rtn_len = 0;
   (void) _DtHelpCeStrcspn(token, " \t", MB_CUR_MAX, &rtn_len);
   tokenEnd = token + rtn_len;
   rtn_len = 0;

   tokenEndChar = *tokenEnd;
   if (tokenEnd) *tokenEnd = EOS;

   if ( NULL == token || _DtHelpCeStrCaseCmp(token, ExecAliasCmd) != 0 )
   {
   ...
   *tokenEnd = tokenEndChar;       /* restore the string */
   ...
   (void) _DtHelpCeStrspn(tokenEnd, " \t", MB_CUR_MAX, &rtn_len);
   token = tokenEnd + rtn_len;
   rtn_len = 0;
   (void) _DtHelpCeStrcspn(token, " \t", MB_CUR_MAX, &rtn_len);
   tokenEnd = token + rtn_len;
   rtn_len = 0;
   ...
   if ( token )
   {
      ...
      char * reptype;
      char rsrc_name[200], rsrc_class[200];
...
void
#ifdef _NO_PROTO
_DtHelpCeWaitAndProcessEvents ( w, waitProc, clientData )
    Widget             w;
...
static Boolean 
#ifdef _NO_PROTO
WaitForBtnActivatedCB(w, clientData)
    Widget w;
    void * clientData;
#else
...
static void 
#ifdef _NO_PROTO
IdentifyActivatedBtnCB(w, clientData, callData)
    Widget w;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
int _DtHelpFilterExecCmd(helpWidget, commandStr, 
                          executionPolicy, 
                          useQueryDialog, pHelpStuff, 
                          ret_filteredCmdStr, hv_path)
    Widget        helpWidget;
    const char *  commandStr;
    unsigned char executionPolicy;
...
{     
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(HUSET, 10,"Execute Anyway"));
   /* give the right title to the Cancel button */
   labelString2 = XmStringCreateLocalized((char *)_DTGETMESSAGE(HUSET, 11,"Don't Execute"));
   n = 0;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpExecFilteredCmd(helpWidget, commandStr, helpLocationId, 
                                  pDisplayStuff, pHelpStuff)
    Widget                   helpWidget;
    char *                   commandStr;
    char *                   helpLocationId;
...
{     
   ...
   ** -------------------------------------------------------------- */
   ...
   if ( (user=getuid()) == ROOT_USER)
   {
      ...
      if (home_dir != NULL
                &&
          strlen(home_dir) >= (size_t) 1)
      {

         if ( (user_info=getpwuid(user)) != NULL
                         &&
              (strcmp(home_dir,user_info->pw_dir)) )
         {
            diff_home_dirs = True;
         }
      }
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static Widget LocateWidgetId(dpy, screen, statusRet, shellWidget, cursorIn)
    Display         *dpy;
    int              screen;
    int             *statusRet;
    Widget           shellWidget;
...

{
    ...
    if (status != GrabSuccess)
      {
        _XmWarning(shellWidget,(char *)_DTGETMESSAGE(HUSET, 3,
    ...
    if (status != GrabSuccess)
      {
        ...
        _XmWarning(shellWidget,(char *)_DTGETMESSAGE(HUSET, 4,
    ...
    while (notDone)
    {
      ...
      XtAppNextEvent(XtWidgetToApplicationContext(shellWidget), &event);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
Boolean _DtHelpResolvePathname(widget, canvas_handle, 
			io_fileName, io_volumeHandle,
                        sysVolumeSearchPath, userVolumeSearchPath) 
   Widget       widget;
   CanvasHandle canvas_handle;
   char * *	io_fileName;
   VolumeHandle * io_volumeHandle;
...
#endif /* _NO_PROTO */
{
   ...
   if (newPath != NULL)       /* We have a valid volume file so open it */
   {
       ...
       if (*io_volumeHandle != NULL)
           _CEVolumeClose(canvas_handle, *io_volumeHandle);

       /* Open the help volume file and save the handle id */
       if (_CEVolumeOpen(canvas_handle, newPath,io_volumeHandle) >= 0)
       {
       ...
       else
       {
           ...
           XtFree(newPath);
           _XmWarning(widget,(char*)UtilMessage2); 
           return (FALSE);
       }      
   }
   else                       /* We have a non-valid path */
   {
       ...
       _XmWarning(widget,(char*)UtilMessage0);        
       return (FALSE);
   }
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
Boolean _DtHelpExpandHelpVolume(w,display,help,print) 
   Widget                      w;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
char *_DtHelpParseIdString(specification) 
    char *specification;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
char *_DtHelpParseAccessFile(specification) 
    char *specification;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
int DtHelpReturnSelectedWidgetId(parent, cursor, widget)
    Widget parent;
    Cursor cursor;
    Widget *widget;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpTopicListAddToHead(locationId, topicTitle,topicType,maxNodes,
                            accessPath, pHead,pTale,totalNodes,scrollPosition)
    char *locationId;
    XmString topicTitle;
    int topicType;
    int maxNodes;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpTopicListDeleteHead(pHead,pTale,totalNodes)
    DtTopicListStruct **pHead;
    DtTopicListStruct **pTale;
    int *totalNodes;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
XtCallbackProc _DtHelpMapCB(w,client_data,call_data)
    Widget w;
    XtPointer client_data;
    XtPointer call_data;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
XtCallbackProc _DtHelpMapCenteredCB(w,client_data,call_data)
    Widget w;
    XtPointer client_data;
    XtPointer call_data;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpDisplayDefinitionBox(parent, definitionBox, path, locationId)
    Widget parent; 
    Widget **definitionBox;  
    char * path;
    char * locationId;
  ...
  if (*definitionBox == NULL)
    {
      ...
      closeString = XmStringCreateLocalized((char *)_DTGETMESSAGE(HUSET, 2,"Close"));
      n =0;
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
static void CloseDefBoxCB( w, client_data, call_data )
    Widget w ;
    XtPointer client_data ;
    XtPointer call_data ;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpDisplayFormatError(displayArea, widget, userError, systemError)
...

{
   ...
   if (systemError != NULL)  
     _XmWarning((Widget)widget, systemError);
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpCommonHelpInit(help)
    _DtHelpCommonHelpStuff * help;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpCommonHelpClean(help,destroy)
    _DtHelpCommonHelpStuff * help;
    Boolean                 destroy;
#else
...
{
   free(help->topLevelId); /* purify reported to use free to synchronize with malloc */
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpSetButtonPositions(btnList, numBtns, 
                               minFormWidth, btnMargins, minBetweenBtnSpace)
    Widget     btnList[];
    int        numBtns;
...
{ /* position buttons */
   ...
   if (minWidthWithSpace > minFormWidth) minFormWidth = minWidthWithSpace;
...
#ifdef _NO_PROTO
Boolean
_DtHelpXmFontListGetPropertyMax(fontList, atom, ret_propertyValue)
        XmFontList fontList;
        Atom atom;
