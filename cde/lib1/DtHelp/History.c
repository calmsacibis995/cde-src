/* $XConsortium: History.c /main/cde1_maint/1 1995/07/17 17:41:40 drk $ */
/************************************<+>*************************************
 ****************************************************************************
 **
 **   File:        History.c
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void CloseHistoryCB (w, client_data, call_data)
    Widget		w;		/*  widget id		*/
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpDisplayHistoryInfo(nw)
    Widget nw;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static Boolean IsTopicInHistoryList(pTopicList, topicTitle)
    DtTopicListStruct *pTopicList;
    XmString topicTitle;
#else
static Boolean IsTopicInHistoryList(
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void UpdateTopicList(pHistoryList, nw, topicType)
    DtHistoryListStruct *pHistoryList;
    Widget nw;
    int topicType;
#else
...
#endif
{
   ...
   XtSetArg(args[n], XmNitemCount, pHistoryList->totalNodes); n++;
   XtSetValues(hw->help_dialog.history.topicList, args, n);
   ...
   switch (topicType)
     {
       case DtHELP_TYPE_TOPIC:
         labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(3, 8,"Help Topics Visited:"));
         break;


       case DtHELP_TYPE_FILE:
         labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(3, 9,"Help Files Visited:"));

         break;


       case DtHELP_TYPE_MAN_PAGE:
         labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(3, 10,"Man Pages Visited:"));
         break;


      case DtHELP_TYPE_STRING:
      case DtHELP_TYPE_DYNAMIC_STRING:
        labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(3, 11,"Help Messages Visited:"));
         break;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void ProcessVolumeSelectionCB(w, client_data, call_data)
    Widget w;
    caddr_t client_data;
    caddr_t call_data;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void ProcessTopicSelectionCB(w, client_data, call_data)
    Widget w;
    caddr_t client_data;
    caddr_t call_data;
#else
...
#endif
{
   ...
   /* Adjust the current selected items */
   XmListDeselectAllItems(hw->help_dialog.history.topicList);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void CreateHistoryDialog(nw)
    Widget nw;
#else
static void CreateHistoryDialog(
    Widget nw)
#endif
{
  ...
  /*  Create the shell and form used for the dialog.  */
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(3, 2,"Help Volumes Visited:"));
   n = 0;
   XtSetArg (args[n], XmNlabelString, labelString);			n++;
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(3, 3,"Close"));
   n = 0;
   XtSetArg (args[n], XmNlabelString, labelString);			n++;
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(3, 4,"Help"));
   n = 0;
   XtSetArg (args[n], XmNlabelString, labelString);			n++;
   XtSetArg (args[n], XmNleftAttachment, XmATTACH_POSITION);		n++;
   XtSetArg (args[n], XmNrightAttachment, XmATTACH_POSITION);		n++;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpUpdateHistoryList(locationId, topicType, nw)
    char *locationId;
    int topicType;
    Widget nw;
#else
...
#endif
{
  ...
  Boolean mod=FALSE;
  static Boolean home_topic = True;
  XmFontList	fontList;
  Arg           args[5];
  int n, status;
  ...
  switch (topicType)
	...
	XtGetValues(hw->help_dialog.history.topicList, args, n);

	/*
	 * Make a copy so that the original fontList is not destroyed
	 */
	...
	status =
        _DtHelpFormatTopicTitle(hw->help_dialog.help.pDisplayArea,
                                hw->help_dialog.display.volumeHandle,
                                locationId, &topicTitle, &fontList, &mod);

        if (status != 0 || topicTitle == NULL)
          topicTitle = XmStringCreateLocalized(locationId);

	else if (mod == True && !home_topic)
	  {
            n = 0;
	    XtSetArg(args[n], XmNfontList, fontList); n++;
	    XtSetValues(hw->help_dialog.history.topicList, args, n);
	 ...
	 */
        n = 0;
	XtSetArg(args[n], XmNfontList, &fontList); n++;
	XtGetValues(hw->help_dialog.history.volumeList, args, n);
	...
	mod = False;
        ...
        if(currentItemTitle == NULL && NULL != hw->help_dialog.display.helpVolume)
          currentItemTitle = XmStringCreateLocalized(hw->help_dialog.display.helpVolume);
	else if (mod == True)
	  {
            n = 0;
	    XtSetArg(args[n], XmNfontList, fontList); n++;
	    XtSetValues(hw->help_dialog.history.volumeList, args, n);
	  }
      ...
      case DtHELP_TYPE_FILE:
        currentItemTitle = XmStringCreateLocalized((char *)_DTGETMESSAGE(3, 5, "Help Files"));
        topicTitle       = hw->help_dialog.display.topicTitleLbl;
        break;


      case DtHELP_TYPE_MAN_PAGE:
        currentItemTitle = XmStringCreateLocalized((char *)_DTGETMESSAGE(3, 6, "Man Pages"));
        topicTitle       = hw->help_dialog.display.topicTitleLbl;
        break;
      ...
      case DtHELP_TYPE_STRING:
      case DtHELP_TYPE_DYNAMIC_STRING:
        currentItemTitle = XmStringCreateLocalized((char *)_DTGETMESSAGE(3, 7, "Help Messages"));
        topicTitle       = hw->help_dialog.display.topicTitleLbl;
        break;
...
#ifdef _NO_PROTO
static
DtHistoryListStruct *AddItemToHistoryList(pHead,title,topicType,duplicateItem)
     DtHistoryListStruct **pHead;
     XmString  title;
     int topicType;
     Boolean *duplicateItem;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static DtTopicListStruct *PullTopicListFromSelVolumeList(nw)
     Widget nw;
#else
static DtTopicListStruct *PullTopicListFromSelVolumeList(
     Widget nw)
