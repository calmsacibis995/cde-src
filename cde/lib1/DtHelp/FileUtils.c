/* $XConsortium: FileUtils.c /main/cde1_maint/2 1995/10/02 15:08:22 lehors $ */
/************************************<+>*************************************
 ****************************************************************************
 **
 **   File:	FileUtils.c
...
#ifdef _NO_PROTO
Boolean
_DtHelpFileTraceLinks (pPathName)
             char * *  pPathName;
#else
Boolean 
...
#ifdef _NO_PROTO
Boolean 
_DtHelpFileTraceToFile (pPathName, accessMode, pFoundPath)
             char * *  pPathName;
             int       accessMode;
             char * *  pFoundPath;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpFileGetSearchPaths(paths, searchHomeDir)
         char * paths[];
...
static char * pathsSet[_DtHELP_FILE_NUM_PATHS];
    ...
    {
       _DtHelpOSGetHomeDirName(tmpPath);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
char * _DtHelpFileLocate (type, filespec, suffixList, searchCurDir, accessMode)
	char *        type;
        char *        filespec;
...
#define NUM_BUGS 1
    ...
    {     /* we're looking at a cwd-relative path; ignore others */
       ...
       /* test all suffixes */
       /* provided no suffix provided already */

       has_suffix = False;
       if (strlen(pathName) > strlen(DtHelpCCDF_VOL_SUFFIX))
	 {
	   hvx  = pathName + strlen(pathName) - strlen(DtHelpCCDF_VOL_SUFFIX);
	   if (strcmp(hvx, DtHelpCCDF_VOL_SUFFIX) == 0)
	     has_suffix = True;
	 }

       if (strlen(pathName) > strlen(DtHelpSDL_VOL_SUFFIX))
	 {
	   sdlx = pathName + strlen(pathName) - strlen(DtHelpSDL_VOL_SUFFIX);

	   if (strcmp(sdlx, DtHelpSDL_VOL_SUFFIX) == 0)
	     has_suffix = True;
	 }

       /* --------------------------------------------------- */
       /* If it had a suffix, it would have found it earlier  */
       /* --------------------------------------------------- */
	      ...
	      *ptr = EOS;
                ...
                pSuffix++ )
          {
             ...
             pathName = _DtHelpCeExpandPathname (curPath, base, type, 
                            (char *) *pSuffix, lang, bugFixSubs, NUM_BUGS);
