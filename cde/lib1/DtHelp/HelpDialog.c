/* $XConsortium: HelpDialog.c /main/cde1_maint/5 1995/10/08 17:19:39 pascale $ */
/************************************<+>*************************************
 ****************************************************************************
 **
 **   File:        HelpDialog.c
...
static XtResource resources[] = {
...

    {
        ...
        XmRImmediate,  (caddr_t) DtHELP_AS_NEEDED_SCROLLBARS
    },
	...
	XmRImmediate, (caddr_t) _DtHelpDefaultLocationId
      },

     {	DtNhelpPrint,
	DtCHelpPrint, XmRString, sizeof (char*),
	XtOffset (DtHelpDialogWidget, help_dialog.print.helpPrint),
	XmRImmediate, (caddr_t) _DtHelpDefaultHelpPrint
      },
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void VariableInitialize(nw)
    DtHelpDialogWidget nw;
#else
static void VariableInitialize(
DtHelpDialogWidget nw)
#endif /* _NO_PROTO */
{
   ...
   */
  if (hw->help_dialog.display.topicTitleStr != NULL)
    hw->help_dialog.display.topicTitleLbl = XmStringCreateLocalized(hw->help_dialog.display.topicTitleStr);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void Initialize(rw, nw, args_init, num_args )
    Widget rw ;
    Widget nw ;
    ArgList args_init;
    Cardinal *num_args ;
...
#endif /* _NO_PROTO */
{
  ...
  _XmChangeNavigationType((Widget)hw, (XtIsShell( XtParent( nw))
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
static void InitialPopupCB(w,clientData,callData)
    Widget w;
    XtPointer clientData;
    XtPointer callData;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static Boolean SetValues(cw, rw, nw, args, numArgs)
    Widget cw ;
    Widget rw ;
    Widget nw ;
    ArgList args ;
...

{
  ...
  if (hw->help_dialog.display.topicTitleStr != NULL &&
	...
	{
          sprintf(hw->help_dialog.display.topicTitleStr, "%s%d", str, count++);
          newTitle = TRUE;
        }
    }
...

{
   DtHelpDialogWidget hw = (DtHelpDialogWidget) w;
     ...
     CleanUpHelpDialog((Widget) hw, DtCLEAN_FOR_DESTROY, NULL);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void MenuBarFix( geoSpec, action, layoutPtr, rowPtr )
    XmGeoMatrix geoSpec ;
    int action ;
    XmGeoRowLayout layoutPtr ;
    XmKidGeometry rowPtr ;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
XmGeoMatrix _DtHelpDialogWidgetGeoMatrixCreate( wid, instigator, desired )
    Widget wid ;
    Widget instigator ;
    XtWidgetGeometry *desired ;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
Boolean _DtHelpDialogWidgetNoGeoRequest( geoSpec )
        XmGeoMatrix geoSpec ;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
static void DisplayTopLevelCB( w, client_data, call_data )
    Widget w ;
    XtPointer client_data ;
    XtPointer call_data ;
#else
...
#endif /* _NO_PROTO */
{
   ...
   if (hw->help_dialog.help.topLevelId != NULL)
     _DtHelpUpdateDisplayArea(hw->help_dialog.help.topLevelId, hw,
                           DtHISTORY_AND_JUMP, DtHELP_TYPE_TOPIC);
   else
     _DtHelpUpdateDisplayArea(NULL, hw, DtHISTORY_AND_JUMP,
                           DtHELP_TYPE_TOPIC);
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void  _DtHelpDisplayBackCB( w, client_data, call_data )
    Widget w ;
    XtPointer client_data ;
    XtPointer call_data ;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpDuplicateWindowCB(w, clientData, callData )
    Widget w ;
    XtPointer clientData ;
    XtPointer callData ;
#else
...
#endif
{
   ...
   */
    ...
    {   /* The application did not register a hypertext callback so
         ...
         */
              _XmWarning((Widget)hw, (char*) HDMessage6);
    }
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
static void CopyTextCB( w, clientData, callData )
    Widget w ;
    XtPointer clientData ;
    XtPointer callData ;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
static void ProcessBMenuBackCB( w, clientData, event, continueToDispatch)
    Widget w ;
    XtPointer clientData ;
    XButtonEvent * event;
    Boolean *continueToDispatch;
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void  _DtHelpDisplayHistoryCB( w, client_data, call_data )
    Widget w ;
    XtPointer client_data ;
    XtPointer call_data ;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void  _DtHelpDisplayBrowserHomeCB( w, client_data, call_data )
    Widget w ;
    XtPointer client_data;
    XtPointer call_data ;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpDisplayIndexCB( w, client_data, call_data )
    Widget w ;
    XtPointer client_data ;
    XtPointer call_data ;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
static void DisplayPrintCB( w, client_data, call_data )
    Widget w ;
    XtPointer client_data ;
    XtPointer call_data ;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
Widget DtCreateHelpDialog(parent, name, al, ac)
    Widget parent;
    char *name;
    ArgList al;
    Cardinal ac;
...
#endif /* _NO_PROTO */
{
    ...
    XtSetArg (argsNew[ac], XmNallowShellResize, TRUE);
    ...
    w = XtCreateWidget( name, dtHelpDialogWidgetClass, shell, al, ac);

    XtAddCallback (w, XmNdestroyCallback, (XtCallbackProc)(_XmDestroyParentCallback),
                   NULL);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
int _DtHelpFilterExecCmdCB(clientData, cmdStr, ret_filteredCmdStr)
    void *   clientData;
    const char *   cmdStr;
...
#endif
{
     ...
     DtHelpDialogWidget    hw   = (DtHelpDialogWidget)     pDAS->clientData;
     char    *hv_path;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void BuildDisplayArea(parent, nw)
    Widget parent;
    DtHelpDialogWidget nw;
#else
static void BuildDisplayArea(
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpSetupDisplayType(nw, updateType)
     DtHelpDialogWidget nw;
     int updateType;
#else
   ...
   switch (hw->help_dialog.display.helpType)
    {
        ...
        if (validAccessPath)
          {
             ...
             _DtHelpUpdateDisplayArea(templocationId, hw, updateType,
                                   DtHELP_TYPE_TOPIC);
      ...
      case DtHELP_TYPE_MAN_PAGE:
        ...
        /* Set the string to the current help dialog */
        status = _DtHelpFormatManPage(hw->help_dialog.help.pDisplayArea->canvas,
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpUpdateDisplayArea(locationId, nw, listUpdateType, topicUpdateType)
    char *locationId;
    DtHelpDialogWidget nw;
    int listUpdateType;
    int topicUpdateType;
    ...
    if (status == 0)  /* success */
    {
	   ...
	   _DtHelpFormatVolumeTitle(hw->help_dialog.help.pDisplayArea,
           ...
           if(labelString == NULL && NULL != hw->help_dialog.display.helpVolume)
             labelString = XmStringCreateLocalized(hw->help_dialog.display.helpVolume);
       ...
       {
           _DtHelpUpdateHistoryList(locationId, topicUpdateType,(Widget)hw);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpUpdateJumpList(topicInfo, topicType, nw)
    char *topicInfo;
    int topicType;
    Widget nw;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void ProcessJumpBack(nw)
    DtHelpDialogWidget nw;
#else
static void ProcessJumpBack(
    DtHelpDialogWidget nw)
  ...
  switch (pTemp->topicType)
    {
      case DtHELP_TYPE_TOPIC:
        ...
        _DtHelpSetupDisplayType(hw, DtHISTORY_ONLY);
	...
	break;
      ...
      default:
        ...
        _XmWarning((Widget)hw, (char*) HDMessage4);
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpDialogHypertextCB (pDisplayAreaStruct, clientData, hyperData)
    DtHelpDispAreaStruct *pDisplayAreaStruct;
    XtPointer clientData;
    DtHelpHyperTextStruct *hyperData;
#else
...
#endif /* _NO_PROTO */
{
   ...
   */
   if (   (   ButtonRelease == hyperData->event->type
     ...
     hyperData->window_hint = CEWindowHint_NewWindow;
  ...
  switch (hyperData->hyper_type)
    {
      ...
      case CELinkType_CrossLink:
	     ...
	     case CEWindowHint_CurrentWindow:
      ...
      case CELinkType_TextFile:
        ...
        if (hw->help_dialog.display.hyperLinkCallback != NULL)
	     ...
	     if (CELinkType_AppDefine == hyperData->hyper_type)
	       {
		 callData.locationId =
		 ...
		 callData.helpVolume =
        ...
        else
          {
              ...
              */
              _XmWarning((Widget)hw, (char*) HDMessage8);
        ...
        default:  /* This catches bogus link types */
             ...
             */
             ...
             /* ERROR-MESSAGE */
             _XmWarning((Widget)hw, (char*) HDMessage7);
             break;
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
static void ResizeHelpDialogCB(clientData)
    XtPointer clientData;
#else
static void ResizeHelpDialogCB (
    XtPointer clientData)
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void  ProcessJumpReuse(nw, hyperData)
    Widget nw;
    DtHelpHyperTextStruct *hyperData;
#else
static void  ProcessJumpReuse(
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpTopicListFree(pHead)
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void CleanUpHelpDialog(nw,cleanUpKind,canvas)
    Widget nw;
    int cleanUpKind;
    CanvasHandle canvas;
#else
...
#endif
{
 ...
 /* use free instead of XtFree - was allocated using malloc - thats what purify says*/
  free(hw->help_dialog.print.printVolume);
  hw->help_dialog.print.printVolume = NULL;
  ...
  if (hw->help_dialog.display.volumeHandle != NULL)
    {
      _DtHelpCeCloseVolume(canvas, hw->help_dialog.display.volumeHandle);
      hw->help_dialog.display.volumeHandle = NULL;
    }
...
#endif /* _NO_PROTO */
{
  ...
  XmAddWMProtocolCallback(XtParent(widget),wm_delete_window,
                          (XtCallbackProc)CloseHelpCB, widget);
 ...
 *********************************************************************/
#ifdef _NO_PROTO
static Widget SetupHelpDialogMenus(parent)
    Widget parent;
#else
static Widget SetupHelpDialogMenus(
    Widget parent)
...

{
  ...
  if (hw->help_dialog.menu.showDupBtn == True)
    {
      /* Duplicate button */
      labelStr = XmStringCreateLocalized((char *)_DTGETMESSAGE(2, 5,"New Window..."));
  ...
  /* Edit button */
  labelStr = XmStringCreateLocalized((char *)_DTGETMESSAGE(2,7,"Copy"));
  ...
  /* Search button */
  labelStr = XmStringCreateLocalized((char *)_DTGETMESSAGE(2, 9,"Index..."));
  ...
  /* On Reference button */
  labelStr = XmStringCreateLocalized((char *)_DTGETMESSAGE(2, 23,"Reference"));
  ...
  labelStr = XmStringCreateLocalized((char *)_DTGETMESSAGE(2, 25,"Mouse and Keyboard"));
  ...
  /* On Version */
  labelStr = XmStringCreateLocalized((char *)_DTGETMESSAGE(2, 27,"About Help"));
 ...
 **********************************************************************/
#ifdef _NO_PROTO
static Widget CreatePopupMenu(nw)
    Widget nw;
#else
static Widget CreatePopupMenu(
    Widget nw)
...

{
   ...
   popup = XmCreatePopupMenu (hw->help_dialog.help.pDisplayArea->dispWid,
                             "popup", args, n);

   /* Add our button three event handler to our newly created display area */
   XtAddEventHandler(hw->help_dialog.help.pDisplayArea->dispWid,
