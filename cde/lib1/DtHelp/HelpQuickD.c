/* $XConsortium: HelpQuickD.c /main/cde1_maint/3 1995/10/08 17:19:55 pascale $ */
/************************************<+>*************************************
 ****************************************************************************
 **
 **   File:        HelpQuickD.c
...
#endif /* _NO_PROTO */
{
  ...
  if (qw->qhelp_dialog.display.topicTitleStr != NULL)
    qw->qhelp_dialog.display.topicTitleLbl = XmStringCreateLocalized(qw->qhelp_dialog.display.topicTitleStr);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static int FilterExecCmdCB(clientData, cmdStr, ret_filteredCmdStr)
    void *   clientData;
    const char *   cmdStr;
...
#endif 
{
     ...
     DtHelpQuickDialogWidget qw   = (DtHelpQuickDialogWidget) pDAS->clientData;
     char    *hv_path;
...
#endif /* _NO_PROTO */
{
  ...
  _XmChangeNavigationType((Widget)qw, (XtIsShell( XtParent( nw))
  ...
  if (qw->qhelp_dialog.qhelp.closeLabelString != NULL)
    ...
    labelStr = XmStringCreateLocalized((char *) _DTGETMESSAGE(HQSET,1,"Close"));
  ...
  if (qw->qhelp_dialog.qhelp.moreLabelString != NULL)
    ...
    labelStr = XmStringCreateLocalized((char *) _DTGETMESSAGE(HQSET,2,"More ..."));
  ...
  if (qw->qhelp_dialog.qhelp.backLabelString != NULL)
    ...
    labelStr = XmStringCreateLocalized((char *) _DTGETMESSAGE(HQSET,3,"Backtrack"));
  ...
  if (qw->qhelp_dialog.qhelp.printLabelString != NULL)
    ...
    labelStr = XmStringCreateLocalized((char *) _DTGETMESSAGE(HQSET,4,"Print ..."));  
  ...
  if (qw->qhelp_dialog.qhelp.helpLabelString != NULL)
    ...
    labelStr = XmStringCreateLocalized((char *) _DTGETMESSAGE(HQSET,5,"Help ..."));
...

{
   DtHelpQuickDialogWidget qw = (DtHelpQuickDialogWidget) w;
     ...
     FreeQuickHelpInfo((Widget)qw, DtCLEAN_FOR_DESTROY, NULL);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
XmGeoMatrix _DtHelpQuickDialogWidgetGeoMatrixCreate( wid, instigator, desired )
    Widget wid ;
    Widget instigator ;
    XtWidgetGeometry *desired ;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
Boolean _DtHelpQuickDialogWidgetNoGeoRequest( geoSpec )
        XmGeoMatrix geoSpec ;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
Widget DtCreateHelpQuickDialog(parent, name, al, ac)
    Widget parent;
    char *name;
    ArgList al;
    Cardinal ac;
...
#endif /* _NO_PROTO */
{
    ...
    ds_name = XtCalloc( 1, (strlen( name) + XmDIALOG_SUFFIX_SIZE + 1)) ;
    strcpy(ds_name,name);
    ...
    w = XtCreateWidget( name, dtHelpQuickDialogWidgetClass, shell, al, ac);
        ...
        unsigned char child )
{
    ...
    switch (child) 
      ...
      case DtHELP_QUICK_PRINT_BUTTON:
          return (w->qhelp_dialog.qhelp.printButton);
          
        case DtHELP_QUICK_MORE_BUTTON: 
          return (w->qhelp_dialog.qhelp.moreButton);
   
        case DtHELP_QUICK_BACK_BUTTON: 
          return (w->qhelp_dialog.qhelp.backButton);
  
        case DtHELP_QUICK_CLOSE_BUTTON: 
          return (w->qhelp_dialog.qhelp.closeButton);
           
        case DtHELP_QUICK_HELP_BUTTON:
          return (w->qhelp_dialog.qhelp.helpButton);
         
        case DtHELP_QUICK_SEPARATOR:  
          return (w->qhelp_dialog.qhelp.separator);
        default: 
          _XmWarning( (Widget) w, (char*)QHDMessage1); 
          break ; 
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpQuickDialogHypertextCB (pDisplayAreaStruct, clientData, hyperData)
    DtHelpDispAreaStruct *pDisplayAreaStruct;
    XtPointer clientData;
    DtHelpHyperTextStruct *hyperData;
#else
             ...
             case CEWindowHint_NewWindow:
               ...
               if (qw->qhelp_dialog.display.hyperLinkCallback != NULL)
                 ...
                 {  /* The application did not register a hypertext callback so
                     ...
                     *  continue!
                     */
                    _XmWarning((Widget)qw, (char*) QHDMessage1); 
      ...
      case CELinkType_TextFile:
        ...
        default:  /* This catches bogus link types */
             ...
             */
             ...
             /* ERROR-MESSAGE */
             _XmWarning((Widget)qw, (char*) QHDMessage2);
             break;
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
static void ResizeQuickDialogCB(clientData)
    XtPointer clientData;
#else
static void ResizeQuickDialogCB (
    XtPointer clientData)
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void SetupDisplayType(nw, updateKind)
     Widget nw;
     int updateKind;
#else
static void SetupDisplayType(
   ...
   switch (qw->qhelp_dialog.display.helpType)
    {
      ...
      case DtHELP_TYPE_MAN_PAGE:
         status = _DtHelpFormatManPage(qw->qhelp_dialog.help.pDisplayArea->canvas,
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void  SetupTopic(nw, updateKind)
    Widget nw;
    int updateKind;
#else
static void SetupTopic(
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void FreeQuickHelpInfo(nw, cleanUpKind, canvas)
    Widget nw;
    int cleanUpKind;
    CanvasHandle canvas;
#else
...
#endif 
{
  ...
  if (qw->qhelp_dialog.display.volumeHandle != NULL)
    {
      _DtHelpCeCloseVolume(canvas, qw->qhelp_dialog.display.volumeHandle);
      qw->qhelp_dialog.display.volumeHandle = NULL;
    }
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void UpdateJumpList(topicInfo, topicType, nw)
    char *topicInfo;
    int topicType;
    Widget nw;
#else
 ...
 ****************************************************************************/
#ifdef _NO_PROTO
static void ProcessBackCB( w, clientData, callData )
    Widget w ;
    XtPointer clientData ;
    XtPointer callData ;
#else
  ...
  switch (pTemp->topicType)
	...
	break;
      ...
      default:  
        ...
        _XmWarning((Widget)qw, (char*) QHDMessage7);
...
#endif /* _NO_PROTO */
{
  ...
  XmAddWMProtocolCallback(XtParent(widget),wm_delete_window,
                          (XtCallbackProc)CloseQuickCB, widget);
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void CloseQuickCB (w, clientData, callData) 
    Widget		w;		/*  widget id		*/
...

{
  ...
  FreeQuickHelpInfo((Widget)qw, DtCLEAN_FOR_REUSE,
				qw->qhelp_dialog.help.pDisplayArea->canvas);
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void HelpButtonCB (w, clientData, callData) 
    Widget		w;		/*  widget id		*/
    XtPointer		clientData;	/*  data from applicaiton   */
    XtPointer     	callData;	/*  data from widget class  */
#else
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void PrintQuickHelpCB (w, clientData, callData) 
    Widget		w;		/*  widget id		*/
    XtPointer		clientData;	/*  data from applicaiton   */
    XtPointer     	callData;	/*  data from widget class  */
#else
