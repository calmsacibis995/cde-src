/* $XConsortium: AccessCCDF.c /main/cde1_maint/1 1995/07/17 17:17:24 drk $ */
/************************************<+>*************************************
 ****************************************************************************
 **
 **   File:        AccessCCDF.c
...
static int
#ifdef _NO_PROTO
GetTopicMap (vol, target_id, level, ret_ids)
	_DtHelpVolume	 vol;
	char		*target_id;
	int		 level;
	char		***ret_ids;
...
static char * 
#ifdef _NO_PROTO
GetResourceString (db, topic, resClass, resName)
    XrmDatabase  db;
    char	*topic;
    char	*resClass;
    char	*resName;
...
static char **
#ifdef _NO_PROTO
GetResourceStringArray (db, topic, resClass, resName)
    XrmDatabase	 db;
    char	*topic;
    char	*resClass;
    char	*resName;
    ...
    if (val != NULL)
      {
        ...
        while (nextC && *nextC != '\0')
	  {
	    nextC = _DtHelpCeGetNxtToken (nextC, &token);
...
static char * 
#ifdef _NO_PROTO
GetNextKeyword (str, delimiter, ret_token)
	char	 *str;
	char	 *delimiter;
	char	**ret_token;
#else
...
static int 
#ifdef _NO_PROTO
TopicFilename (vol, topic, retFname)
     _DtHelpVolume   vol;
     char	 *topic;
     char	**retFname;
#else
...
static int 
#ifdef _NO_PROTO
TopicFilepos (vol, topic, retFpos)
    _DtHelpVolume	 vol;
    char	*topic;
    int		*retFpos;
#else
...
static int 
#ifdef _NO_PROTO
GetResourceInt (db, topic, resClass, resName, ret_value)
    XrmDatabase  db;
    char	*topic;
    char	*resClass;
    char	*resName;
...
static int 
#ifdef _NO_PROTO
LocationIDTopic (vol, locId, retTopic)
     _DtHelpVolume   vol;
     char	 *locId;
     char	**retTopic;
#else
...
#endif /* _NO_PROTO */
{
    ...
    *retTopic = NULL;
	...
	nextTopic && *nextTopic != NULL && *retTopic == NULL; nextTopic++)
      {
	if (_DtHelpCeStrCaseCmp (locId, *nextTopic) == 0)
	...
	for (nextTopic = allTopics;
								...
								nextTopic++)
	  {
		...
		nextLocId != NULL && *nextLocId != NULL && *retTopic == NULL;
								nextLocId++)
	      {
		if (_DtHelpCeStrCaseCmp (locId, *nextLocId) == 0)
		    *retTopic = strdup(*nextTopic);
	      }
...
int
#ifdef _NO_PROTO
_DtHelpCeGetCcdfIdPath (vol, target_id, ret_ids)
    _DtHelpVolume    vol;
    char              *target_id;
    char            ***ret_ids;
#else
...
int 
#ifdef _NO_PROTO
_DtHelpCeGetCcdfTopicChildren (vol, topic_id, retTopics)
    _DtHelpVolume   vol,
    char	  *topic_id;
    char	***retTopics;
#else
	...
	while (*topicList && result == 0)
	  {
		 ...
		 */
		if (parent_id && _DtHelpCeStrCaseCmp (parent_id, topicId) == 0)
	          {
...
int 
#ifdef _NO_PROTO
_DtHelpCeGetCcdfVolIdList (vol, retTopics)
    _DtHelpVolume	vol;
     char	***retTopics;
#else
...
/*****************************************************************************
 * Function: CEBoolean _DtHelpCeFindCcdfId (_DtHelpVolume vol, char *target_id,
...
CEBoolean
#ifdef _NO_PROTO
_DtHelpCeFindCcdfId (vol, target_id, ret_name, ret_offset)
    _DtHelpVolume	vol;
    char		*target_id;
    char		**ret_name;
    int			*ret_offset;
...
int 
#ifdef _NO_PROTO
_DtHelpCeGetCcdfTopicParent (vol, topic, retParent)
     _DtHelpVolume   vol;
     char	 *topic;
     char	**retParent;
#else
...
int
#ifdef _NO_PROTO
_DtHelpCeGetCcdfKeywordList (vol)
    _DtHelpVolume	vol;
#else
...
int 
#ifdef _NO_PROTO
_DtHelpCeGetCcdfVolumeAbstract (vol, retAbs)
    _DtHelpVolume	  vol;
    char		**retAbs;
#else
...
int
#ifdef _NO_PROTO
_DtHelpCeMapCcdfTargetToId (vol, target_id, ret_id)
    _DtHelpVolume	vol;
...
int
#ifdef _NO_PROTO
_DtHelpCeGetCcdfDocStamp (vol, ret_doc, ret_time)
    _DtHelpVolume	  vol;
    char		**ret_doc;
    char		**ret_time;
#else
...
int 
#ifdef _NO_PROTO
_DtHelpCeGetCcdfTopTopic (vol, retTopic)
     _DtHelpVolume   vol;
     char	**retTopic;
#else
...
int 
#ifdef _NO_PROTO
_DtHelpCeGetCcdfVolumeTitle (vol, ret_title)
    _DtHelpVolume	  vol;
    char		**ret_title;
#else
...
char * 
#ifdef _NO_PROTO
_DtHelpCeGetResourceString (db, topic, resClass, resName)
    XrmDatabase  db;
    char	*topic;
    char	*resClass;
    char	*resName;
...
char **
#ifdef _NO_PROTO
_DtHelpCeGetResourceStringArray (db, topic, resClass, resName)
    XrmDatabase	 db;
    char	*topic;
    char	*resClass;
    char	*resName;
