/* $XConsortium: GlobSearch.c /main/cde1_maint/3 1995/10/08 17:18:58 pascale $ */
/************************************<+>*************************************
 ****************************************************************************
 **
 **   File:        GlobSearch.c
...
#if defined(USL)
#include "regexpI.h" /* for _sun_regcomp, _sun_regexec; in $TOP/lib/Xm */
/* It is called Sun's but it is Novell's now. Sun now has XPG4 compliant regular
   expressions functions in the C lib. */
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void MergeFontListIntoWidgetFonts(widget,newFontsList)
     Widget     widget;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static Boolean UpdateCurVolBtnSens(hw,selectVolBtn)
          DtHelpDialogWidget hw;
...
#endif 
{
     ...
     if ( VolumeHasIndexP (
              ...
              hw->help_dialog.help.pDisplayArea->canvas,
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static Boolean VolumeHasIndexP (srchSource,helpType,canvasHandle,baseName)
    _DtHelpGlobSrchSources srchSource;
    int          helpType;
    CanvasHandle canvasHandle;
    char * 	baseName;
...
#endif /* _NO_PROTO */
{
   ...
   if ( _DtHelpCeOpenVolume(canvasHandle,path,&vol) == 0 )
   {
      ...
      _DtHelpCeCloseVolume(canvasHandle,vol);
   }
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static Boolean GetVolumeInfoCB (pDisplayArea, volumePath, 
                           ret_title,ret_titleXmStr, ret_docId, ret_timeStamp, 
                           ret_nameKey, io_fontList,  ret_mod)
...
#endif /* _NO_PROTO */
{
   ...
   /* open volume */
   if ( _DtHelpCeOpenVolume(pDisplayArea->canvas,volumePath,&vol) != 0 )
   ...
   if (ret_title)
   {
      ...
      _DtHelpCeGetVolumeTitle(pDisplayArea->canvas,vol,&locTitle);
      if (locTitle == NULL)
   ...
   _DtHelpCeCloseVolume(pDisplayArea->canvas,vol);
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void AdjustPositionValues(fileList,adjStartAmount,adjNextAmount,adjustThisFile)
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static int HitLoadTopics (hw,file,hit)
          DtHelpDialogWidget hw;
          _DtHelpFileEntry         file;
...
#endif /* _NO_PROTO */
{
    ...
    if ( _DtHelpCeOpenVolume(hw->help_dialog.help.pDisplayArea->canvas,
                                 file->fullFilePath,&volHandle) != 0 ) 
...
#if 1   /* this is the desired code */
        /* I own title memory */
        valid = _DtHelpFormatTopicTitle(hw->help_dialog.help.pDisplayArea,
                    volHandle,origTopicIdList[i], &titleStr, &fontList, &mod);
#else   
        {  /* DBG */ /* this is the non-codeset code */
        char * title = NULL;
        int    val;
        mod = False;
        val = _DtHelpCeGetTopicTitle(hw->help_dialog.help.pDisplayArea->canvas,
                    volHandle,origTopicIdList[i],&title);
        if (NULL!=title) 
    ...
    _DtHelpCeCloseVolume(hw->help_dialog.help.pDisplayArea->canvas, volHandle);
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static int HitNameDisplay (hw,file,hit,hitPos,insertHit)
          DtHelpDialogWidget hw;
          _DtHelpFileEntry     file;
          _DtHelpGlobSrchHit * hit;
          int               hitPos;
...
#endif /* _NO_PROTO */
{
     ...
     LoadPrefixFont(hw);
     prefixString = XmStringCreateLtoR(tmpStr, s_PrefixFontListTag);
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static int HitTopicsDisplay (hw,file,hit,firstTopicListPosition)
          DtHelpDialogWidget hw;
          _DtHelpFileEntry         file;
          _DtHelpGlobSrchHit *     hit;
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static int HitTopicsUndisplay (hw,file,hit,firstTopicListPosition)
          DtHelpDialogWidget hw;
          _DtHelpFileEntry     file;
          _DtHelpGlobSrchHit * hit;
          int               firstTopicListPosition;
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static _DtHelpGlobSrchHit * HitFree (hit,freeHitItself)
          _DtHelpGlobSrchHit * hit;
...
#endif /* _NO_PROTO */
{
   ...
   free((char *) hit->topicTitles); /* its an XmString allocated by malloc */
   ...
   if (freeHitItself) XtFree((String)hit);
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void HitListFree(vol,freeHitsThemselves)
...
static void
#ifdef _NO_PROTO
HitListFreeAllVolHits (hw,freeFullIndex)
        DtHelpDialogWidget hw;
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static int HitListGetNth(volListHead,position,
                     ret_file,ret_hit,ret_locationId,ret_helpFile)
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static int HitListAddFound ( curFile, indexTitle, 
                             indexEntry, insertSorted, ret_hit )
    _DtHelpFileEntry     curFile;
    XmString             indexTitle;
    char *               indexEntry;
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static int CountSelectedVolumes (volListHead, countSearchCompletedVolumes)
    _DtHelpFileList  volListHead;
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void AddVolInfoToFile(file,
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void AddVolInfoToList(listHead,
                 initialSearchStatus, displayStatus, selectedFilesOnly)
        _DtHelpFileList listHead;
        Boolean      initialSearchStatus;
        Boolean      displayStatus;
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void SetVolStatus( listHead, 
                       searchThisVolume, showVolInList, 
                       zeroHitsOk, selectedFilesOnly)
        _DtHelpFileList listHead;
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void  VolNameDisplay(hw,file,insertVol)
          DtHelpDialogWidget hw;
          _DtHelpFileEntry     file;
          Boolean           insertVol;
#else
...
#endif /* _NO_PROTO */
{
   ...
   LoadPrefixFont(hw);
   prefixString = XmStringCreateLtoR(buf, s_PrefixFontListTag);
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static int VolHitsDisplay (hw,file)
          DtHelpDialogWidget hw;
          _DtHelpFileEntry     file;
#else
static int VolHitsDisplay (
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static int VolHitsUndisplay (hw,file)
          DtHelpDialogWidget hw;
          _DtHelpFileEntry     file;
#else
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void ScanStatusCB(count, clientData)
          int           count;
          XtPointer     clientData;
#else
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void VolListBuild(hw,searchStatus,displayStatus,selectedVolumesOnly)
          DtHelpDialogWidget hw;
...
/* sortBy */         _DtHELP_FILE_TITLE, ScanStatusCB, hw);
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void OutputVolInfo( listHead)
        _DtHelpFileList listHead;
#else
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static Boolean SearchContinuePossible(hw,srchWord)
   DtHelpDialogWidget hw;
   char *             srchWord;
#else
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static Boolean SearchOnSameCriteria(hw,srchWord)
   DtHelpDialogWidget hw;
   char *             srchWord;
#else
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void UpdateActionButtonLabel(hw,srchWord,startContAction)
   DtHelpDialogWidget hw;
   char * srchWord;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static Boolean CheckSearchWord(hw,srchWord,updateWidgetFields)
         DtHelpDialogWidget   hw;
         char *                srchWord;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void PrepSearchSourceData(hw,srchWord)
    DtHelpDialogWidget hw;
    char * srchWord;
#else
 ...
 ************************************************************************/
#ifdef _NO_PROTO
static void CloseSearchCB (w, clientData, callData) 
    Widget    w;
    XtPointer clientData;
    XtPointer callData; 
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void DeleteListContents(srch)
        _DtHelpGlobSearchStuff * srch;
#else
static void DeleteListContents(
        _DtHelpGlobSearchStuff * srch)
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void StatusLabelUpdate(hw,status,forceUpdate,intArg)
        DtHelpDialogWidget hw;
        ResultsStatus      status;
        Boolean            forceUpdate;
        int                intArg;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void ResultsListUpdate(hw,newFile)
        DtHelpDialogWidget   hw;
        _DtHelpFileEntry     newFile;
#else
static void ResultsListUpdate(
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static Boolean SearchForPattern(string,pattern,patternLen)
          char * string;
          char * pattern;
          int     patternLen;
#else
...
#endif /* _NO_PROTO */
{
#if defined(_NO_REGEX)  /* don't support regex in string searches */
       char * hit;
...
#else /* if _NO_REGEX */
#if defined(USL)  /* if use regex on Novell */
        _sun_regexp * re;
        int           ret = 0;

        /* sun_regcomp() has no case insensitive option */
        _DtHelpCeUpperCase(string);

        /* a 0 return value indicates failure */
        re = _sun_regcomp(pattern);
        if (NULL != re)
        {
             /* a 0 return value indicates failure */
             ret = _sun_regexec(re,string);
             free((char *)re);
        }
  
        return (ret != 0);                      /* RETURN True or False */
#else /* if use regex and not sun */
        regex_t    re;
        int        ret = -1;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static Boolean OpenVolForSearch(hw,fullFilePath,curVol)
           DtHelpDialogWidget  hw;
           char *               fullFilePath;
           _DtHelpGlobSrchVol * curVol;
#else
...
#endif
{
    ...
    {
       _DtHelpCeOpenVolume(hw->help_dialog.help.pDisplayArea->canvas,
                         fullFilePath,&curVol->volHandle);
       if (NULL == curVol->volHandle) return False;  /* RETURN */

       if (curVol->gotLocale == False)
       {
          curVol->stdLocale = _DtHelpCeGetVolumeLocale(curVol->volHandle);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void CloseVolAfterSearch(hw,curVol)
           DtHelpDialogWidget  hw;
           _DtHelpGlobSrchVol *    curVol;
#else
...
#endif
{
    ...
    if ( NULL != curVol->volHandle )
    {
       _DtHelpCeCloseVolume(hw->help_dialog.help.pDisplayArea->canvas,
							curVol->volHandle);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void SearchTopic( hw, canvasHandle,curFile,curVol,srchWord,
                   srchWordLen,numSearches)
   DtHelpDialogWidget hw;
   CanvasHandle      canvasHandle;
   _DtHelpFileEntry     curFile;
   _DtHelpGlobSrchVol * curVol;
...
#endif /* _NO_PROTO */
{
         ...
         i++, curVol->indexEntriesList++ )
    {
       ...
       validTopic = _DtHelpCeGetTopicTitle(
                         hw->help_dialog.help.pDisplayArea->canvas,
                         curVol->volHandle,
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void SearchIndex( hw, canvasHandle,curFile,curVol,iconvContext,
                          srchWord,srchWordLen,numSearches)
   DtHelpDialogWidget hw;
   CanvasHandle      canvasHandle;
   _DtHelpFileEntry     curFile;
   _DtHelpGlobSrchVol * curVol;
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void SearchVolume(hw,curFile,curVol,numSearches)
         DtHelpDialogWidget hw;
        _DtHelpFileEntry        curFile;
        _DtHelpGlobSrchVol *    curVol;
        int                  numSearches;
...
#endif /* _NO_PROTO */
{
  ...
  {
     SearchTopic(hw, hw->help_dialog.help.pDisplayArea->canvas,
              curFile,curVol,hw->help_dialog.srch.localeWordStr,
  ...
  */
  if ( curVol->indexSearchInProgress )
  {
      ...
      SearchIndex(hw,hw->help_dialog.help.pDisplayArea->canvas,
              curFile, curVol, hw->help_dialog.srch.iconv3Context, 
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void SearchFile(hw,file)
   DtHelpDialogWidget      hw;
   _DtHelpFileEntry        file;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static Boolean DoBackgroundSearchWP(clientData)
    XtPointer clientData;
#else
...
searchComplete:
  ...
  StopSearchCB(hw->help_dialog.srch.actionBtn,hw,&status);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void StartSearchCB(w, clientData, callData)
    Widget w;
    XtPointer clientData;
    XtPointer callData;
#else
...
#endif 
{
   ...
   UpdateSearchStartStatusCB(NULL, (Widget) hw, NULL);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void StartSelectedVolumeSearchCB(w,clientData,callData)
    Widget w;
    XtPointer clientData;
    XtPointer callData;
#else
   ...
   Boolean       noSelectedFiles = True;
    ...
    if (hw->help_dialog.srch.fullIndex == False)
    {
       ...
       {   /* word has changed since last search; do a whole new search */
           StartSearchCB(NULL,(Widget) hw, NULL);
           return;                                   /* RETURN */
       }
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void UpdateSearchStartStatusCB(w, clientData, callData)
    Widget w;
    XtPointer clientData;
    XtPointer callData;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void StopSearchCB(w, clientData, callData)
    Widget w;
    XtPointer clientData;
    XtPointer callData;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void ActionButtonCB(w, clientData, callData)
    Widget w;
    XtPointer clientData;
    XtPointer callData;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void GotoResultCB(w, clientData, callData)
    Widget w;
    XtPointer clientData;
    XtPointer callData;
#else
...

{
   ...
   if (status && topicPosList)
   {
     ...
     _DtHelpSetupDisplayType(hw, DtHISTORY_AND_JUMP);
  }
  else
  {
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void CloseHelpCB( w, clientData, callData)
     Widget          w;
     XtPointer       clientData;
     XtPointer       callData;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void  GotoLocation(hw, helpVolume, locationId)
    DtHelpDialogWidget hw;
    char * helpVolume;
    char * locationId;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void ProcessResultSelectionCB(w, clientData, callData)
    Widget w;
    XtPointer clientData;
    XtPointer callData;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void CloseVolSelDialogCB(w, clientData, callData)
    Widget w;
    XtPointer clientData;
    XtPointer callData;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void CreateVolSelDialog(hw)
            DtHelpDialogWidget hw;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void OpenSelectDialogCB(w, clientData, callData)
    Widget w;
    XtPointer clientData;
    XtPointer callData;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void UpdateSearchVolumesCB(widget, clientData, callData)
    Widget widget;
    XtPointer clientData;
    XtPointer callData;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void UpdateIndexSelectionCB(widget, clientData, callData)
    Widget widget;
    XtPointer clientData;
    XtPointer callData;
#else
...

{
   ...
   if ( False == status->set )
   {
     ...
     if (containsState) /* change focus to the search word */ 
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static Boolean CreateCurVolBtnLabel(hw,
             out_curVolAvailable,out_labelString,out_mnemonic)
     DtHelpDialogWidget hw;
...
#endif
{
   ...
   if ( hw->help_dialog.display.helpType == DtHELP_TYPE_TOPIC )
   {
      ...
      if (NULL == volTitleString)
         volTitleString = XmStringCreateLocalized(hw->help_dialog.display.helpVolume);
   } /* if helpType == TOPIC */
   else
   { /* if helpType != TOPIC */
      curVolAvail = False;
      ...
      case DtHELP_TYPE_DYNAMIC_STRING:
         volTitleString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, 32,"Help Message"));
         break;
      case DtHELP_TYPE_FILE:
         volTitleString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, 33,"Help File"));
         break;
      case DtHELP_TYPE_MAN_PAGE:
         volTitleString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, 34,"Manual Page"));
         break;
      default:
         volTitleString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, 35,"Unknown Format"));
         break;
      } /* switch on helpType */
   } /* if helpType != TOPIC */
   ...
   /* create title */
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, 3,"Current"));
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
static void CreateGlobSrchDialog(nw,searchWord)
    Widget nw;
    char * searchWord;
#else
...
#endif 
{
   ...
   XmAddWMProtocolCallback(srchShell,wm_delete_window,
                          (XtCallbackProc)CloseSearchCB, &hw->help_dialog.srch);
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET,2, "Search"));
   n = 0;
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, 5,"All Volumes"));
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, 6,"Selected"));
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, 7,"Select Volumes..."));
   ...
   /* create the frame title */
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET,8, "Show"));
   n = 0;
   XtSetArg (args[n], XmNlabelString, labelString);             n++;
   XtSetArg (args[n], XmNtraversalOn, False);                   n++;
   XtSetArg (args[n], XmNchildType, XmFRAME_TITLE_CHILD);       n++;
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, 9,"Complete Index"));
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, 10,"Entries with:"));
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, START_SEARCH_CAT,START_SEARCH_STR));
   /*mnemonic = ((char *)_DTGETMESSAGE(GSSET, 20,START_SEARCH_MNEM));*/
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, CLOSE_BTN_CAT,CLOSE_BTN_STR));
   ...
   labelString = XmStringCreateLocalized((char *)_DTGETMESSAGE(GSSET, HELP_BTN_CAT, HELP_BTN_STR));
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpGlobSrchDisplayDialog(w,searchWord,curVolume)
    Widget w;
    char * searchWord;
    char * curVolume;
#else
...
#endif 
{
   ...
   if (NULL == hw->help_dialog.srch.srchForm)
   {
      ...
      status.set = True;
      status.event = (XEvent *) 1; /* thwart == NULL test in Update...() */
      UpdateIndexSelectionCB(sourceBtn,hw,&status);
      StatusLabelUpdate(hw,FIRST_PROMPT_STATUS,False,0);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void _DtHelpGlobSrchUpdateCurVol(w)
    Widget w;
#else
...
#endif 
{
  ...
  /* a different current volume or change of topic */
  {
     ...
     {
         /* cancel the search */
         StopSearchCB(NULL,(Widget) hw, NULL);
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void  _DtHelpGlobSrchInitVars(srch)
       _DtHelpGlobSearchStuff * srch;
#else
 ...
 *****************************************************************************/
#ifdef _NO_PROTO
void  _DtHelpGlobSrchCleanAndClose(srch,destroy)
       _DtHelpGlobSearchStuff * srch;
       Boolean               destroy;
#else
