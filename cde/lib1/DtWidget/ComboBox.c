...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: ComboBox.c /main/cde1_maint/3 1995/10/18 18:08:39 drk $"
#endif
#endif
/***********************************************************
...
static Widget _DtCreateOwnScrolledList( Widget parent, char *name, ArgList args,
        Cardinal argCount );
...
#define StringDirection(w)  w->manager.string_direction
...
static _DtConst char ComboBoxTranslationTable[] = "\
...
static _DtConst char ComboBoxLabelTranslationTable[] = "\
...
static _DtConst char ComboBoxTextTranslationTable[] = "\
        <Key>osfUp:          ComboBoxActivate(label) \n\
        <Key>osfDown:        ComboBoxActivate(label) \n\
";

static _DtConst char ComboBoxButtonTranslationTable[] = "\
        <Key>osfDown:        ComboBoxActivate(label) \n\
        <Btn1Down>:	     ComboBoxActivate(label) \n\
	~s ~m ~a <Key>space: ComboBoxActivate(label) \n\
...
static _DtConst char ComboBoxListTranslationTable[] = "\
...
static XmSyntheticResource syn_resources[] = {
    ...
    {DtNarrowSize, sizeof(Dimension), DtOffset(arrow_size), 
         _DtComboBoxGetArrowSize, _XmToHorizontalPixels},
    ...
    {DtNlistMarginHeight, sizeof(Dimension), 
         ...
         _DtComboBoxGetListMarginHeight, _XmToVerticalPixels},
    {DtNlistMarginWidth, sizeof(Dimension),DtOffset(list_margin_width),
         _DtComboBoxGetListMarginWidth, _XmToHorizontalPixels},
    {DtNlistSpacing, sizeof(Dimension), DtOffset(list_spacing),
         _DtComboBoxGetListSpacing, _XmToVerticalPixels},
...
#endif /* _NO_PROTO */
{
    ...
    */    
	...
	XtSetArg(args[n], XmNstringDirection, StringDirection(new)); n++;
        /* Resolution Independent */
        if (unit_type != XmPIXELS) {
                XtSetArg(args[n], XmNunitType, XmPIXELS); n++;
        }
    ...
    List(new) = _DtCreateOwnScrolledList(Frame(new), "List", args, n);
    XtOverrideTranslations((Widget)List(new), list_trans);

    /* selected_item resource used before selected_position */
    if (SelectedItem(new) && _XmStringIsXmString(SelectedItem(new)) ) {
...
static void 
#ifdef _NO_PROTO
_ComboBoxPrevTabGroup(w, event, params, num_params)
Widget w;
XEvent *event;
char **params;
Cardinal *num_params;
...
static void 
#ifdef _NO_PROTO
_ComboBoxNextTabGroup(w, event, params, num_params)
Widget w;
XEvent *event;
char **params;
Cardinal *num_params;
...
#endif
{
    ...
    if (MarginHeight(combo) < 0) {
	XtWarning(CB_MARGIN_HEIGHT);
	MarginHeight(combo) = 2;
    }
    if (MarginWidth(combo) < 0) {
	XtWarning(CB_MARGIN_WIDTH);
	MarginWidth(combo) = 2;
    }
    if (HorizontalSpacing(combo) < 0) {
	XtWarning(CB_HORIZONTAL_SPACING);
	HorizontalSpacing(combo) = 0;
    }
    if (VerticalSpacing(combo) < 0) {
	XtWarning(CB_VERTICAL_SPACING);
	VerticalSpacing(combo) = 0;
    }
...
#endif
{
    ...
    ResizeResult=XtMakeResizeRequest((Widget)combo, arrow_width + sep_width +
			      ...
			      NULL, NULL);
    /* Dont bother displaying an error if the request fails ... it
       may be for several reasons e.g. the parent is a form */
    OldWidth(combo) = Width(combo);
    OldHeight(combo) = Height(combo);
...
#endif
{
    ...
    if (StringDirection(new) != StringDirection(current)) {
	XtSetArg(args[n], XmNstringDirection, StringDirection(new)); n++;
    }
	...
	if (Type(new) == DtDROP_DOWN_COMBO_BOX) {
	    ...
	    else if (StringDirection(new) != StringDirection(current)) {
		XtSetArg(args[0], XmNstringDirection, StringDirection(new));
		XtSetValues(Label(new), args, 1);
	    }
 ...
 */
	...
	XmString old = combo_p->selected_item;
        combo_p->selected_item = XmStringCopy(item);
	if (old)
	    XmStringFree (old);
        XmStringInitContext(&context, combo_p->selected_item);
 ...
 */
    ...
    if (( combo_p->selected_item ) && (item != combo_p->selected_item))
	XmStringFree(combo_p->selected_item);
    combo_p->selected_item = XmStringCopy(item);
    XtSetArg(args[0], XmNlabelString, combo_p->selected_item);
    XtSetValues(combo_p->label, args, 1);
}
...
#endif
{
    ...
    if ( ShellPoppedUp(((ShellWidget)Shell(combo_w))) &&
	 (event->type == ButtonPress &&
	  (window != XtWindowOfObject(List(combo_w))) &&
          !(ScrollBarVisible(h_scrollbar) && 
	    (window == XtWindowOfObject((Widget)h_scrollbar))) &&
          !(ScrollBarVisible(v_scrollbar) && 
    ...
    if (event->type == ButtonRelease) {
        if ( ScrollBarVisible(h_scrollbar) &&
             window != XtWindowOfObject((Widget)h_scrollbar) )
            XtCallActionProc((Widget)h_scrollbar, "Release", event,NULL,0);
        if ( ScrollBarVisible(v_scrollbar) &&
             window != XtWindowOfObject((Widget)v_scrollbar) )
 ...
 */
	...
	if ( need_sync_flag && 
	     !( (ListItems(combo))[i] && _XmStringIsXmString((ListItems(combo))[i]) )) {
	    SyncWithList(combo_p);
	    need_sync_flag = 0;
	}
 ...
 */
	...
	if ( need_sync_flag && 
	     !( (ListItems(combo))[i] && 
	       _XmStringIsXmString((ListItems(combo))[i]) )) {
	    SyncWithList(combo_p);
	    need_sync_flag = 0;
	}
	if (XmStringCompare(xmStr, (ListItems(combo))[i]))
...
void
#ifdef _NO_PROTO
DtComboBoxDeletePos(combow, pos)
Widget combow;
int pos;
#else
...
#endif
{
    ...
    /* if we are deleting from 0 -> calculate the real position */
...
void
#ifdef _NO_PROTO
DtComboBoxSelectItem(combow, item)
Widget combow;
XmString item;
#else
...
/* MotifBc
 */
    XtSetArg(arg[0], XmNitems, &list_items);
    XtSetArg(arg[1], XmNitemCount, &count);
    XtGetValues(((Widget)List(combo)), arg, 2 );
  
    ItemCount(combo) = count;
...
static Boolean
#ifdef _NO_PROTO
_CvtStringToType(dpy, args, num_args, from, to, data)
Display *dpy;
XrmValuePtr args;
Cardinal *num_args;
XrmValuePtr from, to;
...
static void	
#ifdef _NO_PROTO
input_ungrab ( combo, ungrab_mask )
DtComboBoxWidget combo;
int ungrab_mask;
#else
...
static Widget 
#ifdef _NO_PROTO
_DtCreateOwnScrolledList( parent, name, args, argCount )
        Widget parent ;
        char *name ;
        ArgList args ;
        Cardinal argCount ;
#else
_DtCreateOwnScrolledList(
        Widget parent,
        char *name,
        ArgList args,
        Cardinal argCount )
#endif /* _NO_PROTO */
{
    ...
    s = XtMalloc(((name) ? strlen(name) : 0) + 3); /* Name+NULL+"SW" */
    if (name) {
       strcpy(s, name);
       strcat(s, "SW");
    } else {
