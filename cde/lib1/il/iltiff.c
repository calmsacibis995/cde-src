/* $XConsortium: iltiff.c /main/cde1_maint/2 1995/10/08 21:09:37 pascale $ */
/**---------------------------------------------------------------------
***	
            ...
            *ppTag++ = pTag;                    /* point to converted data, next tag */
            ...
            if (nItems == 0) {
                ...
                switch (tagType) {
                  ...
                  case IL_TAG_ASCII:
                    if (nItems <= 4)
                        bcopy (pFileTag->data.chars, pTagData, nItems);
                    else {
...
/* compatibility problem with long and unsigned long data fields */
                ...
                if (resolutionUnit == 3)            /* centimeters */
                    value = (denom == 0) ? 0 : (double)value * 2 / 2.54 / (double)denom + 0.5;
                else                                /* not centimeters: assume inches */
                    value = (denom == 0) ? 0 : (double)value * 2 / (double)denom + 0.5;
                }
