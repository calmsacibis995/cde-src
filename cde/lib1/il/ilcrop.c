/* $XConsortium: ilcrop.c /main/cde1_maint/2 1995/10/08 17:42:35 pascale $ */
/**---------------------------------------------------------------------
***	
    ...
    if (pInfo->constantStrip && (stripHeight > 0)) {
        ...
        if ((topStrip > 0) || (bottomStrip < ((pInfo->height - 1) / stripHeight))) {
            ...
            pPriv = (ilCropCompPrivPtr)ilAddPipeElement (pipe, IL_FILTER, 
                ...
                &dstData, ilCropCompInit, IL_NPF, IL_NPF, ilCropCompExecute, 0);
            if (!pPriv)
                return FALSE;
       ...
       switch (imdes.nSamplesPerPixel) {
            case 3:
                            pPriv = (ilCropPrivptr) ilAddPipeElement(pipe, IL_FILTER, sizeof(ilCropPriv), 0, (ilSrcElementData *) NULL,
                                                                     &dstdata, ilCropInit, IL_NPF, IL_NPF, ilCrop3ByteExecute, 0);
                     break;

            case 1:  
                     if(bitonal) 
                            pPriv = (ilCropPrivptr) ilAddPipeElement(pipe, IL_FILTER, sizeof(ilCropPriv), 0, (ilSrcElementData *) NULL,
                                                                     &dstdata, ilCropInit, IL_NPF, IL_NPF, ilCropBitonalExecute, 0);
                     else 
                            pPriv = (ilCropPrivptr) ilAddPipeElement(pipe, IL_FILTER, sizeof(ilCropPriv), 0, (ilSrcElementData *) NULL,
                                                                     &dstdata, ilCropInit, IL_NPF, IL_NPF, ilCropByteExecute, 0);
       }
