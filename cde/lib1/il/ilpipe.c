/* $XConsortium: ilpipe.c /main/cde1_maint/2 1995/10/08 21:07:13 pascale $ */
/**---------------------------------------------------------------------
***	
      ...
      case IL_PIPE_EXECUTING:
        ...
        while (TRUE) {                          /* until all or nStrips done */
            ...
            while (pElement != pExecHead) {
                ilError error = (*pElement->Execute) (&pElement->exec, 
                                               pElement->pNext->exec.srcLine, &nLines);
                if (error) {
