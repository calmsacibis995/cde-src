/* $XConsortium: ildecompjpeg.c /main/cde1_maint/1 1995/07/17 18:45:18 drk $ */
/**---------------------------------------------------------------------
***	
    ...
    )
{
    ...
    if (pDes->compInfo.JPEG.reserved & IL_JPEGM_RAW) {
        ...
        pPriv = (ilJPEGPrivPtr)ilAddPipeElement(pipe, IL_FILTER, 
                ...
                ilDecompInit, ilDecompCleanup, IL_NPF, ilDecompRawExecute, 0);
        }
    else {
        ...
        pPriv = (ilJPEGPrivPtr)ilAddPipeElement(pipe, IL_FILTER, 
                sizeof(ilJPEGPrivRec), 0, (ilSrcElementData *)NULL, &dstData, 
                ilDecompInit, ilDecompCleanup, IL_NPF, ilDecompJIFExecute, 0);
        }
