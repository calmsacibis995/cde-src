/* $XConsortium: ActionUtil.c /main/cde1_maint/3 1995/09/06 02:10:33 lehors $ */
/*************************************<+>*************************************
*****************************************************************************
**
**   File:         ActionUtil.c
...
_DtIsSameHost(const char *host1, const char *host2)
{
	...
	if ( !(hostentp = gethostbyname(hostName1)) )
		return False;	/* treat them as different on failure */

	/*
	 * Copy the data from the static area pointed to by "hostentp"
	 * to the local buffer, "hostname1".
	 * Then we can make a subsequent call to gethostbyname().
	 * without loosing the results of the first call.
	 */
	strcpy(hostName1,hostentp->h_name);
	if ( (tp = DtStrchr(hostName1,'.')) != NULL )
		*tp = NULL;
	...
	if ( !(hostentp = gethostbyname(hostName2)) )
		return False;	/* treat them as different on failure */

	if ( !strcmp(hostName1,hostentp->h_name) )
		return	True;	
...
char *_DtGetDtTmpDir(void)
{
   ...
   else
   {
      ...
      strncpy(dirBuf,getenv("HOME"), MAXPATHLEN);
      strcat(dirBuf,"/");
...
_DtActGenerateTmpFile(char *dir,char *format,mode_t mode,int *fd)
{
	...
	int countTrys = 0;	/* count of the number of tmp names tried */
		...
		sprintf(nameBuf,"%x_%d",nameCount++,pid);
