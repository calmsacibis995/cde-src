/* $XConsortium: ActionDb.c /main/cde1_maint/1 1995/07/17 17:57:56 drk $ */
/*************************************<+>*************************************
 *****************************************************************************
 **
 **   File:         ActionDb.c
...
static unsigned long ActionSequenceNumber = 0;
...
Boolean 
#ifdef _NO_PROTO
_DtStringsAreEqual( string1, string2 )
        String string1 ;
        String string2 ;
#else
_DtStringsAreEqual(
...
_DtActValidateFieldValue( long bit, char *value, char *actName, char *filename)
{
	...
	case _ActDb_CWD_SET:
		/*
		 * host:/path format is not supported, so accept any path
		 */
		break;
...
{
	extern unsigned long ActionSequenceNumber;
	...
	act_rec->seq =  ++ActionSequenceNumber;
