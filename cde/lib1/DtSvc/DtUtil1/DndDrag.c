/* $XConsortium: DndDrag.c /main/cde1_maint/1 1995/07/17 18:02:50 drk $ */
 /*********************************************************************
 *
 *	File:		DndDrag.c
 *
...
DtDndDragStart(
	...
	Cardinal	argCount)
{
static  XtCallbackRec	dragDropFinishCbRec[] = { {dndDragDropFinishCallback,
						   NULL}, {NULL, NULL} };
static  XtCallbackRec	topLevelEnterCbRec[]  = { {dndTopLevelEnterCallback,
						   NULL},  {NULL, NULL} };
static  XtCallbackRec	topLevelLeaveCbRec[]  = { {dndTopLevelLeaveCallback,
						   NULL},  {NULL, NULL} };
static  XtCallbackRec	dropStartCbRec[]      = { {dndDropStartCallback,
					    	   NULL}, {NULL, NULL} };
static  XtCallbackRec	dropFinishCbRec[]     = { {dndDropFinishCallback,
						   NULL}, {NULL, NULL} };
...
#ifdef DEBUG
	printf("DtDndDragStart: drag from widget 0x%x\n", dragInitiator);
...
#undef  NUM_DRAG_ARGS
	...
	XtSetArg(args[nn], XmNdragDropFinishCallback, dragDropFinishCbRec);
	nn++;
	XtSetArg(args[nn], XmNdropFinishCallback, dropFinishCbRec);
	nn++;
	...
	if (dtDragInfo->dropOnRootCallback != NULL) {
		...
		XtSetArg(args[nn], XmNtopLevelEnterCallback,topLevelEnterCbRec);
		nn++;
		XtSetArg(args[nn], XmNtopLevelLeaveCallback,topLevelLeaveCbRec);
		nn++;
		XtSetArg(args[nn], XmNdropStartCallback, dropStartCbRec);
		nn++;
	}
