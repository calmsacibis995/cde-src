 ...
 *+SNOTICE
 *
 *	$XConsortium: Dts.c /main/cde1_maint/3 1995/10/19 11:53:14 drk $
 *
...
cleanup(char *ot, type_info_t *info)
{
	...
	else if(info->other_type)
	{
		ot = strdup(info->other_type);
	}
	else
	{
...
get_stat(type_info_t *info)
{
	...
	if (!info->file_stat)
	{
		if(stat(get_file_path(info), &buf) == -1)
		{
			if(errno == ENOENT)
			{
				if(get_lstat(info))
				{
					info->other_type = strdup(DtDTS_DT_BROKEN_LINK);
				}
			}
			info->error = errno;
			info->file_stat = 0;
...
fprintf(stderr, "Load DataCriteria\n");
		...
		if(!dc_db)
		{
			fprintf(stderr, "No DataBase loaded\n", "perror");
			return(NULL);
		}
	}
#ifdef NO_MMAP
...
type_mode(char *attr, type_info_t *info)
{
	...
	do
	{
		...
		case	'd':
			...
			{
				match = 1;
				if(!info->type_has_been_read)
				{
					/* Read suggested type only once */
					info->type_has_been_read = 1;
					info->other_type = 
					      _DtDtsGetDataType(get_file_path(info));
					if(info->other_type) return(match);
				}
			}
			else
			{
...
char	*
DtDtsDataToDataType64(const char *fp,
	const void		*buf,
	const int		bs,
	const struct stat	*fs,
	const char		*ln,
	...
	const char		*on)
{
	...
	for(i = 0; !rec_m && (rec_ptr = name_list(info, db, rec_ptr)); i++)
	{
		...
		for(j=0; fld_m && j < rec_ptr->fieldCount; j++)
		{
			...
			p_atr_m = 1;
			c_atr_m = 1;
			info->orig_attr = _DtDtsMMExpandValue((char *)
...
*/
				...
				{
					info->other_type = strdup(max_buf((size_t)0, info));
				}
...
expand_value(DtShmBoson attr, char *in_pathname)
{
	...
	if(attr && attr != -1)
	{
		tmp = _DtDtsMMExpandValue((char *)_DtDtsMMBosonToString(attr));
...
{
	DtDtsMMDatabase	*db = get_da_db();
	DtDtsMMRecord	*entry = _DtDtsMMGetRecordByName(db, (char *)obj_type);
	DtDtsMMField	*fld_ptr_list;
	DtDtsMMField	*fld_ptr;
...
DtDtsFindAttribute(const char *name, const char *value)
{
	...
	for(i = 0; i < ot->recordCount; i++)
	{
		...
		v = _DtDtsMMExpandValue((char *)_DtDtsMMGetFieldByName(rec_ptr, (char *)name));
		if(v && !strcmp(value, v))
		{
			list[j++] = strdup((char *)_DtDtsMMBosonToString(rec_ptr->recordName));
		}
		_DtDtsMMSafeFree(v);
	}
	list[j] = 0;
