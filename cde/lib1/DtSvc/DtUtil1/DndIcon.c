/* $XConsortium: DndIcon.c /main/cde1_maint/1 1995/07/17 18:03:42 drk $ */
/*********************************************************************
 *
 *	File:		DndIcon.c
 *
	...
	int		component) 
{
	...
	case DtDND_DRAG_OPERATION:
		...
		XtSetArg(args[nn], XmNattachment, XmATTACH_NORTH_WEST);	nn++;
		/* 
		 * Use offset of 0,0 instead of 1,1 so that the composited
		 * drag icon will fit in 32x32 (hardware cursor size)
		 */
		XtSetArg(args[nn], XmNoffsetX, 	  0);			nn++;
		XtSetArg(args[nn], XmNoffsetY,    0);			nn++;
		break;
	}
