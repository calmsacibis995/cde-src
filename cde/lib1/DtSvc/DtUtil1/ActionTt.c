/* $XConsortium: ActionTt.c /main/cde1_maint/2 1995/09/06 02:10:17 lehors $ */
/*************************************<+>*************************************
 *****************************************************************************
 **
 **   File:         ActionTt.c
...
Boolean _DtInitializeToolTalk(Widget w);
...
static void
#ifdef _NO_PROTO
ReportToolTalkError( w, actionName, errorMsg )
        Widget w ;
        String actionName ;
        String errorMsg ;
#else
...
static Boolean 
#ifdef _NO_PROTO
ResolveTtRequestMessagePieces(w, request, relPathHost, relPathDir)
        Widget w ;
        ActionRequest *request ;
        char * relPathHost ;
        char * relPathDir ;
                           ...
                           &(tt->tt_file), False, 0))
   {
      for (i = 0, success = True; (i < tt->value_count) && success; i++)
      {
         success = _DtCompileMessagePiece(w, request, relPathHost, relPathDir,
                           &(tt->tt_argn_value[i]), False, _DTAct_TT_ARG);
      }
      for (i = 0; (i < tt->vtype_count) && success; i++)
      {
         success = _DtCompileMessagePiece(w, request, relPathHost, relPathDir,
                           &(tt->tt_argn_vtype[i]), False, _DTAct_TT_VTYPE);
      }
...
Tt_message
#ifdef _NO_PROTO
Ttmedia_to_Dt_StatusUpdateCB(message, clientdata,
                             op, contents, len, file)
    Tt_message     message;
    void           *clientdata;
    Tttk_op        op;
...
static Tt_callback_action 
#ifdef _NO_PROTO
TtRequestCallbackHandler(message, pattern)
        Tt_message message;
        Tt_pattern pattern;
#else
...
#endif /* _NO_PROTO */
{
   ...
   if ( _DtSvcInitToolTalk(_DtInitTtContextWidget) != TT_OK )
	return False;
