/* $XConsortium: Action.c /main/cde1_maint/4 1995/10/04 11:53:19 gtsang $ */
/*************************************<+>*************************************
 *****************************************************************************
 **
 **   File:         Action.c
...
extern void _DtProcessTtRequest(
                        ...
                        char * relPathHost,
                        char * relPathDir ) ;
extern Boolean _DtInitializeToolTalk(Widget w);
extern char * _DtActMapFileName(
                        const char * curHost,
                        const char * dir,
                        const char * file,
...
static void InitLocalizedStrings() ;
...
static Tt_callback_action _DbReloadCB();
static void _DtActTimerCB();
static void _DtActIndicatorCB();
...
#ifdef _NO_PROTO
DtActionInvoke(w, action, aap, aac, termOpts, execHost,
               cwd, useIndicator, statusUpdateCb, client_data)
	Widget 		w;	
	char   		*action;
	DtActionArg 	*aap;
	int        	aac;
...
#endif /* _NO_PROTO */
{
           ...
           w,action,aap,aac,termOpts,execHost,contextHost,cwd,invp)) != NULL)
    {
        ...
        {
	     ;/* A dialog was posted; request will be freed later */
        }
    }
...
static void 
#ifdef _NO_PROTO
FreeErrorDialog( w, dialog, call_data )
        Widget w ;
        Widget dialog ;
        XtPointer call_data ;
#else
...

{
   ...
   Status status;
   Boolean is_mapped = True;

   /* Create the title string for the dialog */
   title = (char *)XtMalloc((Cardinal)(strlen(PromptDialogTitle) + 
                                       strlen(actionName) +
                                       strlen(ErrorPostfix) + 1));

   (void)sprintf(title, ((char *)Dt11GETMESSAGE(2, 1, "%1$s%2$s%3$s")), 
		 PromptDialogTitle, actionName, ErrorPostfix);

   ok = XmStringCreateLtoR(_DtOkString, XmFONTLIST_DEFAULT_TAG);
   cancel = XmStringCreateLtoR(_DtCancelString, XmFONTLIST_DEFAULT_TAG);
   help = XmStringCreateLtoR(_DtHelpString, XmFONTLIST_DEFAULT_TAG);
   ...
   XtAddCallback(XmMessageBoxGetChild(dialog, XmDIALOG_OK_BUTTON),
                 XmNactivateCallback, (XtCallbackProc)FreeErrorDialog, 
                 (XtPointer)dialog);
   ...
   if (!is_mapped) 
   {
      ...
      XtSetArg (args[0], XmNx,
            (WidthOfScreen(XtScreen(dialog)) - dialog->core.width) / 2);
      XtSetArg (args[1], XmNy,
	       (HeightOfScreen(XtScreen(dialog)) - dialog->core.height) / 2);
...
static void 
#ifdef _NO_PROTO
InvalidFilename( w, actionName, filename )
        Widget w ;
        String actionName ;
        String filename ;
#else
...

{
   ...
   pt1 = XmStringCreateLtoR(InvalidFileMsg, XmFONTLIST_DEFAULT_TAG);
   pt2 = XmStringCreateLtoR(filename, XmFONTLIST_DEFAULT_TAG);
...
static void 
#ifdef _NO_PROTO
HostAccessError( w, actionName, hostName )
        Widget w ;
        String actionName ;
        String hostName ;
#else
...

{
   ...
   /* Construct the error message */
   pt1 = XmStringCreateLtoR(HostErrorMsg, XmFONTLIST_DEFAULT_TAG);
   pt2 = XmStringCreateLtoR(hostName, XmFONTLIST_DEFAULT_TAG);
   pt3 = XmStringCreateLtoR(HostErrorMsg2, XmFONTLIST_DEFAULT_TAG);
...
static void 
#ifdef _NO_PROTO
MultiHostAccessError( w, actionName, hostList )
        Widget w ;
        String actionName ;
        String hostList ;
#else
...
#endif /* _NO_PROTO */
{
   ...
   XmString msg = XmStringCreateLtoR(MapErrorMsg, XmFONTLIST_DEFAULT_TAG);
   _DtCreateErrorDialog(w, actionName, msg);
   XmStringFree(msg);
...
static void 
#ifdef _NO_PROTO
NoActionError( w, origNameQuark, actionName, type, host, dir, file )
        Widget w ;
	DtShmBoson origNameQuark;
        char * actionName ;
        char * type;
...
{
   char     msgbuf[2*MAXPATHLEN];
   XmString msg;
   char *   name = NULL;

   if (actionName == 0) actionName = "";
   /* Construct the error message */
   ...
   msg  = XmStringCreateLtoR(msgbuf, XmFONTLIST_DEFAULT_TAG);

   _DtCreateErrorDialog(w, (origNameQuark==-1)?"Unknown":(char *)_DtDtsMMBosonToString(origNameQuark), msg);
   XmStringFree(msg);
   XtFree(name);
}
...
static void 
#ifdef _NO_PROTO
CommandInvokerError( w, actionName, errorString )
        Widget w ;
        String actionName ;
        String errorString ;
#else
...

{
   ...
   msg = XmStringCreateLtoR(errorString, XmFONTLIST_DEFAULT_TAG);
   _DtCreateErrorDialog(w, actionName, msg);
   XmStringFree(msg);
}
...
static void 
#ifdef _NO_PROTO
NoToolTalkConnectionError( w, actionName)
        Widget w ;
        String actionName ;
#else
...

{
   ...
   msg = XmStringCreateLtoR(NoToolTalkConnMsg, XmFONTLIST_DEFAULT_TAG);
   _DtCreateErrorDialog(w, actionName, msg);
   XmStringFree(msg);
}
...
{
        XmString    msg;
	char        msgBuf[_DtAct_MAX_BUF_SIZE];
...
#ifdef _NO_PROTO
CreateActionRequest( w, actionName, aap, aac, termOpts, execHost, cwdHost, 
                     cwdDir, invp )
        Widget 		w ;
        String 		actionName ;
        DtActionArg 	*aap ;
	...
	int		aac)
{
	 ...
	 */
	 ...
	 if ( aap[i].argClass == DtACTION_BUFFER )
	 {
            ...
            int closeAttempts;
#define _DT_ACTION_MAX_CLOSE_TRYS	5
	   ...
	   DtDtsFreeAttributeValue(is_executable);
	       ...
	       /* Should sprintf() buffer name into the format XXX 4014127 */
		  ...
		  bytesToWrite -= bytesWritten)
	   {
	    ...
	    {
	     /* error closing fd */
		if ( closeAttempts > _DT_ACTION_MAX_CLOSE_TRYS )
		    break;
...
static Boolean 
#ifdef _NO_PROTO
ParseFileArgument( w, request, objectData, hostname, filename, filetype,
                   typeFile )
        Widget w ;
        ActionRequest * request;
        ObjectData * objectData;
...

{
	...
	if ((tmp = getcwd(tmp,MAXPATHLEN)) == NULL )
	{
		perror("getcwd(): unable to get current directory");
		tmp = strdup("/");
	}
	return(tmp);
...
static int 
#ifdef _NO_PROTO
_DtAddEntry( string, arrayPtr, sizePtr )
...
static void 
#ifdef _NO_PROTO
TryToTypeFile( obj, host, dir, file, resolvedPath)
        ObjectData *obj ;
        char * host ;
        char * dir ;
        char * file ;
...
static int 
#ifdef _NO_PROTO
LinkToTypeQuark( host, dir, file, resolvedPath )
        char * host ;
        char * dir ;
        char * file ;
	char **resolvedPath;
...

{
   ...
   path = (char *)_DtActMapFileName(host, dir, file, NULL);
   if (path == NULL)
   {
      *resolvedPath=NULL;
      return(-1);
   ...
   while ((link_len = readlink(file_name, link_path, 1024)) > 0)
   {
...

{
   ...
   if (request->clonedAction == NULL)
   {
	  ...
	  */
         SET_INV_ERROR(_DtActFindInvRec(request->invocId)->state);
         return(True);
      }
       ...
       */
      if (IS_TT_MSG(action->mask) && !_DtInitializeToolTalk(NULL))
      {
         NoToolTalkConnectionError(w, request->clonedAction->label);
         SET_INV_ERROR(_DtActFindInvRec(request->invocId)->state);
         return(True);
      }
...

{
   ...
   */
    ...
    */
   ExtractCWD(request, &relPathHost, &relPathDir, False);
...
static void 
#ifdef _NO_PROTO
ExtractCWD( request, hostPtr, dirPtr, useObjectInfo )
        register ActionRequest *request ;
        char ** hostPtr;
        char ** dirPtr;
        Boolean useObjectInfo;
...

{
   ...
   newAction->arg_types = (DtShmBoson *)XtMalloc(sizeof(XrmQuark) * 
...
static void
#ifdef _NO_PROTO
CloneParsedMessage( old_pmsg, new_pmsg )
        register parsedMsg * old_pmsg ;
        register parsedMsg * new_pmsg ;
#else
CloneParsedMessage(
...
static parsedMsg *
#ifdef _NO_PROTO
CloneParsedMessageArray( pmsgArray, count )
        register parsedMsg * pmsgArray ;
        register int count ;
#else
...
static void 
#ifdef _NO_PROTO
FreeParsedMessageArray( parsedMessageArray, count )
        register parsedMsg * parsedMessageArray ;
        int count;
#else
...
Boolean 
#ifdef _NO_PROTO
_DtCompileMessagePiece(w, request, relPathHost, relPathDir, piece, initialize,
                    processingMask )
        Widget w ;
        ActionRequest *request ;
        char * relPathHost ;
...

{
   ...
   ObjectData tmpObjData = { 0, 0, 0 };

   static Boolean * paramUsed = NULL;
   static int promptDataIndex = 0;
   ...
   if (initialize)
   {
      ...
      XtFree(paramUsed);
      paramUsed = 
        (Boolean *)XtMalloc((Cardinal)(sizeof(Boolean) * request->numObjects));
      for (i = 0; i < request->numObjects; i++)
         paramUsed[i] = False;
   }
...
static Boolean 
#ifdef _NO_PROTO
InsertArgumentString( w, bufPtr, bufSizePtr, request, object, mask, 
                      relPathHost, relPathDir, addLeadingSpace, 
                      processingMask)
        Widget w ;
        register char **bufPtr ;
        int * bufSizePtr ;
      ...
      if (IS_FILE_OBJ(object->mask))
      {
         if (processingMask & _DTAct_TT_VTYPE)
         {
            ...
            if (object->type != (-1))
            {
               ...
               if ((path = (char *)_DtActMapFileName(
...

{
   ...
   if (addLeadingSpace)
       (void)strcat(*bufPtr, " ");
   if (IS_BUFFER_OBJ(object->mask))
   {
      char *base = _DtBasename(object->u.file.origFilename);
      size = strlen(base) + 4;
...
static String 
#ifdef _NO_PROTO
GrowMsgBuffer( buffer, size, count )
        String buffer ;
        int *size ;
        int count ;
#else
...
static void 
#ifdef _NO_PROTO
AddPrompt( argNum, prompt, numPrompts, prompts )
        register int argNum ;
        String prompt ;
        register int *numPrompts ;
        register PromptEntry **prompts ;
...
static int 
#ifdef _NO_PROTO
MatchParamsToAction( request, numPrompts, prompts )
        ActionRequest *request ;
        int *numPrompts ;
        PromptEntry **prompts ;
#else
...
static void 
#ifdef _NO_PROTO
ProcessOneSegment( request, msg, prompts, numPrompts, argsOptionFound,
                   lastArgReferenced, unused, paramUsed )
	ActionRequest * request;
        parsedMsg * msg;
        PromptEntry **prompts ;
...
static void
#ifdef _NO_PROTO
CancelOut( w, event, params, num_params )
        Widget w ;
        XEvent *event;
        XtPointer params;
        XtPointer num_params;
...
static void 
#ifdef _NO_PROTO
CancelPromptDialog( widget, dialog, call_data )
        Widget widget ;
        PromptDialog *dialog ;
        XtPointer call_data ;
#else
...
static void 
#ifdef _NO_PROTO
ChangePromptTraversal( widget, dialog, call_data )
        Widget widget ;
        register PromptDialog *dialog ;
        XtPointer call_data ;
#else
...
static void 
#ifdef _NO_PROTO
ProcessPromptDialog( widget, dialog, call_data )
        Widget widget ;
        register PromptDialog *dialog ;
        XtPointer call_data ;
#else
...

{
   ...
   pt1 = XmStringCreateLtoR(PromptDialogLabel, XmFONTLIST_DEFAULT_TAG);
   n = 0;
   ...
   for (count = 0; count < numPrompts; count++)
   {
      ...
      pt1 = XmStringCreateLtoR(prompts[count].prompt, XmFONTLIST_DEFAULT_TAG);
      n = 0;
   ...
   n = 0;
   labelString = XmStringCreateLtoR(_DtOkString, XmFONTLIST_DEFAULT_TAG);
   ...
   n = 0;
   labelString = XmStringCreateLtoR(_DtCancelString, XmFONTLIST_DEFAULT_TAG);
   XtSetArg (args[n], XmNleftAttachment, XmATTACH_POSITION);	n++;
   ...
   if (!is_mapped) 
   {
      ...
      XtSetArg (args[0], XmNx,
            (WidthOfScreen(XtScreen(bboard)) - bboard->core.width) / 2);
      XtSetArg (args[1], XmNy,
	       (HeightOfScreen(XtScreen(bboard)) - bboard->core.height) / 2);
...
static void 
#ifdef _NO_PROTO
ContinueRequest( widget, dialog, call_data )
        Widget widget ;
        register ContinueDialog *dialog ;
        XtPointer call_data ;
#else
...
static void 
#ifdef _NO_PROTO
CancelRequest( widget, dialog, call_data )
        Widget widget ;
        register ContinueDialog *dialog ;
        XtPointer call_data ;
#else
...

{
   ...
   title = (char *)XtMalloc((Cardinal)
             (strlen(PromptDialogTitle)+ strlen(request->clonedAction->label) + 1));
   (void)sprintf(title, ((char *)Dt11GETMESSAGE(2, 2, "%1$s%2$s")), PromptDialogTitle, request->clonedAction->label);
   label = XmStringCreateLtoR(ContinueMessage, XmFONTLIST_DEFAULT_TAG);
   ...
   XtAddCallback(XmMessageBoxGetChild(dialog->topLevel, XmDIALOG_OK_BUTTON),
                 XmNactivateCallback, (XtCallbackProc)ContinueRequest, (XtPointer)dialog);
   XtAddCallback(XmMessageBoxGetChild(dialog->topLevel, XmDIALOG_CANCEL_BUTTON),
                 XmNactivateCallback, (XtCallbackProc)CancelRequest, (XtPointer)dialog);
...

{
   ...
   if (ResolveCommandInvokerMessagePieces(w, request, relPathHost, relPathDir))
   {
       ...
       */
      ExtractCWD(request, &cwdHost, &cwdDir, True);
...
static Boolean 
#ifdef _NO_PROTO
ResolveCommandInvokerMessagePieces(w, request, relPathHost, relPathDir)
        Widget w ;
        ActionRequest *request ;
        char * relPathHost ;
        char * relPathDir ;
...

{
   ...
   _DtCompileMessagePiece(w, request, relPathHost, relPathDir,
                           &(action->u.cmd.execHosts), True, 0);
   SetExecHost(request);

   if ((_DtCompileMessagePiece(w, request, relPathHost, relPathDir,
                           &(cmd->execString), False, 0) == False) ||
       (_DtCompileMessagePiece(w, request, relPathHost, relPathDir,
                           &(cmd->termOpts), False, 0) == False))
   {
...
static void 
#ifdef _NO_PROTO
InitiateCommandInvokerRequest( w, request, host, dir)
        Widget w ;
        ActionRequest *request ;
        String host ;
        String dir ;
...

{
   ...
   switch ( tt_ptr_error(procId = tt_default_procid()) )
   {
	   ...
	   if ( tt_ptr_error(procId = tt_default_procid()) != TT_OK ) {
...
static void
#ifdef _NO_PROTO
ParseHostList (hostString, hostListPtr, hostListSizePtr, hostCountPtr)
   char * hostString;
   char *** hostListPtr;
   int * hostListSizePtr;
   int * hostCountPtr;
...
static void
#ifdef _NO_PROTO
RemoveDuplicateHostNames (hostList, hostCountPtr)
             char ** hostList;
             int   * hostCountPtr;
#else
...
static void 
#ifdef _NO_PROTO
AddFailedHostToList (request, badHost)
   ActionRequest * request;
   String badHost;
#else
AddFailedHostToList (
...
static void 
#ifdef _NO_PROTO
CmdInvSuccessfulRequest( message, data2 )
        char *message;
        void *data2;
#else
...
static void 
#ifdef _NO_PROTO
CmdInvFailedRequest( error_message, data2 )
        char *error_message;
        void *data2;
#else
CmdInvFailedRequest(
...
char * 
#ifdef _NO_PROTO
_DtActMapFileName( curHost, dir, file, newHost )
        char * curHost;
        char * dir;
        char * file;
        char * newHost;
