	   ...
	   const char *CacheFile)
{
	...
	_DtMMAddActionsToDataAttribute(_DtDtsDbGet(DtDTS_DA_NAME));

	header.pathhash = _DtDtsMMPathHash(dirs);
	header.num_db = num_db;
	header.db_offset = build_new_db(shm_handle, int_handle, num_db,
db_list);
	build_name_list(_DtDtsDbGet("DATA_CRITERIA"), int_handle, &header);
	...
	write_db(&header, tbl_data, tbl_size, CacheFile);
		...
		DtDtsMMHeader *header, const char *suffix)
{
	...
	for(i = 0; dirs->paths[i]; i++)
	{
		...
		{
			char	*c = strrchr(direntp->d_name, suffix[0]);
			if(c && strcmp(c, suffix) == 0)
			{
				sprintf(pathname, "%s/%s", dirs->paths[i], direntp->d_name);
...
{
	DtDtsDbDatabase	*dc = (DtDtsDbDatabase *)
				_DtDtsDbGet(DtDTS_DC_NAME);
	DtDtsDbDatabase	*da = (DtDtsDbDatabase	*)
				_DtDtsDbGet(DtDTS_DA_NAME);
	int		i;
...
write_db(DtDtsMMHeader *header, void *index, int size, const char *CacheFile)
{
	...
	tmpfile = (char *)tempnam(_DTDTSMMTEMPDIR, _DTDTSMMTEMPFILE);
	...
	write(fd, header, sizeof(DtDtsMMHeader));
	write(fd, index,  size);
        ...
        DtDtsMMRecord *record2 )
{
	...
	results =  (int)record1->recordName - (int)record2->recordName;
	if(results)
	{
		return(results);
	}
