/*
 * File:         DbReader.c $XConsortium: DbReader.c /main/cde1_maint/3 1995/10/04 11:56:14 gtsang $
 * Language:     C
 *
...
static int NumberFieldsAllocated = 0;
 ...
 */
static char *tmpLine = NULL; 	/* Temporary input line */
static char *tmpBuffer = NULL;	/* Buffer to store a field value that may
...
static void InitializeLocalizedStrings();
...
Boolean
#ifdef _NO_PROTO
get_variable ( key, key_len, value, escape , bracketSeen)
char *key;
int key_len;
char **value;
int escape;
...
char)*head)))
#else /* AIX_ILS */
	 if (((isalnum (*head)) || (ispunct (*head)) || (iscntrl (*head))) && (!isspace ((unsigned char)*head)))
#endif /* AIX_ILS */
	    tail = head;
	 head++;
      }
 ...
 * Synopsis:
 *
 *	(void) = _DtDbFillVariables (line,lineSize)
 *
 *	char **line;  	The string to scan and replace.
 *	int  lineSize;  The size of the line buffer; used if buffer is
 *		 	longer than strlen.  lineSize of 0 indicates that 
 *			buffersize is the same as strlen.
 *
 *      MODIFIED:	line
 *
 ***********************/
...
static int 
#ifdef _NO_PROTO
ReadNextEntry( versionCanBeSet, fd, fieldPtr, recordDescriptions, 
               numRecordDescripts, returnRecordIndex, file_name )
        Boolean * versionCanBeSet;
        FILE *fd;
        DtDtsDbField ** fieldPtr;
        DtDbRecordDesc * recordDescriptions;
   ...
   while (1) 
   {
	 ...
	 break;
      ...
      if ((strncmp(start, "set ", 4) == 0) &&
          (strlen(start) >= 4))
      {
         if (!DefineVariable(versionCanBeSet, start + 4, file_name))
         {
            /*
	 ...
	 continue;
      ...
      clean_line (&tmpLine);
      if (strchr (tmpLine, '$'))
	 _DtDbFillVariables (&tmpLine,MAX_LINE_LENGTH);
      ...
      SplitField(fieldPtr, &indx, start);
	 ...
	 if (multi_line) 
	 {
		...
		*/
	       clean_line (&tmpBuffer);
               SplitField(fieldPtr, &indx, tmpBuffer);
	       multi_line = False;
	    }
	 }
	 else 
...
static void 
#ifdef _NO_PROTO
ResolveVariableReference( varName, varNameLen, buf, buf_size, start, len, buf_len, escape , brackets)
        char *varName ;
        int varNameLen ;
        char **buf ;
        int *buf_size ;
...
#endif /* _NO_PROTO */
{
   ...
   if ((get_variable (varName, varNameLen, &value, escape, brackets)))
   {
...
static void 
#ifdef _NO_PROTO
SplitField( fields, slotToUse, ptr )
        DtDtsDbField ** fields;
        int * slotToUse;
        char * ptr;
#else
...
static int 
#ifdef _NO_PROTO
MatchKeyword( line, recordDescriptions, numRecordDescriptions )
        char * line;
        DtDbRecordDesc * recordDescriptions;
        int numRecordDescriptions;
#else
