/* $XConsortium: CmdMain.c /main/cde1_maint/7 1995/10/08 22:16:51 pascale $ */
/***************************************************************************
*
* File:         CmdMain.c
...
static void _DtCmdInitializeErrorMessages();
static SPC_ChannelPtr _DtSPCOpen();
static int _DtSPCSpawn();
...
static SPC_Channel_Ptr _DtSPCOpen( char *hostname,
         int iomode,
...
static void
#ifdef _NO_PROTO
QueueRequest (channel, context, execHost, execString, argv, winType, requestNum, 
...
int
#ifdef _NO_PROTO
_DtCmdCommandInvokerExecute (errorMessage, replyContext, windowType, 
			   contextHost, contextDir, contextFile, 
			   execParms, execHost, execString, 
...

{
   ...
   if (!localExecution) 
   {
	 ...
	 (void) snprintf (errorMessage, MAX_BUF_SIZE * 2,
...
#if defined(__hp_osf) || defined(__osf)
         setsid() ;
#else
         (void) setpgrp ();
#endif
...
static void 
#ifdef _NO_PROTO
CheckCommandTerminator( cmdChannel, pid, type, cause, requestNum )
        SPC_Channel_Ptr cmdChannel ;
        int pid ; 			/* NOT USED */
        int type ; 			/* NOT USED */
...
static int 
#ifdef _NO_PROTO
DtCmdGetWindowType( windowTypeMask)
        unsigned long windowTypeMask ;
#else
DtCmdGetWindowType(
        unsigned long windowTypeMask) 
...
void
#ifdef _NO_PROTO
_DtCommandInvokerExecute(request_name, context_host, context_dir, context_file, 
			exec_parameters, exec_host, exec_string,
			success_proc, success_data, failure_proc, failure_data) 
	char *request_name;	/* NOW OBSOLETE -- NO_STDIO is the only choice */
	char *context_host;
	char *context_dir;
	char *context_file;	/* NOW OBSOLETE -- ignored */
...
void 
#ifdef _NO_PROTO
_DtInitializeCommandInvoker( display, toolClass, appClass, reloadDBHandler,
			    appContext)
	Display *display;
	char *toolClass;
	char *appClass;
