/* $XConsortium: ActionFind.c /main/cde1_maint/6 1995/10/20 17:03:35 lehors $ */
/*************************************<+>*************************************
 *****************************************************************************
 **
 **   File:         ActionFind.c
...
static char *		_DtActGetDtsMMField (  );
...
#endif  /* _NO_PROTO */
{
	if(use_in_memory_db)
	{
		return(_DtActionDbDescription(s));
	}
	else
	{
		return(_DtActionMMDescription(s));
	}
}
...
_DtActionMMDescription (  char *s )
{
	...
	DtDtsMMDatabase	*act_db	= _DtDtsMMGet(_DtACTION_NAME);
	int		*start;
	...
	if (!act_rec_list || !start) return NULL;
...
#endif /* _NO_PROTO */
{
	if(use_in_memory_db)
	{
		return(_DtActionDbExists(s));
	}
	else
	{
		return(_DtActionMMExists(s));
	}
}
...
#endif /* _NO_PROTO */
{
	if(use_in_memory_db)
	{
		return(_DtActionDbLabel(s));
	}
	else
	{
		return(_DtActionMMLabel(s));
	}
}
...
#endif /* _NO_PROTO */
{
	if(use_in_memory_db)
	{
		return(_DtActionDbIcon(s));
	}
	else
	{
		return(_DtActionMMIcon(s));
	}
}
...
_DtActMMChooseLabel(DtDtsMMRecord *rec)
{
	...
	if ((label=
		_DtActGetDtsMMField(rec, _DtACTION_LABEL)) != NULL)
		return label;
...
static int 
#ifdef _NO_PROTO
_DtActionCompareRecordQuarks( record1, record2 )
        DtDtsDbRecord ** record1 ;
        DtDtsDbRecord ** record2 ;
#else
...
static void
#ifdef _NO_PROTO
_DtActionGetRecordWeight( record, primaryWeightPtr, secondaryWeightPtr )
        DtDtsDbRecord ** record ;
        long * primaryWeightPtr;
        long * secondaryWeightPtr;
#else
...
_DtActMMGetCmdInfo(DtDtsMMRecord *actRecp, ActionPtr actp)
{
	 ...
	 */
	tmp = XtMalloc( sizeof("-title \"")+strlen(actp->label)+ strlen(s) + 3);
	strcpy(tmp,"-title \"");
...
DtDtsMMRecord *
#ifdef _NO_PROTO
_DtActionLocateRecord( actQuark, obj_mask, arg_type ,arg_count)
	DtShmBoson	actQuark;
...
static Boolean 
#ifdef _NO_PROTO
ValidKeyword( start, end, prompt, keywordId, argNum, isFile )
        char *start ;
        char *end ;
        char **prompt ;
        int *keywordId ;
			...
			DtShmBoson actQuark )
{
	...
	for (actRecp = _DtActionLocateRecord(actQuark,mask,type,num);
		actRecp;
		actRecp = _DtActionLocateRecord(_DtDtsMMStringToBoson(mapto),
				mask,type,num), numSearches++)
	{
		/*
	...
	if ( actRecp )
	{
		...
		actp->file_name_id = actRecp->pathId;
                if ( origLabel )
