/* $XConsortium: Fnmatch.c /main/cde1_maint/2 1995/10/04 12:00:17 gtsang $ */
/*
...
#include <string.h>
#include <stdio.h>
#if !defined(__apollo) && !defined(sun) && !defined(__hp_osf) && !defined(USL) && !defined(__uxp__)
#include <Dt/Setlocale.h>
#endif /* __apollo */
#include <Dt/Regex.h>

/*****************************************************************************/
#if defined(sun) || defined(__uxp__)
#undef NLS16	/* TEMPORARY UNTIL NLS16 COMPILE FIXED ON SUN FOR THIS FILE  */
		/* OR THIS FILE IS NO LONGER INCLUDED IN DTSVC LIB ON SUN    */ 
#endif
/*****************************************************************************/
...
static int matcher() ;
static int bracketcomp() ;
static int gen_range() ;
static int gen_single_match() ;
static int gen_ctype() ;
static u_char * next() ;
static int bracketexec() ;
...
static int matcher( 
                        ...
                        u_char *string,
                        int flag) ;
static int bracketcomp( void ) ;
static int gen_range( 
                        struct logical_char *bchar,
                        struct logical_char *echar,
                        register u_char *ep,
                        u_char *endbuf,
                        int *error) ;
static int gen_single_match( 
                        struct logical_char *lchar,
                        register u_char *ep,
                        u_char *endbuf,
                        int *error) ;
static int gen_ctype( 
...
/*			p++;			 eat the closing ']' */
			...
			case ':':
				...
				*(p-2) = '\0';					/* make the name a string */
				...
				while ((i < sizeof(ctype_funcs)/sizeof(u_char *)) && strcmp((const char *)psave, (const char *)ctype_funcs[i]))
#else
				while ((i < sizeof(ctype_funcs)/sizeof(u_char *)) && strcmp((char *)psave, (char *)ctype_funcs[i]))
#endif
					i++;
				*(p-2) = ':';					/* restore RE */
...
static int 
#ifdef _NO_PROTO
gen_range( bchar, echar, ep, endbuf, error )
        struct logical_char *bchar ;
        struct logical_char *echar ;
        register u_char *ep ;
        u_char *endbuf ;
...
static int 
#ifdef _NO_PROTO
gen_single_match( lchar, ep, endbuf, error )
        struct logical_char *lchar ;
        register u_char *ep ;
        u_char *endbuf ;
        int *error ;
...
static int 
#ifdef _NO_PROTO
gen_ctype( lchar, ep, endbuf, error )
        struct logical_char *lchar ;
        register u_char *ep ;
        u_char *endbuf ;
        int *error ;
	...
	if(*first){  		/* First call */
	    *first = 0;
		...
		if(pnm){
...
#if    defined(__STDC__) || defined(__cplusplus) || defined(c_plusplus)
			    tmpptr = str + strlen((const char *)str);
#else
			    tmpptr = str + strlen((char *)str);
#endif
		    }
		}
		else
	        ...
	        if (pnm){
#if    defined(__STDC__) || defined(__cplusplus) || defined(c_plusplus)
		    if((tmpptr = (u_char *)strchr((const char *)str,'/')) == 0){
#else
		    if((tmpptr = (u_char *)strchr((char *)str,'/')) == 0){
#endif
		        if (per && *str == '.' && lastslash)
			    tmpptr = str;
	                else
