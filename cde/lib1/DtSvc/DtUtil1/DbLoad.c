 ...
 * +SNOTICE
 * 
 * $XConsortium: DbLoad.c /main/cde1_maint/5 1995/10/19 11:22:47 drk $
 * 
...
_DtOAConverter(DtDtsDbField * fields,
	       ...
	       Boolean rejectionStatus)
{
	DtDtsDbDatabase *dc_db = _DtDtsDbGet(DtDTS_DC_NAME);
	DtDtsDbDatabase *da_db = _DtDtsDbGet(DtDTS_DA_NAME);
...
_DtDtsMMCreateDb(DtDirPaths *dirs, const char *CacheFile, int override)
{
	...
	_DtDtsMMCreateFile(dirs, CacheFile);
	if(!_DtDtsMMapDB(CacheFile))
	{
		return(0);
	}
	if(!override)
