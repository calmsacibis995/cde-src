/* $XConsortium: sxpm.c /main/cde1_maint/1 1995/07/17 18:24:58 drk $ */
/*
    ...
    char **argv;
{
	...
	ErrorStatus = _DtXpmReadFileToPixmap(dpy, root, iconFile, &icon.pixmap,
...
#ifdef Debug2
    /* this is just to test the _DtXpmCreateDataFromPixmap function */

    view.attributes.valuemask |= XpmReturnPixels;
    view.attributes.valuemask |= XpmReturnExtensions;
    ErrorStatus = _DtXpmCreatePixmapFromData(dpy, win, plaid,
					  &view.pixmap, &view.mask,
					  &view.attributes);
    ErrorMessage(ErrorStatus, "Plaid");

    ErrorStatus = _DtXpmCreateDataFromPixmap(dpy, &data, view.pixmap, view.mask,
					  &view.attributes);
    ...
    _DtXpmFreeAttributes(&view.attributes);
    view.attributes.valuemask = valuemask;
#endif
	...
	ErrorStatus = _DtXpmCreatePixmapFromData(dpy, win, data,
					      &view.pixmap, &view.mask,
					      &view.attributes);
	...
	ErrorStatus = _DtXpmReadFileToPixmap(dpy, win, input,
					  &view.pixmap, &view.mask,
					  &view.attributes);
    ...
    if (output || stdoutf) {
	ErrorStatus = _DtXpmWriteFileFromPixmap(dpy, output, view.pixmap,
    ...
    int i;
{
	...
	_DtXpmFreeAttributes(&icon.attributes);
    }
    if (view.pixmap) {
	XFreePixmap(dpy, view.pixmap);
	if (view.mask)
