/* $XConsortium: XpmWrFFrI.c /main/cde1_maint/1 1995/07/17 18:22:10 drk $ */
/*
...
int
_DtXpmWriteFileFromImage(display, filename, image, shapeimage, attributes)
    Display *display;
    char *filename;
    XImage *image;
    XImage *shapeimage;
    XpmAttributes *attributes;
{
    ...
    if ((ErrorStatus = _DtxpmWriteFile(filename, &mdata)) != XpmSuccess)
	return (ErrorStatus);
     ...
     */
    ErrorStatus = _DtxpmScanImage(display, image, shapeimage,
			       attributes, &attrib);

    if (ErrorStatus == XpmSuccess) {
	attrib.name = name;
	ErrorStatus = _DtxpmWriteData(&mdata, &attrib, attributes);
    }
...
int
_DtxpmWriteData(mdata, attrib, attributes)
    xpmData *mdata;
    xpmInternAttrib *attrib;
    XpmAttributes *attributes;
{
...
char ***colorTable;
{
	...
	for (key = 1; key <= NKEYS; key++) {
		...
		fprintf(file, "\t%s %s", _DtxpmColorKeys[key - 1], s);
	}
    } else
...
char *rgb_fname;
{
    ...
    if (rgb_fname)
	rgbn_max = _DtxpmReadRgbNames(rgb_fname, rgbn);
	...
	if (c) {
	    colorname = NULL;
	    if (rgbn_max)
		colorname = _DtxpmGetRgbName(rgbn, rgbn_max, xcolors->red,
