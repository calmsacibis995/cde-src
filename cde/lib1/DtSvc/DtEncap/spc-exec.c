/*
 * File:         spc-exec.c $XConsortium: spc-exec.c /main/cde1_maint/3 1995/10/08 22:13:54 pascale $
 * Language:     C
 *
...
#ifdef DEBUG
    fprintf(stderr, (XeString)"got SIGCHLD, pid: %d, channel: %x\n", pid, channel);
#endif
	...
	char 		*context)
{
   ...
   if (!host) {
      ...
      tt_cp = tt_malloc(strlen(file) +1);
      if (tt_ptr_error (tt_cp) != TT_OK) {
	  return (NULL);
      }
      return (strcpy(tt_cp, file));
   }
...
static void
#ifdef _NO_PROTO
resolve_variable_reference(string)
