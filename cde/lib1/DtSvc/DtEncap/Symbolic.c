/*
 * File:         Symbolic.c $XConsortium: Symbolic.c /main/cde1_maint/3 1995/10/18 16:51:52 drk $
 * Language:     C
 *
...
#define D_HASHSIZE 256
    ...
    if (D_sym_table) return D_sym_table;

    return (D_sym_table = Xe_new_symtab(D_HASHSIZE));
}
/*------------------------------------------------------------------------+*/
static XeSymtabList
...
#ifdef DEBUG
   printf("Added data %x in list[%d] @ %x\n", l->data, hash,l);
#endif
