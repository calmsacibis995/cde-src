/*
 * File:         spc-proto.c $XConsortium: spc-proto.c /main/cde1_maint/1 1995/07/14 20:19:17 drk $
 * Language:     C
 *
...
/*----------------------------------------------------------------------+*/
{
  ...
  if (connection->hostinfo != (char*)NULL)
   {
      XeFree(connection->hostinfo);
      connection->hostinfo = (char*)NULL;
   }
  ...
  if (connection != (SPC_Connection_Ptr)NULL)
   {
    XeFree((char *)connection);
    connection = (SPC_Connection_Ptr)NULL;
   }
}
...
/*----------------------------------------------------------------------+*/
{
  ...
  while(numread<request_len) {
	...
	SPC_Error(SPC_Connection_Reset,
		  CONNECTION_HOSTNAME(connection));
      else
  ...
  if(retval<0) {
    ...
    if(retval != SPC_Protocol_Abort) {
      ...
      if (retval == SPC_Cannot_Create_Netfilename) {
         filename = tt_netfile_file(channel->context_dir);
	 SPC_Error(retval, filename, connection->hostname);
	 tt_free(filename);
      } else if (retval == SPC_Cannot_Exec)
	 SPC_Error(retval, channel->path, 0);
      else if (retval == SPC_cannot_Chdir) {
         filename = tt_netfile_file(channel->context_dir);
	 SPC_Error(retval, filename, 0);
	 tt_free(filename);
      } else
			  ...
			  int errid)
{
  ...
  XeString buf;
  int reply_seqno;
  int this_seqno=current_sequence_number;
  int valid_ep = 0;
  char *this_str;
...
/*----------------------------------------------------------------------+*/
{
  ...
  return(SPC_Send_Multi_Packet(connection, prot, environ, ep_count,
    ...
    /* Called by client to register itself to spcd */
{
  ...
  if(!(pwent=getpwuid(this_uid))) {
    ...
    username=(XeString)(pwent->pw_name);
  }
