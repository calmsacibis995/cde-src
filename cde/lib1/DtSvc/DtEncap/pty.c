/*
 * File:         pty.c $XConsortium: pty.c /main/cde1_maint/2 1995/10/04 11:50:08 gtsang $
 * Language:     C
 *
...
#ifdef __hpux_pty
{
  ...
  do
    select_value=select(fd+1,
			(int*)&read_mask, NULL, (int*)&except_mask,
			timeptr);
...
#else /* not __hpux_pty */
{
  ...
  if(result == ERROR)
    if(errno == EIO) {
      ...
      SPC_Error(SPC_Reading, channel->connection ?
                CONNECTION_HOSTNAME(channel->connection) : XeString_Empty);
      return(SPC_ERROR);
    }
...

{
  ...
  if (parentp) {		/* Master process */
	...
	select_value=select(max_fds, NULL, NULL, (int*)&temp_mask, NULL);
...
static int send_eof_pty_channel_object(SPC_Channel_Ptr channel)
{
  ...
  if((wire->slave_termio.c_lflag & ~ICANON) == NULL)
    return(FALSE);
