/*
 * File:         spc-net.c $XConsortium: spc-net.c /main/cde1_maint/2 1995/10/18 12:18:18 drk $
 * Language:     C
 *
...
/*----------------------------------------------------------------------+*/
{
  ...
  host_def=gethostbyname(hostname);
  if(!host_def)
    return(FALSE);
...
SPC_Connection_Ptr SPC_Standalone_Daemon(SPC_Connection_Ptr conn)
{
  ...
  for(;;) {
    ...
    conn->remote = gethostbyaddr((char *)&client_saddr.sin_addr,
