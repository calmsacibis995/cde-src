/*
 * File:         spc-error.c $XConsortium: spc-error.c /main/cde1_maint/2 1995/10/08 22:13:29 pascale $
 * Language:     C
 *
...
/*------------------------------------------------------------------------+*/
static XeString
err_mnemonic(unsigned int errn)
/*------------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
{
  ...
  /* Argument to time is type time_t */
  time_t t;

  if (spc_logging && spc_logF) {
    t = time(NULL);
    fprintf(spc_logF, "%s: %s", str, ctime(&t));
    fflush(spc_logF);
  }
