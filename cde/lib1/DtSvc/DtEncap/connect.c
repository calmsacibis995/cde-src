/*
 * File:         connect.c $XConsortium: connect.c /main/cde1_maint/1 1995/07/14 20:15:35 drk $
 * Language:     C
 *
...
/*------------------------------------------------------------------------+*/
{
   ...
   if (!domainname && (firstPass)) {
      firstPass = FALSE;
      hostinfo = gethostbyname(tmpbuf);
      if (!hostinfo) {
	 _DtSimpleError(XeProgName, XeWarning, NULL, (XeString) "><%s not found in hosts database", tmpbuf);
	 return -1;
      }
      if (ptr = strstr(hostinfo->h_name, (XeString)".")) /* if dot in canonical name */
	 domainname = strdup(ptr);
   }
...
/*------------------------------------------------------------------------+*/
{
   ...
   char env[MAXPATHLEN+10], *current_dir;
