/*
 * File:         noio.c $XConsortium: noio.c /main/cde1_maint/1 1995/07/14 20:16:17 drk $
 * Language:     C
 *
	...
	void)
{
   ...
   dir = malloc (strlen (pwent->pw_dir) +
		 strlen (SPCD_ENV_HOME_DIRECTORY) + 8);

   if (!dir)
      return (NULL);

   (void) sprintf (dir, "%s/%s/tmp", pwent->pw_dir, SPCD_ENV_HOME_DIRECTORY);
   if (is_dir_usable (dir))
      return (dir);

   (void) sprintf (dir, "%s/%s", pwent->pw_dir, SPCD_ENV_HOME_DIRECTORY);
   if (is_dir_usable (dir))
      return (dir);

   (void) sprintf (dir, "%s", pwent->pw_dir);
   if (is_dir_usable (dir))
      return (dir);
