 ...
 ** DtEnvMap.c
 **
 ** $XConsortium: DtEnvMap.c /main/cde1_maint/2 1995/08/26 22:46:04 barstow $
 **
    ...
    int   *mapListCount)
{
    ...
    *mapListCount = 0;
    ...
    while (1) {
	if (firstTime) {
	    tmpPtr2 = DtStrtok( tmpPtr, ":" );
	    firstTime = 0;
	}
	else
	    tmpPtr2 = DtStrtok( (char *) NULL, ":" );
    ...
    char *targetHost)
{
    ...
    while (1) {
	if (!pathListCount)
	    tmpPtr2 = DtStrtok( tmpPtr, separator );
	else
	    tmpPtr2 = DtStrtok( (char *) NULL, separator );
	...
	if (considerMapping) {
		...
		*tmpPtr = swapout;
		...
		if (tmpPtr2) {
		    ...
		    if ((mcnt = mblen(tmpPtr2, MB_CUR_MAX)) < 0) mcnt = 1;
		    *(tmpPtr2 + mcnt) = '\0';
		}
	    }
...
void _DtEnvMapForRemote (char *targetHost)
{
     ...
     */
    netfile = _DtEnv_tt_file_netfile(getenv("HOME"));
    if (tt_ptr_error(netfile) != TT_OK) {
	return;
    }

    host_file = _DtEnv_tt_host_netfile_file(targetHost, netfile);

    switch (tt_ptr_error(host_file))
    {
	case TT_OK:
            tt_free(host_file); /* free up tooltalk memory used for test */
            tt_free(netfile); /* free up tooltalk memory used for test */
	break;

	case TT_ERR_DBEXIST:	/* cannot contact remote host */
	case TT_ERR_NETFILE:	/* value  returned from tt_file_netfile() is invalid */
