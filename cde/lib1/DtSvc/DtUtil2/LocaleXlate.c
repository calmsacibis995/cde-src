/* $XConsortium: LocaleXlate.c /main/cde1_maint/4 1995/10/16 14:38:41 lehors $ */
/************************************<+>*************************************
 ****************************************************************************
...
#ifdef _NO_PROTO
static
void GetHomeDirPath( outptr, max)
         char * outptr;
         unsigned int    max;
#else    /*$DEF$*/
static
...
{        /*$CODE$*/
    ...
    if (ptr && strlen(ptr) < max) strcpy(outptr, ptr);
    else outptr[0] = '\0' ;
}        /*$END$*/
...
#ifdef _NO_PROTO
int _DtLcxXlateOpToStd( xlationDb, platform, version, operation, opValue,
       ret_stdLocale, ret_stdLangTerr, ret_stdCodeset, ret_stdModifier)
...
#ifdef _NO_PROTO
int _DtLcxXlateStdToOp( xlationDb, platform, version, operation,
            stdLocale, stdLangTerr, stdCodeset, stdModifier, ret_opValue)
       const _DtXlateDb   xlationDb;
       const char *       platform;
       const int          version;
       const char *       operation;
...
{       /*$CODE$*/
    ...
    stdValue[0] = EOS;
    if (stdLocale) strcpy(stdValue,stdLocale);
    else 
    {  
       int need = 0;
...
#define NEED_MODIFIER 0x04
       ...
       sprintf(stdValue,"%s%s%s%s%s", 
                     stdLangTerr, sepLC, stdCodeset, sepCM, stdModifier);
    }
    return _DtXlateStdToOpValue(xlationDb,platform,version,operation,stdValue,
                                  ret_opValue,NULL);
}       /*$END$*/
