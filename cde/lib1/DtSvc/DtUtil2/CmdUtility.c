/*
 * File:         CmdUtility.c $XConsortium: CmdUtility.c /main/cde1_maint/2 1995/09/06 02:15:40 lehors $
 * Language:     C
 *
...
static void 
#ifdef _NO_PROTO
GetWordWithQuotes( string, word, position )
	String string;
	String word;
	int *position;  /* MODIFIED - starts at beginning of word, ends at end of word */
#else
GetWordWithQuotes(
        String string,
        String word,
...
void 
#ifdef _NO_PROTO
_DtCmdStringToArrayOfStrings( theString, theArray )
	char theString[];	/* The string to parse. */
        char *theArray[];
#else
...
void 
#ifdef _NO_PROTO
_DtCmdFreeStringVector( stringv )
	char **stringv;      /* Pointer to an array of string pointers. */
#else
