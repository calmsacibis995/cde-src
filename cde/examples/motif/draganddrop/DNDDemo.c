...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: DNDDemo.c /main/cde1_maint/1 1995/07/14 13:58:42 drk $"
#endif
#endif
/*
...
static Widget
#ifdef _NO_PROTO
GetDragIcon(w, icon, iconMask, width, height, background, foreground)
Widget w;
Pixmap icon;
Pixmap iconMask;
Dimension width;
...
static Widget
#ifdef _NO_PROTO
GetDragIconFromBits(w, bits, mask, width, height, background, foreground)
Widget w;
char *bits;
char *mask;
Dimension width;
...
static void
#ifdef _NO_PROTO
TransferProcCallback(w, closure, seltype, type, value, length, format)
Widget w;
XtPointer closure ;
Atom *seltype ;
Atom *type ;
