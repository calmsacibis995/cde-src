/* $XConsortium: file.c /main/cde1_maint/1 1995/07/17 16:42:36 drk $ */
/*****************************************************************************
 *****************************************************************************
 **
 **   File:         file.c
        ...
        XtPointer       callData)
{
	...
	if (convertInfo->dragData->protocol != DtDND_FILENAME_TRANSFER ||
		 ...
		 */
                ...
                fileDraw = XtNameToWidget(topLevel, "*fileDraw");
	...
	Widget		fileDraw)
{
        ...
        WM_DELETE_WINDOW = XmInternAtom(XtDisplay(topLevel), "WM_DELETE_WINDOW",
                False);
        XmAddWMProtocolCallback(topLevel, WM_DELETE_WINDOW, fileShutdown,
                (XtPointer)NULL);
