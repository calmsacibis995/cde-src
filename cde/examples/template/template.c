/* $XConsortium: template.c /main/cde1_maint/2 1995/09/06 02:06:59 lehors $ */
/*
...
main(int argc, char **argv)
{
	...
	Fatal(catgets(msgCatalog, 1, 7, "ttdt_open failed"));
    }

    ttrc = ttmedia_ptype_declare(ToolTalkPType, 0, HandleTtMedia,
				 NULL, True);
    if (tt_is_err(ttrc)) {
	Fatal(catgets(msgCatalog, 1, 8, "ttmedia_ptype_declare failed"));
    }

    ttpat = ttdt_session_join(NULL, NULL, NULL, NULL, True);
    if (tt_ptr_error(ttpat) != TT_OK) {
	Fatal(catgets(msgCatalog, 1, 9, "ttdt_session_join failed"));
    }
