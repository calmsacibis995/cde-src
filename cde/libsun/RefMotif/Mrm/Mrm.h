 ...
 * Motif Release 1.2.3
*/ 
/*   $RCSfile: Mrm.h,v $ $Revision: 1.9 $ $Date: 94/04/19 07:43:31 $ */
 ...
 * up to a fullword. This is done to help satisfy alignment
 * request for MIPS, CRAY, and similar machines.  
 */
...
#define	URMPortableVersion	"URM 1.3"
...
typedef union {
	unsigned		external_id ;	/* Looks like a fullword */
        struct {
/* BEGIN OSF Fix CR 5122 */
#ifdef WORD64
          /* external_id is 64 bits, so these fields should sum to that
          ** number of bits, too; external_id must include the whole of
          ** internal_id
          */
          IDBRecordNumber       map_rec B32;    /* resource map record */
          IDBResourceIndex      res_index B32;  /* resource index into map */
#else
          /* assume unsigned is 32 bits and short ints are 16 */
...
typedef struct {
	...
	MrmCount	annex1 ;	/* annex allows compatible growth */
