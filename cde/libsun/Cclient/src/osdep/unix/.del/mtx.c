			    ...
			    unsigned long *len)
{
  ...
  *len = mtx_size (stream,msgno) - hdrsize;
		       ...
		       unsigned long *len,long flags)
{
  ...
  elt = mtx_elt (stream,msgno);
				/* make sure have a body */
  if (!(mtx_fetchstructure (stream,msgno,&b,NIL) && b && s && *s &&
			    ...
			    unsigned long *size)
{
  ...
  long msiz = mtx_size (stream,msgno);
				/* is size known? */
...
void mtx_setflag (MAILSTREAM *stream,char *sequence,char *flag,long flags)
{
  ...
  long i;
  unsigned long uf;
  short f = mail_parse_flags (stream,flag,&uf);
...
void mtx_snarf (MAILSTREAM *stream)
{
  ...
  if (sbuf.st_size) {		/* non-empty? */
	...
	iov[1].iov_base = cpystr (mail_fetchheader_full (sysibx, i, NIL,
							 (unsigned long *)&j, 
							 NIL));
	iov[1].iov_len = j;
	iov[2].iov_base = mail_fetchtext_full (sysibx, i,
					       (unsigned long *)&j,
					       NIL);
	iov[2].iov_len = j;
	...
	sprintf(LOCAL->buf + strlen (LOCAL->buf),",%ld;0000000000%02o\015\012",
		 iov[1].iov_len + iov[2].iov_len,(fDRAFT * elt->draft) +
		 (fOLD * !(elt->recent)) +
		   ...
		   STRING *message)
{
  ...
  long ret = LONGT;
  unsigned long uf = 0;
  short f = mail_parse_flags (stream,flags,&uf);
