 ...
 *
 * Edit log:
 * 	[10-21-96, clin] Passing stream in routine: auth_login_server.
 *			 and server_login.
 *			 
 * 	[10-31-96, clin] Passing a NIL for IMAPD_GLOBALS and stream in responder.
 */
#include "os_sv5.h"
			...
			NETMBX *mb,void *stream,unsigned long trial)
{
  ...
  fs_give ((void **) &challenge);
				/* prompt user */
  mm_login ((MAILSTREAM *)stream,mb,user,pwd,trial);
			...
			MAILSTREAM *stream)
{
  char *ret = NIL;
  char *user,*pass;
  if (user = (*responder) (PWD_USER,sizeof (PWD_USER),NIL, NIL, stream)) {
    if (pass = (*responder) (PWD_PWD,sizeof (PWD_PWD),NIL, NIL, stream)) {
      if (server_login (user,pass,argc,argv, stream)) ret = myusername (stream);
      fs_give ((void **) &pass);
    }
    fs_give ((void **) &user);
  }
