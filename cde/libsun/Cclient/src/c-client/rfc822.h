...
void	rfc822_header(char 	* header,
		      ENVELOPE 	* env,
		      BODY 	* body,
		      int 	  return_receipt,
		      HEADERLIST* header_list);

void 	rfc822_address_line(char	** header,
			    char 	*  type,
			    ENVELOPE 	*  env,
			    ADDRESS 	*  adr);

void 	rfc822_header_line(char 	** header,
			   char 	*  type,
			   ENVELOPE 	*  env,
			   char 	*  text);
...
void 	rfc822_write_body_header(char ** eader, BODY * body);
void 	rfc822_parse_msg(ENVELOPE 	** en,
			 BODY 		** bdy,
			 char 		*   s,
			 unsigned long 	i,
			 STRING 	*  bs,
...
ADDRESS *rfc822_parse_address(ADDRESS 	** lst,
			      ADDRESS 	*  last, char **string, 
			       char *defaulthost);
ADDRESS *rfc822_parse_group(ADDRESS 	** lst,
			    ADDRESS 	*  last,
			    char 	** string, 
			    char 	*  defaulthost);
...
typedef long(*soutr_t)(TCPSTREAM *stream, char *string,  MAILSTREAM *ms);

long 	rfc822_output(char 	* t,
		      ENVELOPE 	* env,
		      BODY 	* body,
		      soutr_t 	  f,
		      TCPSTREAM * s, 
...
long	rfc822_output_body(BODY 	* body,
			   soutr_t 	  f,
			   TCPSTREAM 	* s,
			   MAILSTREAM   * ms);
void *	rfc822_base64(unsigned char 	* src,
		      unsigned long 	  srcl,
		      unsigned long 	* len);
unsigned char *rfc822_binary(void 	*    src,
			     unsigned long   srcl,
			     unsigned long * len);
