...
DRIVER *nntp_isvalid (char *name,char *mbx)
{
  ...
  if (ret && (mbx[0] == '#')) {	/* namespace format name? */
	...
	(mbx[4] == 's') && (mbx[5] == '.')) memmove (mbx,mbx+6,strlen (mbx+6));
...

/* NNTP find list of all newsgroups
 * Accepts: mail stream
 *	    reference
 *	    pattern to search
 */
...
void nntp_list (MAILSTREAM *stream,char *ref,char *pat)
{
  ...
  if (nntp_canonicalize (ref,pat,pattern) &&
      ((stream && LOCAL && LOCAL->nntpstream) ||
       (stream = mail_open (NIL,pattern,OP_HALFOPEN,NIL))) &&
...

  if (nstream) {		/* now try to open newsgroup */
	...
	if ((k < nmsgs) &&	/* only bother if there appear to be holes */
	  ...
	  while ((s = tcp_getline (nstream->tcpstream)) && strcmp (s,".")) {
			...
			STRINGLIST *lines,unsigned long *len,long flags)
{
  ...
  if (!hdr) {			/* if don't have header already, must snarf */
    ...
    hdr = netmsg_slurp_text (LOCAL->nntpstream->tcpstream,&elt->data2);
			   ...
			   unsigned long *len,long flags)
{
  ...
  if (len) *len = 0;		/* no data returned yet */
  txt = stream->scache ? ((LOCAL->txn == msgno) ? (char *) LOCAL->txt : NIL) :
  ...
  if (!txt) {			/* if don't have header already, must snarf */
    ...
    if (stream->scache) {	/* get new current text if short caching */
      if (LOCAL->txt) fclose (LOCAL->txt);
      LOCAL->txt = netmsg_slurp (LOCAL->nntpstream->tcpstream,&elt->data4,NIL);
      LOCAL->hdn = msgno;
      ...
      (txt = netmsg_slurp_text (LOCAL->nntpstream->tcpstream,&elt->data4));
  }
...
SENDSTREAM *nntp_open (char **hostlist,long options)
{
  ...
  if (!(hostlist && *hostlist)) mm_log ("Missing NNTP service host",ERROR);
	...
	if ((reply == NNTPGREET) ||
      ...
      else if (reply != NNTPGREET) stream = nntp_close (stream);
    }
...
long nntp_mail (SENDSTREAM *stream,ENVELOPE *env,BODY *body)
{
  ...
  if (!(nntp_send (stream,"POST",NIL) == NNTPREADY)) return NIL;
				/* set up error in case failure */
  nntp_fake (stream,NNTPSOFTFATAL,"NNTP connection went away!");
  ...
  ret = tcp_soutr (stream->tcpstream,tmp) &&
    rfc822_output (tmp,env,body,nntp_soutr,stream->tcpstream, NIL) &&
...
long nntp_send (SENDSTREAM *stream,char *command,char *args)
{
  ...
  strcat (tmp,"\015\012");
				/* send the command */
  return tcp_soutr (stream->tcpstream,tmp) ? nntp_reply (stream) :
    nntp_fake (stream,NNTPSOFTFATAL,"NNTP connection went away!");
}
...
long nntp_reply (SENDSTREAM *stream)
{
  ...
  if (!(stream->reply = tcp_getline (stream->tcpstream)))
    return nntp_fake (stream,NNTPSOFTFATAL,"NNTP connection went away!");
