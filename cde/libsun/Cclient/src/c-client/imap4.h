 ...
 * Edit Log:
 *
 * [12-31-96 ] Add a new data structure: IMAP_GLOBALS.
 *
 */
...
#define IMAPLOOKAHEAD 20	/* fetch lookahead */
...
#define SEQTMPLEN 8*MAILTMPLEN
#define MAXAUTHENTICATORS 8

/*
 * MAX look ahead is the number of "n," sequences
 * we can stuff in the tmp buffer where n is 10
 * digits. Right now it is 409.
 * C'est assez bien pour longtemps! */
#define IMAPMAXLOOKAHEAD ((SEQTMPLEN-2)/10/2)
...
typedef struct imap_local {
  ...
  /* SUN - imap4 additions */
  unsigned int scache : 1;      /* LOCAL short cache flag */
  unsigned int csstatus: 1;	/* True is status checksum/validity */
  unsigned int sync_cache: 1;   /* True if syncronizing cache */
  unsigned int hdr_lines: 1;	/* True if RFC822.HEADER.LINES fetch */
  unsigned int sunvcap : 1;     /* True if IMAP4SUNVERSION capability */
  unsigned long sync_start;	/* For starting the search for UIDs */
  char *prefix;			/* find prefix */
  char tmp[IMAPTMPLEN];		/* temporary buffer */
  char sbuf[SEQTMPLEN];		/* second temp buffer */
} IMAPLOCAL;
...
typedef struct imap_argument {
  imap4Type_t 	  type;			/* argument type */
  void 		* text;			/* argument text */
} IMAPARG;


/* Data structure holds all the globals */

typedef struct imap_globals {
  long imap_maxlogintrials;
  long imap_lookahead;
  long imap_uidlookahead;
  long imap_defaultport;
  long imap_prefetch;
  long imap_closeonerror;
} IMAP_GLOBALS;  
...
long imap_status (MAILSTREAM *stream,char *mbx,long flags,void *userdata);
...
ENVELOPE *imap_fetchenvelope (MAILSTREAM *stream,unsigned long msgno,
			      long flags); /* 25-aout-96 */
...
long imap_append(MAILSTREAM 	* stream,
		 char 		* mailbox,
		 char 		* flags,
		 char 		* date,
		 char		* xUnixFrom,
...
IMAPPARSEDREPLY *imap_send_astring(MAILSTREAM *stream,char *tag,char **s,
				   char *t,unsigned long size,boolean_t wildok);
...
int imap_sunversion (MAILSTREAM *stream);
void imap_checkpoint(MAILSTREAM *stream, unsigned long min_msgno,long flags); 
...
void imap_fetchshort (MAILSTREAM *stream, unsigned long msgno, SHORTINFO *sinfo,
		     long flags);
...
void imap_parse_validchecksum(MAILSTREAM *stream, char *text);
void imap2_process_1flag (MAILSTREAM *stream, char pm, char *sequence, char *flag);
void imap4_process_1flag (MAILSTREAM *stream, char pm, char *sequence, char **flag);
void imap_parse_uidsequence (MAILSTREAM *stream, char *msgseq, char **txtptr);
void imap_parse_shortinfo (MAILSTREAM *stream,SHORTINFO *sinfo,	char **txtptr,
			   IMAPPARSEDREPLY *reply);
