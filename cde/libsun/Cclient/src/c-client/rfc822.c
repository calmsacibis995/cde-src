 ...
 *
 * Edits chez SunSoft (c):
 *   1. body->contents.msg.hdrsize = j; Bill Oct-14-96.
 *   2. "cc" ==> "Cc" ENCORE merde quoi. Bill 29-oct-96
 *   
 * [10-26-96, clin] Pass mailstream in rfc822_output and rfc822_output_body.
 * [10-28-96, clin] Make the last five globals "static".
 * [11-08-96, clin] Pass a NIL stream in all mm_log calls.
 * [11-19-96, clin] Pass a NIL stream in all fatal calls since the calling
 *			routines do not contain stream.
 * [01-31-97, clin] Integrate Bill's fix in rfc822_parse_content_header. 
 */
...
const char *errhst = ".SYNTAX_ERROR";	/* syntax error host string */


/* Body formats constant strings, must match definitions in mail.h */

const char *body_types[TYPEMAX+1] = {
  "TEXT", "MULTIPART", "MESSAGE", "APPLICATION", "AUDIO", "IMAGE", "VIDEO",
  "X-UNKNOWN"
};


const char *body_encodings[ENCMAX+1] = {
 ...
 *	    message body
 *          return receipt
 *          and a list of additional headers  LS 7/19/96
 */

void rfc822_header (char *header,ENVELOPE *env,BODY *body,int return_receipt,
...
void rfc822_parse_content (BODY *body,STRING *bs,char *h,char *t)
{
  ...
  case TYPEMESSAGE:		/* encapsulated message */
	...
	mm_log ("Ignoring nested encoding of message contents",PARSE,NIL);
				/* hunt for blank line */
      ...
      body->contents.msg.hdrsize = j;	/* header size: BY 14-oct-96 */
  ...
  case TYPEMULTIPART:		/* multiple parts */
	...
	|| (body->encoding == ENCOTHER))
      mm_log ("Ignoring nested encoding of multipart contents",PARSE,NIL);
				/* find cookie */
 ...
 *	    remainder of header
 * There is a code here for a MailTool MIME bug:
 *   MailTool will at times generate two Content-Type headers,
 *   where the first is wrong. It just has a type which itself
 *   is bogus, and NO subtype. In this case we try the 2nd.
 *     "Ouais, c'est completement bete!" Bill Yeager.
 */
...
void rfc822_parse_content_header (BODY *body,char *name,char *s)
{
  ...
  case 'D':			/* possible Content-Description */
    if (strcmp (name+1,"ESCRIPTION") == 0 && !body->description)
      body->description = strdup (s);
    break;
  case 'M':			/* possible Content-MD5 */
    if (!(strcmp (name+1,"D5")) || body->md5) body->md5 = cpystr (s);
    break;
  case 'T':			/* possible Content-Type/Transfer-Encoding */
				/* If there are TWO Content-Type headers */
				/* AND the first was bogus, try the second */
    if (strcmp (name+1,"YPE") == 0 && 
	(!(body->type || body->subtype ||  body->parameter) ||
	 (body->type && body->subtype && 
	  !strcmp(body->subtype,"UNKNOWN")))) {
      if (body->subtype && !strcmp(body->subtype,"UNKNOWN"))
	/* A 2nd content type. First bad. Free any parameters */
	if (body->parameter) mail_free_body_parameter(&body->parameter);
				/* get type word */
	...
	*name = '\0';		/* tie off attribute name */
	...
	if (!*s) *name = c;	/* must have an attribute name */
	  ...
	  if (body->parameter) { /* BUG FIX: 10-mai-96. YEAGER */
	    param->next = mail_newbody_parameter ();
	    param = param->next;
	  }
	...
	mm_log (tmp, PARSE, NIL); /* OK to pass a NIL here. -clin */
      }
    ...
    *ret = end;			/* update return pointer */
    ...
    if (end = rfc822_parse_word ((string = end),hspecials)) {
      ...
      *end = c;			/* restore delimiter */
      ...
      mm_log ("Missing or invalid host name after @",PARSE, NIL);
      adr->host = cpystr (".MISSING_HOST");
    }
  }
...
long rfc822_output (char *t,ENVELOPE *env,BODY *body,soutr_t f,TCPSTREAM *s,
		    int return_receipt, HEADERLIST *header_list, MAILSTREAM *ms)
{
  rfc822_header (t,env,body,return_receipt,header_list);   /* build RFC822 header */
                                                           /* output header and body */
  return (*f) (s,t, ms) && (body ? rfc822_output_body (body,f,s, ms) : T);
}
...
void rfc822_encode_body_7bit (ENVELOPE *env,BODY *body)
{
  ...
  case TYPEMULTIPART:		/* multi-part */
    if (!body->parameter) {	/* cookie not set up yet? */
      ...
      set_mime_boundary(tmp);   /* General call - MT safe */
      body->parameter = mail_newbody_parameter ();
  ...
  case TYPEMESSAGE:		/* encapsulated message */
    ...
    case ENC7BIT:
      break;
    case ENC8BIT:
      mm_log ("8-bit included message in 7-bit message body",WARN, NIL);
      break;
    case ENCBINARY:
      mm_log ("Binary included message in 7-bit message body",WARN, NIL);
      break;
    default:
      fatal ("Invalid rfc822_encode_body_7bit message encoding",NIL);
    }
  ...
  default:			/* all else has some encoding */
    ...
    case ENC8BIT:		/* encode 8BIT into QUOTED-PRINTABLE */
      ...
      body->contents.text = rfc822_8bit (body->contents.text,body->size.bytes,
					 &body->size.bytes);
      body->encoding = ENCQUOTEDPRINTABLE;
      (*body->contents_deallocate)(&f);	/* flush old binary contents */
...
void rfc822_encode_body_8bit (ENVELOPE *env,BODY *body)
{
  ...
  case TYPEMESSAGE:		/* encapsulated message */
    ...
    case ENC8BIT:
      break;
    case ENCBINARY:
      mm_log ("Binary included message in 8-bit message body",WARN,NIL);
      break;
    default:
      fatal ("Invalid rfc822_encode_body_7bit message encoding",NIL);
    }
...
long rfc822_output_body (BODY *body,soutr_t f,TCPSTREAM *s, MAILSTREAM *ms)
{
  PART *part;
  PARAMETER *param;
  char *cookie = NIL;
  ...
  case TYPEMULTIPART:		/* multipart gets special handling */
    ...
    do {			/* for each part */
      ...
      if (!((*f) (s,tmp,ms) && rfc822_output_body (&part->body,f,s, ms))) return NIL;
    } while (part = part->next);/* until done */
  ...
  default:			/* all else is text now */
    ...
    break;
				...
				/* output final stuff */
  if (t && *t && !((*f) (s,t, ms) && (*f) (s,"\015\012", ms))) return NIL;
  return LONGT;
}
...
unsigned char *rfc822_binary (void *src,unsigned long srcl,unsigned long *len)
{
  ...
  *d = '\0';			/* tie off string */
  if ((d - ret) != *len) fatal ("rfc822_binary logic flaw",NIL);
...
unsigned char *rfc822_qprint (unsigned char *src,unsigned long srcl,
			      unsigned long *len)
{
  unsigned char *ret = (unsigned char *) fs_get (srcl+1);
  unsigned char *d = ret;
  unsigned char *s = d;
  unsigned char c,e;
  *len = 0;			/* in case we return an error */
