...
/* The following extra configuration file has been added:
   OSVersion.tmpl
   This file, if present, is included before anything else and sets
   OSMajorVersion and OSMinorVersion.
*/
/* The following defines are used in any directory containing an
   Imakefile to allow local users to override global defines and macros.
   This program will include the following files if they are present.
   localdefines is included just after OSVersion.tmpl and should be used to 
   set defines.
   localmacros is included after everything else and should be used to
   set make macro values.
   admindefines is included after localdefines and should be used by the
   build administrator to changed defines in a directory.
   adminmacros is included just before localmacros and should be used by
   the build administrator to set make macro values.
 */
/* The following extra configuration file has been added:
   treedefines
   This file, if present, is included before Imake.tmpl
   and sets cpp  macros for a whole tree. This is for local use.
   Do not check this file in.
*/
#ifdef __hpux
#include "OSVersion.tmpl"
#endif

#include <stdio.h>
#if (defined(SVR4) || defined(_IBMR2) || defined(SYSV386)) && defined(__STDC__)
FILE * fdopen();
#endif
#include <ctype.h>
#include "Xosdefs.h"
...
#ifndef DEFAULT_CPP
# ifdef USE_CC_E
#   if (defined(__hpux) && ( OSMajorVersion >= 10 ))
#     define DEFAULT_CPP "/opt/ansic/bin/cc"
#   else
...
char    *Localdefs = "localdefines";
char    *Localmacros = "localmacros";
char    *Admindefs = "admindefines";
char    *Adminmacros = "adminmacros";
/* The following is used to get the correct path for included files
 * such as OSVersion.tmpl or BuildTree.name.
 * You must grab the TOPDIR passed to imake and append the appropriate
 * path.
 */
char    *ConfigDir = "/config/";
char     TopConfigPath[256];
char     OSVersionFilePath[256];
char     TreeTemplatePath[256];
char     BuildTreeNameFilePath[256];
char     *GlobalDefinesFile;
char     *GlobalMacrosFile;
char    *BuildTreeNameFile = "BuildTree.name";
char    *OSVersionFile = "OSVersion.tmpl";
char    *TreeTemplateFile = "treedefines";
/* The following set the macros for X11R4,R5,etc */
char    *R3macro = "#define Release3 3";
char    *R4macro = "#define Release4 4";
char    *R5macro = "#define Release5 5";
char    *R6macro = "#define Release6 6";

/* The following macros are for determining which HP architecture
 * You are running imake on.
 * This will allow you to ifdef for architectures inside Imakefiles,
 * config template files, etc.
 */
#ifdef __hp9000s700
   char    *HPSystemMacro = "#define __hp9000s700";
#else 
#  ifdef __hp9000s300
     char    *HPSystemMacro = "#define __hp9000s300";
#  else
#    ifdef __hp9000s800
       char    *HPSystemMacro = "#define __hp9000s800";
#    endif
#  endif
#endif
    ...
    char    **argv;
{
    ...
    for(argc--, argv++; argc; argc--, argv++) {
        ...
        if (argv[0][0] == '-') {
        if (argv[0][1] == 'D') {
                /* this is for OSVersion.tmpl,etc.  include */
        /* need to grab the top path */
            if (strncmp(&argv[0][2],"TOPDIR=",7) == 0)
                strcpy(TopConfigPath,&argv[0][9]);
            AddCppArg(argv[0]);
        } else if (argv[0][1] == 'I') {
            AddCppArg(argv[0]);
        } else if (argv[0][1] == 'f') {
    ...
    char    *outfname;
{
    ...
    if (pid) {  /* parent */
        ...
        /* place the macros for r4, r5 etc into the Makefile */ 
        fprintf(pipeFile, "%s\n",R3macro);
        fprintf(pipeFile, "%s\n",R4macro);
        fprintf(pipeFile, "%s\n",R5macro);
        fprintf(pipeFile, "%s\n",R6macro);
	    /* place the system macro for hp */
#ifdef __hpux
        fprintf(pipeFile, "%s\n",HPSystemMacro);
#endif
            /* Build the Build Tree name file Path */
        strcpy(BuildTreeNameFilePath,TopConfigPath);
    strcat(BuildTreeNameFilePath,ConfigDir);
    strcat(BuildTreeNameFilePath,BuildTreeNameFile);
            /* include BuildTree.name if present */
        if (!access(BuildTreeNameFilePath,R_OK))
        {
            fprintf(pipeFile, "#define BUILDTREENAMEFILE\t\"%s\"\n",
                BuildTreeNameFile);
            fprintf(pipeFile, "#include BUILDTREENAMEFILE\n");
        }
            /* include OSVersion.tmpl if present */
        strcpy(OSVersionFilePath,TopConfigPath);
        strcat(OSVersionFilePath,ConfigDir);
    strcat(OSVersionFilePath,OSVersionFile);
            /* include OSVersion.tmpl if present */
        if (!access(OSVersionFilePath,R_OK))
        {
            fprintf(pipeFile, "#define OSVERSIONFILE\t\"%s\"\n",
                OSVersionFile);
            fprintf(pipeFile, "#include OSVERSIONFILE\n");
        }
        else /* warn user that he should set OSVersion.tmpl */
        {
            printf("\nWarning! OSMajorVersion and OSMinorVersion defaulting to some\n\tUnknown default.  These should be set on a per-tree basis in \n\tconfig/OSVersion.tmpl. If you do not want to set \n\tOSMajorVersion and OSMinorVersion, create an empty \n\tOSVersion.tmpl in config.\n");
        }
            
    /* stuff for local defines */
        if (!access(Localdefs,R_OK))
        {
            printf("\nWarning! You are using a local file, %s, \n",Localdefs);
            printf("         to override global cpp macros.\n");
            fprintf(pipeFile, "#define LOCALDEFS\t\"%s\"\n",
                Localdefs);
            fprintf(pipeFile, "#include LOCALDEFS\n");
        }
        if (GlobalDefinesFile = getenv("IMAKE_DEFINES_FILE"))
        {
            if (!access(GlobalDefinesFile,R_OK)) {
                printf("\nWarning! The environment variable IMAKE_DEFINES_FILE is set:\n");
                printf("\tIncluding the file %s.\n",GlobalDefinesFile);
                fprintf(pipeFile, "#define GLOBALDEFSFILE\t\"%s\"\n",
                    GlobalDefinesFile);
                fprintf(pipeFile, "#include GLOBALDEFSFILE\n");
            }
        }
        if (!access(Admindefs,R_OK))
        {
            printf("\nWarning! There exists a file overwriting global cpp macros, %s.\n",Admindefs);
            fprintf(pipeFile, "#define ADMINDEFS\t\"%s\"\n",
                Admindefs);
            fprintf(pipeFile, "#include ADMINDEFS\n");
        }
            /* include treedefines if present */
        strcpy(TreeTemplatePath,TopConfigPath);
        strcat(TreeTemplatePath,ConfigDir);
        strcat(TreeTemplatePath,TreeTemplateFile);
        if (!access(TreeTemplatePath,R_OK))
        {
            fprintf(pipeFile, "#define TREETEMPLATEFILE\t\"%s\"\n",
                TreeTemplateFile);
            fprintf(pipeFile, "#include TREETEMPLATEFILE\n");
        }
        ...
        if (!access(Adminmacros,R_OK))
        {
            printf("\nWarning! There exists a file overwriting global make macros, %s.\n",Adminmacros);
            fprintf(pipeFile, "#define ADMINMACROS\t\"%s\"\n",
                Adminmacros);
            fprintf(pipeFile, "#include ADMINMACROS\n");
        }
        if (GlobalMacrosFile = getenv("IMAKE_MACROS_FILE"))
        {
            if (!access(GlobalMacrosFile,R_OK)) {
                printf("\nWarning! The environment variable IMAKE_MACROS_FILE is set:\n");
                printf("\tIncluding the file %s.\n",GlobalMacrosFile);
                fprintf(pipeFile, "#define GLOBALMACROSFILE\t\"%s\"\n",
                    GlobalMacrosFile);
                fprintf(pipeFile, "#include GLOBALMACROSFILE\n");
            }
        }
        if (!access(Localmacros,R_OK))
        {
            printf("\nWarning! You are using a local file, %s, \n",Localmacros);
            printf("         to override global make macros.\n\n");
            fprintf(pipeFile, "#define LOCALMACROS\t\"%s\"\n",
                Localmacros);
            fprintf(pipeFile, "#include LOCALMACROS\n");
        }
        fclose(pipeFile);
        while (wait(&status) > 0) {
            errno = 0;
