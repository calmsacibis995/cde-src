...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: UilP2Reslv.c /main/cde1_maint/1 1995/07/14 20:53:57 drk $"
#endif
#endif
...

{
	...
	*/
	if (((unsigned) object_entry->header.b_type != object_type) &&
	    (uil_gadget_variants[object_entry->header.b_type] != object_type) &&
	    ((unsigned) uil_gadget_variants[object_type] !=
	     object_entry->header.b_type))
	    {
	    diag_issue_diagnostic
		(d_ctx_req,
		 _sar_source_pos2(fwd_entry),
