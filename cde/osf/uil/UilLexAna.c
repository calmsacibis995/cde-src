...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: UilLexAna.c /main/cde1_maint/1 1995/07/14 20:51:54 drk $"
#endif
#endif
...
#include <Xm/XmosP.h>	/* Need this for _XmOSSetLocale, MB_CUR_MAX */
...
found_token:
  ...
  case token_comment:       /* RAP preserve comments */
	...
	comment_text[0]=0;
      ...
      size = strlen((char *)az_current_lex_buffer->c_text)+1;
...

{
    ...
    for (i=0;  i<length;  i++)
    {
	if ((class_table[ (int) buffer[ i ] ] == class_illegal) ||
