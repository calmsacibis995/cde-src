...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: UilLstMac.c /main/cde1_maint/2 1995/08/29 17:29:03 drk $"
#endif
#endif
...
#define _Offset(type,base,field)    \
        ((unsigned long)(base + XtOffsetOf(type, field)))

#define _OffsetArray(type,base,field)    \
        ((unsigned long)((short)(base + XtOffsetOf(type, field))))
#else
#define _Offset(type,base,field)    \
	((unsigned long) (((char *) (& (((type *) NULL)->field))) + base))
...

{
    ...
    */
	...
	case sym_k_rgb_value:  /* rgb data type RAP */
	    ...
	    for (offset = 0; offset < rc_size; offset += sizeof(int), index++)
		{
...
for ( sym_code=sym_k_error_object+1;
      ...
      sym_code++ )
    if ( (unsigned) uil_widget_compr[sym_code] == class )
