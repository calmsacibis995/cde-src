...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: UilSarObj.c /main/cde1_maint/1 1995/07/14 20:55:26 drk $"
#endif
#endif
...
*/
    if ( (unsigned) obj_entry->header.b_type != uil_sym_user_defined_object )
...
/*    Check if this name was previously defined for a different usage.	*/
	...
	if ( ref_entry->header.b_tag==sym_k_widget_entry ||
		...
		((ref_entry->header.b_type!=obj_entry->header.b_type) &&
		 ((unsigned) uil_gadget_variants[ref_entry->header.b_type]!=
		  obj_entry->header.b_type) &&
		 ((unsigned) uil_gadget_variants[obj_entry->header.b_type]!=
