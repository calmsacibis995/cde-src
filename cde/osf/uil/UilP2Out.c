...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: UilP2Out.c /main/cde1_maint/1 1995/07/14 20:53:43 drk $"
#endif
#endif
...
if( urm_status != MrmSUCCESS)
    ...
    ** creation routine
    */
    if ( (unsigned) widget_entry->header.b_type == uil_sym_user_defined_object )
	{
	widget_class_name = 
...

{
    ...
    */
	...
	case sym_k_compound_string_value:
	    value_size = XmStringLength(( XmString)value_entry->value.xms_value);
	    break;
...
/* BEGIN OSF Fix CR 4859 */
	case sym_k_wchar_string_value:
	    ...
	    break;
/* END OSF Fix CR 4859 */
	case sym_k_localized_string_value: /* delayed evaluation */
	    sem_evaluate_value_ls(value_entry);
	    /* fall through */
	...
	case sym_k_string_table_value:
		...
		{
		value_count++;
		value_size += XmStringLength (value_segment->value.xms_value)
		              + sizeof( RGMTextEntry );
		}
	    break;
	    }
    ...
    */
	...
	case sym_k_keysym_value:
	case sym_k_compound_string_value:
	    _move( buffer, value_entry->value.xms_value, value_size );
	    break;
...
/* END OSF Fix CR 4859 */
	...
	case sym_k_string_table_value:
	    ...
	    for (value_segment=value_entry->az_first_table_value;
		 ...
		 value_segment = value_segment->az_next_table_value)
		{
		segment_size = XmStringLength(value_segment->value.xms_value);
		text_offset -= segment_size;
		_move(&(buffer[text_offset]), 
		      value_segment->value.xms_value, segment_size);
		index--;
...

{
    ...
    case sym_k_localized_string_value: 	/* fall through */
