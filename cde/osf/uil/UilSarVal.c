...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: UilSarVal.c /main/cde1_maint/1 1995/07/14 20:56:04 drk $"
#endif
#endif
...
int	    value_type;
{
    ...
    */
    ...
    else if (value_type == sym_k_compound_string_value)
      {
...

{
	...
	/*
	**  If the attr_frame is not null, it must be a value frame with
	** contains a pointer to the value entry for the userdefined charset,
	** or a token frame which contains the charset token subclass.
	*/
...
/*
 * Fix for CR 5428 - The color table CANNOT be inline.  Check the colorTable's
 *                   obj_header.az_name.  If it is NULL, the color table was
 *                   created inline.  Print diagnostic.
 */
                 if (table_entry->obj_header.az_name == NULL)
                 {
                  diag_issue_diagnostic
                    ( d_wrong_type,
                      _sar_source_pos2(table_entry),
                      "inline color_table",
                      "color_table reference" );
		  target_type = sym_k_error_value;
		  table_entry = NULL;
		  return;
                 }
