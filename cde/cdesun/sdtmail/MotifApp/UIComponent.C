...
#pragma ident "@(#)UIComponent.C	1.8 11/27/96"
#endif
...
UIComponent::UIComponent (const char *name) : _w(NULL)
{
...
UIComponent::~UIComponent()
{
  ...
  if (_w) {
    ...
    XtRemoveCallback (_w, XmNdestroyCallback,
                      &UIComponent::widgetDestroyedCallback, (XtPointer) this);	
			   ...
			   const int numResources)
{
  ...
  if (XtParent(_w))
    XtGetSubresources (XtParent(_w), (XtPointer) this, _name,
                       className(), resources, numResources, NULL, 0);
  else 
    XtGetSubresources (_w , (XtPointer) this, _name, className(),
