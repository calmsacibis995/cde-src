...
#pragma ident "@(#)Application.C	1.35 05/31/97"
#endif
  ...
  for ( int i = 0; i < _numWindows; i++ )
    {
      ...
      _windows[i]->initialize(*argcp, argv);
...
void Application::unregisterWindow ( MainWindow *window )
{
  ...
  // Copy all objects, except the one to be removed, to the new list
  index = 0;
  for ( i = 0; i < _numWindows; i++ )
...
int
Application::windowExists(MainWindow  *window)
{
    for ( int i = 0; i < _numWindows; i++ )
    	if (_windows[i] == window)
		return 1;
