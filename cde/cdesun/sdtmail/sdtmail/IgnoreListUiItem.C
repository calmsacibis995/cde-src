...
#pragma ident "@(#)IgnoreListUiItem.C	1.22 11/12/96"
#endif
...
IgnoreListUiItem::IgnoreListUiItem(Widget w, 
		       int source, 
		       char *search_key,
		       Widget text_entry_widget):ListUiItem(w, source, search_key, (DtVirtArray<char *> *)NULL)
{
...
void handleIgnoreSelection(Widget w, XtPointer, XtPointer calldata)
{
  ...
  if (list_items != NULL) {
      // motif index is 1 based, virtarry is 0 based
      ...
      XtSetSensitive(od->h_field_delete, True);
      XtSetSensitive(od->h_field_change, True);
    }
}
...
//      list_len = char_list->length();
  ...
  if (list_items && list_items->length() > 0)
	...
	OptCmd *oc = theRoamApp.mailOptions();
        DtbOptionsDialogInfo od = oc->optionsDialog();
        XtSetSensitive(od->h_field_delete, False);
        XtSetSensitive(od->h_field_change, False);
  }
...
void IgnoreListUiItem::handleAddButtonPress()
{
  ...
  if (test_str && (i = strlen(test_str)) > 0) {
 	for (i--; i >= 0; i--) {
                if (test_str[i] == ' ') {
                        DtMailGenDialog *dialog = theRoamApp.genDialog();
                        dialog->setToErrorDialog(catgets(DT_catd, 3, 48, "Mailer"),
                                catgets(DT_catd, 15, 20, "A header field cannot contain spaces."));
                        dialog->post_and_return(catgets(DT_catd, 1, 1, "OK"), DTMAILHELPIGNORELISTNOSPACES);
                        free(test_str);
                        return;
                }
        }
      ...
      if (test_str && (i = strlen(test_str)) > 0) {
	    for (i--; i >= 0; i--) {
                if (test_str[i] == ' ') {
                        ...
                        dialog->setToErrorDialog(catgets(DT_catd, 3, 48, "Mailer"),
                                catgets(DT_catd, 15, 20, "A header field cannot contain spaces."));
                        dialog->post_and_return(catgets(DT_catd, 1, 1, "OK"), DTMAILHELPIGNORELISTNOSPACES);                        free(test_str);
                        return;
                }
            }
	    ...
	    XmListReplaceItemsPos(this->getWidget(), &replace_string,
				  1, pos_list[0]);
