...
DraftModuleDialogInfoRec	module_dialog = 
{
    False	 /* initialized */
};
 ...
 */
static int module_dialog_create(
    DraftModuleDialogInfo	instance,
    Widget	parent
);
static int module_dialog_button_panel_create(
    DraftModuleDialogInfo	instance,
    Widget	parent
);
static int module_controlpane_create(
    DraftModuleDialogInfo	instance,
    Widget	parent
);
static int module_draft_name_create(
    DraftModuleDialogInfo	instance,
    Widget	parent
);
static int module_separator_create(
    DraftModuleDialogInfo	instance,
    Widget	parent
);
static int module_save_button_create(
    DraftModuleDialogInfo	instance,
    Widget	parent
);
static int module_cancel_button_create(
    DraftModuleDialogInfo	instance,
    Widget	parent
);
static int module_help_button_create(
    DraftModuleDialogInfo	instance,
    Widget	parent
);

int 
DraftDialogInfo_clear(DraftModuleDialogInfo instance)
{
...
)
{
    ...
    module_dialog_create(instance,
        parent);
    module_dialog_button_panel_create(instance,
        instance->dialog_panedwin);
    module_controlpane_create(instance,
        instance->dialog_form);
    module_draft_name_create(instance,
        instance->controlpane);
    module_separator_create(instance,
        instance->controlpane);
    module_save_button_create(instance,
        instance->dialog_button_panel);
    module_cancel_button_create(instance,
        instance->dialog_button_panel);
    module_help_button_create(instance,
        instance->dialog_button_panel);
	...
	XtVaSetValues(instance->separator,
        XmNtopWidget, instance->draft_name_rowcolumn,
        NULL);
     ...
     */
    XtVaGetValues(instance->draft_name_rowcolumn,
     ...
     */
		...
		XmNactivateCallback, draft_save_butCB,
    		(XtPointer)&(*instance));
	XtAddCallback(instance->cancel_button,
		XmNactivateCallback, draft_cancel_butCB,
		(XtPointer)&(*instance));
 	XtAddCallback(instance->help_button,
                XmNactivateCallback, &HelpErrorCB,
                DTMAILHELPDRAFTDIALOGHELPBUTTON);
...
)
{
    ...
    if (instance->draftDialog == NULL) {
        n = 0;
        XtSetArg(args[n], XmNallowShellResize, True);  ++n;
        XtSetArg(args[n], XmNtitle, "Custom Dialog");  ++n;
        XtSetArg(args[n], XmNbackground, dtb_cvt_string_to_pixel(parent, "white"));  ++n;
        instance->draftDialog =
            XmCreateDialogShell(parent,
                "module_dialog", args, n);
    }
    if (instance->draftDialog == NULL)
        return -1;

    XtVaSetValues(instance->draftDialog,
                XmNtitle, catgets(DT_catd, 1, 291, "Mailer - Save As Draft"),
                NULL);

    if (instance->dialog_shellform == NULL) {
        instance->dialog_shellform =
            XtVaCreateWidget("module_dialog_shellform",
                xmFormWidgetClass,
                instance->draftDialog,
                XmNdefaultPosition, False,
                XmNmarginHeight, 0,
                XmNmarginWidth, 0,
                XmNbackground, dtb_cvt_string_to_pixel(instance->draftDialog, "white"),
                NULL);
    }
    if (instance->dialog_shellform == NULL)
        return -1;
...
static int 
module_dialog_button_panel_create(
    DraftModuleDialogInfo instance,
    Widget parent
)
{
...
)
{
    ...
    if (instance->draft_name_rowcolumn == NULL) {
        instance->draft_name_rowcolumn =
            XtVaCreateWidget("draft_name_rowcolumn",
    ...
    label_xmstring = XmStringCreateLocalized(catgets(DT_catd, 1, 333, "Draft Name:"));
    if (instance->draft_name_label == NULL) {
        instance->draft_name_label =
            XtVaCreateWidget("draft_name_label",
                xmLabelWidgetClass,
                instance->draft_name_rowcolumn,
    ...
    if (instance->draft_name == NULL) {
        ...
        instance->draft_name =
            XmCreateTextField(instance->draft_name_rowcolumn,
                "draft_name", args, n);
    }
    if (instance->draft_name == NULL)
        return -1;
