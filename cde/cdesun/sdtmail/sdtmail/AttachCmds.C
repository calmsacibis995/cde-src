...
#pragma ident "@(#)AttachCmds.C	1.15 09 Sep 1994"
#endif
...
void AttachAddCmd::ok( Widget , XtPointer callData )
{
	...
	for(i=0;i<count;i++) {
	    XmStringGetLtoR(string_table[i], XmSTRING_DEFAULT_CHARSET, 
					&filename);
	    add_file(filename);
	}
    }
...
void AttachFetchCmd::doit()
{
    ...
    unsigned long size;
    char error[1024];
    SdmBoolean compressed;

    if (_attachArea->get_mailbox()->get_disconnected() == Sdm_True)
	return;
...
// 	    XmStringGetLtoR(cbs->value, XmSTRING_DEFAULT_CHARSET, &label);
