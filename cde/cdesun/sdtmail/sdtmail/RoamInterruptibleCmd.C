...
#pragma ident "07/19/96 @(#)RoamInterruptibleCmd.C	1.6"
#else
static const char * __sccs_FILE = "07/19/96 @(#)RoamInterruptibleCmd.C	1.6";
#endif
...
RoamInterruptibleCmd::execute()
{
    ...
    _done  = Sdm_False;  // Initialize flag
    ...
    if (_done && !_interrupted)
	  ...
	  if ( _callback )  // the FALSE is to say it was not interrupted.
	      ( *_callback )( this, Sdm_False, _clientData );
      }
...
RoamInterruptibleCmd::workProc()
{
	...
	if ( _callback )  
	    ( *_callback )( this, Sdm_True, _clientData );
     }	 
