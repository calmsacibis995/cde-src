#ifndef lint
static char sccsid[] = "@(#)dither.c 1.8 96/05/29";
#endif
    ...
    unsigned long  pixel;
{
    ...
    for (i = ncolors; i > 0; i--) {
      ...
      colors[i].blue = colors[i-1].blue;
      colors[i].flags = colors[i-1].flags;
    }
 ...
 */
    ...
    *offset = image->offset;
    if (!background_found && *offset > 0 && *ncolors > 0) {
    ...
    if (!black_pixel_found && *offset > 0 && *ncolors > 0)
      if (*ncolors < maxcolors) 
    ...
    if (!white_pixel_found && *ncolors > 0) {
    ...
    int         depth;
{
    ...
    if ((XMatchVisualInfo (xdpy, DefaultScreen(xdpy), depth, image_display->visual->class, 
			   &vinfo)) == 0) {
      if (prog->verbose)
	fprintf (stderr, catgets (prog->catd, 1, 16,
		 "%s: Could not get %d-bit visual in set_cmap8.\n"), 
		 prog->name, depth);
      return (0);
    }
 ...
 */
	...
	if ((XMatchVisualInfo (xdpy, DefaultScreen(xdpy), depth, 
	    ...
	    if (prog->verbose) {
		fprintf (stderr, catgets (prog->catd, 1, 17,
		    "%s: Could not get %d bit visual in set_cmap24to8.\n"),
		    prog->name, depth);
	    }
	    return;
	}
	...
	if (XAllocColorCells (xdpy, image->cmap, 1, pmask, 0, pixels, 
	    ...
	    if (prog->verbose) {
	      fprintf (stderr, 
		   catgets (prog->catd, 1, 18,
			   "%s: XAllocColorCells failed in set_cmap24to8.\n"), 
		   prog->name);
	    }
	    return;
	}
    ...
    ImageInfo     *image;
{
    ...
    (*xil->lookup_get_values) (cmap, 0, 16, cmap_data);
	...
	if (prog->verbose) {
	    fprintf (stderr, 
		catgets (prog->catd, 1, 21,
			 "%s: Could not get 4-bit visual in set_cmap4.\n"), 
		prog->name);
	}
	return;
    }
	...
	if (prog->verbose) {
	    fprintf (stderr, 
		catgets (prog->catd, 1, 22,
			 "%s: XAllocColorCells failed in set_cmap4.\n"), 
		prog->name);
	}
	return;
    }
    ...
    int         depth;
{
    ...
    if ((XMatchVisualInfo (xdpy, DefaultScreen(xdpy), depth, 
      ...
      if (prog->verbose)
    	fprintf (stderr, 
		 catgets (prog->catd, 1, 23, 
		  "%s: Could not get %d-bit visual in set_grayscale_cmap.\n"), 
		 prog->name, depth);
      return (0);
    }
 ...
 */
  ...
  tmp = (*xil->create) (image_display->state, 256, 1, 3, XIL_BYTE);
  (*xil->export_ptr) (tmp);
  ...
  src_lut = (*xil->lookup_create) (image_display->state, XIL_BYTE, XIL_BYTE, 3,
			       image->colors, 0, cmap_data);
  dst_lut = (*xil->choose_colormap) (tmp, 216);
  tmp_lut = (*xil->lookup_convert) (src_lut, dst_lut); /* 0 - 216 */ 

  (*xil->lookup_destroy) (src_lut);
  (*xil->destroy) (tmp);

  dst = (*xil->create) (image_display->state, width, height, nbands, datatype);
  (*xil->lookup) (image->orig_image, dst, tmp_lut);

  image->colors = (*xil->lookup_get_num_entries) (dst_lut);
  ...
  (*xil->rescale) (dst, tmp_image, mult, offset);
  (*xil->destroy) (dst);
 ...
 */
    rgb_image = (*xil->create) (image_display->state, width, height, 3, XIL_BYTE);
    ...
    lookup = (*xil->lookup_create) (image_display->state, XIL_BYTE, XIL_BYTE,
			        3, image->colors, 0, data);
    (*xil->lookup) (image->orig_image, rgb_image, lookup);
    (*xil->lookup_destroy) (lookup);

    if (mask == NULL)
      mask = (*xil->dithermask_create) (image_display->state, 4, 4, 3, dmask);
    ...
    if (cmap == NULL) {
      ...
      cmap = (*xil->colorcube_create) (image_display->state, datatype, datatype, 
		                   3, image->offset, multipliers, dimensions);
    }

    image->colors = (*xil->lookup_get_num_entries) (cmap);

    if (current_display->depth == 4) 
      set_cmap4 (current_display->xdisplay, current_display->win, cmap, image);

    tmp_image = (*xil->create) (image_display->state, width, height, 1, datatype);
    (*xil->ordered_dither) (rgb_image, tmp_image, cmap, mask);
    ...
    lookup = (*xil->lookup_create) (image_display->state, XIL_BYTE, XIL_BYTE, 3,
				image->colors, 0, data);
    (*xil->lookup) (image->orig_image, rgb_image, lookup);
 ...
 */ 
    grayimage = (*xil->create) (image_display->state, width, height, 1, XIL_BYTE);  
    if (rgb_colorspace == NULL)
      rgb_colorspace = (*xil->colorspace_get_by_name)
                        (image_display->state, "rgblinear");
    (*xil->set_colorspace) (rgb_image, rgb_colorspace);

    if (y_colorspace == NULL)
      y_colorspace = (*xil->colorspace_get_by_name) 
                      (image_display->state, "ylinear");
    (*xil->set_colorspace) (grayimage, y_colorspace);

    (*xil->color_convert) (rgb_image, grayimage);
    (*xil->destroy) (rgb_image);
/*
 * Do thresholding on grayimage going from 8 to 1.
 */
    bit_image = (*xil->create) (image_display->state, width, height, 1, XIL_BIT); 
    lowvalue[0] = 0.0;
    highvalue[0] = THRESHOLD;
    mapvalue[0] = 0.0;
    (*xil->threshold) (grayimage, grayimage, lowvalue, highvalue, mapvalue);
      
    lowvalue[0] = highvalue[0]+1;
    highvalue[0] = 256.0;
    mapvalue[0] = 1.0;
    (*xil->threshold) (grayimage, grayimage, lowvalue, highvalue, mapvalue);
    (*xil->cast) (grayimage, bit_image);
    (*xil->destroy) (grayimage);
    /* Take this line out when you can use mult to be -1 (xil bug)*/
    (*xil->xor_const) (bit_image, &xor_value, bit_image);
/*
 ...
 */
    ...
    tmp_image = (*xil->create) (image_display->state, width, height, 1, XIL_BYTE);
    (*xil->cast) (bit_image, tmp_image);
    (*xil->destroy) (bit_image);
    (*xil->xor_const) (tmp_image, &xor_value, tmp_image); 
#endif
    ...
    XilLookup lut;
{
    ...
    image->offset = (*xil->lookup_get_offset) (lut); 
    image->colors = (*xil->lookup_get_num_entries) (lut);

    (*xil->lookup_get_values) (lut, image->offset, image->colors, lut_data);
