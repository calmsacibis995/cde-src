#ifndef lint
static char sccsid[] = "@(#)rast.c 1.8 97/05/15";
#endif
 ...
 */
static int read_encoded (int incount, u_char *out, int outcount);
    ...
    switch (rh->ras_type) {
       ...
       case RT_BYTE_ENCODED :  
	    if (lineb == scanline_stride)
	       ...
	       if (rh->ras_depth == 8)
		 encoded_len = lineb;
  	       else if (rh->ras_depth == 1)
	         encoded_len = rast_linelen (rh->ras_width, rh->ras_depth);
	       else
 ...
 */
    if (rh.ras_type == RT_BYTE_ENCODED) {
      ...
      /* Bug#  1212109
       *
       * Calculate the size of the image using linesize rather than
       * scanline_stride, because they are not always the same, especially
       * on images with an odd image->width.
       */
      size = linesize * image->height;
      bp = data;
	...
	/* Bug#  1212109
	 *
	 * Pad out to linesize (always even). This character is always
	 * ignored by programs reading in the encoded raster file.
	 */
        else if ( scanline_stride != linesize &&
		  i % linesize == linesize -1 )
	  c=0x00;
	else 
          c = *bp++;
