#ifndef lint
static char sccsid[] = "@(#)jfif.c 1.6 96/05/29";
#endif
...
ImageInfo	*image;
{
    ...
    image->cis = (*xil->cis_create) (image_display->state, "Jpeg");
    ...
    (*xil->cis_put_bits_ptr) (image->cis, image->file_size, 1, cis_data, 
		          jfif_done_with_data);
    
    xiltype = (*xil->cis_get_output_type) (image->cis);
    (*xil->imagetype_get_info) (xiltype, &width, &height, &nbands, &datatype); 
    ...
    (*xil->decompress) (image->cis, image->orig_image); 
 ...
 */
      ...
      (*xil->rescale) (image->orig_image, image->orig_image, scale, offset);
 ...
 */
       ...
       if (ycc_colorspace == NULL) 
          ycc_colorspace = (*xil->colorspace_get_by_name)
                                     (image_display->state, "ycc601");
       (*xil->set_colorspace) (image->orig_image, ycc_colorspace);

       tmp_image = (*xil->create) (image_display->state, image->width,
                               image->height, nbands, datatype);

       if (rgb_colorspace == NULL)
          rgb_colorspace = (*xil->colorspace_get_by_name)
                         	(image_display->state, "rgb709");
       (*xil->set_colorspace) (tmp_image, rgb_colorspace);
       (*xil->color_convert) (image->orig_image, tmp_image);
...
ImageInfo	*image;
{
    ...
    new_image = (*xil->create) (image_display->state, width, height, nbands,
			    datatype);
    
    if (rgb_colorspace == NULL)
      rgb_colorspace = (*xil->colorspace_get_by_name)
      (image_display->state, "rgb709");
    (*xil->set_colorspace) (image->orig_image, rgb_colorspace);

    if (ycc_colorspace == NULL) 
      ycc_colorspace = (*xil->colorspace_get_by_name)
      (image_display->state, "ycc601");
    (*xil->set_colorspace) (new_image, ycc_colorspace);
    
    (*xil->color_convert) (image->orig_image, new_image);
 ...
 */
    if (nbands == 3) {
      ...
      (*xil->rescale) (new_image, new_image, scale, offset);
    }
 
    (*xil->compress) (new_image, image->cis);
 ...
 */
    ...
    while ((*xil->cis_has_data) (image->cis) > 0) {
        
      image_data = (unsigned char *) (*xil->cis_get_bits_ptr) (image->cis,
							   &nbytes, &frames);
