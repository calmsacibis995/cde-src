#ifndef lint
static char sccsid[] = "@(#)tiff.c 1.6 96/05/29";
#endif
 ...
 */
    ...
    if (TIFFGetField (tif, TIFFTAG_BITSPERSAMPLE, &bitspersample) > 0) {
       if (bitspersample > 8) {
          if (prog->verbose)
             fprintf (stderr, catgets (prog->catd, 1, 346,
		      "tiff.c: 8-bits per sample max permitted\n"));
          TIFFClose(tif);
          return (-1);
	  }
       }
    ...
    if (TIFFGetField (tif, TIFFTAG_SAMPLESPERPIXEL, &samplesperpixel) > 0) {
	   ...
	   (samplesperpixel != 4) ) {
          if (prog->verbose)
             fprintf (stderr, catgets (prog->catd, 1, 347,
		      "tiff.c : 1,3 or 4 chanel images only allowed\n")); 
          TIFFClose(tif);
          return (-1);
	  }
       }
...
/*
       if (prog->verbose)
           printf (stderr, catgets (prog->catd, 1, -1,
	                   "tiff.c : No width specified\n"));
*/
       TIFFClose(tif);
       return (-1);
       }
...
*/
    ...
    switch (photometric) {
       ...
       default:
	    if (prog->verbose)
  	      fprintf (stderr, catgets (prog->catd, 1, 348,
		       "tiff.c: This type of image not supported\n"));
   	    TIFFClose (tif);
	    return (-1);
       }
 ...
 */
    ...
    if (buf == NULL) {
       if (prog->verbose)
         fprintf (stderr, catgets (prog->catd, 1, 349,
				   "tiff.c: Buf malloc failed\n"));
       TIFFClose (tif);
       return (-1);
       }
...
*/
	...
	iptr += scanline_stride;
	   ...
	   if (prog->verbose)
             fprintf (stderr, catgets (prog->catd, 1, 350,
		      "%d: Error writing TIFF scanline\n"), prog->name);
