#ifndef lint
static char sccsid[] = "@(#)ppm.c 1.6 95/04/26";
#endif
...
pbm_getc()
{
	...
	fprintf (stderr, catgets (prog->catd, 1, 150,
				  "%s: EOF / read error in pbm_getc.\n"), 
		 prog->name);
    ch = (char) ich;
	...
	do
	    ...
	    ich = im_fgetc( );
	    if ( ich == EOF && prog->verbose )
		fprintf (stderr, catgets (prog->catd, 1, 151,
			 "%s: EOF / read error in pbm_getc.\n"), prog->name);
	    ch = (char) ich;
	    }
	while ( ch != '\n' && ch != '\r' );
...
pbm_getint( )
{
	...
	fprintf (stderr, catgets (prog->catd, 1, 152,
		 "%s: Junk in file where an integer should be in pbm_getint.\n"),
		 prog->name);
    ...
    pixval* maxvalP;
{
    ...
    *rowsP = pbm_getint( );
    ...
    if ( maxval > PPM_MAXMAXVAL && prog->verbose )
        fprintf (stderr, catgets (prog->catd, 1, 153,
		 "%s: Maxval is too large - try reconfiguring with PGM_BIGGRAYS or without PPM_PACKCOLORS\n"), 
		 prog->name);
    *maxvalP = maxval;
}
    ...
    gray* maxvalP;
{
    ...
    *rowsP = pbm_getint( );
	...
	fprintf (stderr, catgets (prog->catd, 1, 154,
		 "%s: Maxval is too large - try reconfiguring with PGM_BIGGRAYS\n"), 
		 prog->name);
    *maxvalP = maxval;
}
...
/*
    if ( ich1 == EOF )
	fprintf( stderr, catgets (prog->catd, 1, -1, 
	         "EOF / read error reading magic number 1\n" ));
*/
    ich2 = im_fgetc( );
/*
    if ( ich2 == EOF )
	fprintf( stderr, catgets (prog->catd, 1, -1,
	         "EOF / read error reading magic number 2\n" ));
*/
    ...
    pixval* maxvalP;
{
	...
	*maxvalP = ppm_pbmmaxval;
	...
	default:
	if (prog->verbose)
 	  fprintf (stderr, catgets (prog->catd, 1, 155,
		   "%s: Bad magic number - not a ppm, pgm, or pbm file.\n"),
		   prog->name);
	return 0;
	}
    return 1;
...
pbm_getrawbyte( )
{
	...
	fprintf (stderr, catgets (prog->catd, 1, 156,
		 "%s: EOF / read error in pbm_getrawbyte.\n"),
		 prog->name);
...
pbm_getbit( )
{
	...
	fprintf (stderr, catgets (prog->catd, 1, 157,
		 "%s: Junk in file where bits should be in pbm_getbit.\n"),
		 prog->name);
    ...
    ImageInfo *image;
{
    ...
    (*xil->get_memory_storage) (image->orig_image, (XilMemoryStorage *)&storage);
    image->bytes_per_line = storage.scanline_stride;
    image_data = (unsigned char *)malloc(image->width * image->height *
	image->nbands);
    if (!image_data) {
	if (prog->verbose) {
	    fprintf(stderr, catgets (prog->catd, 1, 158,
		    "%s: Unable to allocate ppm data.\n"), prog->name);
	}
	return(0);
    }
    ...
    (*xil->set_memory_storage) (image->orig_image, (XilMemoryStorage *)&storage);
	...
	(*xil->multiply_const) (image->orig_image, &c, image->orig_image);
    }
