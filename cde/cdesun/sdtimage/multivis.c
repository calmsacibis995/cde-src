#ifndef lint
static char sccsid[] = "@(#)multivis.c	3.2 06 Aug 1992 Copyright 1987-1991 Sun Microsystem, Inc." ;
#endif
/*-
 ...
 *		used with snapshot.
 * 12-14-95	Phuong Nguyen 
 *		Made some changes to integrate this with sdtimage tool.
 */

#include <X11/Xlib.h>
#include "multivis.h"

static MVColmap *mvFindColormap(Colormap);
static XVisualInfo *mvMatchVisual(VisualID);

static void mvGetColormap(MVWinVisInfo *);
static void mvCalculateComposite(MVWinVisInfo *);

static unsigned long mvCompositePixel(unsigned long, MVColmap *);
...
static int num_vis;			/* Number of visuals. */
static XVisualInfo *vlist;		/* List of visuals. */
...
init_multivis ()
{
    ...
    vinfo_template.screen = DefaultScreen(current_display->xdisplay);
    vlist = XGetVisualInfo (current_display->xdisplay, VisualScreenMask, 
...
int wi, hi; 	/* size of requested rectangle */
{
    ...
    if (!XGetWindowAttributes(current_display->xdisplay, win, &xwa) || 
 ...
 */
    ...
    if (XQueryTree(current_display->xdisplay, win, &root, &parent, &children, &nchild)) {
       for (n=0; n<nchild; n++) {
...
mvDoWindowsFrontToBack()
{
    ...
    for (i=winList.used-1; ((nPixelsUnLabeled > 0) && i >= 0); i--) {
        pWI = &(winList.wins[i]);
	if (pWI->depth != 0) {
           if (!(xim = XGetImage(current_display->xdisplay, pWI->window,
                       pWI->x, pWI->y, pWI->width, pWI->height, (~0), ZPixmap))) 
              return;
 ...
 */
       ...
       XQueryColors(current_display->xdisplay, pCmp->cmap, pCmp->Colors, size);
       }
 ...
 * Assumes that a global vlist for this screen has already
 * been created. Uses globals num_vis, and vlist.
...
VisualID vid;
{
    XVisualInfo *pVis = vlist;

    while (pVis < (vlist+num_vis)) {
...
/*
 * Create an image data from the Img we have.
...
unsigned char *
CreateImageFromImg(ImageInfo *image)
{
    unsigned char		*data = NULL;
    register unsigned char	*im_ptr = NULL;
