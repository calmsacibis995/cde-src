...
main(int argc, char * argv[])
{
	...
	while (done == SAFalse) {
		...
		if (SAFileRead(&safile, buffer + bytes_left,
		    ...
		    if (bytes_left == 0) {
			DEBUGMSG(saplay, "no more data in file\n");
			if (total_bytes != safile.data_size) {
			    fprintf(stderr, "file size off by %d\n",
				    total_bytes - safile.data_size);
			}
