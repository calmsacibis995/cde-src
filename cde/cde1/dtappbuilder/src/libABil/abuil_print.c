/*
 *	$XConsortium: abuil_print.c /main/cde1_maint/2 1995/10/16 14:46:49 rcs $
 *
...
)
{
	...
	if (next_sibling != NULL)
	{
	    ...
	    {
		ret_val = -1;
		fprintf(stderr, "Bad prev_sibling 0x%x %s\n",
	...
	if (prev_sibling != NULL)
	{
	    ...
	    {
		ret_val = -1;
		fprintf(stderr, "Bad next_sibling 0x%x %s\n",
			obj, nullstr(obj_get_name(obj)));
	    }
	}
	...
	if (parent != NULL)
	{
	    ...
	    {
		ret_val = -1;
		fprintf(stderr,"Bad parent 0x%x %s\n",
		       obj, nullstr(obj_get_name(obj)));
	    }
	}
    }
