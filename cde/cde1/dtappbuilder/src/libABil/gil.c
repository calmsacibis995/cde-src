 ...
 * $Revision: 1.1 $
 * 
 * @(#)gil.c	1.14 04/17/96	cde_app_builder/src/libABil
 * 
...
{
    AB_CHOICE_TYPE	choiceType = AB_CHOICE_UNDEF;

    if (util_streq(s_choice_type, ":check"))
    {
	choiceType = AB_CHOICE_NONEXCLUSIVE;
    }
    else if (util_streq(s_choice_type, ":exclusive"))
    {
	choiceType = AB_CHOICE_EXCLUSIVE;
    }
    else if (util_streq(s_choice_type, ":nonexclusive"))
    {
	choiceType = AB_CHOICE_NONEXCLUSIVE;
    }
    else if (util_streq(s_choice_type, ":stack"))
    {
	choiceType = AB_CHOICE_OPTION_MENU;
    }
...
#define ott obj_type_table
    ...
    ott[AB_TYPE_LABEL] = istr_const(":message");
    /*ott[AB_TYPE_LABEL] = istr_const(":drop-target");*/
    ott[AB_TYPE_LAYERS]=	istr_const(":stack");
...
gilP_string_to_object_type(STRING string_type)
{
	...
	else if (util_streq(string_type, ":drop-target"))
	{
	    objType = AB_TYPE_LABEL;
	}
