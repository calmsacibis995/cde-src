...
/*
 * $XConsortium: gil_loadact.c /main/cde1_maint/3 1995/10/16 14:51:30 rcs $
 * 
...
#pragma ident "@(#)gil_loadact.c        1.20 94/02/13 SMI"
...
static BOOL	action_dup_exists(ABObj action, ABObj module);
static BOOL	action_equals(ABObj action1, ABObj action2);
...
gilP_load_att_actions(FILE * inFile, ABObj obj, ABObj module)
{
    ...
    while (!abio_get_list_end(inFile))
    {
	    ...
	    {
		/* unsupported :when value. Ferget it! */
		obj_destroy(action);
	    }
	    else
	    {
                if (install_action(obj, module, action) < 0)
		{
		    obj_destroy(action);
		}
	    }
...
install_action(ABObj obj, ABObj module, ABObj action)
{
    ...
    if (action_dup_exists(action, module))
    {
	/* this is a duplicate - don't install it. */
	/* if (debugging())
	{
 	    util_dprintf(2, "Dup action detected: \n");
	    obj_print_indented(action, 4, util_get_verbosity());
	} */
     ...
     * intra-module connections sit under the module
     */
...
static BOOL	
action_dup_exists(ABObj action, ABObj module)
{
    BOOL		dupExists = FALSE;
    ABObj		project = obj_get_project(module);
    AB_TRAVERSAL	trav;
    ABObj		dupAction = NULL;
    ABObj		from = obj_get_from(action);
    AB_WHEN		when = obj_get_when(action);
    ABObj		to = obj_get_to(action);
    AB_FUNC_TYPE	funcType = obj_get_func_type(action);

    for (trav_open(&trav, module, AB_TRAV_ACTIONS_FOR_OBJ);
	(!dupExists) && ((dupAction = trav_next(&trav)) != NULL); )
    {
	if (dupAction == action)
	{
	    continue;
	}
	dupExists = action_equals(action, dupAction);
    }
    trav_close(&trav);

    if ((!dupExists) && (project != NULL))
    {
...
{
    BOOL	isDup = FALSE;
    ABObj	from1 = obj_get_from(action1);
    AB_WHEN	when1 = obj_get_when(action1);
    ABObj	to1 = obj_get_to(action1);
    ABObj	from2 = obj_get_from(action2);
    AB_WHEN	when2 = obj_get_when(action2);
    ABObj	to2 = obj_get_to(action2);

	/* consider to==NULL and to==from to be equivalent */
	if (   (from1 == from2)
	    && (when1 == when2)
	    && (   (to1 == to2)
		|| ((to1 == NULL) && (to2 == from1))
	    ...
	    && (obj_get_func_type(action1) == obj_get_func_type(action2))
	   )
	{
	    switch (obj_get_func_type(action1))
	    {

            case AB_FUNC_BUILTIN:        /* ab-defined action */
		isDup = (obj_get_func_builtin(action1)
				== obj_get_func_builtin(action2));
	    break;

            case AB_FUNC_USER_DEF:       /* function name */
		isDup = util_streq(obj_get_func_name(action1),
					obj_get_func_name(action2));
	    break;

            case AB_FUNC_CODE_FRAG:      /* actual code entered by user */
		isDup = util_streq(obj_get_func_code(action1),
					obj_get_func_code(action2));
	    break;
