...
/*
 *      $XConsortium: bil_store.c /main/cde1_maint/3 1995/10/16 14:50:29 rcs $
 *
 *      @(#)bil_store.c	1.146 05/16/96
 *
...
)
{
    ...
    *buf = 0;
	...
	case AB_BIL_C_HEADER_FILE_EXTENSION:
	    return (obj_get_c_header_file_extension(obj) == NULL);

	case AB_BIL_C_IMPL_FILE_EXTENSION:
	    return (obj_get_c_impl_file_extension(obj) == NULL);
...
)
{
    ...
    /*
     * For separator objects:
     * If attachments exist on opposite sides, don't write out width/height
     */
    if (obj_is_separator(obj))
    {
	/*
	 * Check if attachments exist for left AND right sides
	 */
        if (attr == AB_BIL_WIDTH)
        {
	    AB_ATTACH_TYPE	ltype, rtype;

	    rtype = obj_get_attach_type(obj, AB_CP_EAST);
	    ltype = obj_get_attach_type(obj, AB_CP_WEST);

	    /*
	     * If both the right/left attachments are *not* either undef or none,
	     * skip the width
	     */
	    if (!((rtype == AB_ATTACH_UNDEF) || (rtype == AB_ATTACH_NONE)) && 
	        !((ltype == AB_ATTACH_UNDEF) || (ltype == AB_ATTACH_NONE)))
		return TRUE;
        }

	/*
	 * Check if attachments exist for top AND bottom sides
	 */
        if (attr == AB_BIL_HEIGHT)
        {
	    AB_ATTACH_TYPE	ttype, btype;

	    ttype = obj_get_attach_type(obj, AB_CP_NORTH);
	    btype = obj_get_attach_type(obj, AB_CP_SOUTH);

	    /*
	     * If both the top/bottom attachments are *not* either undef or none,
	     * skip the height
	     */
	    if (!((ttype == AB_ATTACH_UNDEF) || (ttype == AB_ATTACH_NONE)) && 
	        !((btype == AB_ATTACH_UNDEF) || (btype == AB_ATTACH_NONE)))
		return TRUE;
        }
    }

    /*
     * Don't write out width/height of layer objects.
     * It will shrink to fit it's child panes
     */
    if (obj_is_layers(obj) && ((attr == AB_BIL_WIDTH) || (attr == AB_BIL_HEIGHT)))
	return TRUE;
...
)
{
	...
	switch (type)
	{
			...
			AB_BIL_C_HEADER_FILE_EXTENSION,
			AB_BIL_C_IMPL_FILE_EXTENSION,
			AB_BIL_WRITE_MAIN_FUNC,
                        -1,
                };
...
)
{
    ...
    if ((iRet = abio_put_keyword(outFile, obj_type)) >= 0)
	if ((iRet = abio_putc(outFile, ' ')) >= 0)
	    if ((iRet = abio_put_keyword_name(outFile, obj_get_name(obj))) >= 0)
    		if ((iRet = abio_putc(outFile, '\n')) >= 0)
		   if ((iRet = abio_puts(outFile, abio_bil_object_begin_string())) >= 0)
		   {
			iRet = abio_putc(outFile, '\n');
	 ...
	 * type.
	 */
	for (attrp = get_object_attrs(obj); (iRet >= 0) && (attrp != NULL) && (*attrp != -1); 
		++attrp)
    	{
            iRet = store_attribute(outFile, obj, *attrp, save_type);
    	}
    	abio_set_indent(outFile, 0);

    	if ( !(obj_is_module(obj) || obj_is_project(obj)) && (iRet >= 0))
    	{
	    iRet = abio_puts(outFile, abio_bil_object_end_string());
    	}
    	if (iRet >= 0)
	    iRet = abio_putc(outFile, '\n');
    }
...
)
{
    ...
    iRet = abio_puts(outFile, attrbuf);
	...
	case AB_BIL_TYPE:
	    iRet = abio_puts(outFile, util_object_type_to_string(obj_get_type(obj)));
	    break;

	case AB_BIL_X:
 	    iRet = abio_puts(outFile, abio_integer_string(obj_get_x(obj)));
	    break;

	case AB_BIL_Y:
 	    iRet = abio_puts(outFile, abio_integer_string(obj_get_y(obj)));
	    break;

	case AB_BIL_WIDTH:
 	    iRet = abio_puts(outFile, abio_integer_string(obj_get_width(obj)));
	    break;

	case AB_BIL_HEIGHT:
 	    iRet = abio_puts(outFile, abio_integer_string(obj_get_height(obj)));
	    break;

	case AB_BIL_DRAG_CURSOR:
	    iRet = abio_put_string(outFile, obj_get_drag_cursor(obj));
	break;

	case AB_BIL_DRAG_CURSOR_MASK:
	    iRet = abio_put_string(outFile, obj_get_drag_cursor_mask(obj));
	break;

	case AB_BIL_DRAG_ENABLED:
	    iRet = abio_puts(outFile, 
		abio_bil_boolean_string(obj_get_drag_initially_enabled(obj)));
	break;

	case AB_BIL_DRAG_OPS:
	    iRet = store_dnd_ops(outFile, obj_get_drag_ops(obj));
	break;
	...
	case AB_BIL_DRAG_TYPES:
	    iRet = store_dnd_types(outFile, obj_get_drag_types(obj));
	break;

	case AB_BIL_DRAWAREA_WIDTH:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_drawarea_width(obj)));
	    break;

        case AB_BIL_DRAWAREA_HEIGHT: 
            iRet = abio_puts(outFile, abio_integer_string(obj_get_drawarea_height(obj))); 
            break; 
	...
	case AB_BIL_DROP_OPS:
	    iRet = store_dnd_ops(outFile, obj_get_drop_ops(obj));
	break;

	case AB_BIL_DROP_TYPES:
	    iRet = store_dnd_types(outFile, obj_get_drop_types(obj));
	break;

        case AB_BIL_BG_COLOR:
	    iRet = abio_put_string(outFile, obj_get_bg_color(obj));
	    break;

	case AB_BIL_FG_COLOR:
	    iRet = abio_put_string(outFile, obj_get_fg_color(obj));
	    break;

	case AB_BIL_ICON_LABEL:
            iRet = abio_put_string(outFile, obj_get_icon_label(obj));
	    break;

	case AB_BIL_LABEL:
	    iRet = abio_put_string(outFile, obj_get_label(obj));
	    break;
	...
	case AB_BIL_LABEL_TYPE:
	{
	    ...
	    iRet = abio_puts(outFile, bilP_label_type_to_string(lbl_type));
	    break;
	}

	case AB_BIL_LABEL_POSITION:
	    iRet = abio_puts(outFile, bilP_compass_point_to_string(obj_get_label_position(obj)));
	    break;
/*
	case AB_BIL_LABEL_STYLE:
	{
	    ...
	    if (lbl_style)
	    {
		iRet = abio_puts(outFile, bilP_label_style_to_string(lbl_style));
	    }
	    break;
	}
*/
	...
	case AB_BIL_FILES:
	{
	    ...
	    {
                if ((iRet = abio_puts(outFile, abio_bil_object_begin_string())) >= 0)
		    if ((iRet = abio_puts(outFile, bil_files)) >= 0)
			iRet = abio_puts(outFile, abio_bil_object_end_string());
		util_free(bil_files);
	    }
	    else
	...
	case AB_BIL_WIN_CHILDREN: 
	{
		...
		for (i = 0; i < numRefs && iRet >= 0; i++)
		{
			...
			{
			    if ((iRet = abio_puts(outFile, obj_get_name(refObj))) >= 0)
				iRet = abio_putc(outFile, '\n');
			}
			else
			{
			    refObj_mod_name = obj_get_name(refModule);
			    iRet = abio_printf(outFile, "%s.%s\n", 
					refObj_mod_name,
					obj_get_name(refObj));
			}
		    }
	...
	case AB_BIL_WIN_PARENT:
	    ...
	    if (win_parent == NULL)
	    {
		iRet = abio_puts(outFile, bilP_token_to_string(AB_BIL_UNDEF_KEYWORD));
		break;
	    }
...
>= 0)
	...
	case AB_BIL_BORDER_FRAME:
	    iRet = abio_puts(outFile, bilP_border_frame_to_string(obj_get_border_frame(obj)));
	    break;

	case AB_BIL_PACKING:
	    iRet = abio_puts(outFile, bilP_packing_to_string(obj_get_packing(obj)));
	    break;

	case AB_BIL_PROCESS_STRING:
            iRet = abio_put_string(outFile, obj_get_process_string(obj));
	    break;

	case AB_BIL_RES_FILE_TYPES:
	    iRet = store_res_file_types(outFile, obj);
	break;

	case AB_BIL_ROW_ALIGNMENT:
	    iRet = abio_puts(outFile, bilP_alignment_to_string(obj_get_row_align(obj)));
	    break;

	case AB_BIL_COLUMN_ALIGNMENT:
	    iRet = abio_puts(outFile, bilP_alignment_to_string(obj_get_col_align(obj)));
	    break;
...
/*
	case AB_BIL_HEIGHT_RESIZABLE:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_height_resizable(obj)));
	    break;

	case AB_BIL_WIDTH_RESIZABLE:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_width_resizable(obj)));
	    break;
*/
	case AB_BIL_RESIZABLE:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_resizable(obj)));
	    break;

	case AB_BIL_CONTAINER_TYPE:
	    iRet = abio_puts(outFile, bilP_container_type_to_string(obj_get_container_type(obj)));
	    break;

	case AB_BIL_MENU:
	    iRet = abio_puts(outFile, obj_get_menu_name(obj));
	    break;

        case AB_BIL_MENU_TITLE:
            iRet = abio_put_string(outFile, obj_get_menu_title(obj));
            break;
/*
	case AB_BIL_MENU_TYPE:
	    iRet = abio_puts(outFile, bilP_menu_type_to_string(obj_get_menu_type(obj)));
	    break;
*/

	case AB_BIL_GROUP_TYPE:
	    iRet = abio_puts(outFile, bilP_group_type_to_string(obj_get_group_type(obj)));
	    break;
	...
	case AB_BIL_LABEL_ALIGNMENT:
		...
		case AB_TYPE_BUTTON:
		case AB_TYPE_LABEL:
		    iRet = abio_puts(outFile, bilP_alignment_to_string(obj_get_label_alignment(obj)));
		    break;
	    }
	    break;

	case AB_BIL_NUM_COLUMNS:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_num_columns(obj)));
	    break;

	case AB_BIL_HATTACH_TYPE:
	    iRet = abio_puts(outFile, bilP_attachment_to_string(obj_get_hattach_type(obj)));
	    break;

	case AB_BIL_VATTACH_TYPE:
	    iRet = abio_puts(outFile, bilP_attachment_to_string(obj_get_vattach_type(obj)));
	    break;

	case AB_BIL_HOFFSET:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_hoffset(obj)));
	    break;

	case AB_BIL_HSPACING:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_hspacing(obj)));
	    break;

	case AB_BIL_REFERENCE_POINT:
	    iRet = abio_puts(outFile, bilP_compass_point_to_string(obj_get_ref_point(obj)));
	    break;

	case AB_BIL_NUM_ROWS:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_num_rows(obj)));
	    break;

        case AB_BIL_VENDOR:
            iRet = abio_put_string(outFile, obj_get_vendor(obj));
            break;

        case AB_BIL_VERSION:
            iRet = abio_put_string(outFile, obj_get_version(obj));
            break;
        ...
        case AB_BIL_ROOT_WINDOW:
	{
	    ...
	    if (root_win != NULL)
	    {
                ...
                if ((iRet = abio_puts(outFile, root_win_mod_name)) >= 0)
                   if ((iRet = abio_putc(outFile, '.')) >= 0)
                	iRet = abio_puts(outFile, obj_get_name(root_win));
            }
            break;
	}
	case AB_BIL_VISIBLE:
            iRet = abio_puts(outFile, abio_bil_boolean_string(obj_is_initially_visible(obj)));
	    break;

	case AB_BIL_VOFFSET:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_voffset(obj)));
	    break;

	case AB_BIL_VSPACING:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_vspacing(obj)));
	    break;

        case AB_BIL_BUTTON_TYPE:
	    iRet = abio_puts(outFile, bilP_button_type_to_string(obj_get_button_type(obj)));
	    break;

	case AB_BIL_C_HEADER_FILE_EXTENSION:
	    iRet = abio_put_string(outFile, obj_get_c_header_file_extension(obj));
	    break;

	case AB_BIL_C_IMPL_FILE_EXTENSION:
	    iRet = abio_put_string(outFile, obj_get_c_impl_file_extension(obj));
	    break;

	case AB_BIL_ORIENTATION:
	    iRet = abio_puts(outFile, bilP_orientation_to_string(obj_get_orientation(obj)));
	    break;

	case AB_BIL_DIRECTION:
	    iRet = abio_puts(outFile, bilP_direction_to_string(obj_get_direction(obj)));
	    break;

	case AB_BIL_SELECTION_REQUIRED:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_selection_required(obj)));
	    break;

	case AB_BIL_HSCROLLBAR:
	    iRet = abio_puts(outFile, bilP_scrollbar_policy_to_string(
	...
	case AB_BIL_IS_HELP_ITEM:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_is_help_item(obj)));
	    break;

	case AB_BIL_ITEM_TYPE:
	    iRet = abio_puts(outFile, bilP_item_type_to_string(obj_get_item_type(obj)));
	    break;

        case AB_BIL_ACCELERATOR:
	    iRet = abio_put_string(outFile, obj_get_accelerator(obj));
	    break;

        case AB_BIL_MNEMONIC:
            iRet = abio_put_string(outFile, obj_get_mnemonic(obj));
            break;

	case AB_BIL_TEAR_OFF:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_tearoff(obj)));
	    break;

	case AB_BIL_EXCLUSIVE:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_exclusive(obj)));
	    break;

	case AB_BIL_DECIMAL_POINTS:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_decimal_points(obj)));
            break;

  	case AB_BIL_INCREMENT:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_increment(obj)));
            break;

	case AB_BIL_MIN_VALUE:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_min_value(obj)));
	    break;

	case AB_BIL_MAX_VALUE:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_max_value(obj)));
	    break;

	case AB_BIL_TEXT_TYPE:
	    iRet = abio_puts(outFile, bilP_text_type_to_string(obj_get_text_type(obj)));
	    break;

	case AB_BIL_MAX_LENGTH:
	    iRet = abio_puts(outFile, abio_integer_string(obj_get_max_length(obj)));
	    break;
	...
	case AB_BIL_INITIAL_VALUE:
	{
		...
		iRet = abio_puts(outFile, abio_integer_string(ival));
	    } 
	    else if (type == AB_TYPE_TEXT_FIELD || type == AB_TYPE_TEXT_PANE)
	    {
		val = obj_get_initial_value_string(obj);
		if (val != NULL)
		    iRet = abio_put_string(outFile, val);
	    }
            break;
        }    

	case AB_BIL_READ_ONLY:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_read_only(obj)));
	    break;
        ...
        case AB_BIL_ACTION: 
	{
		...
		case AB_FUNC_BUILTIN:
		    iRet = abio_puts(outFile, bilP_builtin_action_to_string(
			obj_get_func_builtin(obj)));
		    break;
		case AB_FUNC_USER_DEF:
		    iRet = abio_puts(outFile, obj_get_func_name(obj));
		    break;
		case AB_FUNC_CODE_FRAG:
		    iRet = abio_put_string(outFile, obj_get_func_code(obj));
		    break;
	    }
	    break;
	}
	    ...
	    iRet = abio_puts(outFile, bilP_func_type_to_string(obj_get_func_type(obj)));
	    break;
        ...
        case AB_BIL_FROM:
	{
	    ...
	    {
                iRet = abio_puts(outFile, bilP_token_to_string(AB_BIL_APPLICATION));
	    }
	    else
	    {
            	type = obj_get_func_type(obj);
                   ...
                   case AB_FUNC_BUILTIN:
	           {
		    	...
		    	if (to_module == from_module)
	    	    	{
			    iRet = abio_puts(outFile, get_from_name(obj));
		    	}
		    	else  /* It is a cross-module connection */
		    	{
			    from_mod_name = obj_get_name(from_module);
			    if ((iRet = abio_puts(outFile, from_mod_name)) >= 0)
				if ((iRet = abio_putc(outFile, '.')) >= 0)
				    iRet = abio_puts(outFile, get_from_name(obj));
		    	}
		    	break;
	      ...
	      case AB_FUNC_BUILTIN:
		{
		    ...
		    if (to_module == from_module)  
		    { 
			iRet = abio_puts(outFile, get_to_name(obj));
		    }
		    else  /* It is a cross-module connection */ 
		    { 
			to_mod_name = obj_get_name(to_module);
			if ((iRet = abio_puts(outFile, to_mod_name)) >= 0)
			    if ((iRet = abio_putc(outFile, '.')) >= 0)
				iRet = abio_puts(outFile, get_to_name(obj)); 
		    }
		}
        ...
        case AB_BIL_WHEN:
	    iRet = abio_puts(outFile, bilP_when_to_string(obj_get_when(obj)));
	    break;
	
	case AB_BIL_ARG_TYPE:
	    iRet = abio_puts(outFile, bilP_arg_type_to_string(obj_get_arg_type(obj)));
	    break;
	...
	case AB_BIL_ARG_VALUE:
		...
		{
		    case AB_ARG_FLOAT:
			iRet = abio_put_float(outFile, obj_get_arg_float(obj));
			break;
		    case AB_ARG_INT:
			iRet = abio_put_integer(outFile, obj_get_arg_int(obj));
			break;
		    case AB_ARG_VOID_PTR:
		    case AB_ARG_STRING:
			if (obj_get_arg_string(obj))
			{
			    iRet = abio_put_string(outFile, 
	    ...
	    iRet = abio_puts(outFile, bilP_choice_type_to_string(obj_get_choice_type(obj))); 
            break; 
	...
	case AB_BIL_SELECTION_MODE:
	    iRet = abio_puts(outFile, bilP_selection_mode_to_string(obj_get_selection_mode(obj)));
	    break;

	case AB_BIL_DEFAULT:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_is_default(obj)));
            break;
	...
	case AB_BIL_CHILDREN:
	{
	    ...
	    {
	        if ((iRet = abio_puts(outFile, obj_get_name(child))) >= 0)
		    iRet = abio_putc(outFile, '\n');
	    }
	    trav_close(&trav);
	    abio_set_indent(outFile, 1);
	...
	case AB_BIL_NORTH_ATTACHMENT: 
	{
		...
		if ((iRet = abio_puts(outFile, bilP_attachment_to_string(attach_type))) >= 0)
		    iRet = abio_putc(outFile, ' ');
	    if (iRet >= 0)
	    {
	    switch (attach_type)
	    {
		    ...
		    iRet = abio_puts(outFile, obj_get_name(obj_get_root( (ABObj)obj_get_attach_value(obj, AB_CP_NORTH))));
		    break;
                case AB_ATTACH_GRIDLINE:
                case AB_ATTACH_CENTER_GRIDLINE:
		    iRet = abio_put_integer(outFile, (int)obj_get_attach_value(obj, AB_CP_NORTH));
		    break;
		case AB_ATTACH_POINT:
		case AB_ATTACH_NONE:
		    iRet = abio_put_integer(outFile, 0);
		    break;
            }
	    if (iRet >= 0)
	    {
		if ((iRet = abio_putc(outFile, ' ')) >= 0)
		    if (obj->attachments != NULL)
	            	iRet = abio_put_integer(outFile, obj_get_attach_offset(obj, AB_CP_NORTH));
		    else
			iRet = abio_put_integer(outFile, 0); 
		if (iRet >= 0)
		    iRet = abio_putc(outFile, ')');
	...
	case AB_BIL_SOUTH_ATTACHMENT:
	{
	    ...
	    if (attach_type != AB_ATTACH_UNDEF)
		iRet = abio_puts(outFile, bilP_attachment_to_string(attach_type));
	    if (iRet >= 0)
	    {
	    if ((iRet = abio_putc(outFile, ' ')) >= 0)
	    {
            switch (attach_type)
            {
                case AB_ATTACH_OBJ:
                case AB_ATTACH_ALIGN_OBJ_EDGE:
                    iRet = abio_puts(outFile, obj_get_name(obj_get_root((ABObj)obj_get_attach_value(obj, AB_CP_SOUTH))));
                    break;
                case AB_ATTACH_GRIDLINE:
                case AB_ATTACH_CENTER_GRIDLINE:
                    iRet = abio_put_integer(outFile, (int)obj_get_attach_value(obj, AB_CP_SOUTH));
                    break;
                default:
		    iRet = abio_put_integer(outFile, 0);
                    break;
	    ...
	    if ((iRet = abio_putc(outFile, ' ')) >= 0)
		if (obj->attachments != NULL)
                    iRet = abio_put_integer(outFile, obj_get_attach_offset(obj, AB_CP_SOUTH));
		else
		    iRet = abio_put_integer(outFile, 0);
	    if (iRet >= 0)
	        iRet = abio_putc(outFile, ')');
	...
	case AB_BIL_EAST_ATTACHMENT:
	{
	    ...
	    if ((iRet = abio_putc(outFile, ' ')) >= 0)
	    {
            switch (attach_type)
            {
                case AB_ATTACH_OBJ:
                case AB_ATTACH_ALIGN_OBJ_EDGE:
                    iRet = abio_puts(outFile, obj_get_name(obj_get_root((ABObj)obj_get_attach_value(obj, AB_CP_EAST))));
                    break;
                case AB_ATTACH_GRIDLINE:
                case AB_ATTACH_CENTER_GRIDLINE:
                    iRet = abio_put_integer(outFile, (int)obj_get_attach_value(obj, AB_CP_EAST));
                    break;
                default:
		    iRet = abio_put_integer(outFile, 0);
                    break;
	    ...
	    if ((iRet = abio_putc(outFile, ' ')) >= 0)
		if (obj->attachments != NULL)
		    iRet = abio_put_integer(outFile,obj_get_attach_offset(obj, AB_CP_EAST));
                else
		    iRet = abio_put_integer(outFile, 0);
	    if (iRet >= 0)
                iRet = abio_putc(outFile, ')');
	...
	case AB_BIL_WEST_ATTACHMENT:
	{
	    ...
	    if ((iRet = abio_putc(outFile, ' ')) >= 0)
	    {
            switch (attach_type)
            {
                case AB_ATTACH_OBJ:
                case AB_ATTACH_ALIGN_OBJ_EDGE:
                    iRet = abio_puts(outFile, obj_get_name(obj_get_root((ABObj)obj_get_attach_value(obj, AB_CP_WEST))));
                    break;
                case AB_ATTACH_GRIDLINE:
                case AB_ATTACH_CENTER_GRIDLINE:
                    iRet = abio_put_integer(outFile, (int)obj_get_attach_value(obj, AB_CP_WEST));
                    break;
                default:
		    iRet = abio_put_integer(outFile, 0);
                    break;
		...
		if ((iRet = abio_putc(outFile, ' ')) >= 0)
		    if (obj->attachments != NULL)
                    	iRet = abio_put_integer(outFile, obj_get_attach_offset(obj, AB_CP_WEST));
            	    else
		    	iRet = abio_put_integer(outFile, 0);
		if (iRet >= 0)
            	    iRet = abio_putc(outFile, ')');
	...
	case AB_BIL_ICON_FILE:
	    iRet = abio_put_string(outFile, obj_get_icon(obj));
	    break;

	case AB_BIL_ICON_MASK_FILE:
	    iRet = abio_put_string(outFile, obj_get_icon_mask(obj));
	    break;

	case AB_BIL_HELP_LOCATION:
	    if (!obj_is_action(obj))
		iRet = abio_put_string(outFile, obj_get_help_location(obj));
	    else
		iRet = abio_put_string(outFile, obj_get_func_help_location(obj));
	    break;

	case AB_BIL_HELP_TEXT:
	    iRet = abio_put_string(outFile, obj_get_help_text(obj));
	    break;

	case AB_BIL_HELP_VOLUME:
	    if (!obj_is_action(obj))
		iRet = abio_put_string(outFile, obj_get_help_volume(obj));
	    else
		iRet = abio_put_string(outFile, obj_get_func_help_volume(obj));
	    break;

	case AB_BIL_WORD_WRAP:
            iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_word_wrap(obj)));
            break;

	case AB_BIL_WRITE_MAIN_FUNC:
            iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_write_main_func(obj)));
            break;
	...
	case AB_BIL_ARROW_STYLE:
	    ...
	    break;
	case AB_BIL_ICONIC:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_is_initially_iconic(obj)));
	    break;

	case AB_BIL_ACTIVE:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_is_initially_active(obj)));
	    break;

	case AB_BIL_SHOW_VALUE:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_show_value(obj)));
            break; 

	case AB_BIL_FILTER_PATTERN:
	    iRet = abio_put_string(outFile, obj_get_filter_pattern(obj)); 
	    break;

	case AB_BIL_OK_LABEL:
	    iRet = abio_put_string(outFile, obj_get_ok_label(obj));
	    break;

	case AB_BIL_AUTO_DISMISS:
	    iRet = abio_puts(outFile, abio_bil_boolean_string(obj_get_auto_dismiss(obj)));
	    break;

	case AB_BIL_DIRECTORY:
	    iRet = abio_put_string(outFile, obj_get_directory(obj));
	    break;

	case AB_BIL_FILE_TYPE_MASK:
	    iRet = abio_puts(outFile, bilP_file_type_mask_to_string(obj_get_file_type_mask(obj)));
	    break;

	case AB_BIL_MESSAGE_TYPE:
	    iRet = abio_puts(outFile, bilP_msg_type_to_string(obj_get_msg_type(obj)));
	    break;
	
	case AB_BIL_MESSAGE: 
	    iRet = abio_put_string(outFile, obj_get_msg_string(obj));
	    break;
	...
	case AB_BIL_SESSION_MGMT:
	{
		...
		{
		    abio_set_indent(outFile, 2);
		    if ((iRet = store_attribute(outFile, obj, AB_BIL_SESSIONING_METHOD, save_type)) >= 0)
		    {
			abio_set_indent(outFile, 1);
			iRet = abio_puts(outFile, abio_bil_object_end_string());
		    }
	...
	case AB_BIL_I18N:
	{
		...
		{
		    abio_set_indent(outFile, 2);
	    	    if ((iRet = store_attribute(outFile, obj, AB_BIL_I18N_ENABLED, save_type)) >= 0)
		    {
			abio_set_indent(outFile, 1);
			iRet = abio_puts(outFile, abio_bil_object_end_string());
		    }
	...
	case AB_BIL_I18N_ENABLED:
		...
		{
		    abio_set_indent(outFile, 2);
            	    if ((iRet = store_attribute(outFile, obj, AB_BIL_TOOLTALK_DESKTOP_LEVEL, save_type)) >= 0)
		    {
            		abio_set_indent(outFile, 1);
            		iRet = abio_puts(outFile, abio_bil_object_end_string());
		    }
	...
	case AB_BIL_HELP_BUTTON:
		...
		ABObj help_b = obj_get_help_act_button(obj);
            	if (help_b != NULL)
                    iRet = abio_puts(outFile, obj_get_name(help_b));
        ...
        case AB_BIL_ACTION1_LABEL:
            iRet = abio_put_string(outFile, obj_get_action1_label(obj));
            break;

        case AB_BIL_ACTION2_LABEL:
            iRet = abio_put_string(outFile, obj_get_action2_label(obj));
            break;

        case AB_BIL_ACTION3_LABEL:
            iRet = abio_put_string(outFile, obj_get_action3_label(obj));
            break;
	...
	case AB_BIL_DEFAULT_BUTTON:
		...
		ABObj default_b = obj_get_default_act_button(obj);
            	if (default_b == NULL)
            	{
                    ...
                    iRet = abio_puts(outFile, obj_get_name(default_b));
            }
	    else
	    	iRet = abio_puts(outFile,
	...
	case AB_BIL_PANE_MIN_HEIGHT:
 	    iRet = abio_puts(outFile, abio_integer_string(obj_get_pane_min(obj)));
	    break;

	case AB_BIL_PANE_MAX_HEIGHT:
 	    iRet = abio_puts(outFile, abio_integer_string(obj_get_pane_max(obj)));
	    break;
    }
    if (iRet >= 0)
	iRet = abio_putc(outFile, '\n');
...
store_res_file_types(FILE *outFile, ABObj obj)
{
    ...
    iRet = abio_puts(outFile, abio_bil_object_begin_string());
    for (i = 0; (i < AB_ARG_CLASS_NUM_VALUES) && (iRet >= 0); ++i)
    {
	...
	if ((resFileArgClasses & argClassFlag) != 0)
	{
	    ...
	    if (iRet >= 0)
		if ((iRet = abio_puts(outFile, bilP_arg_class_to_string(argClass))) >= 0)
		    ++classCount;
	}
    }
    if (iRet >= 0)
...
store_dnd_ops(FILE *outFile, ABDndOpFlags opFlags)
{
    ...
    iRet = abio_puts(outFile, abio_bil_object_begin_string());
    for (flagCount = 0; (flagCount < 3) && (iRet >= 0); ++flagCount)
    {
	switch (flagCount)
	{
	    ...
	    if (iRet >= 0)
		if ((iRet = abio_puts(outFile, tokenString)) >= 0)
		    ++numWritten;
        }
    }
    if (iRet >= 0)
...
store_dnd_types(FILE *outFile, ABDndTypeFlags typeFlags)
{
    ...
    iRet = abio_puts(outFile, abio_bil_object_begin_string());
    for (flagCount = 0; (flagCount < 4) && (iRet >= 0); ++flagCount)
    {
	switch (flagCount)
	{
...
)
{
	...
	util_dabort(1);
#endif
    if ((iRet = bilP_write_tree_ui_objs(outFile, root, save_type)) >= 0)
	iRet = bilP_write_tree_conn_objs(outFile, root, save_type);

    if ((iRet >= 0) && (obj_is_project(root) || obj_is_module(root)))
    {
	/*
	* This ends the module (module) section of the BIL out file
	*/
	if ((iRet = abio_puts(outFile, abio_bil_object_end_string())) >= 0)
	    iRet = abio_putc(outFile, '\n');
    }
...
)
{
    ...
    {
	iRet = store_object(outFile, root, save_type);
    }
    for( trav_open(&trav, root, AB_TRAV_SALIENT_CHILDREN);
        ((child = trav_next(&trav)) != NULL) && (iRet >= 0); )
    {
	...
	iRet = bilP_write_tree_ui_objs(outFile, child, save_type);
    }
    trav_close(&trav);
    return iRet;
}
...
)
{
    ...
    for (attrp = get_object_attrs(obj); (*attrp != -1) && (iRet >= 0); attrp++)
	iRet = store_attribute(outFile, obj, *attrp, save_type);
...
)
{
	...
	((action_obj = trav_next(&trav)) !=NULL) && (iRet >= 0); ) 
    {
	...
	switch (type)  
	{
				...
				saveCrossModule) )
		{
		    ...
		    iRet = store_action(outFile, action_obj, save_type);
		}
		break;
	    case AB_FUNC_USER_DEF:
	    case AB_FUNC_CODE_FRAG:
...
)
{
    ...
    bil_list = (char*)util_malloc(MAXPATHLEN * sizeof(char));
    if (bil_list == NULL)
	return bil_list;
...
)
{
	...
	iRet = abio_open_bil_output(file_name, old_file, &bilOutFile);
	if (iRet != OK)
	{
	    goto epilogue;
	}
	 ...
	 */
	iRet = bilP_write_tree(bilOutFile, root, save_type);

	/*
	 * See if we need to write out the modules, as well.
	 */
	if ((iRet >= 0) && (save_type != BIL_SAVE_FILE_PER_MODULE) && (!obj_is_module(root)))
	{
	    ABObj		module = NULL;
	    AB_TRAVERSAL	trav;

	    for (trav_open(&trav, root, AB_TRAV_MODULES);
		((module = trav_next(&trav)) != NULL) && (iRet >= 0); )
	    {
		iRet = bilP_write_tree(bilOutFile, module, save_type);
	    }
	    trav_close(&trav);
	}

	/* write out end-of-data marker */
	if ((iRet >= 0) && (save_type == BIL_SAVE_ENCAPSULATED))
	{
	    iRet = abio_printf(bilOutFile, "%s\n", 
