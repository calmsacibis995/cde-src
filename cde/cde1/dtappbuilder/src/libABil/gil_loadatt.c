...
/*
 * $XConsortium: gil_loadatt.c /main/cde1_maint/3 1995/10/16 14:51:52 rcs $
 * 
...
#pragma ident "@(#)gil_loadatt.c        1.26 94/02/13 SMI"
...
/* string for a drop target object that gets converted to a label */
static STRING	dropTargetLabel = "DnD";
...
/*
 * convert from text field to text pane 
 */
static int
cvt_text_field_to_text_pane(ABObj text)
{
    /*
     * Grab relevant data (text field is one line of text)
     */
    int			num_columns = obj_get_num_columns(text);
    int			max_length = obj_get_max_length(text);
    ISTRING		initial_value_string = 
				istr_create(obj_get_initial_value_string(text));
    int			initial_value_int = 
		    		obj_get_initial_value_int(text);
    BOOL		has_border = obj_has_border(text);
    AB_COMPASS_POINT	label_position = obj_get_label_position(text);
    ISTRING		menu_title = istr_create(obj_get_menu_title(text));

    if (obj_get_type(text) == AB_TYPE_TEXT_PANE)
    {
	/* it's already a text pane! */
	return 0;
    }
    if (obj_get_type(text) != AB_TYPE_TEXT_FIELD)
    {
	/* we can't convert this... */
	return  -1;
    }

    obj_set_type(text, AB_TYPE_TEXT_PANE);

    obj_set_num_columns(text, num_columns);
    obj_set_max_length(text, max_length);
    obj_set_initial_value_string(text, istr_string(initial_value_string));
    obj_set_initial_value_int(text, initial_value_int);
    obj_set_has_border(text, has_border);
    obj_set_label_position(text, label_position);
    obj_set_menu_title(text, istr_string(menu_title));

    istr_destroy(initial_value_string);
    istr_destroy(menu_title);
...
/*
 * Rearranges items so that the match the order of the list of labels.
 */
static int
match_items_with_labels(ABObj obj, ISTRING_ARRAY *labels)
{
    ABObj		item = NULL;
    ABObj		itemParent = NULL;
    ABObj		labelItem = NULL;
    int			labelIndex = 0;
    ...
    itemParent = obj_get_parent(item);	/* *immediate* parent of item */

    labelIndex = -1;
    for (labelIndex = 0; labelIndex < numLabels; ++labelIndex)
    {
	label = labels->strings[labelIndex];
	labelItem = obj_find_child_by_label(itemParent, istr_string(label));
	if (labelItem != NULL)
	{
	    /* move this item left */
	    for (i = numLabels+1; 
	         (--i >= 0) && (obj_get_item_num(labelItem) > labelIndex); )
	    {
	        childNum = obj_get_child_num(labelItem);
		if (childNum > 0)
		{
	            obj_insert_child(itemParent, labelItem, childNum-1);
		}
	    }
	    /* move this item right */
	    for (i = numLabels+1; 
	         (--i >= 0) && (obj_get_item_num(labelItem) < labelIndex); )
	    {
	        childNum = obj_get_child_num(labelItem);
		if (childNum < (obj_get_num_children(itemParent)-1))
		{
	            obj_insert_child(itemParent, labelItem, childNum+1);
		}
	    }
...
static int
set_items_initially_selected(ABObj obj, BOOL_ARRAY * selects)
{
    ...
    {
        obj_set_is_initially_selected(item, selects->bools[i]);
    }
    return 0;
}
...
set_item_labels(ABObj obj, ISTRING_ARRAY * strings)
{
    ...
    match_items_with_labels(obj, strings);
    for (trav_open(&trav, obj, AB_TRAV_ITEMS), i = 0;
         (item = trav_next(&trav)) != NULL; ++i)
    {
	...
	else
	{
	    ...
	    /* don't replace non-empty label with empty label */
	    if (! (   util_strempty(label) 
		   && (!util_strempty(obj_get_label(item)))))
	    {
	...
	/*
	 *
	 * REMIND: objxm_config() does not seem to handle toggle
	 *         buttons with label = NULL, or "" (no big deal,
	 *         one space seems to work well).
	 */
  	if (   (obj_get_label_type(item) == AB_LABEL_STRING)
	    && (util_strempty(obj_get_label(item))) )
	{
	    obj_set_label(item, " ");
	}
	...
	/*
	 * Finish defining this beastie
	 */
	if (obj_get_item_type(item) == AB_ITEM_FOR_UNDEF)
	{
	    obj_set_item_type(item, ab_get_proper_item_type(obj_get_type(obj)));
	}
	obj_set_is_defined(item, TRUE);		/* don't forget this! */
    }
    trav_close(&trav);
    return 0;
}
...
/*
 * GIL borizontal-spacing maps to BIL hoffset, so we ignore :horizontal-offset
 */
static int
load_att_hoffset(FILE * inFile, ABObj obj, ABObj root_obj)
{
    ...
    if (!abio_get_integer(inFile, &hoffset))
        return (abil_print_load_err(ERR_WANT_INTEGER), -1);
...
{
    int		hspacing = 0;
    if (!abio_get_integer(inFile, &hspacing))
        return (abil_print_load_err(ERR_WANT_INTEGER), -1);

    obj_set_hoffset(obj, hspacing);
...
load_att_initial_list_glyphs(FILE * inFile, ABObj obj, ABObj root_obj)
{
    int                 	return_value = 0;
    int                 	rc = 0; /* r turn code */
    ISTRING_ARRAY       	glyphNames;
    AB_TRAVERSAL		trav;
    ABObj			item = NULL;
    int				itemNum = 0;

    if ((rc = gilP_load_strings(inFile, &glyphNames)) < 0)
    {
        return rc;
    }
    return_value = rc = set_item_labels(obj, &glyphNames);

    /*
     * For those items that actually had their glyph labels set, mark them
     * as glyphs
     */
    for (itemNum = -1, trav_open(&trav, obj, AB_TRAV_ITEMS);
      (++itemNum < glyphNames.count) && ((item = trav_next(&trav)) != NULL); )
    {
	if (((int)istr_len(glyphNames.strings[itemNum])) > 0)
	{
	    obj_set_label_type(item, AB_LABEL_GLYPH);
	}
    }
    trav_close(&trav);
...
load_att_initial_selections(FILE * inFile, ABObj obj, ABObj root_obj)
{
    ...
    return_value = rc = set_items_initially_selected(obj, &bools);
    return return_value;
}
...
load_att_initial_value(FILE * inFile, ABObj obj, ABObj root_obj)
{
    ...
    else if (abio_get_integer(inFile, &tmp_int))
    {
	value_is_int = TRUE;
    }
    else
    ...
    if (value_is_int)
    {
	obj_set_initial_value_int(obj, tmp_int);
    }
    else
    {
...
load_att_label(FILE * inFile, ABObj obj, ABObj root_obj)
{
    ...
    if (   (tmp_str != NULL)
	&& (!(   obj_is_label(obj)
	      && util_streq(obj_get_label(obj), dropTargetLabel)
	      && (((int)istr_len(tmp_str)) < 1)))
       )
    {
	/* if it was a drop target, and the :label property is empty, 
	 * leave it with the default label.
	 */
...
load_att_label_type(FILE * inFile, ABObj obj, ABObj root_obj)
{
    ...
    if ((label_type = gilP_string_to_label_type(istr_string(tmp_str))) == ERROR)
        return (abil_print_load_err(ERR_UNKNOWN), -1);
    obj_set_label_type(obj, label_type);
    istr_destroy(tmp_str);
    return 0;
}
...
load_att_layout_type(FILE * inFile, ABObj obj, ABObj root_obj)
{
	...
	{
	    /* if we've set rows or columns, ignore orientation. GIL and
	     * BIL don't jibe properly, so we'll infer it from #rows, #columns
	     */
...
load_att_menu(FILE * inFile, ABObj obj, ABObj root_obj)
{
    ...
    if ((((int)istr_len(menu_name)) > 0) && obj_is_button(obj))
    {
	obj_set_button_type(obj, AB_BUT_MENU);
    }
...
load_att_show_border(FILE * inFile, ABObj obj, ABObj root_obj)
{
    ...
    if (!abio_gil_get_boolean(inFile, &show_border))
        return (abil_print_load_err(ERR_WANT_BOOLEAN), -1);
    if (show_border)
    {
	obj_set_border_frame(obj, AB_LINE_ETCHED_IN);
    }
...
{
    ISTRING             tmp_str = NULL;
    AB_TEXT_TYPE        text_type = AB_TEXT_UNDEF;
    ABObj		textObj = NULL;
    ...
    if (text_type == AB_TEXT_NUMERIC)
    {
	/* convert to a spinbox, preserving any necessary data from the
	 * text field.
	 */
	textObj = obj_dup(obj);
	obj_set_type(obj, AB_TYPE_SPIN_BOX);
	if (obj_get_initial_value_string(textObj) != NULL)
	{
	    obj_set_initial_value_int(obj, 
		atoi(obj_get_initial_value_string(textObj)));
	}
	else if (obj_get_initial_value_int(textObj) >= 0)
	{
	    obj_set_initial_value_int(obj, obj_get_initial_value_int(textObj));
	}
	obj_destroy(textObj);
    }
    else if (   (text_type == AB_TEXT_ALPHANUMERIC)
	     && (istr_equalstr(tmp_str, ":multiline")) )
	   
    {
	cvt_text_field_to_text_pane(obj);
    }
    else if (text_type == AB_TEXT_ALPHANUMERIC)
    {
	/* GIL files can have bogus #rows for text fields - set to 1 */
...
load_att_type(FILE * inFile, ABObj obj, ABObj root_obj)
{
	...
	case AB_TYPE_LABEL:
	    /* it may be a drop target */
	    if (istr_equalstr(tmp_str, ":drop-target"))
	    {
		obj_set_label_type(obj, AB_LABEL_STRING);
		obj_set_label(obj, dropTargetLabel);
	    }
	break;
	...
	case AB_TYPE_SCALE:
	    if (istr_equalstr(tmp_str, ":gauge"))
	    {
...
/*
 * GIL vertical-spacing maps to BIL voffset, so we ignore :vertical-offset
 */
static int
load_att_voffset(FILE * inFile, ABObj obj, ABObj root_obj)
{
    ...
    if (!abio_get_integer(inFile, &voffset))
        return (abil_print_load_err(ERR_WANT_INTEGER), -1);
...
{
    int		vspacing = 0;
    if (!abio_get_integer(inFile, &vspacing))
        return (abil_print_load_err(ERR_WANT_INTEGER), -1);

    obj_set_voffset(obj, vspacing);
