/* 
 * $XConsortium: bil_loadatt.c /main/cde1_maint/3 1995/10/16 14:49:16 rcs $
 * 
...
bilP_get_string()
{
    ...
    /* allocate space for the temp string */
...
STRING
bilP_load_att_c_header_file_extension(BIL_TOKEN valueToken)
{
    ABObj               obj = bilP_load.obj;
    ISTRING		value = NULL;

    nset_att(AB_BIL_LABEL);
    if ((value = bilP_get_string()) != NULL)
    {
        obj_set_c_header_file_extension(obj, istr_string(value));
    }
    istr_destroy(value);
    return NULL;
}
...
STRING
bilP_load_att_c_impl_file_extension(BIL_TOKEN valueToken)
{
    ABObj               obj = bilP_load.obj;
    ISTRING		value = NULL;
    ...
    if ((value = bilP_get_string()) != NULL)
    {
        obj_set_c_impl_file_extension(obj, istr_string(value));
    }
    istr_destroy(value);
    return NULL;
}
...
STRING
bilP_load_att_write_main_func(BIL_TOKEN valueToken)
{
    ABObj               obj = bilP_load.obj;
    BOOL                write_main_func = FALSE;

    util_dprintf(3, "bilP_load_att_resizable %d/%s\n",
	       valueToken,
	       str_safe(bilP_token_to_string(valueToken)));

    nset_att(AB_BIL_WRITE_MAIN_FUNC);
    if (!bilP_token_is_bool(valueToken))
    {
	abil_print_load_err(ERR_WANT_BOOLEAN);
    }
    write_main_func = bilP_token_to_bool(valueToken);
    obj_set_write_main_func(obj, write_main_func);
    return NULL;
}
