%{
/*
 *	$XConsortium: bil_parse.y /main/cde1_maint/1 1995/07/14 22:33:20 drk $
 *
...
#pragma ident "@(#)bil_parse.y	1.80 94/10/16 SMI"
 ...
 */
%token	AB_BIL_MARKER_FIRST	/* placeholder to determine first value */
%token	AB_BIL_UNDEF
...
%token	AB_BIL_C_HEADER_FILE_EXTENSION
%token	AB_BIL_C_IMPL_FILE_EXTENSION
...
att		: AB_BIL_ACCELERATOR	token	{bilP_load_att_accelerator($2);}
		...
		| AB_BIL_C_HEADER_FILE_EXTENSION token	{bilP_load_att_c_header_file_extension($2);}
		| AB_BIL_C_IMPL_FILE_EXTENSION token	{bilP_load_att_c_impl_file_extension($2);}
		...
		| AB_BIL_WRITE_MAIN_FUNC token	{bilP_load_att_write_main_func($2); }
...
token	: AB_BIL_UNDEF {$$=TOK;}
	...
	| AB_BIL_C_HEADER_FILE_EXTENSION {$$=TOK;}
	| AB_BIL_C_IMPL_FILE_EXTENSION {$$=TOK;}
	| AB_BIL_CALLBACK {$$=TOK;}
