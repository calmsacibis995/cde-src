/*
 *	$XConsortium: abuil_trav.c /main/cde1_maint/2 1995/10/16 14:47:31 rcs $
 *
 * @(#)abuil_trav.c	1.9 01/20/96	cose/unity1/cde_app_builder/src/libABil
 *
...
)
{
      ...
      default:
	{
	    ...
	    printf("%x  unknown type: %d  size: %d  byte: 0x%x\n",
			  az_symbol_entry,
...
)
{
    ...
    printf(
    "  controls: 0x%x  callbacks: 0x%x  arguments: 0x%x  parent_list: 0x%x\n",
...
)
{
    ...
    printf(
    "  CALLBACK name: %s  proc ref: 0x%x  proc ref list: 0x%x\n",
		  name_str,
...
)
{
    ...
    printf("%s LIST(0x%x)  count: %d  gadget count: %d\n",
...
{
    printf("NAME(0x%x)  size: %d  next name: 0x%x  object: 0x%x",
     az_name_entry,
...
{
    printf("%s MODULE(0x%x)  version: 0x%x \n",
...
{
    printf(
    "COLOR ITEM(0x%x)  size: %d  letter: %c  index: %d  color: 0x%x  next: 0x%x\n",
     az_color_item_entry,
...
{
    printf("PARENT LIST(0x%x)  parent: 0x%x  next: 0x%x \n",
     az_parent_list_item,
...
{
    printf("EXTERNAL DEF(0x%x)  next external: 0x%x  object: 0x%x \n",
...
)
{
    ...
    printf("PROC DEF(0x%x)  name: 0x%x %s%s%s%s  count: %d  %s\n",
	 az_proc_def_entry,
...
)
{
    ...
    printf("PROC REF(0x%x)  proc def: 0x%x  value: 0x%x\n",
	 az_proc_ref_entry,
...
{
    printf(
    "FORWARD REF(0x%x)  next ref: 0x%x  location: 0x%x  %s  parent: 0x%x\n",
...
)
{
    ...
    printf("  VALUE(0x%x)  size: %d  name: 0x%x  %s%s%s%s",
	 az_value_entry,
...
check_for_table_value:
	...
	printf("  icon  width: %d  height: %d  colors: 0x%x  rows: 0x%x \n",
...
common_table:
	...
	printf("  %s  first table entry: 0x%x\n",
      ...
      case sym_k_color_table_value:
	{
		...
		printf("    letter: %c  index: %d  color: 0x%x\n",
...
)
{
    ...
    {
	printf("  name: 0x%x", az_obj_entry->obj_header.az_name);
    }
    ...
    {
	printf("  next: 0x%x", az_obj_entry->obj_header.az_next);
    }
...
{
    printf("INCLUDE FILE(0x%x)  file name: %s  full file name: %s\n",
	       az_symbol_entry,
...
{
    printf("%s SECTION(0x%x)  entries: 0x%x\n",
...
{
    printf(
    "DEFAULT OBJ VAR(0x%x)  next: 0x%x  object info: %d, variant_info: %d\n",
		  az_symbol_entry,
...
{
    printf("ROOT(0x%x)  tag: %d  module: 0x%x  sections: 0x%x\n  module tail: ",
		  az_symbol_entry,
		  az_symbol_entry->header.b_tag,
