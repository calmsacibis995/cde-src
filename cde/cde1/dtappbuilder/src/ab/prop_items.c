/*
 *      $XConsortium: prop_items.c /main/cde1_maint/3 1995/10/16 14:00:34 rcs $
 *
...
)
{
    ...
    if (HasLabelType(pis->item_type))
    	prop_options_set_value(pis->item_label_type_pos, AB_LABEL_STRING, False);
	...
	prop_options_set_value(pis->item_line_style_pos, AB_LINE_ETCHED_OUT, False);
...
)
{
    ...
    /* rhale util_free(iobj_list); */

    if (pos > 1) util_free(iobj_list);
...
)
{
	...
	label_type = iobj? iobj->label_type : AB_LABEL_STRING;
    	prop_options_set_value(pis->item_label_type_pos, label_type, False);
    }
