...
/*
 *      $XConsortium: appfw_stubs.c /main/cde1_maint/3 1995/10/16 13:35:39 rcs $
 *
...
#define APPFW_DONT_GEN_MAIN_KEY	124 /* needed for Checkbox initialization */
#define NO_DISMISS	0
#define DISMISS		1
#include "dtb_utils.h"
...
typedef struct  APPFW_EDITOR_SETTINGS
{
    ...
    PropCheckboxSettingRec	dont_gen_main;
    PropComboboxSettingRec	c_impl_ext;
    PropComboboxSettingRec	c_hdr_ext;
...
)
{
	...
	/* Enabled */
        ...
        prop_checkbox_init(&(afs->i18n), 
		appfw_cgen->i18n_checkbox_label,
	...
	/* Don't gen main checkbox -> OFF */
        n = 0;
        item[n] = appfw_cgen->gencode_dont_gen_main_checkbox_items.nolabel_item;
        item_val[n] = APPFW_DONT_GEN_MAIN_KEY; n++;
        prop_checkbox_init(&(afs->dont_gen_main), 
		appfw_cgen->gencode_dont_gen_main_label,
                appfw_cgen->gencode_dont_gen_main_checkbox, n, item, item_val,
                appfw_cgen->gencode_dont_gen_main_cb);

	prop_combobox_init(&(afs->c_impl_ext), 
		appfw_cgen->gencode_c_impl_ext_combobox_label,
		appfw_cgen->gencode_c_impl_ext_combobox, 
		appfw_cgen->gencode_c_impl_cb);
	prop_combobox_init(&(afs->c_hdr_ext), 
		appfw_cgen->gencode_c_hdr_ext_combobox_label,
		appfw_cgen->gencode_c_hdr_ext_combobox, 
		appfw_cgen->gencode_c_hdr_cb);
...
)
{
     ...
     * Generated Code       *
     ************************/
    /* note - "dont gen main" is a negative option */
    prop_checkbox_set_value(&(afs->dont_gen_main), APPFW_DONT_GEN_MAIN_KEY,
		(!obj_get_write_main_func(project)), False);
    prop_combobox_set_value(&(afs->c_impl_ext), 
	obj_get_c_impl_file_extension(project), False);
    prop_combobox_set_value(&(afs->c_hdr_ext), 
	obj_get_c_header_file_extension(project), False);
    ...
    prop_options_set_value(&(afs->session_mgt), 
		obj_get_sessioning_method(project), False);
    ...
    prop_options_set_value(&(afs->tooltalk),
		obj_get_tooltalk_level(project), False);
...
)
{
     ...
     * Generated Code       * 
     ************************/ 
    if (prop_changed(afs->dont_gen_main.changebar))
    {
	abobj_set_write_main_func(project,
	    !((BOOL)prop_checkbox_get_value(&(afs->dont_gen_main), 
						APPFW_DONT_GEN_MAIN_KEY)));
    }
    if (prop_changed(afs->c_impl_ext.changebar))
    {
	STRING	newval = prop_combobox_get_value(&(afs->c_impl_ext));
	if (!util_streq(newval, obj_get_c_impl_file_extension(project)))
	{
	    obj_set_c_impl_file_extension(project, newval);
	    abobj_set_save_needed(project, TRUE);

	}
    }
    if (prop_changed(afs->c_hdr_ext.changebar))
    {
	STRING	newval = prop_combobox_get_value(&(afs->c_hdr_ext));
	if (!util_streq(newval, obj_get_c_header_file_extension(project)))
	{
   	    obj_set_c_header_file_extension(project, newval);
	    abobj_set_save_needed(project, TRUE);
	}
    }
...
)
{
    ...
    prop_set_changebar(afs->dont_gen_main.changebar,	PROP_CB_OFF);
    prop_set_changebar(afs->c_impl_ext.changebar,	PROP_CB_OFF);
    prop_set_changebar(afs->c_hdr_ext.changebar,	PROP_CB_OFF);
