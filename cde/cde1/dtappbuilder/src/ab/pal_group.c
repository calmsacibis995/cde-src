...
/*
 *      $XConsortium: pal_group.c /main/cde1_maint/3 1995/10/16 13:52:04 rcs $
 *
...
)
{
    ...
    */
	...
	prop_radiobox_init(&(pgs->layout_type), cgen->layout_rbox_label,
			cgen->layout_rbox, n, item, item_val,
			cgen->layout_cb1);
	...
	*/
	...
	prop_radiobox_init(&(pgs->grid_rowcol), NULL,
			cgen->gridrowcol_rbox, n, item, item_val,
			cgen->gridrowcol_cb);
...
)
{
    ...
    /* Clear Border Frame */
    prop_options_set_value(&(pgs->frame), AB_LINE_NONE, False);

    /* Clear Layout Type */
    prop_radiobox_set_value(&(pgs->layout_type), AB_GROUP_IGNORE, False);

    /* Clear Grid Rows/Columns */
    prop_radiobox_set_value(&(pgs->grid_rowcol), AB_ORIENT_HORIZONTAL, False);
    prop_field_set_numeric_value(&(pgs->grid_rowcol_count), NUMROWCOL, False);

    /* Clear Vertical Alignment */
    prop_options_set_value(&(pgs->valign), AB_ALIGN_LEFT, False);
    prop_field_set_numeric_value(&(pgs->vspacing), 0, False);
 
    /* Clear Horizontal Alignment */
    prop_options_set_value(&(pgs->halign), AB_ALIGN_TOP, False);
...
)
{
    ...
    {
	/* Load Border Frame */
	prop_options_set_value(&(pgs->frame), obj_get_border_frame(obj), False);
 
	/* Load Layout Type */
	gtype = obj_get_group_type(obj);
	prop_radiobox_set_value(&(pgs->layout_type), gtype, False);
