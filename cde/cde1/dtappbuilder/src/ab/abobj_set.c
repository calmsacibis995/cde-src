...
/*
 *      $XConsortium: abobj_set.c /main/cde1_maint/3 1995/10/16 13:34:32 rcs $
 *
...
/* brianc added code for bugid xxxxx
 * where label is set to NULL, reset it to its previous glory
 */


    if ((util_strcmp(label,"") == 0) && (obj->type == AB_TYPE_CHOICE))
        strcpy(label, " ");


/* END of brianc bug fix code,
 * the rationale behind this fix is as follows
 *
 * prevent the user from having a Null label. Although there is nothing wrong with
 * a null laber, if two changes are applied at the same time one of which is a null
 * label, then updating the widget structure becomes unstable. By forcing the Null
 * label to revert to its previous value, we thereby reduce the number of changes to
 * one, removing the change to the label. This blocks the condition that causes this
 * crash.
 *
 * This fix is applied to all widget types, to maintain consistency. If users want
 * to have a widget with no label, then they must use a space as the label.
 *
 */
...
)
{
    ...
    ui_size_to_row_col(objxm_get_widget(tObj), width, height,
                &num_rows, &num_cols);
...
void
abobj_set_write_main_func(
			ABObj		project,
			BOOL		write_main_func
)
{
    ...
    if (util_xor(obj_get_write_main_func(project), write_main_func))
    {
	obj_set_write_main_func(project, write_main_func);
	abobj_set_save_needed(project, TRUE);
    }
}
