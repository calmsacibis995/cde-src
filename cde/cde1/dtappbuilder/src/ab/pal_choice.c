/*
 *      $XConsortium: pal_choice.c /main/cde1_maint/3 1995/10/16 13:49:17 rcs $
 *
...
)
{
	...
	if (type == AB_PROP_FIXED)
	{
		...
		n, item, item_val, cgen->labeltype_cb);
	...
	/* Item Label Type */
		...
		n, item, item_val, cgen->itemlist_cb);
...
)
{
    ...
    prop_options_set_value(&(pcs->label_type), (int)AB_LABEL_STRING, False);
    prop_options_set_value(&(pcs->label_pos), (int)AB_CP_WEST, False);
    ...
    /* Clear Choice Type*/ 
    prop_options_set_value(&(pcs->choice_type), (int)AB_CHOICE_EXCLUSIVE, False); 

    /* Clear Row/Columns */
    prop_radiobox_set_value(&(pcs->row_col), (int)AB_ORIENT_HORIZONTAL, False); 
...
)
{
	...
	prop_options_set_value(&(pcs->label_type), (int)obj->label_type, False);
	prop_options_set_value(&(pcs->label_pos), obj_get_label_position(obj), False);
	...
	prop_options_set_value(&(pcs->choice_type), ctype, False);
	setup_type_settings(type, ctype);

	/* Load Row/Column*/
	prop_radiobox_set_value(&(pcs->row_col), obj_get_orientation(obj), False);
...
)
{
	...
	prop_options_get_value(&(pcs->label_type)) == AB_LABEL_GLYPH &&
