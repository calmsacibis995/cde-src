...
/*
 *      $XConsortium: pal_label.c /main/cde1_maint/3 1995/10/16 13:52:32 rcs $
 *
...
)
{
	...
	prop_label_field_init(&(pls->label), cgen->graphic_hint, item, n);
        ...
        prop_radiobox_init(&(pls->size_policy), cgen->szpolicy_rbox_label,
                cgen->szpolicy_rbox, n, item, item_val,
                cgen->szpolicy_cb);
...
)
{
    ...
    prop_options_set_value(&(pls->label_align), AB_ALIGN_CENTER, False);
    ...
    /* Clear Size Policy */
    prop_radiobox_set_value(&(pls->size_policy), SIZE_OF_CONTENTS_KEY, False);
...
)
{
	...
	prop_options_set_value(&(pls->label_align), obj_get_label_alignment(obj), False);

	/* Load Size Policy */
	prop_radiobox_set_value(&(pls->size_policy),
				abobj_width_resizable(obj)? SIZE_FIXED_KEY : SIZE_OF_CONTENTS_KEY, False);
