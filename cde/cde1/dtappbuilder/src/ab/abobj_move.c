...
/*
 *      $XConsortium: abobj_move.c /main/cde1_maint/2 1995/10/16 13:33:27 rcs $
 *
...
/*
 * For some reason, extern is needed here. Otherwise, the Devpro (Sun)
 * compiler does not export the symbol out of this file. This only seems
 * to be the case for C++.
 */
extern const int	AB_drag_threshold = 4;
...
)
{
	 ...
	 */
	(void)abobj_set_undo_move(sel.list, sel.count);
...
/*
 * Need public function for setting the undo info for move because 
 * 'undo_move' is static to this function, and undo of move is needed
 * in various places - abobj_move.c, abobj_align.c
 */
void
abobj_set_undo_move(
    ABObj		*objlist,
...
{
    (void)abobj_set_undo(objlist, count, undo_move, AB_UNDO_MOVE);
}
...
)
{
     ...
     */
    (void)abobj_set_undo_move(undo_rec->list, undo_rec->count);
