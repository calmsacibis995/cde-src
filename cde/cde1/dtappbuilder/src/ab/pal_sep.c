...
/*
 *	$XConsortium: pal_sep.c /main/cde1_maint/3 1995/10/16 13:55:22 rcs $
 *
...
)
{
    ...
    /* Clear Line Style */
    prop_options_set_value(&(pss->line_style), AB_LINE_ETCHED_OUT, False);
...
)
{
    ...
    {
	/* Load Orientation */
	prop_radiobox_set_value(&(pss->orient), obj_get_orientation(obj), False);

	/* Load Line Style */
	prop_options_set_value(&(pss->line_style), obj_get_line_style(obj), False);
