...
/*
 *      $XConsortium: conn_stubs.c /main/cde1_maint/3 1995/10/16 13:42:14 rcs $
 *
 * @(#)conn_stubs.c	1.60 02/14/97 cde_app_builder/src/ab
 *
...
)
{
	    ...
	    && !obj_is_project(obj))
	{
            ...
            /* rhale */
            if(!info->old_name) info->old_name = 0;
...
)
{
	...
	case AB_TYPE_ITEM:
	    ...
	    else if (obj_is_menu_item(cur_obj))
	    	/* We have to make sure that the "Activated" when
	     	 * option menu item is sensitive, since it can be
	     	 * made insensitive by choosing a radio/checkbox.
	     	 */
		XtSetSensitive(w_list[ConnP_conn_whens[2].when_type-1], True);
	    break;
