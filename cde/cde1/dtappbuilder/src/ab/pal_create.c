...
/*
 *      $XConsortium: pal_create.c /main/cde1_maint/3 1995/10/16 13:50:19 rcs $
 *
...
)
{
    ...
    XtInsertEventHandler(widget, ButtonPressMask, FALSE,
            palitem_select_event, (XtPointer)subtype, XtListHead);
    XtInsertEventHandler(widget, ButtonReleaseMask, FALSE,
            palitem_release_event, (XtPointer)subtype, XtListHead);
    XtAddEventHandler(widget, EnterWindowMask|LeaveWindowMask, FALSE,
	    palitem_update_type, (XtPointer)subtype);
...
)
{
    ...
    *ModuleCreated = FALSE;
	...
	if (obj_is_menubar(obj))
	{
        ...
        {
	    ABObj	new_obj_parent;

	    new_obj_parent = abobj_get_ctrl_pane_or_group(obj_parent);

	    /*
	     * Check if obtained handle to control panel/group 
	     */
	    if (new_obj_parent && (new_obj_parent != obj_parent))
	    {
                Window	rootwin = RootWindowOfScreen(XtScreen(AB_toplevel)),
			child = NULL;
		Widget	new_parent_win = (Widget)obj_get_root(new_obj_parent)->ui_handle;
		int	new_x = 0, new_y = 0;

		/*
		 * If obtained handle to new parent
		 * the coordinates have to be translated to the new parent's
		 * space.
		 */
		if (XTranslateCoordinates(dpy, rootwin, 
				XtWindow(new_parent_win), x, y,
				&new_x, &new_y, &child) == True)
		{
		    /*
		     * XTranslateCoordinates() succeeded, apply to object
		     */
        	    obj->x = new_x - drag_cursor_xhot;
        	    obj->y = new_y - drag_cursor_yhot;
		}

	    }

	    obj_parent = new_obj_parent;
