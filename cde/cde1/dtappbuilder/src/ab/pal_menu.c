/*
 *      $XConsortium: pal_menu.c /main/cde1_maint/3 1995/10/16 13:53:45 rcs $
 *
...
)
{
	...
	pms->edit_menu = cgen->menuedit_mbutton_menu_editmenu;
        ...
        prop_radiobox_init(&(pms->tearoff), cgen->tearoff_rbox_label,
                cgen->tearoff_rbox, n, item, item_val,
                cgen->tearoff_cb);
	...
	/* Item Line Style */
        ...
        prop_options_init(&(pms->item_line_style), cgen->linestyle_opmenu_label,
                ...
                n, item, item_val,    
                cgen->itemlist_cb);
 	prop_options_set_value(&(pms->item_line_style), AB_LINE_ETCHED_OUT, False);
...
)
{
    ...
    /* Clear Tearoff */
    prop_radiobox_set_value(&(pms->tearoff),  False, False);
