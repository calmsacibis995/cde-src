...
/*
 *      $XConsortium: pal_button.c /main/cde1_maint/3 1995/10/16 13:48:56 rcs $
 *
...
)
{
	...
	if (type == AB_PROP_FIXED)
	{
        ...
        prop_radiobox_init(&(pbs->button_type), cgen->buttontype_rbox_label,
                                cgen->buttontype_rbox, n, item, item_val,
                                cgen->buttontype_cb);
        for(i=0; i < n; i++)
	...
	XtVaSetValues(cgen->label_field_label, 
        ...
        prop_radiobox_init(&(pbs->size_policy), cgen->sizepolicy_rbox_label,
                cgen->sizepolicy_rbox, n, item, item_val,
                cgen->sizepolicy_cb);
...
)
{
    ...
    /* Clear Button Type*/ 
    prop_radiobox_set_value(&(pbs->button_type), (int)AB_BUT_PUSH, False); 
 
    /* Clear LabelType */
    prop_options_set_value(&(pbs->label_type), (int)AB_LABEL_STRING, False);
    ...
    prop_options_set_value(&(pbs->arrow_dir), AB_LABEL_ARROW_DOWN, False); 
 
    /* Clear Label Alignment */
    prop_options_set_value(&(pbs->label_align), AB_ALIGN_CENTER, False);

    /* Clear Menu Name */
    prop_menuname_set_value(&(pbs->menuname), "", False);

    /* Clear Size Policy */
    prop_radiobox_set_value(&(pbs->size_policy), SIZE_OF_CONTENTS_KEY, False);
...
)
{
    ...
    if (load_all)
    {
    	...
    	prop_radiobox_set_value(&(pbs->button_type), btype, False);
	...
	switch (obj_get_label_type(obj))
	{
		...
		/* AB_LABEL_ARROW_* types are mapped into AB_LABEL_ARROW_DOWN */
			prop_options_set_value(&(pbs->label_type), (int)AB_LABEL_ARROW_DOWN, False);
			break;
		default:
			prop_options_set_value(&(pbs->label_type), (int)obj_get_label_type(obj), False);
			break;
	}
	...
	prop_options_set_value(&(pbs->label_align), 
		obj_get_label_alignment(obj), False);
	...
	prop_radiobox_set_value(&(pbs->size_policy),
			...
			SIZE_FIXED_KEY : SIZE_OF_CONTENTS_KEY, False);
...
)
{
	...
	prop_options_get_value(&(pbs->label_type)) == AB_LABEL_GLYPH &&
...
)
{
	...
	if ((btype == AB_BUT_DRAWN) || (btype == AB_BUT_MENU))
	{
	    switch((AB_LABEL_TYPE)prop_options_get_value(&(pbs->label_type)))
	    {
	        ...
	        case AB_LABEL_ARROW_LEFT:
	        case AB_LABEL_ARROW_RIGHT:
		    prop_options_set_value(&(pbs->label_type), (int)AB_LABEL_STRING, True);
		    break;
	        default:
		    break;
	    }
