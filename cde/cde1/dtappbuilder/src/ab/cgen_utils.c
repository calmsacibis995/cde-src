/*
 *      $XConsortium: cgen_utils.c /main/cde1_maint/3 1995/10/16 13:39:34 rcs $
 *
 ...
 *
 * MAX_CGEN_FIXED_ARGS = dtcodegen, -[no]freshen -changed, -nomerge, -v or -s,
 * -main, -p, <project_name>, NULL, +1(extra) = 10
 */
...
term_execute_command(CG_SUBCOMMAND cmd_code, STRING cmd, STRING argv[])
{
	...
	else
	{
	    ...
	    /*util_dprintf(3, "process launched: %d\n", actual_process_pid);*/
...
build_dtcodegen_arg_list(STRING *argList, int *iInOut)
{
    ...
    argList[i++]= "-nofreshen";		/* everybody gets this one */
...
print_cmd_not_found_message(STRING cmd)
{
    ...
    *msg = 0;
    ...
    "****>         done via the File->Options->Environment menu).\n",
	cmd);
    print_to_term(msg);
...
careful_kill_group(pid_t pgid)
{
    ...
    /*util_dprintf(2, "careful_kill_group(%d)\n", pgid);*/
    ...
    /*util_dprintf(2,"%s: %d\n", (killed? "Killed":"COULD NOT KILL"), pgid);*/
...
)
{
    ...
    /* BT#1227288 - core dump bringing up code gen window
     *   The crash occurs in DtTerm widget, and I don't know why (there
     *   was no reproducible test case). We don't seem to be passing in
     *   NULL, but this check can't hurt. [dunn 3 Apr 1996]
     */
    if (input_termWidget == NULL)
    {
	return -1;
    }
