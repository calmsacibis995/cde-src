...
/*
 *      $XConsortium: pal_list.c /main/cde1_maint/3 1995/10/16 13:52:56 rcs $
 *
...
)
{
	...
	/* Width */
        ...
        prop_radiobox_init(&(pls->wth_policy), cgen->wpolicy_rbox_label,
                cgen->wpolicy_rbox, n, item, item_val,
                cgen->wpolicy_cb);
...
)
{
    ...
    prop_options_set_value(&(pls->label_type), (int)AB_LABEL_STRING, False);
    prop_options_set_value(&(pls->label_pos), (int)AB_CP_WEST, False);
    ...
    prop_options_set_value(&(pls->sel_mode), (int)AB_SELECT_SINGLE, False); 
    ...
    /* Clear Width */
    prop_radiobox_set_value(&(pls->wth_policy), SIZE_FIXED_KEY, False);
...
)
{
	...
	prop_options_set_value(&(pls->label_type), (int)obj->label_type, False);
	prop_options_set_value(&(pls->label_pos), obj_get_label_position(obj), False);
	...
	prop_options_set_value(&(pls->sel_mode), 
		(AB_SELECT_TYPE)obj_get_selection_mode(obj), False);
	...
	/* Load Width */
        ...
        {
            prop_options_set_value(&(pls->hgt_metric), SIZE_IN_PIXELS_KEY, False);
            height = abobj_get_comp_height(obj);
        }
        else /* Size in Chars */
        {
            prop_options_set_value(&(pls->hgt_metric), SIZE_IN_CHARS_KEY, False);
...
)
{
    ...
    {
	metric = prop_options_get_value(&(pls->hgt_metric));
...
)
{
	...
	prop_options_get_value(&(pls->label_type)) == AB_LABEL_GLYPH &&
