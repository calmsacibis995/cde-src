...
/*
 *      $XConsortium: pal_textf.c /main/cde1_maint/3 1995/10/16 13:56:31 rcs $
 *
...
)
{
    ...
    prop_options_set_value(&(pts->label_type), (int)AB_LABEL_STRING, False);
    prop_options_set_value(&(pts->label_pos), AB_CP_WEST, False);
...
*/
    ...
    /* Clear Operation */
    prop_radiobox_set_value(&(pts->op), False, False);
...
)
{
	...
	prop_options_set_value(&(pts->label_type), (int)obj->label_type, False);
	prop_options_set_value(&(pts->label_pos), obj_get_label_position(obj), False);

	/* Load Label */
	prop_setup_label_field(&(pts->label), NULL, 
				obj->label_type, obj_get_label(obj), AB_LINE_UNDEF); 
	/* Load Operation */
	prop_radiobox_set_value(&(pts->op), obj_get_read_only(obj), False);
...
)
{
	...
	prop_options_get_value(&(pts->label_type)) == AB_LABEL_GLYPH &&
