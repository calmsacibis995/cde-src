...
/*
 *      $XConsortium: pal_custdlg.c /main/cde1_maint/3 1995/10/16 13:50:39 rcs $
 *
...
)
{
	...
	if (type == AB_PROP_FIXED)
	{
        ...
        prop_radiobox_init(&(pcs->resize_mode), cgen->resizemode_rbox_label,
                cgen->resizemode_rbox, n, item, item_val,
                cgen->resizemode_cb);
	...
	prop_obj_options_init(&(pcs->help_but), cgen->helpb_opmenu_label,
        ...
        prop_radiobox_init(&(pcs->size_policy), cgen->size_policy_rbox_label,
                cgen->size_policy_rbox, n, item, item_val,
                cgen->size_policy_cb);
...
)
{
    ...
    /* Clear Resize Mode */
    prop_radiobox_set_value(&(pcs->resize_mode), (int)True, False);
    ...
    /* Clear Size Policy */
    prop_radiobox_set_value(&(pcs->size_policy), SIZE_FIXED_KEY, False);
...
)
{
	...
	/* Load Resize Mode */
	prop_radiobox_set_value(&(pcs->resize_mode), obj_get_resizable(obj), False);
...
)
{
	...
	if (area != NULL && !area_set) /* Remove Button Panel */
	{
	    ...
	    /*
	     * NULLify default/help button
	     */
	    obj_set_default_act_button(pcs->current_obj, NULL);
	    obj_set_help_act_button(pcs->current_obj, NULL);

	    /*
	     * Change default button option menu to display
	     * "None"
	     */
	    prop_obj_options_load(&(pcs->default_but), NULL);
	    prop_obj_options_set_value(&(pcs->default_but),
	    ...
	    /*
	     * Change help button option menu to display
	     * "None"
	     */
	    prop_obj_options_load(&(pcs->help_but), NULL);
	    prop_obj_options_set_value(&(pcs->help_but),
	...
	else if (area == NULL && area_set) /* Add Button Panel */
	{
	    ...
	    /*
	     * Load default button option menu
	     */
	    prop_obj_options_load(&(pcs->default_but), 
	        objxm_comp_custdlg_get_area(pcs->current_obj, AB_CONT_BUTTON_PANEL));
	    prop_obj_options_set_value(&(pcs->default_but),
	        obj_get_default_act_button(pcs->current_obj), False);

	    /*
	     * Load help button option menu
	     */
	    prop_obj_options_load(&(pcs->help_but), 
	        objxm_comp_custdlg_get_area(pcs->current_obj, AB_CONT_BUTTON_PANEL));
	    prop_obj_options_set_value(&(pcs->help_but),
	        obj_get_help_act_button(pcs->current_obj), False);
	}
