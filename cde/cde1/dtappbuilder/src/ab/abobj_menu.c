...
/*
 *      $XConsortium: abobj_menu.c /main/cde1_maint/3 1995/10/16 13:33:05 rcs $
 *
...
)
{
	...
	/* Loop2: look at selected objects to TURN-OFF menu items */
	    ...
	    if (obj_is_group_member(selobj) && 
			(obj_get_group_type(selparent) != AB_GROUP_IGNORE))
	    {
		/*
		 * Disable Align/Distribute if selected object is member 
		 * of a group that is NOT of type "As is"
		 */
		item_flags &= (~ABMenuAlign);
		item_flags &= (~ABMenuDistribute);
	    }
	...
	{
	    /*
	     * If only one object selected, disable Align/Distribute
	     */
            item_flags &= (~ABMenuAlign);
            item_flags &= (~ABMenuDistribute);
	}
