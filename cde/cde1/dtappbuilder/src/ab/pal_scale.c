...
/*
 *      $XConsortium: pal_scale.c /main/cde1_maint/3 1995/10/16 13:54:58 rcs $
 *
...
)
{
	...
	if (type == AB_PROP_FIXED)
	{
        ...
        prop_radiobox_init(&(pss->scale_type), cgen->scaletype_rbox_label,
                                cgen->scaletype_rbox, n, item, item_val,
                                cgen->scaletype_cb);
        ...
        prop_radiobox_init(&(pss->orient), cgen->orient_rbox_label,
                                cgen->orient_rbox, n, item, item_val,
                                cgen->orient_cb);
...
)
{
    ...
    prop_options_set_value(&(pss->label_type), (int)AB_LABEL_STRING, False);
    prop_options_set_value(&(pss->label_pos), (int)AB_CP_WEST, False);
    ...
    /* Clear Scale Type*/ 
    prop_radiobox_set_value(&(pss->scale_type), (int)False, False); 

    /* Clear Orientation */
    prop_radiobox_set_value(&(pss->orient), (int)AB_ORIENT_HORIZONTAL, False);

    /* Clear Direction */
    prop_options_set_value(&(pss->dir), AB_DIR_LEFT_TO_RIGHT, False);
...
)
{
	...
	prop_options_set_value(&(pss->label_type), (int)obj->label_type, False);
	prop_options_set_value(&(pss->label_pos), obj_get_label_position(obj), False);
	...
	/* Load Scale Type */
	prop_radiobox_set_value(&(pss->scale_type), obj_get_read_only(obj), False);

	/* Load Orientation */
	orient = obj_get_orientation(obj);
	prop_radiobox_set_value(&(pss->orient), orient, False); 
	setup_direction_setting(type, orient);

	/* Load Direction */
	prop_options_set_value(&(pss->dir), obj_get_direction(obj), False);
...
)
{
	...
	prop_options_get_value(&(pss->label_type)) == AB_LABEL_GLYPH &&
...
)
{
	...
	if (value == AB_ORIENT_HORIZONTAL)
		...
		AB_DIR_LEFT_TO_RIGHT, True);
	else
	    prop_options_set_value(&(pss->dir),
		AB_DIR_BOTTOM_TO_TOP, True);
    }
}
