...
/*
 *	$XConsortium: pal_fchooser.c /main/cde1_maint/3 1995/10/16 13:51:21 rcs $
 *
...
)
{
	...
	if (type == AB_PROP_FIXED)
	{
        ...
        prop_radiobox_init(&(pcs->pattern_type), cgen->pattern_type_rbox_label,
                cgen->pattern_type_rbox, n, item, item_val,
...
)
{
    ...
    prop_radiobox_set_value(&(pcs->pattern_type), (int)True, False);
...
)
{
	...
	prop_radiobox_set_value(&(pcs->pattern_type), 
			obj_get_file_type_mask(obj), False);
