...
/*
 *      $XConsortium: pal_drawp.c /main/cde1_maint/2 1995/10/16 13:51:01 rcs $
 *
...
)
{
	...
	/* Scrolling */
        ...
        prop_radiobox_init(&(pds->scrolling), cgen->scrolldisp_rbox_label,
                cgen->scrolldisp_rbox, n, item, item_val,
                cgen->scroll_cb);
...
)
{
    ...
    /* Clear Scrolling */
    prop_radiobox_set_value(&(pds->scrolling), AB_SCROLLBAR_WHEN_NEEDED, False); 

    /* Clear Border Frame */
    prop_options_set_value(&(pds->frame), AB_LINE_NONE, False);
...
)
{
	...
	prop_radiobox_set_value(&(pds->scrolling), sb_policy, False);
	setup_scroll_settings(type, sb_policy);

	/* Load Border Frame */
	prop_options_set_value(&(pds->frame), obj_get_border_frame(obj), False);
...
)
{
	...
	hgt = prop_geomfield_get_value(&(pds->geom), GEOM_HEIGHT);
        ...
        border = /* assumption! */
            ((prop_options_get_value(&(pds->frame)) != AB_LINE_NONE)? 4 : 0);
