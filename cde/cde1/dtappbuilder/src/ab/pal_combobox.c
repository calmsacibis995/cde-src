...
/*
 *	$XConsortium: pal_combobox.c /main/cde1_maint/3 1995/10/16 13:49:39 rcs $
 *
...
)
{
	...
	prop_label_field_init(&(pcs->label), cgen->graphic_hint, item, n);
        ...
        prop_radiobox_init(&(pcs->combobox_type), cgen->comboboxtype_rbox_label,
                       cgen->comboboxtype_rbox, n, item, item_val,
...
)
{
    ...
    /* Clear Read Only */ 
    prop_radiobox_set_value(&(pcs->combobox_type), True, False);
    ...
    /* Clear Width */
    prop_radiobox_set_value(&(pcs->wth_policy), SIZE_OF_CONTENTS_KEY, False);
...
)
{
    ...
    if (load_all || loadkey & LoadSize)
    {
        ...
        prop_radiobox_set_value(&(pcs->wth_policy),
                editable? SIZE_FIXED_KEY : SIZE_OF_CONTENTS_KEY, False);
