...
/*
 *	$XConsortium: abobj_edit.c /main/cde1_maint/3 1995/10/16 13:31:10 rcs $
 *
...
)
{
	...
	case AB_ATTACH_OBJ:
	    origAttachObj = (ABObj)obj_get_attach_value(origObj, dir);
	    origParentObj = obj_get_parent(origObj);

	    offset = obj_get_attach_offset(origObj, dir);

	    attachObj = (ABObj)obj_get_attach_value(obj, dir);
...
)
{
	...
	else
	{
		...
		else if (obj_is_menubar(obj))
		{
        	...
        	{
		    newroot = abobj_get_ctrl_pane_or_group(root);
...
ABObj
abobj_get_ctrl_pane_or_group(
    ABObj	paste_target
)
{
    ABObj	parent = paste_target;
