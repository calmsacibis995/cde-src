/*
 *	$XConsortium: prop.c /main/cde1_maint/3 1995/10/16 13:59:55 rcs $
 *
...
)
{
    ...
    XtAddCallback(objlist, XmNdefaultActionCallback,
	    (XtCallbackProc)objlist_dclickCB, (XtPointer)NULL);
...
{
    pcs->label = label;
    pcs->field = XtNameToWidget(combobox, "Text");
    pcs->changebar = changebar;
    /* Setup Prop Sheet changebar mechanism */
    XtVaSetValues(pcs->field, XmNuserData, (XtArgVal)PROP_EDIT, NULL);
    propP_changebar_init(changebar, pcs->field);
 
    XtAddCallback(pcs->field, XmNvalueChangedCallback,
                propP_field_chgCB, (XtPointer)changebar);

}

STRING   
prop_combobox_get_value(
    PropComboboxSetting pcs
)
{
    STRING      value;
...
int      
prop_combobox_set_value(
    PropComboboxSetting pcs,
    STRING              value,
    BOOL                trip_changebar
)
{
    return(propP_field_set_value(pcs->field, value, trip_changebar));
}
...
)
{
    ...
    if (num_items > 0)
    	prop_options_set_value(pos, item_values[0], False);
}

int
prop_options_get_value(
    PropOptionsSetting	pos
)
{
    XtArgVal value = -1;
...
)
{
    ...
    for (i = 0; i < num_children; i++)
    {
	    ...
	    cur_val = prop_options_get_value(pos);
		...
		if ((i == 0) && (num_children != 1))
		{
                    XtVaGetValues(children[1],
                        ...
                        NULL);
		    prop_options_set_value(pos, reset_val, trip_changebar);
		}
		else 
		    if (i != 0)
		        prop_options_set_value(pos, reset_val, trip_changebar);
...
{
    return(prop_options_set_value(&(pos->options), (int)obj, trip_changebar));
}
...
)
{
	...
	prop_options_set_value(label_line_style_pos, line_style, False);
...
)
{
    ...
    XtAddCallback(cgen->objlist, XmNdefaultActionCallback, 
            (XtCallbackProc)objlist_dclickCB, 
		(XtPointer)NULL); 
...
)
{
    ...
    /* objlist_selectCB was just called, so the obj corresponding to
     * the list item which was double-clicked has already been loaded
     * into props; just need to make the obj selected in the interface.
     */
    pstate = get_prop_state_info(widget);
    if (pstate->loaded_obj != NULL) 
    {
	abobj_deselect_all(obj_get_project(pstate->loaded_obj));
	abobj_select(pstate->loaded_obj);
    }
}
