...
/*
 *	$XConsortium: pal_spinbox.c /main/cde1_maint/3 1995/10/16 13:55:45 rcs $
 *
...
)
{
	...
	if (type == AB_PROP_FIXED)
	{
        ...
        prop_radiobox_init(&(pss->spinbox_type), cgen->spinboxtype_rbox_label,
                       cgen->spinboxtype_rbox, n, item, item_val,
                       cgen->spinboxtype_cb);
        for(i=0; i < n; i++)
...
)
{
    ...
    /* Clear Spinbox Type */ 
    prop_radiobox_set_value(&(pss->spinbox_type), AB_TEXT_NUMERIC, False);

    /* Clear Arrow Style*/
    prop_options_set_value(&(pss->arrow_style), AB_ARROW_END, False);
...
)
{
	...
	prop_radiobox_set_value(&(pss->spinbox_type), spinbox_type, False);
	setup_content_settings(type, spinbox_type);

	/* Load Arrow Style */
	prop_options_set_value(&(pss->arrow_style), obj_get_arrow_style(obj), False);
