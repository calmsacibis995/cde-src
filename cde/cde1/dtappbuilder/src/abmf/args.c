...
/*
 * $XConsortium: args.c /main/cde1_maint/4 1995/10/17 11:49:32 rcs $
 * 
...
#undef _NO_PROTO		/* I don't know WHO is setting this! */
...
)
{
    ...
    *strval = 0;
		...
		!util_streq(argName, XmNaccelerator)) 
            {
                ...
                wroteResource = TRUE;
                abio_printf(fp, "%s, ", c_arg_name);
                abmfP_write_catgets_call(genCodeInfo, obj, str);
            }
            else
            {
	...
	else
	{
		...
		if (argValue == NULL)
		{
		    write_str_literal_resource(c_arg_name, nullCStr);
		    wroteResource = TRUE;
		}
		else
		{
		    ...
		    wroteResource = TRUE;
                    ...
                    if (genCodeInfo->i18n_method == ABMF_I18N_XPG4_API)
                    {
                        ...
                        abio_printf(fp, "XmRString, ");
			abmfP_write_catgets_call(genCodeInfo, obj, unq_str_val);
			abio_printf(fp, ", strlen(");
			abmfP_write_catgets_call(genCodeInfo, obj, unq_str_val);
			abio_printf(fp, ")+1");
                    }
                    else
...
)
{
	...
	if (menuObj != NULL) {
	  ...
	  abmfP_write_arg_val_line(
				   genCodeInfo,
				   abmfP_arg_is_dumped(genCodeInfo,
						       argName, menuObj),
				   argName,
				   objName,
				   menuObj,
...
int
abmfP_comp_get_num_args_of_classes(ABObj wholeObj, ABMF_ARG_CLASS argClasses)
{
    int			num_args = 0;
    AB_TRAVERSAL	compTrav;
    ABObj		subobj = NULL;
    for (trav_open(&compTrav, wholeObj, AB_TRAV_COMP_SUBOBJS);
	(subobj = trav_next(&compTrav)) != NULL; )
    {
	num_args += abmfP_get_num_args_of_classes(subobj, argClasses);
    }
    trav_next(&compTrav);
