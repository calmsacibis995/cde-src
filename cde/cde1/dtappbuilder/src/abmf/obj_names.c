...
/*
 * $XConsortium: obj_names.c /main/cde1_maint/3 1995/10/16 14:27:31 rcs $
 * 
...
STRING
abmfP_get_project_init_proc_name(ABObj obj)
{
    static char	name[MAX_NAME_SIZE];
    ...
    *name = 0;
    strcat(name, "dtb_");
    strcat(name, obj_get_name(project));
...
abmfP_get_project_c_file_name(ABObj obj)
{
    ...
    *name = 0;
    strcat(name, obj_get_name(project));
    strcat(name, obj_get_c_impl_file_extension(project));
    return name;
}
...
abmfP_get_project_header_file_name(ABObj obj)
{
    ...
    *name = 0;
    strcat(name, obj_get_name(project));
    strcat(name, obj_get_c_header_file_extension(project));
    return name;
}
...
STRING 
abmfP_get_stubs_c_file_name(ABObj obj)
{
    static char	name[MAX_NAME_SIZE];
    ...
    *name = 0;
    assert(module != NULL);
    assert(project != NULL);
    sprintf(name, "%s_stubs%s", 
	obj_get_name(module), obj_get_c_impl_file_extension(project));
    return name;
}
...
abmfP_get_ui_header_file_name(ABObj obj)
{
    ...
    *name = 0;
    ...
    assert(project != NULL);
    sprintf(name, "%s_ui%s", 
	obj_get_name(module), obj_get_c_header_file_extension(project));
    return name;
}
