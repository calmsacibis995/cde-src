...
/*
 * $XConsortium: abmf.c /main/cde1_maint/4 1995/10/17 11:49:07 rcs $
 * 
...
main(int argc, STRING *argv)
{
    ...
    startupStartCPUSeconds = loadStartCPUSeconds = configStartCPUSeconds
				= progStartCPUSeconds;
    /* 
     * Print date of executable
     */
    if (debugging())
    {
	char		exePath[MAXPATHLEN+1];
	char		*exeNamePtr = NULL;
	struct stat	fileInfo;
	time_t		mtime;
	*exePath = 0;

	dtb_save_command(argv[0]);
	strcpy(exePath, dtb_get_exe_dir());
	strcat(exePath, "/");
	exeNamePtr = strrchr(argv[0], '/');
	if (exeNamePtr == NULL)
	{
	    exeNamePtr = argv[0];
	}
	...
	strcat(exePath, exeNamePtr);

	if (stat(exePath, &fileInfo) == 0)
	{
	    mtime = fileInfo.st_mtime;
	    printf("(DEBUG): %s dated %s\n", exeNamePtr, ctime(&mtime));
	}
    ...
    if (strlist_is_empty(cmdline->files))
    {
	if (cmdline->write_project_files)
	{
	...
	*projNameBuf = 0;
    ...
    if (cmdline->debug_profile_what != ABMF_PROFILE_UNDEF)
    {
        util_printf("load real:%lg  CPU:%lg\n", loadSeconds, loadCPUSeconds);
    }
#endif /* DEBUG */
...
parse_args(int argc, char *argv[], CmdlineArgs cmdline)
{
    ...
    cmdline->write_project_files = FALSE;
    ...
    for (argCount = 1; argCount < argc; ++argCount)
    {
		...
		cmdline->cgen_restriction = ABMF_CGEN_BY_DATE;
            ...
            {
                cmdline->write_project_files = TRUE;
            }
	     ...
	     */
#ifdef DEBUG
	    else if (util_streq(arg, "-debugloadall"))
	    {
		cmdline->force_load_all = TRUE;
	    }
	    else if (util_streq(arg, "-debugtime"))
	    {
		cmdline->debug_profile_what = ABMF_PROFILE_LOAD;
	    }
	    else if (util_streq(arg, "-debugtree"))
	    {
		cmdline->dump_tree = TRUE;
	    }
	    else if (strncmp(arg, "-V", 2) == 0)
	    {
		cmdline->verbosity += (strlen(arg)-1);
	    }
#endif /* DEBUG */
            else
            {
		/* ain't never heard of this option */
		char *prog_name_string = util_get_program_name();
...
)
{
	...
	*explodedModuleFile = 0;
	    ...
	    /*
	     * The string list data specifies whether the the file name
	     * was seen on the command line or not.
	     */
	    strlist_set_str_data(fileNames, explodedModuleFile, (void*)TRUE);

	    /*
	     * Decide whether or not we're supposed to gen code for this
	     * module.
	     */
