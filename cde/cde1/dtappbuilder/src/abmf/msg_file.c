/*
 * $XConsortium: msg_file.c /main/cde1_maint/3 1995/10/16 14:27:10 rcs $
 * 
    ...
    )
{
    ...
    if (util_be_verbose())
    {
	util_printf(catgets(Dtb_project_catd,1,91, "reading existing %s\n"), filename);
    }
    ...
    )
{
    ...
    if (   (!that->is_referenced)
	|| (!that->allow_msg_delete) )
    {
	for (i = 0; i < that->num_msgs; i++)
