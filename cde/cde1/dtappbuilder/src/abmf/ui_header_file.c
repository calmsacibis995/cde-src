...
/*
 * $XConsortium: ui_header_file.c /main/cde1_maint/3 1995/10/16 14:30:31 rcs $
 * 
...
/* dtb_utils.h is dynamically named, depending on the file extension. Leave
 * as NULL in debug version to force a core dump if the name is not set.
 */
#ifndef DEBUG
static IncludeFileRec incDtbUtils = {"<DYNAMIC>"};
#else
static IncludeFileRec incDtbUtils = {(char*)NULL};
#endif
...
static IncludeFileRec *std_includes[] =
{
    &incStandardLib,
    &incIntrinsic,
    &incDtbUtils,	/* NOTE!: placeholder - get name dynamically */
    NULL,
};
...
#define add_inc(in_entry, in_class_name, in_include) \
    ...
    istr_const(incDtbUtils.include_file);
...
" *           and callbacks."
	...
	{
	    abmfP_write_c_include(genCodeInfo, std_includes[i]->include_file);
	}
    }
