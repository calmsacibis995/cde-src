...
/*
 * $XConsortium: utils.c /main/cde1_maint/3 1995/10/16 14:30:52 rcs $
 * 
...
abmfP_get_root_window(ABObj project)
{
    ...
    static ABObj	root_window= NULL;	/* NOTE: static! */
    AB_TRAVERSAL	trav;
    ABObj		window= NULL;
    ...
    if (   ((root_window = obj_get_root_window(project)) == NULL)
	&& (obj_get_write_main_func(project))
       )
    {
	/*
	 * The root window has not been set for this project. Since
	 * we are generating main() (according to obj_get_write_main_func()),
	 * we are in the driver's seat, and we'll find the best window for 
	 * the job.
	 */
