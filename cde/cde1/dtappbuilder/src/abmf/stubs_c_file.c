...
/*
 * $XConsortium: stubs_c_file.c /main/cde1_maint/4 1995/10/16 14:29:16 rcs $
 * 
...
)
{
    ...
    {
	/*
	 * 'actionName' here will substitute "%s" in the format string.
	 * Currently, only 6 instances of 'actionName' is needed but
	 * we use more so that if in the future we add more "%s"s in
	 * abmfP_lib_default_dragCB->def, this will take care of it
	 * automatically. It will also prevent core dumps!
	 *
	 * The compiler shouldn't complain since abio_printf() does
	 * take a specific number of parameters.
	 */
	 ...
	 * Currently, only 6 instances of 'actionName' is needed but
	 * we use more so that if in the future we add more "%s"s in
	 * abmfP_lib_default_dropCB->def, this will take care of it
	 * automatically. It will also prevent core dumps!
	 *
	 * The compiler shouldn't complain since abio_printf() does
	 * take a specific number of parameters.
