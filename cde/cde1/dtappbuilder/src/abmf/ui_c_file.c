...
/*
 * $XConsortium: ui_c_file.c /main/cde1_maint/3 1995/10/16 14:30:09 rcs $
 * 
...
#pragma ident "@(#)ui_c_file.c	3.115 95/03/21 SMI"
...
epilogue:
    ...
    {
	char	name[1024];
	obj_get_safe_name(obj, name, 1024);
        util_dprintf(1, 
	    "obj_should_be_managed_when_initialized(%s,[field %s]) -> %s\n",
		name, 
...
write_msg_init_proc(GenCodeInfo genCodeInfo, ABObj parent)
{
    ...
    {
        abio_printf(codeFile, "XmStringCreateLocalized(");
        if (genCodeInfo->i18n_method == ABMF_I18N_XPG4_API) 
	{
	    abmfP_write_catgets_call(genCodeInfo, parent, str);
	}
	else
	{
	...
	if (genCodeInfo->i18n_method == ABMF_I18N_XPG4_API) 
	{
	    ...
	    abmfP_write_catgets_call(genCodeInfo,parent,help_text);
            abio_puts(codeFile, ";\n");
            abio_printf(codeFile,
