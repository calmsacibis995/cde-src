/* $XConsortium: parse_c.c /main/cde1_maint/2 1995/09/06 07:48:37 lehors $ */
/*
 * File: parse_c.c
 *
...
static int	bfile_construct(BFile, FILE *file, STRING fileName);
static int	bfile_destruct(BFile);
static STRING	bfile_get_name(BFile);
...
int
abmfP_parse_c_file(FILE *cFile, STRING cFileName, CSegArray *segmentsOutPtr)
{
    int			return_value = 0;
    ...
    bfile_construct(bfile, cFile, cFileName);
	...
	{
	    int		i = 0;
	    util_dprintf(1, "Seg: %s  lines:%d-%d length:%d userSegs:%d\n", 
	    ...
	    {
		STRING	string = NULL;
		util_dprintf(1, "    =>UserSeg: %d-%d [%d+%d bytes]\n",
	...
	*/
	...
	if (segArray->segs == NULL)
	{
	    ...
	    util_printf_err("Error: out of memory!\n");
	    return_code(-1);
	}
...
)
{
    ...
    *namePtr = 0;
    ...
    while ((segType == CSEG_UNDEF) && (!bfile_eof(file)))
    {
	    ...
	    if (magicCommentType == MAGIC_CMT_USER_START)
	    {
		/*
	    ...
	    else if (magicCommentType == MAGIC_CMT_USER_END)
	    {
		/* Bad comment! */
		util_printf_err(
	   "Error: %s comment without preceding %s in %s, line %d\n",
			magicCommentUserEnd,
			magicCommentUserStart,
			bfile_get_name(file),
	...
	while (lookForIdent)
	{
		...
		{
		    util_printf_err(
		"Error: bad function definition at or near %s, line %d\n",
			bfile_get_name(file),
		...
		{
		    util_printf_err("Error: unbalanced '[' at or near %s, line %d\n",
			bfile_get_name(file),
	...
	reverse_string(ident);		/* was read backwards */
	    ...
	    *ident = 0;
	    ...
	    {
	        util_printf_err("Error: syntax error at or near %s, line %ld\n",
			bfile_get_name(file),
	...
	/* { vi hack */
        ...
        {
	    util_printf_err("Error: unbalanced '{' at %s, line %d.\n", 
								/* } vi hack */
		bfile_get_name(file),
...
find_user_seg_end(BFile file, CUserSegs userSegs)
{
	...
	return_value = -1;
	util_printf_err(
	    "Error: nested DTB_USER segment (not allowed) at %s, line %d\n", 
		bfile_get_name(file),
		cvt_offset_to_line(file, bfile_get_off(file)-1));
    }
    else if (magicCommentType != MAGIC_CMT_USER_END)
	...
	segmentIsBad = TRUE;
	return_value = -1;
	util_printf_err("Error: generated comment required but not found: %s\n",
	    magicCommentUserEnd);
    }
    else
    {
    ...
    {
	util_printf_err(
	    "Error: bad DTB_USER segment %s, beginning on line %d\n", 
		bfile_get_name(file),
...
)
{
	...
	if (userSegs != NULL)
	{
	    switch (magicCommentType)
	    {
		     ...
		     */
		    util_printf_err(
		"Error: bad DTB_USER segment beginning at %s, line %d\n", 
			    bfile_get_name(file),
			    cvt_offset_to_line(file, bfile_get_off(file))-1);
		    return_code(-1);
	        break;
...
static int
bfile_construct(BFile file, FILE *stream, STRING fileName)
{
    file->name = NULL;
    file->stream = NULL;
    file->reverse = FALSE;
...
cvt_offset_to_line(BFile file, long offset)
{
    ...
    /*
     * Return file to its state upon entry
     */
...
main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
	util_printf_err("Error: usage %s <filename>\n", 
    ...
    if (numSegs < 0)
    {
	util_printf_err("Error: failed parsing C file. Aborting\n");
	exit(1);
    }
    util_dprintf(1, "segs found in file: %d\n", numSegs);
    if (numSegs < 1)
    {
	exit(0);
    }
	...
	util_dprintf(1, "Seg: %s  offset:%d length:%d userSegs:%d\n", 
