/*
 *	$XConsortium: obj.h /main/cde1_maint/3 1995/10/16 10:34:13 rcs $
 *
 ...
 *
 * The data fields of an ABObj and its components are private! All
 * accesses must be made through obj_ functions, or data integrity
 * cannot be guaranteed. To help enforce this, many data fields are
 * given obfuscated names that make them difficult to type and
 * use directly.
 *
...
/*
 * Note: some of these string fields don't use obfuscated names because
 * the connection code uses the AB_ACTION_INFO structure to create
 * its own data structures. This should be fixed (connection should
 * use its own data structure, or an entire ABObj)!
 */
 ...
 *
 * Note: some these string fields don't use obfuscated names because
 * the connection code uses the AB_ACTION_INFO structure to create
 * its own data structures. This should be fixed (connection should
 * use its own data structure, or an entire ABObj)!
 * 
...
typedef struct _AB_ACTION_INFO
{
    ...
    ISTRING		func_name_suffix_ABOBJPRIVDDDD6703250194; 
...
/*
 * Extra info for types: AB_TYPE_BUTTON
 */
...
/*
 * Extra info for types: AB_TYPE_LABEL
 */
...
typedef struct _AB_DRAWING_AREA_INFO
{
	...
	AB_SCROLLBAR_POLICY	vscrollbar;
        int                     pane_min;
        int                     pane_max;
	ISTRING			menu_title_ABOBJPRIVDDDD0784928506;
} AB_DRAWING_AREA_INFO;

/*
 * Extra info for types: AB_TYPE_CONTAINER
 */
typedef struct _AB_CONTAINER_INFO
{
	...
	int			pane_min;
	int			pane_max;
	ISTRING			menu_title_ABOBJPRIVDDDD7908351082;
} AB_CONTAINER_INFO;
...
typedef struct _AB_FILE_CHOOSER_INFO
{
	...
	ISTRING			ok_label_ABOBJPRIVDDDD6150724264;
	BOOL			auto_dismiss;
	ISTRING			directory_ABOBJPRIVDDDD1975670834;
...
typedef struct _AB_MESSAGE_INFO 
{
        ...
        ISTRING                 msg_string_ABOBJPRIVDDDD6570852873;
	ISTRING			action1_label_ABOBJPRIVDDDD9274932408;
	ISTRING			action2_label_ABOBJPRIVDDDD7867242890;
	ISTRING			action3_label_ABOBJPRIVDDDD3150720971;
...
/*
 * Extra info for types: AB_TYPE_iTEM
 */
...
/*
 * Extra info for types: AB_TYPE_MENU
 */ 
...
typedef struct _AB_PROJECT_INFO
{
	...
	ISTRING			vendor_ABOBJPRIVDDDD5626203074;
	ISTRING			version_ABOBJPRIVDDDD1768753946;
	ISTRING			c_header_file_extension_ABOBJPRIVDDDD3268106051;
	ISTRING			c_impl_file_extension_ABOBJPRIVDDDD7635964785;
 	ABObj			root_window;
	StringList		obj_names_list;
	...
	BOOL			write_main_func;	/* write main()? */
} AB_PROJECT_INFO;
...
/*
 * Extra information for types: AB_TYPE_SCALE
 */
...
typedef struct _AB_LIST_INFO
{
	...
	AB_SCROLLBAR_POLICY	vscrollbar;
        AB_COMPASS_POINT 	label_position;
	ISTRING			menu_title_ABOBJPRIVDDDD8023717698;
} AB_LIST_INFO;

/*
 * Extra info for types: AB_TYPE_TEXT_FIELD, AB_TYPE_TEXT_PANE
 */
typedef struct _AB_TEXT_INFO
{
	...
	int			pane_min;
	int			pane_max;
	ISTRING			menu_title_ABOBJPRIVDDDD8529237357;
} AB_TEXT_INFO;

/*
 * Extra info for types: AB_TYPE_TERM_PANE
 */
typedef struct _AB_TERM_INFO
{
	...
	ISTRING			process_string_ABOBJPRIVDDDD3271493068;
        int                     pane_min;
        int                     pane_max;
 	ISTRING			menu_title_ABOBJPRIVDDDD7696870908;
} AB_TERM_INFO;

/*
 * Extra info for types: AB_TYPE_BASE_WINDOW, AB_TYPE_POPUP_WINDOW
 */
typedef struct _AB_WINDOW_INFO
{
	...
	ISTRING		icon_mask_ABOBJPRIVDDDD93883729879;
...
typedef	struct _AB_OBJ
{
	...
	unsigned long	impl_dnd_flags_ABOBJPRIVDDDD7849439167;
...
int		obj_set_c_header_file_extension(ABObj, STRING ext);
STRING		obj_get_c_header_file_extension(ABObj);
int		obj_set_c_impl_file_extension(ABObj, STRING ext);
STRING		obj_get_c_impl_file_extension(ABObj);
...
int		obj_set_write_main_func(ABObj, BOOL write_main_func);
BOOL		obj_get_write_main_func(ABObj);
