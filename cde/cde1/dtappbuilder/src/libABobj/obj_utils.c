...
/*
 * $XConsortium: obj_utils.c /main/cde1_maint/3 1995/10/16 16:01:18 rcs $
 * 
...
static void	sig_fault_handler(int param);
...
{
#ifndef DEBUG
    obj = obj;		/* avoid compiler warnings */
    return 0;
#else
    int                 spaces = 0;
...
#define print_flag(flag, flag_str) \
    ...
    *namebuf = 0;
    ...
    util_dprintf(0, "0x%08x: %s  type:%s\n",
		 obj, namebuf,
...
#define check_str(obj, _field) \
	...
	return_value = -1;
	util_dprintf(0,
	    "ERROR - bad object ptr in obj_verify(): %#x\n", obj);
	goto epilogue;
    }
	...
	if (ok)
	{
	    ...
	    sprintf((STRING)obj_name, "(ABObj %#x", obj);
...
#ifdef DEBUG
static void
sig_fault_handler(int param)
{
    param = param;		/* avoid unused param warning */
...
{
#ifndef DEBUG
    root = root;		/* avoid compiler warnings */
    return 0;
#else
    int                 return_value = 0;
    ...
    /* 
     * Check the names list for this object, if it is project or module
     */
    ...
    /* 
     * check all the modules' name lists, if this is a project
     */
