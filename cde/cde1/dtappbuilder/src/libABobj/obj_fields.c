...
/*
 * $XConsortium: obj_fields.c /main/cde1_maint/3 1995/10/16 15:54:34 rcs $
 * 
...
int
obj_set_write_main_func(ABObj obj, BOOL write_main_func)
{
    verify_for_write(obj);
    if (obj->type == AB_TYPE_PROJECT)
    {
	obj->info.project.write_main_func = write_main_func;
	return 0;
    }
    member_error(obj, "write_main_func");
    return -1;
}

BOOL
obj_get_write_main_func(ABObj obj)
{
    verify_for_read(obj);
    if (obj->type == AB_TYPE_PROJECT)
    {
	return obj->info.project.write_main_func;
    }
    return FALSE;
}
...
int
obj_set_c_header_file_extension(ABObj obj, STRING ext)
{
    verify_for_write(obj);
    if (obj->type == AB_TYPE_PROJECT)
    {
        istr_destroy(obj->info.project.c_header_file_extension);
        obj->info.project.c_header_file_extension = istr_create(ext);
	return 0;
    }
    return -1;
}
...
STRING
obj_get_c_header_file_extension(ABObj obj)
{
    verify_for_read(obj);
    return istr_string(obj->info.project.c_header_file_extension);
}

int
obj_set_c_impl_file_extension(ABObj obj, STRING ext)
{
    verify_for_write(obj);
    if (obj->type == AB_TYPE_PROJECT)
    {
        istr_destroy(obj->info.project.c_impl_file_extension);
        obj->info.project.c_impl_file_extension = istr_create(ext);
	return 0;
    }
    return -1;
}
...
STRING
obj_get_c_impl_file_extension(ABObj obj)
{
    verify_for_read(obj);
    return istr_string(obj->info.project.c_impl_file_extension);
}
