...
/*
 *	$XConsortium: obj_init.c /main/cde1_maint/3 1995/10/16 15:55:37 rcs $
 *
...
obj_construct_project(ABObj obj)
{
	...
	info->obj_names_list = strlist_create();
	strlist_set_sort_order(info->obj_names_list, STRLIST_SORT_BEST);
	info->c_header_file_extension = istr_const(".h");
	info->c_impl_file_extension = istr_const(".c");
