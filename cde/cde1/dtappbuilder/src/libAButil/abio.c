...
/*
 * $XConsortium: abio.c /main/cde1_maint/3 1995/10/16 17:04:23 rcs $
 * 
 * @(#)abio.c	1.86 03/27/97
 * 
...
int iRet;
	...
	if (iRet < 0)
		return iRet;
	iRet = ftruncate(fileno(outFile), (size_t)ftell(outFile));
	if (iRet < 0)
		return(iRet);
	return(util_fclose(outFile));
    }
    return 0;
#undef outFile
}
...
#define outFile (*pOutFile)
    ...
    *backup = 0;
     ...
     */
    if (fprintf(outFile, "%s\t%d %d\n", AB_VERSION_PREFIX,
		Util_major_version, Util_minor_version) < 0)
	iRet = ERR_WRITE;
...
abio_printf(FILE *outFile, STRING fmt, ...)
{
    ...
    if (Newline)
	if (fputs(Indent, outFile) == EOF)
	    iRet = ERR_WRITE;

    va_start(args, fmt);
    if (iRet != ERR_WRITE) 
    {
	if (vfprintf(outFile, fmt, args) == -1)
	    iRet = ERR_WRITE;
    }
    va_end(args);
...
abio_print_line(FILE *outFile, STRING fmt,...)
{
	...
	if (iRet >= 0)
	   if (vfprintf(outFile, fmt, args) >= 0)
		Newline = (fmt[strlen(fmt)-1] == '\n');
	    else iRet = ERR_WRITE;
    }
    va_end(args);
...
int iRet = 0;
	...
	iRet = abio_puts(outFile, ":project");
	break;

    case AB_TYPE_MODULE:
	iRet = abio_puts(outFile, ":module");
	break;

    case AB_TYPE_ACTION:
	iRet = abio_puts(outFile, ":connection");
	break;

    case AB_TYPE_BASE_WINDOW:
	iRet = abio_puts(outFile, ":element");
	break;
...
abio_put_handler(FILE * outFile, STRING s)
{
	...
	iRet = fputc(*abio_string_begin_string(), outFile);
	for (p = newstr + 1; (*p) && (iRet != EOF); p++)
	{
	    if ((*p == *abio_string_end_string()) || (*p == '\\'))
		iRet = fputc('\\', outFile);

	    if (iRet != EOF)
		iRet = fputc(*p, outFile);
	}
	if (iRet != EOF)
	    if (fputc(*abio_string_end_string(), outFile) == EOF)
		iRet = ERR_WRITE;
	else iRet = ERR_WRITE;
    ...
    {
	iRet = abio_gil_put_name(outFile, s);
    }
    return iRet;
}
...
abio_put_string(FILE * outFile, STRING s)
{
    ...
    if ((iRet = abio_puts(outFile, abio_string_begin_string())) >= 0)
	if ((iRet = abio_put_string_to_file(outFile, s)) >= 0)
	    iRet = abio_puts(outFile, abio_string_end_string());
...
abio_put_string_to_file(FILE * outFile, STRING string)
{
    ...
    if (string != NULL)
    {
	for (ptr = string; (*ptr != 0) && (iRet >= 0); ++ptr)
	{
	    curChar = *ptr;
