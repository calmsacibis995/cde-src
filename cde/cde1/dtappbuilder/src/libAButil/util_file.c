...
/*
 *	$XConsortium: util_file.c /main/cde1_maint/4 1995/10/23 12:17:16 rcs $
 *
...
util_fopen_locked(const char *filename, const char *accessType)
{
    ...
    {
	if (   (util_flock_named(filename, file, TRUE, lockType, 0, 0) >= 0)
	    && truncateFile)
	{
	    util_ftruncate(file, 0);
	}
    }
...
{
    return util_flock_named(NULL, file, wait, lockType, offset, length);
}
...
)
{
...
#ifndef DEBUG
    fileName = fileName;	/* avoid unused param warnings */
#else
    if (debugging() && wait)
    {
	...
	{
	    STRING	lockString = "?Unknown Type?";
	    const char *lockName = "";
	    ...
	    switch (lockType)
	    {
		case F_RDLCK: lockString = "READ"; break;
		case F_WRLCK: lockString = "WRITE"; break;
	    }
	    /* didn't get the lock - we're going to block */
	    util_dprintf(1, "Waiting for lock %s[%s]...\n", 
		lockName, lockString);
	}
    }
#endif /* DEBUG */
...
int
util_ftruncate(FILE *file, off_t length)
{
    int		fd = fileno(file);
    ...
    /*
     * We must invalidate the stream buffer. Unfortunately, there is
     * no way to do this via standard or nonstandard library calls.
     */
#if defined(__hpux)
    file->__ptr = file->__base;
    file->__cnt = 0;
#else
