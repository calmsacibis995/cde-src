...
/*
 *      $XConsortium: objxm_inst.c /main/cde1_maint/3 1995/10/16 17:15:47 rcs $
 *
...
)
{
    ...
    else
    {
        ...
        if (parentobj == 0) {widget = 0; return ERROR;} /* rhale */ 
...
)
{
	...
	else
	{
        ...
        if (util_get_verbosity() > 3)
        {
            fprintf(stderr,"INSTANTIATING: %s=%x part_of(%s), parent=%s, class=%s.\n",
...
) 
{
	...
	{
            util_dprintf(2,"WIDGET DESTROY(CB): %s=%x part_of(%s)\n",
...
)
{
    ...
    {
        util_dprintf(2,"WIDGET DESTROY: %s=%x part_of(%s)\n",
