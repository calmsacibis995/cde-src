/*
static char rcsid[] =
  "$XConsortium: SrvPalette.c /main/cde1_maint/4 1995/10/09 10:58:28 pascale $";
*/
/*                                                                      *
...
int 
#ifdef _NO_PROTO
InitializeDtcolor( dpy, sessionType )
        Display *dpy ;
        short sessionType ;
#else
InitializeDtcolor(
...
#endif /* _NO_PROTO */
{
  ...
  /* find out what type of monitor(s?) is being used */
   ...
   /* Set the *background: resource 
     ...
     /* For HIGH_COLOR the default primary colorset is 4, else 1 */
    if(colorSrv.TypeOfMonitor[0] != B_W) 
    {
       int chipnum = colorSrv.TypeOfMonitor[0] == HIGH_COLOR ? 4 : 1;
       sprintf(xrdb_string,
     ...
     /* For B_W the resources depended on whether the default palette is
...
#endif /* _NO_PROTO */
{
  ...
  pixel_set_atom = XInternAtom(dpy, _SUN_SDT_COLOR_OBJECT_CACHE, FALSE) ;
  ...
  *(palette->converted + palette->converted_len) = SUN_SDT_COLOR_OBJECT_VERSION ;
  ...
  XChangeProperty(dpy, win, pixel_set_atom, XA_STRING, 8, PropModeAppend, 
		 XtNewString(palette->converted), palette->converted_len) ;
}
...
#endif /* _NO_PROTO */
{
   ...
   /* cycle through each screen */
    for(screen_number=0;screen_number != colorSrv.NumOfScreens;screen_number++)
    {
       ...
       XtSetArg (args[n], XmNwidth, 1); n++;
       XtSetArg (args[n], XmNheight, 1); n++;
       shell[screen_number] = XtAppCreateShell(screenStr, COLOR_SRV_NAME, 
                                               applicationShellWidgetClass, 
                                               dpy, args, n);
	...
	*/
       ...
       sprintf(cust_msg,"%s%d", CUST_DATA, screen_number);
       colorSrv.XA_CUSTOMIZE[screen_number] = 
	   ...
	   sprintf(tmpStr,"%s colorSrv.XA_CUSTOMIZE[%d].\n", 
		   MSG1, screen_number);
	   _DtSimpleError(COLOR_SRV_NAME, DtWarning, NULL, tmpStr, NULL);
	   SRV_FREE(tmpStr);
	   return(-1);
       }
      ...
      /* write out the color or monochrome palette resource for the screen */
       ...
       if (colorSrv.TypeOfMonitor[0] == HIGH_COLOR || 
           colorSrv.TypeOfMonitor[0] == MEDIUM_COLOR ||
           colorSrv.TypeOfMonitor[0] == LOW_COLOR)
       {
           sprintf(xrdb_string, "*%d*ColorPalette: %s%s\n",
                   screen_number, colorSrv.pCurrentPalette[0]->name, PALETTE_SUFFIX);
       }
       else /* B_W */
       {
...
static char *
#ifdef _NO_PROTO
convert_pixel_set( typeOfMonitor, color )
        int typeOfMonitor;
        ColorSet *color;
#else
convert_pixel_set(
...
#endif /* _NO_PROTO */
{
  ...
  const int colormapping [4][MAX_NUM_COLORS] = {
    {0, 1, 2, 3, 4, 5, 6, 7}, /* HIGH_COLOR */
    {0, 1, 2, 3, 1, 1, 2, 1}, /* MEDIUM_COLOR */
    {0, 1, 1, 1, 1, 1, 1, 1}, /* LOW_COLOR */
    {0, 1, 1, 1, 1, 1, 1, 1}  /* B_W */
  };
  ...
  const int colormapping [4][MAX_NUM_COLORS] = {
    {0, 1, 2, 3, 4, 5, 6, 7}, /* HIGH_COLOR */
    {0, 1, 2, 3, 3, 3, 3, 3}, /* MEDIUM_COLOR */
    {0, 1, 1, 0, 0, 0, 0, 0}, /* LOW_COLOR */
    {0, 1, 1, 0, 0, 0, 0, 0}  /* B_W */
  };
  #endif
  ...
  {
    case HIGH_COLOR:   colormappingindex = 0; break;
    case MEDIUM_COLOR: colormappingindex = 1; break;
    case LOW_COLOR:    colormappingindex = 2; break;
    case B_W:          colormappingindex = 3; break;
  } 
...
#ifdef SUN_SDT_COLOR_OBJECT_CACHE
     Atom pixel_set_atom ;
     pixel_set_atom = XInternAtom(XtDisplay(w), _SUN_SDT_COLOR_OBJECT_CACHE, FALSE) ;
   ...
   if(status == False)
   {
       ...
       else
       {
        ...
        /* Now actually allocate R/W pixels */
              ...
              {
                 SRV_FREE((char *) pixels);
                 _DtSimpleError(COLOR_SRV_NAME, DtWarning, NULL, ((char *)GETMESSAGE(26, 5, "Warning, can't allocate enough pixels.\n")), NULL);
                 return(False);
              }
           }
       }
