 ...
 ****************************************************************************
 ************************************<+>*************************************/
/* $XConsortium: ColorFile.c /main/cde1_maint/4 1995/10/23 11:09:52 gtsang $ */
/*+++++++++++++++++++++++++++++++++++++++*/
/* include files                         */
/*+++++++++++++++++++++++++++++++++++++++*/
...
static void SwitchAItoPS();
static palette *RemovePaletteFromList();
...
void 
#ifdef _NO_PROTO
ReadPalette( directory, in_filename, length )
        char *directory ;
        char *in_filename ;
        short length ;
#else
...
/****************************************************************************
 *
 * SwitchAItoPS - used to in a LOW_COLOR system to switch the Acitive and
 ...
 * The check is done by issuing a pathconf(..., _PC_NAME_MAX) call on the
...
#endif /* _NO_PROTO */
{
     ...
     /*
      * Allocate space for the name of the directory to which the long file
      * name support query will pertain.
      */
     dirname = (char *)XtMalloc(strlen(style.home) + strlen(DT_PAL_DIR) + 1);
     if (dirname == NULL)
     {
 	/* Report a conservative result. */
 	style.longfilename = False;
 	return;
     }
 
     (void) sprintf(dirname, "%s%s", style.home, DT_PAL_DIR);
     rv = pathconf(dirname, _PC_NAME_MAX);
     if (rv == -1)
     {
 	style.longfilename = False;
     }
     ...
     {
 	style.longfilename = rv > 14 ? True : False;
     }
