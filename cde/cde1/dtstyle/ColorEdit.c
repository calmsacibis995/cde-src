/* $XConsortium: ColorEdit.c /main/cde1_maint/3 1995/10/23 11:08:41 gtsang $ */
/************************************<+>*************************************
 ***************************************************************************
 **
 **   File:        ColorEdit.c
...
static void UpdateOldButton( ColorSet *oldButton);
...
#endif /* _NO_PROTO */
{
        ...
        if (!XtIsManaged(edit.DialogShell))
	{
	    ...
	    CopyPixelSet(&edit.oldButtonColor,edit.color_set);
            ...
            if(TypeOfMonitor != B_W && !style.dynamicColor)
	      UpdateOldButton(&edit.oldButtonColor);
...
#endif /* _NO_PROTO */
{
	...
	XmStringFree(string);
        ...
        if(TypeOfMonitor != B_W && !style.dynamicColor)
	  UpdateOldButton(edit.color_set);
...
#endif /* _NO_PROTO */
{
	...
	/* if static color visual, update the color chip selected by 
	   setting it's resources */

        if(TypeOfMonitor != B_W && !style.dynamicColor)
	   XtSetValues(edit.chip_selected, args,n);
...
#endif /* _NO_PROTO */
{
    ...
    if(TypeOfMonitor != B_W && style.dynamicColor)
       XStoreColors(style.display, style.colormap, colors, j );
         ...
         /* free the cell from last selection */
       for (i=0; i< j; i++) {
         XFreeColors( style.display, style.colormap, &colors[i].pixel, 1, 0);
 
        switch(i) {
          case 0:
           if ((XAllocColor(style.display, style.colormap, &colors[i])) == 0)
            break;
           else
                edit.color_set->bg.pixel = colors[i].pixel;
            break;
          case 1:
           if ((XAllocColor(style.display, style.colormap, &colors[i])) == 0)
            break;
           else
                edit.color_set->sc.pixel = colors[i].pixel;
            break;
          case 2:
           if ((XAllocColor(style.display, style.colormap, &colors[i])) == 0)
            break;
           else
                edit.color_set->fg.pixel = colors[i].pixel;
            break;
          case 3:
           if ((XAllocColor(style.display, style.colormap, &colors[i])) == 0)
            break;
           else
                edit.color_set->ts.pixel = colors[i].pixel;
            break;
          case 4:
           if ((XAllocColor(style.display, style.colormap, &colors[i])) == 0)
            break;
           else
                edit.color_set->bs.pixel = colors[i].pixel;
            break;
          default:
                break;
        }
...
static void
#ifdef _NO_PROTO
UpdateOldButton( ColorSet *oldButton)
#else
UpdateOldButton(
        ColorSet *oldButton)
#endif /* _NO_PROTO */
{
 ...
 static unsigned long   pixels[MAX_NUM_COLORS*5];
  ...
  colors[j++] =  oldButton->bg;
  colors[j++] =  oldButton->sc;
  colors[j++] =  oldButton->fg;
  colors[j++] =  oldButton->ts;
  colors[j++] =  oldButton->bs;
  ...
  for (i=0; i< j; i++) {
        switch(i) {
          case 0:
           if ((XAllocColor(style.display, style.colormap, &colors[i])) == 0)
            break;
                oldButton->bg.pixel = colors[i].pixel;
            break;
          case 1:
           if ((XAllocColor(style.display, style.colormap, &colors[i])) == 0)
            break;
                oldButton->sc.pixel = colors[i].pixel;
            break;
          case 2:
           if ((XAllocColor(style.display, style.colormap, &colors[i])) == 0)
            break;
                oldButton->fg.pixel = colors[i].pixel;
            break;
          case 3:
           if ((XAllocColor(style.display, style.colormap, &colors[i])) == 0)
            break;
                oldButton->ts.pixel = colors[i].pixel;
            break;
          case 4:
           if ((XAllocColor(style.display, style.colormap, &colors[i])) == 0)
            break;
                oldButton->bs.pixel = colors[i].pixel;
            break;
          default:
                break;
        }
        pixels[count++] = colors[i].pixel;
 } /* for */
 
 n=0;
 XtSetArg(args[n], XmNbackground, colors[0].pixel); n++;
 XtSetArg(args[n], XmNarmColor,   colors[1].pixel); n++;
 XtSetArg(args[n], XmNforeground, colors[2].pixel); n++;
 if(UsePixmaps == FALSE)
 {
    XtSetArg(args[n], XmNtopShadowColor,
                                      colors[3].pixel); n++;
