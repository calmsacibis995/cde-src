/* $XConsortium: calctool.c /main/cde1_maint/3 1995/10/05 17:17:48 mgreess $ */
/*									*
 *  calctool.c                                                          *
...
int n ;
{
  return((v->curwin == FCP_KEY) ? buttons[n].resname :
          mode_buttons[MODEKEYS * ((int) v->modetype - 1) + n].resname) ;
}
...
int n ;
{
  return((v->curwin == FCP_KEY) ? buttons[n].str2 :
          mode_buttons[MODEKEYS * ((int) v->modetype - 1) + n].str2) ;
}
...
char **argv ;
{
  ...
  write_cmdline() ;                 /* Setup dtcalc command line. */
...
/*  Added in by brianc for bug id 1258470

Set DEC and FIX accuracy = 2
0.19 / 2 displays as 0.0A, change accuracy to 3 answer is 0.095

This is caused by the storage structure used to store the fraction part of
the number. To determine how this is displayed  we determine the integer part of the number
then add the decimal point, and keep multiplying the fraction part by 10, adding the integer value
(one char) onto the display string. This works in general for 99.9999999% of numbers but the
encoding fails to round off for some.

For DEC and FIX, with accuracy set to 2 enter 0.095 and hit = answer is 0.0a
for DEC and FIX with accuracy set to 3 enter 0.0095 and hit = answer is 0.00a

Works correctly for any other accuracy setting

For DEC and FIX, with accuracy set to 2 enter 0.09500000000000000000000000000000000001 and hit = answer is 0.10
for DEC and FIX with accuracy set to 3 enter 0.00950000000000000000000000000000000001 and hit = answer is 0.010

when evaluating the 0.95 it fails to round up to 1, so 0 gets added to the display string
then evaluating 9.5 does round up to 10, it tries to add the 10th Char in digits onto the
display string. digits = {"0123456789ABCDEF"}, which results in the A being added.

To trap this error we check that the value received is not greater that the Base we are using,
I have added code for BIN, OCT and HEX as well as DEC. Although I have been unable to generate
this code under the other bases I think that an encoding does exist that can have the number
2 appear in binary 8 in Octal, if a coding does exist in HEX this bug would crash dtcalc.

Because this has not happened yet, may only be due toi the fact that no-one has encountered
the number yet.     */
...
/* SunOS 5.4 doesn't define struct exception with _XOPEN_SOUNCE,
   so this will generate a compilation error.  The declaration above
   in the function arglist gives a warning, but we'll just tolerate
   that for now
*/
...
read_rcfiles()   /* Read .dtcalcrc's from home and current directories. */
{
  ...
  char name[MAXLINE] ;          /* Full name of users .dtcalcrc file. */
