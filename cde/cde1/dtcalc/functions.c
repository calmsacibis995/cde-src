/* $XConsortium: functions.c /main/cde1_maint/4 1995/10/10 15:37:29 pascale $ */
/*									*
 *  functions.c                                                         *
...
/* Get rid of IEEE math lib. dependency */
 ...
 */
                              ...
                              || v->funstate == 0)
      {
          ...
          else
          {
             accSav = v->accuracy;
             v->accuracy = 2;
             MPstr_to_num(make_number(v->MPdisp_val, FALSE), 
                                                   DEC, v->MPdisp_val);
             /* set RATE register */
      ...
      accSav = v->accuracy;
      v->accuracy = 2;
      MPstr_to_num(make_number(v->MPdisp_val, FALSE), DEC, v->MPdisp_val);
      v->accuracy = accSav;
      if(!v->error)
         make_registers(FIN) ;
      v->funstate = 1;
  ...
  if (IS_KEY(v->current, KEY_LPAR))
    {
      if (v->noparens == 0)
      {
         ...
         if(tmpdb != 0.0  && (v->cur_op == '?' || v->cur_op == 'x'))
         {
