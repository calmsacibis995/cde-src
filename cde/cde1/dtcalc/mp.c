/* $XConsortium: mp.c /main/cde1_maint/1 1995/07/17 19:37:08 drk $ */
/*                                                                      *
 *  mp.c                                                                *
...
/* next two defines added by brianc */
  int tmpcpy[MP_SIZE], tmpacc; /* temp store for the number and the accuracy precision */
  char tmpdisp[MAXLINE];       /* setup a string to store what would be displayed */
  enum num_type tmpdtype;      /* setup a temp display type variable */

  static int i, il, ll;        /* ll added to record num of integer cells in the Number */
...
/* The above section of code sets the fraction part of a number to 0, however the
   following calculation  ( 10000 * 0.58 ) displays a value of 5800, this is stored
   as Integer part 5799  fraction part .99999999999999. So when we do an INT on the 
   value displayed we get 5799 not 5800 as we would expect. 
   
   When a value is displayed it is allways rounded up to the Accuracy setting. If
   Accuracy is set to two places decimal and we type in 2.999, then hit the EQUALS 
   (=) button the value changes to 3.00. However clicking on INT will produce 2 as
   it should.
   
   As the Maximum Accuracy is 9 places decimal, we temporarily set the accuracy to 9
   
   Temporarily set the display type to FIX. This is because FIX will show 1.0 or 0.xx
   all other display types shoe 1.0 -e5. As we are looging for a fraction part that
   will round up to 1.0 we need to set it to FIX.
   
   Determine the Fraction part of the Number
   
   if it is 1.0 then add one to the integer part and return the users settings
   for accuracy and display type to what they were.
 ...
 v->accuracy = ACC_END;
 mpcmf(&x[1], tmpcpy);
 STRCPY(tmpdisp, make_number(tmpcpy, TRUE));
