/* $XConsortium: proxy.c /main/cde1_maint/2 1995/09/06 07:16:31 lehors $ */
...
/* DSDM - protocol shared with dtwm */
    ...
    Window dsdm_win;
{
    ...
    ATOM_MOTIF_DRAG_WIN = XInternAtom (dpy, "_MOTIF_DRAG_WINDOW", False);
    ATOM_MOTIF_PROXY_WIN = XInternAtom(dpy, "_MOTIF_DRAG_PROXY_WINDOW", False);
    ATOM_MOTIF_DND_MESS = XInternAtom(dpy, "_MOTIF_DRAG_AND_DROP_MESSAGE", False);
    ATOM_MOTIF_INITIATOR_INFO = XInternAtom(dpy, "_MOTIF_DRAG_INITIATOR_INFO", False);
    ATOM_MOTIF_RECEIVER_INFO = XInternAtom(dpy, "_MOTIF_DRAG_RECEIVER_INFO", False);
    ATOM_MOTIF_SUCCESS = XInternAtom(dpy, "XmTRANSFER_SUCCESS", False);
    ATOM_MOTIF_FAILURE = XInternAtom(dpy, "XmTRANSFER_FAILURE", False);
    ATOM_MOTIF_TARGETS = XInternAtom (dpy, "_MOTIF_DRAG_TARGETS", False);

    ATOM_SUN_DND_TRIGGER = XInternAtom(dpy, "_SUN_DRAGDROP_TRIGGER", False);
    ATOM_SUN_DND_DONE = XInternAtom(dpy, "_SUN_DRAGDROP_DONE", False);
    ATOM_SUN_SELECTION_END = XInternAtom(dpy, "_SUN_SELECTION_END", False);
    ATOM_SUN_DND_ACK = XInternAtom(dpy, "_SUN_DRAGDROP_ACK", False);

    ATOM_TARGETS = XInternAtom(dpy, "TARGETS", False);
    ATOM_MULTIPLE = XInternAtom(dpy, "MULTIPLE", False);

    /* DSDM - initialize atoms used in protocol with dtwm */
    ATOM_DT_PROXY_STOP = XInternAtom(dpy, "_DT_PROXY_STOP", False);
    ATOM_DT_PROXY_START = XInternAtom(dpy, "_DT_PROXY_START", False);
    ATOM_DT_PROXY_WINDOW = XInternAtom(dpy, "_DT_PROXY_WINDOW", False);

    for (i=0; i<DROP_TABLE_MAX; i++) {
	sprintf(buf, "DND_PROXY_HANDLE_%d", i);
	drop_table[i].proxy_handle = XInternAtom(dpy, buf, False);
...
#ifdef DEBUG
    printf("motif_drag_win=%d, proxy_win=%d\n", motif_drag_win, proxy_win);
#endif
    ...
    XEvent *event;
{
	...
	/* DSDM - messages from the dtwm */
	else if (event->xclient.message_type == ATOM_DT_PROXY_STOP) {
#ifdef DEBUG
	  printf("dsdm: Got stop command.\n");
#endif
	  XSelectInput(dpy, DefaultRootWindow(dpy), NoEventMask);
	}
	else if (event->xclient.message_type == ATOM_DT_PROXY_START) {
#ifdef DEBUG
	  printf("dsdm: Got start command.\n");
#endif
	  XSelectInput(dpy, DefaultRootWindow(dpy), SubstructureNotifyMask);
	}
	else if (event->xclient.message_type == ATOM_DT_PROXY_WINDOW) {
#ifdef DEBUG
	  printf("dsdm: Got window message.\n");
#endif
	  XSelectInput(dpy, DefaultRootWindow(dpy), NoEventMask);
	  AdvertiseMotifDropSite(dpy, event->xclient.window);
	}
	...
	if (client_win) {
	    /* if client has olit dnd interest then attach motif dnd
	       interest to client */
	    if (GetPropertyWindow(dpy, client_win, ATOM_DRAGDROP_INTEREST)) {
    ...
    case PropertyNotify:
/*	printf("PropertyNotify, win=0x%x\n", 
	...
	drop_info_t *drop_info;
#ifdef DEBUG
	printf("SelectionRequest, selection=%s, target=%s, prop=%s, win=0x%x, time=%d\n",
...
#ifdef DEBUG
	printf("SelectionNotify, sel=%s, target=%s, prop=%s, w=0x%x, time=%d\n",
