/* $XConsortium: Icon.c /main/cde1_maint/3 1995/10/23 10:10:45 gtsang $ */
#include <stdio.h>
...
static XtResource resources[] =
{
    ...
    {
    GuiNtopLabelString, XmCXmString, XmRXmString, sizeof(_XmString),
    XtOffsetOf(struct _IconRec, icon.top_label), XmRImmediate,
    (XtPointer) NULL
    },
    {
    GuiNbottomLabelString, XmCXmString, XmRXmString, sizeof(_XmString),
    ...
    {
    GuiNiconShadowThickness, GuiCIconShadowThickness, XmRDimension,
    sizeof(Dimension),
    ...
    {
    GuiNiconMarginThickness, GuiCIconMarginThickness, XmRDimension,
    sizeof(Dimension),
    ...
    {
    GuiNfields, GuiCFields, XmRPointer, sizeof(XmRPointer),
    ...
    )
{
    ...
    if (!GuiIsIcon(w))
      {
        _XmWarning(w, "Cannot set field name width of non-subclass of GuiIcon");
        return;
      }
    ...
    )
{
    ...
    if (!GuiIsIcon(w))
      {
        _XmWarning(w, "Cannot get field name width of non-subclass of GuiIcon");
        return 0;
      }
    if (fields = Fields(w))
    ...
    Boolean active)
{
    ...
    if (!GuiIsIcon(w))
      {
        _XmWarning(w, "Cannot set icon fields of non-subclass of GuiIcon");
        return;
      }
    if (fields = Fields(w))
      {
    ...
    Boolean *active)
{
    ...
    if (!GuiIsIcon(w))
      {
        _XmWarning(w, "Cannot get icon fields of non-subclass of GuiIcon");
        return;
      }
    if (fields = Fields(w))
      {
   ...
   )
{
    if (!GuiIsIcon(w))
      {
        _XmWarning(w, "Cannot get icon rectangles of non-subclass of GuiIcon");
        return;
      }
   ...
   )
{
    ...
    if (Font(new) == NULL)
      {
        ...
        defaultFont = _XmGetDefaultFontList((Widget) new, XmLABEL_FONTLIST);
    ...
    if (Pixmap(new) == XmUNSPECIFIED_PIXMAP && Label(new) == NULL)
      {
        XmString string;
        string = (XmString)_XmOSGetLocalizedString((char *)NULL, 
	    (Widget) new, XmNlabelString, new->core.name);
        Label(new) = _XmStringCreate(string);
        XmStringFree (string);
      }
    else if (Label(new) != NULL)
      {
        if (_XmStringIsXmString((XmString) Label(new)))
            Label(new) = _XmStringCreate((XmString) Label(new));
        else
          {
            XmString string;
            string = (XmString)_XmOSGetLocalizedString((char *)NULL,
    ...
    if (TopLabel(new) && _XmStringIsXmString((XmString) TopLabel(new)))
	TopLabel(new) = _XmStringCreate((XmString) TopLabel(new));
    else
	TopLabel(new) = NULL;
    if (BottomLabel(new) && _XmStringIsXmString((XmString) BottomLabel(new)))
	BottomLabel(new) = _XmStringCreate((XmString) BottomLabel(new));
    else
   ...
   )
{
    ...
    _XmFontListGetDefaultFont(Font(w), &fs);
...
{
    if (Label(w))
        _XmStringFree(Label(w));
    if (TopLabel(w))
        _XmStringFree(TopLabel(w));
    if (BottomLabel(w))
        _XmStringFree(BottomLabel(w));
    if (Font(w))
   ...
   )
{
    ...
    if (Label(w) != NULL)
      {
        _XmStringExtent(Font(w), Label(w), &LabelWidth(w), &LabelHeight(w));
        LabelWidth(w) += 4;
        LabelHeight(w) += 4;
      }
    else
	...
	Dimension max_width;
        ...
        if (TopLabel(w))
	  {
            _XmStringExtent(Font(w), TopLabel(w), &TopLabelWidth(w),
	        &TopLabelHeight(w));
	    TopLabelWidth(w) += 4;
	  }
	else
	  {
        ...
        if (BottomLabel(w))
	  {
            _XmStringExtent(Font(w), BottomLabel(w), &BottomLabelWidth(w),
	        &BottomLabelHeight(w));
   ...
   )
{
	...
	if (Selected(w) == True)
		...
		_XmStringDraw(XtDisplay(w), XtWindow(w), Font(w), Label(w),
		   label_gc, LabelX(w) + 2, LabelY(w) + 2, LabelWidth(w) - 4,
		   Alignment(w), StringDirection(w), NULL);
              }
            if (TopLabel(w) != NULL)
              {
		_XmStringDraw(XtDisplay(w), XtWindow(w), Font(w), TopLabel(w),
            ...
            if (BottomLabel(w) != NULL)
              {
		_XmStringDraw(XtDisplay(w), XtWindow(w), Font(w),
    ...
    if (Font(new) != Font(cur))
      {
        ...
        if (Font(new) == NULL)
            Font(new) = _XmGetDefaultFontList((Widget) new, XmLABEL_FONTLIST);
        Font(new) = XmFontListCopy(Font(new));
        _XmStringUpdate(Font(new), Label(new));
      }
   ...
   )
{
    ...
    string = _XmStringCreateExternal(Font(w), Label(w));
   ...
   )
{
    ...
    string = _XmStringCreateExternal(Font(w), TopLabel(w));
   ...
   )
{
    ...
    string = _XmStringCreateExternal(Font(w), BottomLabel(w));
