...
extern "C" {
   extern void _XmFontListGetDefaultFont(XmFontList, XFontStruct **);
} 
	...
	: MotifUI(NULL, name, appClassName, appClassName)
{
   ...
   fs = (XFontStruct *)XmFontListEntryGetFont(userFont, &_type_return);
   if (_type_return != XmFONT_IS_FONT)
      _XmFontListGetDefaultFont(userFont, &fs);
   font = fs->fid;
