      ...
      Application(progname, "Dtprintinfo", argc, argv)
{
   ...
   if (app_mode == CONFIG_PRINTERS)
      window = new DtMainW("Printers", this, app_name, SCROLLED_WORK_AREA,
			   SINGLE_SELECT, OpenClose2, NULL, PreferenceCB2, NULL,
...
#ifdef aix
	  {
	    ...
	    if (rc->FindByName(MESSAGE(EmptyL)))
	    ...
	    else
	     {
           ...
           if (checkedForDefault == 2) {
               	window->status_line->Name("");
           }
           else if (checkedForDefault == 1) {
               	sprintf(msg2, MESSAGE(PrinterNotFound1L), argv[2]);
...
// OpenClose is an overloaded function.  OpenClose2 is provided here as a 
// non-overloaded function that can be passed by address.  This is needed
// for some C++ compliers (including SunPro 3.0.1) that don't allow passing
// of overloaded function addresses. 

void OpenClose2(void *data, BaseUI *obj)
{
   DtApp::OpenClose(data, obj);
}
...
// PreferenceCB is an overloaded function.  PreferenceCB2 is provided here
// as a non-overloaded function that can be passed by address.  This is needed
// for some C++ compliers (including SunPro 3.0.1) that don't allow passing
// of overloaded function addresses. 

void PreferenceCB2(void *data, PreferenceRequest req, char *value)
{
   DtApp::PreferenceCB(data, req, value);
}
      ...
      if (argc >= 3)
       {
         ...
         if (found == -1) {
	 // Issues error message and return
         window->container->EndUpdate();
         window->PointerShape(LEFT_SLANTED_ARROW_CURSOR);
         checkedForDefault = 1;
         UpdateStatusLine();
	 char *tmp = new char [strlen(MESSAGE(PrinterNotFound2L)) + 5];
	 sprintf(tmp, "%s", MESSAGE(PrinterNotFound2L));
         Dialog *dialog = new Dialog(window, (char *)Name(), tmp,
				     INFORMATION, MESSAGE(OKL));
         dialog->Visible(true);
         delete prt;
         delete tmp;
