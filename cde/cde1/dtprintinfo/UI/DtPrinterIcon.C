    ...
    if ( stat(dir, &file_status) != 0) {
        ...
        if ( (file_status.st_mode & dir_mode) != dir_mode ) {
            ...
            if ( chmod(dir, file_status.st_mode | dir_mode) == 0) {
                fprintf(stdout, "Set permissions on %s\n", dir);
            } else {
            	fprintf(stderr, "Unable to set permissions on %s\n", dir);
            }
        }
    }
}
...
char *DtPrinterIcon::CreateActionFile()
{
   ...
   if (app_mode == INITIALIZE_PRINTERS || app_mode == CONFIG_PRINTERS)
	...
	dtmkdir("/etc", 0755);
	dtmkdir("/etc/dt", 0755);
	dtmkdir("/etc/dt/appconfig", 0755);
	dtmkdir("/etc/dt/appconfig/types", 0755);
	dtmkdir(filename, 0755);
...
boolean DtPrinterIcon::PrintActionExists()
{
   char buf[60]; 
   sprintf(buf, "%s_Print", queue->Name());
   return (DtActionExists(buf) ? true : false);
}
