...
void DtFindSet::InitComboBox(BaseUI *obj, void * /*data*/)
{
   ...
   findSet->Refresh();
   ComboBoxCB(findSet->comboBox, findSet->dirs[0], 0);
}
...
void DtFindSet::ComboBoxCB(ComboBoxObj *obj, char *dir, int position)
{
   ...
   if (findSet->last_position != -1)
    {
      filenames = findSet->filenames[findSet->last_position];
      FindSetIcon **icon = filenames->icons;
      for (i = 0; i < filenames->n_icons; i++, icon++)
         (*icon)->Visible(false);
    }
   filenames = findSet->filenames[position];
