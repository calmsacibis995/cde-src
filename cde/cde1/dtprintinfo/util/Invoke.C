...
Invoke::Invoke(const char *command,   // Command to Run
               ...
               int _uid)              // run command as this UID
{
   ...
   while (trap_out || trap_err)
    {
      ...
      for (i = 0; i <= _NFILE / 32; i++)
	 rdmask[i]= wrmask[i]= exmask[i]= 0;
      ...
      // check the status
      if (select(_NFILE,rdmask,wrmask,exmask,(struct timeval *)NULL) == -1)
#else
      // reset the file descriptor masks
      FD_ZERO(&rdmask);
      FD_ZERO(&wrmask);
...
#ifdef hpux
      if (trap_out && (rdmask[m_stdout[0]/BITS(int)] & (1 << (m_stdout[0]%32))))
#else
...
#ifdef hpux
      if (trap_err && (rdmask[m_stderr[0]/BITS(int)] & (1 << (m_stderr[0]%32))))
#else
