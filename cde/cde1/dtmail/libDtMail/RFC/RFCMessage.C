...
#pragma ident "@(#)RFCMessage.C	1.68 03/02/96"
#endif
...
RFCMessage::toBuffer(DtMailEnv & error, DtMailBuffer & buf)
{
    ...
    memcpy(buf.buffer, _msg_start, buf.size);
}
...
RFCMessage::getDeleteTime(DtMailEnv & error)
{
    ...
    _envelope->getHeader(error, RFCDeleteHeader, DTM_FALSE, value);
    if (error.isNotSet()) {
	delete_time = strtol(*(value[0]), NULL, 16);
    }
			   ...
			   char ** msgBodyStart, long &msgBodyLen)
{
  ...
  //
  const size_t maxHeaderLength = ((RFCEnvelope *)_envelope)->headerLength();
...
RFCMessage::adjustMessageLocation(char * oldStart, char * newStart, long newLength, int msgTemporary, long newBodyOffset)
{
  ...
  //
  int len = (_msg_end - _msg_start) + 1 + newBodyOffset;
  ...
  if (_alternativeMultipart == DTM_TRUE) {
    ...
    for (int altCacheSlot = 0; altCacheSlot < altCacheMaxSlot; altCacheSlot++) {
      ...
      int altHeaderLength = ((RFCEnvelope *) amc->amc_msg->_envelope)->headerLength();
		     ...
		     const char * end_of_file)
{
    ...
    // "\n<any-white-space>\n"
    //
    for (const char * hdr_end = _msg_start; hdr_end <= end_of_file; hdr_end++) {
	if (*hdr_end == '\n') {
	    int blanks_only = 1;
	...
	//
	for (const char * next_from = hdr_end + 1;
...
RFCMessage::findMsgEnd(DtMailEnv & error, const char * eof)
{
	...
	//
	if (next_msg <= eof) {
	    for (const char *white = next_msg; white <= eof && isspace((unsigned char)*white);
		 white++) {
		continue;
	    }
...
RFCMessage::parseBodies(DtMailEnv & error)
{
	...
	if (error.isSet()) {
		...
		//
		for (const char **cp = SCANLIST; *cp; cp++) {
...
RFCMessage::parseV3Bodies(DtMailEnv & error)
{
  error.clear();
	...
	//
	for (const char * body = _body_start - 1; body <= _msg_end; body++) {
	    if (*body == '\n' && 
		strncmp(body + 1, "----------", 10) == 0 &&
		(*(body + 11) == '\n' || 
		 *(body + 11) == '\r' && *(body + 12) == '\n')) {
...
int
RFCMessage::sizeMIMEBodies(DtMailEnv & /* error */)
{
...
int
RFCMessage::sizeV3Bodies(DtMailEnv & /* error */)
{
  // We will use _msg_end - _body_start because this is
  // both the correct body length, and the one we updated.
  //
...
RFCMessage::extractBoundary(const char * content_type)
{
	...
	sprintf(new_bdry, "%08x-%08x-%08x", new_bdry, time(NULL), &new_bdry);
    }
    ...
    //
    for (const char * b_end = boundary; *b_end; b_end++) {
	if (quoted) {
	    if (*b_end == '"') {
		break;
	    }
...
RFCMessage::hasHeaders(const char * buf, const unsigned long len)
{
    ...
    //
    for (const char * colon = start; colon < (buf + len) && *colon != ':'; colon++) {
	continue;
    }
