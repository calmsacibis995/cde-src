...
#pragma ident "@(#)SunV3.C	1.35 15 Feb 1995"
#endif
...
// End of For CHARSET
	...
	buf.appendData(bp_contents, bp_len);
...
// For CHARSET
// [gregl 8/16/95]	In order to be compatable with V3 mailtool, we should
//			not encode multipart messages.  Just set the
//			X-Sun-Charset header to iso-8859-1.
//
// 	if ( strcasecmp(v3type, "text") == 0 ) {
// 	   if ( sevenbit == DTM_FALSE ) {
// 		from_cs = NULL;
...
// End of For CHARSET
	    ...
	    //  Set the Charset to iso-8859-1 for compatability with
	    //  Openwindows V3 mailtool.
	    if ( strcasecmp(v3type, "text") == 0 ) {
		buf.appendData("X-Sun-Charset: ", 15);
		appendString(buf, "iso-8859-1");
		crlf(buf);
	    }
	      ...
	      const unsigned long inputLen)
{
	...
	*end = 0;
	...
	if (interimBp != inputBp) {	// in an interim buffer - slosh to output side
	    *outputBp = interimBp;
	    interimBp = 0;
	    outputLen = interimLen;
	    interimLen = 0;
	}
	else {	// original source - must copy into newly allocated buffer
	    *outputBp = (char *)malloc(inputLen);
	    memcpy(*outputBp, inputBp, inputLen);
	    outputLen = inputLen;
	}
    }
		...
		const unsigned long len)
{
    ...
    for (const unsigned char * cur = ubp; cur < (ubp + whole_lines - 2); cur += 45) {
	buf.appendData("M", 1);
