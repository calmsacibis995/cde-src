...
#pragma ident "@(#)V3BodyPart.C	1.41 02/13/96"
#endif
...
: RFCBodyPart(error, parent, start, 0, NULL)
{
	...
	*end = body_end;
    ...
    if (error.isNotSet()) {
      ...
      //
      int content_length = strtol(*(value[0]), NULL, 10);
	...
	//
	int content_lines = strtol(*(value[0]), NULL, 10);
	int lines = 0;
...
//    error.clear();
    ...
    char ** types = DtDtsFindAttribute("SUNV3_TYPE",
				       v3_type);
...
void
V3BodyPart::loadBody(DtMailEnv & error)
{
// For CHARSET
    char cs[64], *to_cs = NULL, *from_cs = NULL;
// End of For CHARSET
    int do_decode = 1;
...
char *
V3BodyPart::csFromContentType(DtMailValueSeq &value)
{
   return NULL;
}
