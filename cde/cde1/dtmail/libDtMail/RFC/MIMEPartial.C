...
#pragma ident "@(#)MIMEPartial.C	1.4 06/22/94"
#endif
			     ...
			     RFCMessage	* message)
{
  ...
  if (message != NULL) {
	...
	// the new message.
	//
	totalSize = (totalSize > 10*1024) ? (totalSize * 1.1) : (20 * 1024);
	...
	if (error.isNotSet()) {
	  ...
	  if (error.isNotSet()) {
		...
		for (offset = 1; offset < totalParts ; offset++) {
		  ...
		  strncat(newMessage, (const char *)contents, length);
		  messageSize += length;
		  newMessage[messageSize] = '\0';
		}
		//
