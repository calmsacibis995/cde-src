...
#pragma ident "@(#)RFCTransport.C	1.70 02/06/96"
#endif
...
RFCTransport::submit(DtMailEnv & error, DtMail::Message * msg, DtMailBoolean log_msg)
{
    ...
    if ((new_we = (waitentry_t *) malloc(sizeof(*new_we))) == NULL)
    {
...
//    thr_error->clear();
    ...
    if (log_count) {
      ...
      assert(log_files);	// if log files open, must have array

      logFmt->msgToBuffer(tmp_error, *dtmsg, DTM_TRUE, DTM_FALSE, DTM_TRUE,
		      ...
		      int & log_count)
{
    ...
    assert(log_files);		// must provide -> log_files ->
    ...
    *log_files = 0;
    ...
    // We add the "-i" parameter to sendmail so that it will ignore
                             ...
                             char ** argv)
{
    ...
    // Use vfork because the only purpose of the child is to do an exec
    //
...
#if defined(_AIX)
        (void) execvp(mailer, (const char **)argv);
#else
...
RFCTransport::signalRegister(void)
{
	...
	// When a SIGCHLD signal is received, call the child_handler
#if (MAXSIG > 43)
		...
		Boolean append)
{
    ...
    fmt->msgToBuffer(error, *msg, DTM_TRUE, DTM_TRUE, DTM_TRUE,
		     headers, bodies);
    ...
    if (append)
    	fd = SafeOpen(path, O_RDWR | O_APPEND | O_CREAT, 0600);
    else
    	fd = SafeOpen(path, O_RDWR | O_CREAT | O_TRUNC, 0600);
