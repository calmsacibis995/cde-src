...
#pragma ident "@(#)RFCEnvelope.C	1.51 11/29/95"
#endif
			    ...
			    DtMailValueSeq & value)
{
    ...
    int slot = _parsed_headers.index((ParsedHeader *)last);
    if (slot < 0) {
	return(NULL);
    }
			...
			DtMailValueSeq & value)
{
	...
	//
	for (int abs = 0; abstract_map[abs].abstract; abs++) {
			    ...
			    DtMailValueSeq & value)
{
    ...
    //
    for (const char * end = hdr.value_start; *end && !isspace((unsigned char)*end); end++) {
	continue;
    }
