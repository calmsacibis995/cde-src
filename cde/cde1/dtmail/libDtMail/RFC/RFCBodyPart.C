...
#pragma ident "@(#)RFCBodyPart.C	1.45 02/13/96"
#endif
...
RFCBodyPart::adjustBodyPartsLocation(char * start)
{
	...
	if (*start != '-' && *(start + 1) != '-') {
	  fprintf(stderr, "RFCBodyPart::adjustBodyPartLocation(%.20s): Not a seperator\n",
		start);
	} else {
		while (*start != '\n')
			start++;
...
void
#ifdef _NO_PROTO
RFCBodyPart::DtXlateOpToStdLocale (operation, opLocale, ret_stdLocale, ret_stdLang, ret_stdSet)
     char       *operation;
     char       *opLocale;
     char       **ret_stdLocale;
     char       **ret_stdLang;
...
void
#ifdef _NO_PROTO
RFCBodyPart::DtXlateStdToOpLocale (operation, stdLocale, dflt_opLocale, ret_opLocale)
     char       *operation;
     char       *stdLocale;
     char       *dflt_opLocale;
     char       **ret_opLocale;
