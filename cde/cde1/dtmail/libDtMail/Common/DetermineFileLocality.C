...
#pragma ident "@(#)DetermineFileLocality.C	1.5 02/01/95"
#endif
...
DtMail::DetermineFileLocality(const char * path)
{
  ...
  for (int i = 0; ; i++)
  {
	...
	if (len >= fsTypeLen) {
	  fsTypeLen = len;
	  (void) strncpy(fsType, mnt->mnt_fstype ? mnt->mnt_fstype : "unknown-fstype", MAXPATHLEN-1);
	}
      }
      fclose(mfp);
      break;
...
#else
  // no code for this OS to determine locality -- return unknown
  //
  return (Dtm_FL_UNKNOWN);
#endif
}
