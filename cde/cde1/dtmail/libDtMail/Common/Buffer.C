...
#pragma ident "@(#)Buffer.C	1.7 02/20/96"
#endif
...
BufferMemory::iterate(Buffer::CallBack callback, ...)
{
    ...
    for (c = _firstchunk; c; c = c->_nextchunk) {
        ...
        // For each iteration through the loop, setup va_list object to be passed to callback
	...
	// At the end of each iteration through the loop, end access to variable arguments list
	va_end(va);
