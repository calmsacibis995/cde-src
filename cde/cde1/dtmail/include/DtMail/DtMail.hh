...
#pragma ident "@(#)DtMail.hh	1.95 03/21/96"
#endif
    ...
    class Session : public DtCPlusPlusAllocator {
      public:
	    ...
	    //
            ...
            void registerDisableGroupPrivilegesCallback(DisableGroupPrivilegesCallback,
							  void * client_data);
            void registerEnableGroupPrivilegesCallback(EnableGroupPrivilegesCallback,
							  void * client_data);
	    ...
	    // library may wish to delay the busy event for a while.
            ...
            void registerLastInteractiveEventTimeCallback(LastInteractiveEventTimeCallback,
							  void * client_data);
...
//==================NEW MAILRC CLASS
    class MailRc {
      ...
      public:
	    ...
	    void getValue(DtMailEnv &, const char * var, const char ** value);
	    void setValue(DtMailEnv &, const char * var, const char * value);
	    ...
	    // Methods for filters
	    void getFilterList(DtVirtArray<struct filter*> *);
	    struct filter* getFilter(char *);
	    void setFilter(DtMailEnv &, DtVirtArray<struct filter*> *);
            void removeFilter(DtMailEnv &, struct filter *);
	    void insertFilter(struct filter*, struct filter *);
            void update_forward(int);
	    ...
	    static void mt_puthash(char *name, char * val, struct var **hasharray);
	    ...
	    static void wunset(const char * verbatim, char ** arglist, FILE * outf);
	    ...
	    static void nalternates(char * key, void * value, void * client_data);
	    
	    static char *nullfield;
	    static Boolean clearAliases;
    	    static Boolean clearFilters;
	  ...
	  protected:
	    ...
	    int getrawlist2(char line[], char ** argv, int argc);
    ...
    class BodyPart : public DtCPlusPlusAllocator {
      public:
        ...
        virtual int csConvert(char **, unsigned long &, int, char *, char *) = 0;
        //-------------------------------------------
    ...
    class MailBox : public DtCPlusPlusAllocator {
      public:
	    ...
	    // must exist, and be readable (at least) by the user. If the container
	    // is writable by the user it will be opened for update.
	    //
	    // Parameters:
	    ...
	    //  DTME_ObjectCreationFailed - A required object could not
	    //       be created.
	    //  DTME_NoSuchFile - The file doesn't exist and auto_create is false.
	    ...
	    virtual DtMailMessageHandle getFirstMessageSummary(DtMailEnv & error,
							       const DtMailHeaderRequest & header_list,
							       DtMailHeaderLine & header_line) = 0;
	    
	    virtual DtMailMessageHandle getNextMessageSummary(DtMailEnv & error,
							      DtMailMessageHandle last,
							      const DtMailHeaderRequest & header_list,
							      DtMailHeaderLine & header_line) = 0;

	    virtual void getMessageSummary(DtMailEnv & error,
					   DtMailMessageHandle handle,
					   const DtMailHeaderRequest &,
					   DtMailHeaderLine &) = 0;

	    virtual Message * getMessage(DtMailEnv & error,
...
friend class Session;
    static void setError(Session &, DtMailEnv & error, DTMailError_t minor_code);
};
