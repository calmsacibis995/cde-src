    ...
    :ListUiItem(w, source, search_key, NULL)
{
    ...
    if (!rmw) {
	// DEBUG
	printf("No RoamMenuWindow...sorry.\n");
	return;
    }
...
void ToolbarListUiItem::writeFromUiToSource()
{
    ...
    XtVaGetValues(w,
		  ...
		  NULL);
    if (count < 1) {
	mail_rc->setValue(error, "toolbarcommands", " ");
	if (error.isSet()) {
    ...
    mail_rc->setValue(error, "toolbarcommands", mailrc_str);
    if(error.isSet()) {
...
void ToolbarListUiItem::writeFromSourceToUi()
{
    ...
    error.clear();
    mail_rc->getValue(error, "toolbarusetext", &value);
