...
RuleCmd::update_fields()
{
    ...
    if (_filter->f_state) {
	    XtVaSetValues(_rule_dlog->rule_on_off_checkbox_items.On_item, XmNset, True, NULL);
	    XtVaSetValues(_rule_dlog->rule_on_off_checkbox_items.Off_item, XmNset, False, NULL);
	}
	else {
	    XtVaSetValues(_rule_dlog->rule_on_off_checkbox_items.On_item, XmNset, False, NULL);
	    XtVaSetValues(_rule_dlog->rule_on_off_checkbox_items.Off_item, XmNset, True, NULL); 
	}
    ...
    // Update the criteria and action fields, if there is any expression or action that
    // is not supported by the UI, popup a warning. This UI only accepts a small subset
    // from the rule spec
    int err = 0;
	...
	genDialog->post_and_return(catgets(DT_catd, 3, 72, "OK"), DTMAILHELPUNSUPPORTRULE);
