...
#pragma ident "@(#)FindDialog.C	1.36 02/13/96"
#endif
...
FindDialog::FindDialog(RoamMenuWindow *parent) : Dialog("find", parent)
{
  ...
  //
  _buttonData[0].label = strdup(catgets(DT_catd, 1, 183, "Search"));
...
FindDialog::createWorkArea(Widget dialog)
{
  ...
  _name = catgets(DT_catd, 1, 192, "Mailer - Search");
...
FindDialog::directionCallback(Widget	widget,
			      ...
			      XtPointer)
{
  int	       which = (int) closure;	// closure contains button #
  FindDialog	*find; 
