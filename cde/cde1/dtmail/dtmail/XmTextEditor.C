...
#pragma ident "@(#)XmTextEditor.C	1.56 05/22/96"
#else
static char *sccs__FILE__ = "@(#)XmTextEditor.C	1.56 05/22/96";
#endif
...
// in a single call.
int
XmTextEditor::append_to_contents_and_cache(
    const char *contents,
    const unsigned long len
)
{
...
XmTextEditor::spell()
{
    ...
    if (spellgLibHandle2 != (void *) NULL)
	  ...
	  SdtInvokeSpell_FP = (Boolean (*)(Widget)) dlsym(spellgLibHandle2, "SDtInvokeSpell");
...
XmTextEditor::loadFile(const char * path, const int pos)
{
    ...
    map = mmap(0, map_size, PROT_READ, MAP_PRIVATE, fd, 0);
    if (map == (char *)-1) {
...
void disp_xm_editor_err(const char*, const int line)
{
    char* helpId;
