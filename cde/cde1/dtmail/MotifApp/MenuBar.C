...
#pragma ident "@(#)MenuBar.C	1.48 05/13/96"
#endif
	...
	unsigned char type)
{
	...
	if (cascade != NULL && *cascade != NULL)
	    ...
	    *cascade = XtCreateWidget (name, 
		...
		XtAddCallback(*cascade, XmNhelpCallback, HelpCB, DTMAILEDITMENUID);
	    } else if (helpId == "Message") {
		XtAddCallback(*cascade, XmNhelpCallback, HelpCB, DTMAILMESSAGEMENUID);
	    } else if (helpId == "Attachments") {
		XtAddCallback(*cascade, XmNhelpCallback, HelpCB, DTMAILATTACHMENUID);
	    } else if (helpId == "View") {
