...
static char rcs_id[] = "$Header: TermEnhance.c,v 1.10 94/12/21 10:45:12 tom_hp_cv Exp $";
...
_DtTermEnhProc(Widget w, enhValues values, TermEnhInfo info)
{
	...
	} else if (ourFont == FONT_LINEDRAW) {
	    /* DKS: can't do bold with the linedraw font... */
	    info->font = td->renderFonts[RENDER_FONT_LINEDRAW].termFont;
	} else if (ourFont == FONT_GLYPH) {
	    /* ... or with the glyph font */
	    info->font = td->renderFonts[RENDER_FONT_GLYPH].termFont;
	}
    } else {
