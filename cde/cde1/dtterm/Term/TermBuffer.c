...
static char rcs_id[] = "$XConsortium: TermBuffer.c /main/cde1_maint/2 1995/10/09 11:47:06 pascale $";
	...
	if (lines != NULL)
	    ...
	    */
		...
		if (lines[i])
                  {
		    DT_ENH(lines[i]) = NULL;
		    BUFFER(lines[i]) = (termChar *)
                    malloc((unsigned) MAX_COLS(tb) * BYTES_PER_CHAR(tb));
			...
			WIDTH(lines[i])   = 0;
			WRAPPED(lines[i]) = False;
			START_SELECTION(lines[i]) = NON_SELECTION_COL;
			END_SELECTION(lines[i]) = NON_SELECTION_COL;
                      }
		    else 
                      {
			/*
