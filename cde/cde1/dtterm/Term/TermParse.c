...
static char rcs_id[] = "$XConsortium: TermParse.c /main/cde1_maint/2 1995/09/29 15:05:38 lehors $";
...
_DtTermParseHexDigit(Widget w) /* vt220 DECUDK store hex digit*/
{
  ...
  Debug('P', fprintf(stderr,">>In func _DtTermLoadSoftKey\n")) ;
  context = GetParserContext(w) ;
  if ( context->stringParms[0].length < 2*STR_SIZE-1 ) {
      ...
      if ( left_byte  )
           ...
           context->stringParms[0].str[i] |= c & '\x0F' ;
...
void
_DtTermParseFF    /* Form Feed (NP) Ctrl-L */
(
    Widget w
)
{
  DtTermWidget vtw = (DtTermWidget)w ;
  Debug('P', fprintf(stderr,">>In func _vtParseFF\n")) ;
  if (vtw->vt.formFeedClearsScreen)
    _DtTermFuncClearBuffer(w, 1, fromParser);
  else
    _DtTermParseLF(w);
}
...
)
{
  ...
  switch (context->parms[1])
  {
    ...
    case 20:
      ...
      sprintf(buf,"\033]L%s\033\\",icon) ;
      sendEscSequence(w,buf) ;
      break ;
    case 21:
      XtVaGetValues(sw,XmNtitle, &title, NULL);
      sprintf(buf,"\033]l%s\033\\",title) ;
      sendEscSequence(w,buf) ;
      break ;
    case 22:
      _DtTermSetGlyph(w, context->parms[2], context->parms[3]);
      break ;
   }
}
