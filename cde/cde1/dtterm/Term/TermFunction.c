...
static char rcs_id[] = "$Header: TermFunction.c,v 1.30 94/12/21 10:45:13 tom_hp_cv Exp $";
...
)
{
	...
	*/
	...
	break;
      ...
      case eraseBuffer:
	(void) _DtTermFuncClearBuffer(w, NULL /* not used*/, functionSource);
	break;
...
_DtTermFuncClearToEndOfBuffer(Widget w, int count, FunctionSource functionSource)
{
    ...
    /* turn the cursor off... */
    (void) _DtTermPrimCursorOff(w);
    /* clean out the glyph gutter... */
    if ( tw->term.glyphGutter ) {
	if ( tpd->clearGC.gc ) {
	    y = tpd->cursorRow * tpd->cellHeight + tpd->offsetY;
...
_DtTermFuncHardReset(Widget w, int count, FunctionSource functionSource)
{
  ...
  Boolean reverseVideo = vtw->term.reverseVideo;
  ...
  _DtTermFuncClearBuffer(w, NULL , NULL ) ;
  _DtTermFunctionKeyClear(w) ;

  _DtTermPrimSetCursorVisible(w, True);
  _DtTermPrimCursorOn(w);  

  /* ---------------------------------------------------------------- */
  /*  restore the reverseVideo flag, many people prefere it that way  */
  /* ---------------------------------------------------------------- */
  vtw->term.reverseVideo = reverseVideo;
  _DtTermFuncClearBuffer(w, NULL, NULL);
...
Boolean _DtTermGetUserKeyLock(
         Widget         w)
{
  return(((DtTermWidget)w)->vt.td->userKeysLocked);
}
...
Boolean _DtTermGetAutoLineFeed(
         Widget         w)
{
      return(((DtTermWidget)w)->vt.td->tpd->autoLineFeed);
}
