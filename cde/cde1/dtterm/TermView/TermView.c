...
static char rcs_id[] = "$XConsortium: TermView.c /main/cde1_maint/3 1995/09/29 12:09:16 lehors $";
...
)
{
    ...
    XtGetValues(tw->termview.term, al, ac);
...
)
{
    ...
    rows = (* (short *) value);
    XtSetArg(al[ac], DtNrows, rows); ac++;
    XtSetValues(tw->termview.term, al, ac);
...
static XtResource resources[] =
{
    ...
    {
	DtNuserFontIndex, DtCUserFontIndex, XmRInt, sizeof(int),
	XtOffsetOf( struct _DtTermViewRec, termview.userFontIndex),
	XmRImmediate, 0
    },
    {
	DtNfontIndexSet, DtCFontIndexSet, XmRBoolean, sizeof(Boolean),
	XtOffsetOf( struct _DtTermViewRec, termview.fontIndexSet),
	XmRImmediate, (XtPointer) False
    },
    {
	"glyphFont", "GlyphFont", XmRPointer, sizeof(void *),
	XtOffsetOf( struct _DtTermViewRec, termview.glyphFont),
	XmRImmediate, 0
    },
    {
	"glyphGutter", "GlyphGutter", XmRInt, sizeof(int),
	XtOffsetOf( struct _DtTermViewRec, termview.glyphGutter),
	XmRImmediate, 0
    },
};
...
SetSizeInfo(Widget w, int width, int height)
{
  ...
  i = 0;
  XtSetArg(arglist[i], DtNrows, &tw->termview.rows); i++;
  XtSetArg(arglist[i], DtNcolumns, &tw->termview.columns); i++;
  XtSetArg(arglist[i], DtNwidthInc, &tw->termview.widthInc); i++;
  XtSetArg(arglist[i], DtNheightInc, &tw->termview.heightInc); i++;
  XtGetValues(tw->termview.term, arglist, i);
  ...
  if (width > 0)
    tw->termview.baseWidth =
    width - tw->termview.columns * tw->termview.widthInc;
  ...
  if (XtIsShell(XtParent(w)))
    {
      i = 0;
      XtSetArg(arglist[i], XmNheight, &shellHeight); i++;
  ...
  if (change && XtIsShell(XtParent(w)))
    {
      ...
      XtSetArg(arglist[i], DtNwidthInc, tw->termview.widthInc); i++;
      XtSetArg(arglist[i], DtNheightInc, tw->termview.heightInc); i++;
      XtSetArg(arglist[i], DtNbaseWidth, tw->termview.baseWidth); i++;
      XtSetArg(arglist[i], DtNbaseHeight, tw->termview.baseHeight); i++;
      XtSetValues(XtParent(w), arglist, i);
    }
...
ResetSizeInfo(Widget w, int width, int height)
{
	...
	XtSetArg(arglist[i], DtNheightInc, tw->termview.heightInc); i++;
	XtSetArg(arglist[i], DtNbaseWidth, tw->termview.baseWidth); i++;
	XtSetArg(arglist[i], DtNbaseHeight, tw->termview.baseHeight);
		i++;
	XtSetValues(XtParent(w), arglist, i);
    }
...
Initialize(Widget rw, Widget w, Arg *args, Cardinal *num_args)
{
    ...
    /* Widget helpWidget */
    tw->termview.helpWidget = NULL;

    /* Widget globalOptionsDialog */
    tw->termview.globalOptionsDialog = NULL;

    /* Widget terminalOptionsDialog */
    tw->termview.terminalOptionsDialog = NULL;
	...
	XtManageChild(tw->termview.menuBar.widget);
	...
	if (XtIsShell(XtParent(w)))
	    ...
	    XtGetValues(XtParent(w), arglist, i);
            ...
            /* -------------------------------------------------- */
            /*  Adjust for menuBar height if -geometry specified  */
            /* -------------------------------------------------- */
    ...
    i = 0;
    XtSetArg(arglist[i], XmNscrollingPolicy, XmAPPLICATION_DEFINED); i++;
    ...
    /* Let's make sure we are setting everthing that term needs...      */
    ...
    /* char *emulationId						*/
    i = 0;
    XtSetArg(arglist[i], DtNemulationId, tw->termview.emulationId); i++;

    /* unsigned char charCursorStyle					*/
    XtSetArg(arglist[i], DtNcharCursorStyle,
             tw->termview.charCursorStyle); i++;

    /* int blinkRate							*/
    XtSetArg(arglist[i], DtNblinkRate, tw->termview.blinkRate); i++;

    /* Boolean visualBell		set via setvalues		*/
    XtSetArg(arglist[i], DtNvisualBell, tw->termview.visualBell); i++;

    /* Boolean marginBell		set via setvalues		*/
    XtSetArg(arglist[i], DtNmarginBell, tw->termview.marginBell); i++;

    /* Boolean nMarginBell		set via setvalues		*/
    XtSetArg(arglist[i], DtNnMarginBell, tw->termview.nMarginBell);
	    i++;

    /* Boolean jumpScroll		set via setvalues		*/
    XtSetArg(arglist[i], DtNjumpScroll, tw->termview.jumpScroll); i++;
    ...
    if (tw->termview.pty >= 0)
      {
	XtSetArg(arglist[i], DtNtermDevice, tw->termview.pty); i++;
      }

    /* Boolean ptyAllocate						*/
    XtSetArg(arglist[i], DtNtermDeviceAllocate, tw->termview.ptyAllocate); i++;

    /* char *ptySlaveName						*/
    if (tw->termview.ptySlaveName)
      {
	XtSetArg(arglist[i], DtNtermSlaveName,
    ...
    XtSetArg(arglist[i], "glyphFont",
		    tw->termview.glyphFont); i++;

    XtSetArg(arglist[i], "glyphGutter",
		    tw->termview.glyphGutter); i++;
    ...
    XtAddCallback(tw->termview.term, DtNsubprocessTerminationCallback,
	...
	i = 0;
	_DtTermViewCreatePopupMenu(w, tw->termview.term, arglist, i);
	Debug('T',
    ...
    i = 0;
    XtSetArg(arglist[i], DtNuserFont, &tw->termview.fontList);
	    i++;
    XtGetValues(tw->termview.term, arglist, i);
	...
	for (sw = w; !XtIsShell(sw); sw = XtParent(sw))
	    ;
	XtAddEventHandler(sw, (EventMask) StructureNotifyMask,
...
Resize(Widget w)
{
  ...
  if (tw->termview.menuBar.widget && tw->termview.menuBar.managed)
    {
      ...
      desired.width =
      tw->core.width - 2 * tw->termview.menuBar.widget->core.border_width;

      desired.height = tw->termview.menuBar.widget->core.height;
      desired.request_mode = CWWidth;

      if (XtQueryGeometry(tw->termview.menuBar.widget,
                          &desired, &preferred) != XtGeometryYes)
        {
      ...
      XtResizeWidget(tw->termview.menuBar.widget,
                     desired.width,
                     desired.height,
  ...
  /* now, the terminal widget gets the height that is left and the
   * full width (i.e., goes from the left border to the right border)... */

  configWidth =
  ourWidth(tw) - 2 * tw->termview.scrolledWindowFrame->core.border_width;

  configHeight = ourHeight(tw) - 2 *
  tw->termview.scrolledWindowFrame->core.border_width - menuBarHeight;
  ...
  XtConfigureWidget(tw->termview.scrolledWindowFrame,
                    ourWidthOffset(tw),                  /* x */
                    ourHeightOffset(tw) + menuBarHeight, /* y */
                    (Dimension) configWidth,		 /* width */
                    (Dimension) configHeight,		 /* height */
                    tw->termview.scrolledWindowFrame->core.border_width);

  /* update our rows, columns, widthInc, heightInc, baseWidth, baseHeight */
  SetSizeInfo(w, -1, -1);
  ...
  if (XtIsRealized(w))
    {
      ...
      XClearArea(XtDisplay(w), XtWindow(w), 0, 0, tw->core.width,
	...
	Cardinal *num_args)
{
	...
	if (tw->termview.scrollBarVisible) {
	    ...
	    XtManageChild(tw->termview.scrollBar.widget);
	    tw->termview.scrollBar.managed = True;
	} else {
	    /* make it invisible... */
	    if (tw->termview.scrollBar.widget) {
		/* unmanage it... */
		XtUnmanageChild(tw->termview.scrollBar.widget);
	    }
	    tw->termview.scrollBar.managed = False;
	}
    }
	...
	if (tw->termview.menuBarVisible) {
	    ...
	    if (tw->termview.menuBar.widget) {
		/* unmanage it... */
		XtUnmanageChild(tw->termview.menuBar.widget);
		tw->termview.menuBar.managed = False;
	    }
	}
    }
	...
	XtSetArg(al[ac], DtNblinkRate, tw->termview.blinkRate);
									ac++;
	...
	XtSetArg(al[ac], DtNcharCursorStyle,
	...
	XtSetArg(al[ac], DtNvisualBell, tw->termview.visualBell);
									ac++;
	...
	XtSetArg(al[ac], DtNmarginBell, tw->termview.marginBell);
									ac++;
	...
	XtSetArg(al[ac], DtNnMarginBell, tw->termview.nMarginBell);
									ac++;
	...
	XtSetArg(al[ac], DtNjumpScroll, tw->termview.jumpScroll);
									ac++;
	tw->termview.jumpScroll = (Boolean) -1;
    }

    if (tw->termview.glyphFont != cur_tw->termview.glyphFont) {
	XtSetArg(al[ac], "glyphFont", tw->termview.glyphFont);
									ac++;
    }

    if (tw->termview.glyphGutter != cur_tw->termview.glyphGutter) {
	XtSetArg(al[ac], "glyphGutter", tw->termview.glyphGutter);
									ac++;
    }
	...
	XtSetArg(al[ac], DtNcolumns, tw->termview.columns); ac++;
	XtSetArg(al[ac], DtNrows, tw->termview.rows); ac++;
	...
	XtSetArg(al[ac], DtNuserFont, tw->termview.fontList); ac++;
	...
	XtWidgetGeometry *allowed)
{
    ...
    memcpy((char *) allowed, (char *) desired, sizeof(*desired));
	...
	if ((result == XtGeometryYes) &&
	    ...
	    ResetSizeInfo((Widget) tw, width, height);
		...
		XtMakeResizeRequest((Widget) tw, widthReturn,
...
ChangeManaged(Widget w)
{
    ...
    if (tw->termview.menuBar.widget && tw->termview.menuBar.managed) {
	XtConfigureWidget(tw->termview.menuBar.widget,
		0,
		0,
	...
	case XtGeometryAlmost:
	     ...
	     */
	    XtMakeResizeRequest(w, widthReturn, heightReturn,
	...
	Dimension *childWidth)
{
    ...
    *childWidth = widgetWidth(tw->termview.scrolledWindow) -
	 ...
	 */
	i = 0;
	XtSetArg(arglist[i], DtNscrollBarPlacement,
		&scrollBarPlacement); i++;
	XtSetArg(arglist[i], DtNspacing, &scrolledWindowSpacing); i++;
	XtGetValues(tw->termview.scrolledWindow, arglist, i);
	...
	XtPointer call_data)
{
	...
	memset(&termviewCB, '\0', sizeof(termviewCB));
...
DtTermViewCloneCallback(Widget w, XtPointer client_data, XtPointer call_data)
{
    ...
    /* unsigned char charCursorStyle */
    XtSetArg(arglist[i], DtNcharCursorStyle, tw->termview.charCursorStyle);
	    i++;
    ...
    XtSetArg(arglist[i], DtNshadowType, tw->termview.shadowType); i++;

    /* XmFontList fontList */
    XtSetArg(arglist[i], XmNfontList, tw->termview.fontList); i++;
    ...
    XtSetArg(arglist[i], DtNmarginHeight, tw->termview.marginHeight); i++;

    /* Dimension marginWidth */
    XtSetArg(arglist[i], DtNmarginWidth, tw->termview.marginWidth); i++;

    /* Dimension spacing */
    XtSetArg(arglist[i], XmNspacing, tw->termview.spacing); i++;
    ...
    /* int ptyAllocate							*/
    XtSetArg(arglist[i], DtNtermDeviceAllocate, True);

    /* char *ptySlaveName		set by new widget		*/
    /* int subprocessPid		set by new widget		*/

    /* char *subprocessCmd */
    XtSetArg(arglist[i], DtNsubprocessCmd, tw->termview.subprocessCmd);
	    i++;

    /* char **subprocessArgv */
    XtSetArg(arglist[i], DtNsubprocessArgv, tw->termview.subprocessArgv);
	    i++;
    ...
    /* Boolean subprocessExec */
    XtSetArg(arglist[i], DtNsubprocessExec, tw->termview.subprocessExec);
	    i++;

    /* Boolean scrollBarVisible */
    XtSetArg(arglist[i], DtNscrollBar,
	    tw->termview.scrollBarVisible); i++;

    /* Boolean menuBarVisible */
    XtSetArg(arglist[i], DtNmenuBar, tw->termview.menuBarVisible); i++;

    termCB->arglist = arglist;
    termCB->argcount = i;

    if (tw->termview.newCallback) {
	XtCallCallbackList(w, tw->termview.newCallback,
		(XtPointer) termCB);
    }
}
...
)
{
	...
	XtSetArg(al[ac], DtNmanPage, hyperData->specification);	ac++;
	XtSetArg(al[ac], DtNcolumns, 80);			ac++;
	XtSetArg(al[ac], DtNrows, 20);				ac++;
	XtSetArg(al[ac], XmNtitle, GETMESSAGE(NL_SETN_View, 4, "Terminal - Man Page"));ac++;
	quickHelpDialog = DtCreateHelpQuickDialog(w, "helpDialog", al, ac);
#ifdef	HPVUE
	XtAddCallback(quickHelpDialog, XmNokCallback,
...
_DtTermViewMapHelp(Widget w, char *volume, char *id)
{
    ...
    i = 0;
    XtSetArg(arglist[i], DtNhelpType, DtHELP_TYPE_TOPIC); i++;
    XtSetArg(arglist[i], DtNlocationId, id); i++;
    XtSetArg(arglist[i], DtNhelpVolume, volume); i++;
    XtSetArg(arglist[i], XmNtitle, GETMESSAGE(NL_SETN_View, 5,"Terminal - Help")); i++;
    if (tw->termview.helpWidget) {
	XtSetValues(tw->termview.helpWidget, arglist, i);
    } else {
	tw->termview.helpWidget = DtCreateHelpDialog(w, "helpDialog",
		arglist, i);
#ifdef	BOGUS
#ifdef	HPVUE
	XtAddCallback(tw->termview.helpWidget, XmNokCallback,
		closeHelp, (XtPointer) 0);
#else	/* HPVUE */
	XtAddCallback(tw->termview.helpWidget, DtNcloseCallback,
		closeHelp, (XtPointer) 0);
#endif	/* HPVUE */
#endif	/* BOGUS */
	XtAddCallback(tw->termview.helpWidget, DtNcloseCallback,
		closeHelp, (XtPointer) 0);
	XtAddCallback(tw->termview.helpWidget, DtNhyperLinkCallback,
		hyperlinkCallback, (XtPointer) 0);
    }
    XtManageChild(tw->termview.helpWidget);

    for (sw = tw->termview.helpWidget; !XtIsShell(sw); sw = XtParent(sw))
	;
    if (sw) {
	XRaiseWindow(XtDisplay(sw), XtWindow(sw));
    }
}
