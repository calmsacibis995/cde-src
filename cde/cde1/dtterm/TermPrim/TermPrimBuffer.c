...
static char rcs_id[] = "$XConsortium: TermPrimBuffer.c /main/cde1_maint/1 1995/07/15 01:23:15 drk $";
...
static TermLine *referenceLines = (TermLine *) 0;
static Boolean  *referenceFlags = (Boolean *) 0;
static int	referenceLineCount = 0;
...
)
{
    ...
    if (!referenceLines || (referenceLineCount != ROWS(tb))) {
	referenceLineCount = ROWS(tb);
	referenceLines = (TermLine *) XtRealloc((char *) referenceLines,
		ROWS(tb) * sizeof(TermLine));
	referenceFlags = (Boolean *) XtRealloc((char *) referenceFlags,
		ROWS(tb) * sizeof(Boolean));
    }
...
)   
{
           ...
           TermLine     line;
    static termChar     overflowBuffer[BUFSIZ];
           termChar    *start;
	...
	*/
	status =
        _DtTermPrimBufferInsertWc(tb, row, col,
                                  (wchar_t *)newChars,
                                  numChars, insertFlag,
    ...
    */
    ...
    if (insertFlag == False)
    {
        ...
        *returnCount = numChars - insertCount;
        if (*returnCount > 0)
        {
            memcpy(overflowBuffer, newChars + insertCount,
    ...
    else
    {
        ...
        *returnCount = MAX(0, (WIDTH(line) + numChars) - COLS(tb));
        if (*returnCount > 0)
        {
            ...
            */
            memcpy(overflowBuffer + insertOverflow, BUFFER(line) + 
...
)
{
    ...
    DebugF('B', 1, RememberTermBuffer(tb));
...
)
{
    ...
    (void)memset(BUFFER(line) + WIDTH(line),
                 '\x20', (newWidth - WIDTH(line)) * BYTES_PER_CHAR(tb));
    if (CLEAR_ENH(tb))
    {
...
/*
** Set the glyph for this row.
*/
void
_DtTermPrimBufferSetLineGlyph
(
    TermBuffer  tb,
    short       row,
    int		glyph
)
{
    if (VALID_ROW(tb, row))
    {
	GLYPH(LINE_OF_TBUF(tb, row)) = glyph;
    }
    return;
}

/*
** Return the glyph for this row, if any
*/
int
_DtTermPrimBufferGetLineGlyph
(
    TermBuffer  tb,
    short       row
)
{
    if (VALID_ROW(tb, row))
    {
	return(GLYPH(LINE_OF_TBUF(tb, row)));
    }
    else 
    {
        return(0);
