...
static char rcs_id[] = "$XConsortium: TermPrimCursor.c /main/cde1_maint/1 1995/07/15 01:24:14 drk $";
...
_DtTermPrimCursorChangeFocus(Widget w)
{
	...
	if ((!tpd->IMHasFocus) ||
	    ...
	    point.x = tpd->IMCursorColumn * tpd->cellWidth + tpd->offsetX + tw->term.glyphGutter;
...
cursorToggle(Widget w)
{
	...
	(void) XFillRectangle(XtDisplay(w),
		...
		tw->term.columns - 1 :
		tpd->cursorColumn) * tpd->cellWidth + tpd->offsetX + tw->term.glyphGutter,
					/* x				*/
		y,			/* y				*/
		tpd->cellWidth,		/* width			*/
		height);		/* height			*/
...
_DtTermPrimCursorOn(Widget w)
{
	...
	tpd->IMCursorColumn = tpd->cursorColumn;
	tpd->IMCursorRow = tpd->cursorRow;
	point.x = tpd->cursorColumn * tpd->cellWidth + tpd->offsetX + tw->term.glyphGutter;
