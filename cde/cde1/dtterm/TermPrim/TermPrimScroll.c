...
static char rcs_id[] = "$XConsortium: TermPrimScroll.c /main/cde1_maint/1 1995/07/15 01:34:10 drk $";
...
waitOnCopyArea(Widget w)
{
	...
	case Expose:
	    ...
	    _DtTermPrimExposeText(w, ev->xexpose.x, ev->xexpose.y,
...
_DtTermPrimScrollWait(Widget w)
{
	...
	if (tpd->scrollHeight > 0) {
	    ...
	    tpd->scrollWidth = tpd->cellWidth * tw->term.columns + tw->term.glyphGutter;
	...
	for (i = 0; i < (exposeY - tpd->offsetY) / tpd->cellHeight; i++) {
		...
		_DtTermPrimRefreshText(w, 0, i,
                                              tw->term.columns, i, False);
	    }
	}
  ...
  if (lines > 0)
    {
      ...
      if (lines >= tpd->scrollBottomRow - tpd->scrollTopRow + 1)
      ...
      else
        {
          ...
          exposeY = tpd->offsetY +
          tpd->cellHeight * (tpd->scrollBottomRow + 1 - lines);
      ...
      if (lines >= tw->term.rows)
        {
          ...
          exposeHeight =
          tpd->cellHeight * (tpd->scrollBottomRow - tpd->scrollTopRow + 1);
	}
      else
        {
          tpd->scrollHeight = tpd->cellHeight *
          ...
          tpd->scrollSrcY =
          tpd->offsetY + tpd->cellHeight * (tpd->scrollTopRow);

          tpd->scrollDestY =
          tpd->offsetY + tpd->cellHeight * (tpd->scrollTopRow + lines);
  ...
  if (tpd->scrollHeight > 0)
    {
      ...
      if ( tw->term.glyphGutter > 0 )
        tpd->scrollWidth += tw->term.glyphGutter;

      tpd->scrollSrcX = tpd->offsetX;
      tpd->scrollDestX = tpd->offsetX;

      /* copy the area... */
      XCopyArea(XtDisplay(w), XtWindow(w), XtWindow(w), tpd->renderGC.gc,
                tpd->scrollSrcX, tpd->scrollSrcY, tpd->scrollWidth,
                tpd->scrollHeight, tpd->scrollDestX, tpd->scrollDestY);

      tpd->scrollInProgress = True;
    }

  /* clear the old area... */
  XFillRectangle(XtDisplay(w), XtWindow(w), tpd->clearGC.gc,
                 tpd->offsetX, exposeY, tpd->scrollWidth, exposeHeight);

  /* expose the old area... */
  _DtTermPrimExposeText(w, tpd->offsetX, exposeY,
                        tpd->scrollWidth, exposeHeight, False);
...
_DtTermPrimScrollText(Widget w, short lines)
{
    ...
    if (tpd->scrollLockTopRow > 0) {
	_DtTermPrimBufferMoveLockArea(tpd->termBuffer,
	...
	if (lines2 > 0) {
	    for (i = 0; i < lines2; i++) {
		_DtTermPrimBufferClearLine(tpd->termBuffer, i, 0);
	    }
	    _DtTermPrimBufferMoveLockArea(tpd->termBuffer,
		    0,
		    lines2,
...
_DtTermPrimScrollTextTo(Widget w, short topRow)
{
    ...
    _DtTermPrimScrollText(w, topRow - tpd->topRow);
    return;
}
	...
	short scrollDistance)
{
	...
	tpd->scrollTopRow = scrollStart;
	tpd->scrollBottomRow = scrollStart + scrollLength - 1;
	doActualScroll(w, scrollDistance);
	tpd->scrollInProgress = True;
    }
}
...
_DtTermPrimScrollComplete(Widget w, Boolean flush)
{
	...
	if (!tw->term.jumpScroll && tpd->scroll.nojump.pendingScroll) {
	    ...
	    doActualScroll(w, tpd->scroll.nojump.pendingScrollLines);
