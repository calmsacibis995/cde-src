...
static char rcs_id[] = "$XConsortium: TermPrim.c /main/cde1_maint/9 1995/11/13 10:03:01 gtsang $";
...
static XtResource resources[] =
{
    ...
    {
        SDtNglyphGutter, SDtCGlyphGutter, XmRInt, sizeof(int),
	XtOffsetOf(struct _DtTermPrimitiveRec, term.glyphGutter),
	XtRImmediate, 0
    }
...
static char defaultTranslations[] = "\
	...
	<Key>KP_Multiply:	XtDisplayTranslations()\n\
	Ctrl<Key>Cancel:	stop(long)\n\
	~Ctrl<Key>Cancel:	stop()\n\
	<Key>Tab:		tab()\n\
	<Key>KP_Tab:		tab()\n\
...
)
{
    ...
    *font = (XFontStruct *) 0;
	...
	fprintf(stderr, "XmFontListInitFontContext() failed\n");
	exit(1);
    }
	...
	*fontSet = XCreateFontSet(XtDisplay(w),
	...
	if (missingCharsetCount > 0) {
		...
		fprintf(stderr, "missing charset in fontset \"%s\"\n",
...
)
{
    ...
    _DtTermPrimGetFontSet(w, fontList, &fontSet, &font);
...
)
{
	...
	for (i1 = 0; i1 < modifierMapping->max_keypermod; i1++) {
	    thisKeyCode = modifierMapping->modifiermap[mapIndex *
		    modifierMapping->max_keypermod + i1];
	    if (thisKeyCode >= minKeycodes && thisKeyCode <= maxKeycodes)
...
Initialize(Widget ref_w, Widget w, Arg *args, Cardinal *num_args)
{
    ...
    memset(tpd, '\0', sizeof(DtTermPrimDataRec));
    ...
    */
    ...
    if (!tw->term.fontList) {
	tw->term.fontList = _XmGetDefaultFontList(w, XmTEXT_FONTLIST);
    }
	...
	if (tw->term.fontSet) {
	    ...
	    *c2 = '\0';
		...
		if (tw->term.boldFontSet) {
		    XFreeFontSet(XtDisplay(w), tw->term.boldFontSet);
		    tw->term.boldFontSet = (XFontSet) 0;
		}
	    }
    ...
    _DtTermPrimParserInitContext(w);
    ...
    XDisplayKeycodes(XtDisplay(w), &minKeycodes, &maxKeycodes);
    ...
    XFree(keyboardMapping);
    XFreeModifiermap(modifierMapping);
	...
	XtAddEventHandler(sw, (EventMask) PropertyChangeMask,
...
InitializeVerticalScrollBar(Widget w, Boolean initCallbacks)
{
...
#ifdef	NOTDEF
	XtAddCallback(tw->term.verticalScrollBar, XmNdecrementCallback,
		VerticalScrollBarCallback, (XtPointer) w);
	XtAddCallback(tw->term.verticalScrollBar, XmNincrementCallback,
		VerticalScrollBarCallback, (XtPointer) w);

	XtAddCallback(tw->term.verticalScrollBar,
	...
	XtAddCallback(tw->term.verticalScrollBar, XmNdragCallback,
		VerticalScrollBarCallback, (XtPointer) w);
	XtAddCallback(tw->term.verticalScrollBar,
...
InitOrResizeTermBuffer(Widget w)
{
    ...
    newColumns = MAX(1, (tw->core.width - tw->term.glyphGutter -
			 (2 * (tw->primitive.shadow_thickness +
	    tw->primitive.highlight_thickness + tw->term.marginWidth))) /
	    tw->term.widthInc);
    newRows = MAX(1, (tw->core.height - (2 * (tw->primitive.shadow_thickness +
	    tw->primitive.highlight_thickness + tw->term.marginHeight))) /
	    tw->term.heightInc);

    if ((newColumns == tw->term.columns) &&
        (newRows == tw->term.rows) && tpd->termBuffer)
	/* we already have a buffer and the size didn't change */
	return;
	...
	if (tpd->useHistoryBuffer && tpd->historyBufferRows > 0 )
	{
	    ...
	    */
	    if (tpd->topRow < 0)
	    {
		_DtTermPrimScrollTextTo((Widget) tw, 0);
	    }
		...
		if (newBufferRows < tpd->bufferRows)
		{
			...
			_DtTermPrimBufferSetLineWidth(tpd->historyBuffer,
							     tpd->lastUsedHistoryRow,
							     0);
			_DtTermPrimBufferInsert(tpd->historyBuffer,
						       tpd->lastUsedHistoryRow,
						       0, c1, length, False,
						       &overflowChars,
						       &overflowCount);
		    ...
		    if (linesNeeded > 0) {
			_DtTermPrimBufferInsertLineFromTB(tpd->termBuffer,
								 tpd->bufferRows - 1,
								 linesNeeded,
								 insertFromTop);
			/* adjust everything... */
	...
	*/
	...
	{
	    XtFree(tpd->scrollRefreshRows);
	    tpd->scrollRefreshRows = (Boolean *) XtMalloc(newBufferRows *
							  sizeof(Boolean));
	    memset(tpd->scrollRefreshRows, '\0', newBufferRows *
			  sizeof(Boolean));
	}
	...
	if (tpd->useHistoryBuffer) {
	    tpd->historyBuffer = (*(termClassPart->buffer_create_proc))(
		    ...
		    termClassPart->sizeOfEnh);
	    if (tpd->termBuffer)
	      _DtTermPrimBufferSetLinks(tpd->termBuffer,
					       tpd->historyBuffer,
					       NULL);
            if (tpd->historyBuffer)
	      _DtTermPrimBufferSetLinks(tpd->historyBuffer,
					       NULL,
					       tpd->termBuffer);
	}
	...
	if (!tpd->termBuffer ||
	    ...
	    fprintf(stderr, "unable to create termBuffer\n");
	    exit(1);
	}
    ...
    */
	...
	_DtTermPrimPtySetWindowSize(tw->term.pty,
...
Redisplay(Widget w, XEvent *event, Region region)
{
	...
	_DtTermPrimExposeText(w, event->xexpose.x, event->xexpose.y,
	...
	Boolean *cont)
{
	...
	if (tpd->scroll.nojump.pendingScroll) {
	    _DtTermPrimScrollComplete(w, False);
	}

	/* process any pending input... */
	if (TextIsPending(tpd->pendingRead)) {
	    readPty((XtPointer) w, &fd, &tpd->ptyInputId);
	}
        ...
        Boolean *cont)
{
	...
	if (tpd->mapWarningDialog && tpd->warningDialog) {
	    XtManageChild(tpd->warningDialog);
	...
	Cardinal *num_args)
{
    ...
    /* change in rows, columns, or glyph gutter... */
    if ((cur_tw->term.rows != tw->term.rows) ||
	    (cur_tw->term.columns != tw->term.columns ||
	     cur_tw->term.glyphGutter != tw->term.glyphGutter)) {
	...
	/* handle changes in gutter width... */
	if (cur_tw->term.glyphGutter != tw->term.glyphGutter) {
	    request.width = tw->term.columns  * tw->term.widthInc +
		    2 * (tw->primitive.shadow_thickness +
		    tw->primitive.highlight_thickness + tw->term.marginHeight ) +
			 tw->term.glyphGutter;
    ...
    if (cur_tw->core.background_pixel != tw->core.background_pixel) {
	    XClearArea(XtDisplay(w), XtWindow(w),
			0, 0, 0, 0, True);
	    _DtTermPrimRefreshText(w, 0, 0, tw->term.columns,
				   tw->term.rows, False);
...
Realize(Widget w, XtValueMask *p_valueMask, XSetWindowAttributes *attributes)
{
     ...
     */
    tw->term.columns = (tw->core.width - (2 * (tw->primitive.shadow_thickness +
	    tw->primitive.highlight_thickness + tw->term.marginWidth))) /
	    tw->term.widthInc;
    tw->term.rows = (tw->core.height - (2 * (tw->primitive.shadow_thickness +
	    tw->primitive.highlight_thickness + tw->term.marginHeight))) /
	    tw->term.heightInc;
    ...
    XmImRegister(w, (unsigned int) 0);

    imPoint.x = 0;
    imPoint.y = 0;
    XmImVaSetValues(w,
                    XmNspotLocation, &imPoint,
                    XmNfontList, tw->term.fontList,
                    XmNbackground, tw->core.background_pixel,
                    XmNforeground, tw->primitive.foreground,
                    XmNbackgroundPixmap, tw->core.background_pixmap,
                    XmNlineSpace, tw->term.heightInc,
                    NULL);

    /* block sigchld while we do this... */
    sigemptyset(&sigNew);
    sigaddset(&sigNew, SIGCHLD);
    sigprocmask(SIG_BLOCK, &sigNew, (sigset_t *) 0);
	...
	if (tw->term.pty < 0) {
	    _XmWarning(w, "unable to get pty");

	    /* popup a warning dialog... */
	    _DtTermPrimWarningDialog(w, GETMESSAGE(
                          NL_SETN_Prim,3,"unable to get pty"));
	}
    ...
    */
    if (tw->term.pointerBlank) {_DtTermPrimPointerOn((Widget)tw); }
	...
	sprintf(buffer, "LINES=%d", tw->term.rows);
	newEnvStrings[i1] = XtMalloc(strlen(buffer) + 1);
	strcpy(newEnvStrings[i1++], buffer);

	sprintf(buffer, "COLUMNS=%d", tw->term.columns);
	newEnvStrings[i1] = XtMalloc(strlen(buffer) + 1);
	strcpy(newEnvStrings[i1++], buffer);
#endif	/* SETENV_LINES_AND_COLS */

	if (tw->term.termName && *tw->term.termName) {
	    sprintf(buffer, "TERM=%s", tw->term.termName);
	    newEnvStrings[i1] = XtMalloc(strlen(buffer) + 1);
	    strcpy(newEnvStrings[i1++], buffer);
	}
    ...
    sigprocmask(SIG_UNBLOCK, &sigNew, (sigset_t *) 0);
...
Destroy(Widget w)
{
    ...
    /* remove our handlers on our shell widget... */
    {
        ...
        for (sw = w; !XtIsShell(sw); sw = XtParent(sw))
	    ;
        XtRemoveEventHandler(sw, (EventMask) StructureNotifyMask,
		False, handleProcessStructureNotifyEvent, (Opaque) w);
	XtRemoveEventHandler(sw, (EventMask) PropertyChangeMask,
		False, handlePropertyChangeEvents, (Opaque) w);
    }
    ...
    */
	...
	XtRemoveInput(tw->term.tpd->ptyInputId);
	...
	kill(-tw->term.subprocessPid, SIGHUP);
    }
    /* turn off suid root...
     */
    _DtTermPrimToggleSuidRoot(False);
	...
	close(tw->term.pty);
      ...
      if (tw->term.ptySlaveName) {
	    _DtTermPrimReleasePty(tw->term.ptySlaveName);
	}
    }
    ...
    if (tw->term.boldFont) {
       XFreeFont(XtDisplay(w), tw->term.boldFont);
     }
    if (tw->term.boldFontSet) {
       XFreeFontSet(XtDisplay(w), tw->term.boldFontSet);
     }
	...
	if (tw->term.tpd->cursorTimeoutId) {
	    XtRemoveTimeOut(tw->term.tpd->cursorTimeoutId);
	...
	if (tw->term.tpd->renderGC.gc) {
	    XFreeGC(XtDisplay(w), tw->term.tpd->renderGC.gc);
	    tw->term.tpd->renderGC.gc = (GC) 0;
	}
	if (tw->term.tpd->renderReverseGC.gc) {
	    XFreeGC(XtDisplay(w), tw->term.tpd->renderReverseGC.gc);
	    tw->term.tpd->renderReverseGC.gc = (GC) 0;
	}
	/* clear GC... */
	if (tw->term.tpd->clearGC.gc) {
	    XFreeGC(XtDisplay(w), tw->term.tpd->clearGC.gc);
	    tw->term.tpd->clearGC.gc = (GC) 0;
	}
	/* cursor GC... */
	if (tw->term.tpd->cursorGC.gc) {
	    XFreeGC(XtDisplay(w), tw->term.tpd->cursorGC.gc);
	...
	if (tw->term.tpd->scrollRefreshRows) {
	    XtFree((char *) tw->term.tpd->scrollRefreshRows);
	    tw->term.tpd->scrollRefreshRows = (Boolean *) 0;
	}

        /* free up the selection information */
	if (tw->term.tpd->selectInfo) {
	    _DtTermPrimSelectDestroy(w, tw->term.tpd->selectInfo);
	    tw->term.tpd->selectInfo = (TermSelectInfo) 0;
	}

	/* free up pending text... */
	if (tw->term.tpd->pendingRead) {
	    _DtTermPrimPendingTextDestroy(tw->term.tpd->pendingRead);
	    tw->term.tpd->pendingRead = (PendingText) 0;
	}
	if (tw->term.tpd->pendingWrite) {
	    _DtTermPrimPendingTextDestroy(tw->term.tpd->pendingWrite);
	    tw->term.tpd->pendingWrite = (PendingText) 0;
	}

        if (tw->term.tpd->capsLockKeyCodes)
                 XtFree((char *)tw->term.tpd->capsLockKeyCodes) ;

        _DtTermPrimDestroyFont(w,tw->term.tpd->boldTermFont) ;
        _DtTermPrimDestroyFont(w,tw->term.tpd->termFont) ;

        if (tw->term.ptyAllocate && tw->term.ptySlaveName)
          free((char *)tw->term.ptySlaveName);

        if (tw->term.tpd->context)
          XtFree((char *)tw->term.tpd->context);

	/* free up the termData structure... */
	free((char *) tw->term.tpd);
...
moreInput(int pty)
{
    ...
    timeout.tv_sec = 0;
    timeout.tv_usec = 0;
    if (!((select(pty + 1, (fd_set *) &readFD, 0, 0,
	    &timeout) > 0) && (readFD & (1 << pty))))
	return(False);
    return(True);
}
...
readPty(XtPointer client_data, int *source, XtInputId *id)
{
	...
	len = chunk->len;
	memcpy(buffer, chunk->bufPtr, len);
    } else {
        bzero( (char*)buffer, sizeof(buffer));
	len = read(*source, buffer, sizeof(buffer));

        /* ------------------------------------------------------- */
        /*  This is a fix or workaround for csh when filec is set  */
        /*  The read sys call returns ^G or '\007' for all chars   */
        /*  entered after around 265 chars or so. Only leave one   */
        /*  of these extra bells ^G chars in the buffer - one ^G   */
        /*  char causes the Bell to ring is enough. Bug Id 1256731 */
        /* ------------------------------------------------------- */
        {
          int i;
          for ( i = 0 ; i < len - 1 ; i++ )
            if (buffer[i] == '\007' && buffer[i+1] == '\007')
              len = i + 1;
        }
        /* --------------------------- */
        /*  End fix of Bug Id 1256731  */
        /* --------------------------- */
...
#pragma BBA_IGNORE
	...
	if (!_DtTermPrimParseInput((Widget) tw, buffer, len,
		 ...
		 */
		_DtTermPrimPendingTextReplace(chunk, dangleBuffer,
			dangleBufferLen);
	    } else {
		_DtTermPrimPendingTextAppend(tpd->pendingRead,
			dangleBuffer, dangleBufferLen);
	    }
	    XtFree((char *) dangleBuffer);
	} else if (chunk) {
...
_DtTermPrimForcePtyRead(XtPointer client_data, XtIntervalId *id)
{
    ...
    _DtTermPrimStartOrStopPtyInput((Widget) tw);

    if (TextIsPending(tpd->pendingRead)) {
	readPty(client_data, &fd, &tpd->ptyInputId);
    }
}
...
_DtTermPrimLoopBackData(Widget w, char *data, int dataLength)
{
    ...
    _DtTermPrimPendingTextAppend(tpd->pendingRead, (unsigned char *) data,
	    dataLength);
...
)
{
     ...
     */
    _DtTermPrimLoopBackData(w, (char *) buffer, length);
}
...
writePty(XtPointer client_data, int *source, XtInputId *id)
{
    ...
    */
    if(tw->term.subprocessExec == True)
	...
	Cardinal *num_params)
{
	...
	fprintf(stderr, "_DtTermPrimActionKeyInput: incorrect event type %d\n",
		keyEvent->type);
	return;
    }
	...
	XtCallCallbackList(w, tw->term.inputVerifyCallback, &cb);
	...
	if (tw->term.marginBell &&
        ...
        if (synEscape && *string != '\r' && *string != '\x0B')
            _DtTermPrimSendInput(w, (unsigned char *) "\033", 1);

        /* for pointer blanking                               */
        if (tw->term.pointerBlank && *string != '\r' && *string != '\x0B')
	...
	for (end = string; nbytes > 0; )  {
	    ...
	    _DtTermPrimSendInput(w, start, end - start);
	    if (tpd->autoLineFeed && (end[-1] == '\r')) {
		_DtTermPrimSendInput(w, (unsigned char *) "\n", 1);
	    }
	}
    }
...
)
{
    ...
    if (length > 0) {
	if (tpd->pendingWrite &&
		_DtTermPrimPendingTextIsPending(tpd->pendingWrite)) {
	    /* queue up the text... */
	    _DtTermPrimPendingTextAppend(tpd->pendingWrite, buffer,
		    length);
	    _DtTermPrimStartOrStopPtyOutput(w);
	} else {
	    /* just write it out... */
	    if (isDebugFSet('i', 2)) {
#ifdef	BBA
#pragma BBA_IGNORE
		...
		fprintf(stderr,
			">>_DtTermPrimSendInput() writing %d bytes", length);
		if (length > 0) {
	    ...
	    write(tw->term.pty, buffer, length);
	}
...
)
{
    ...
    if (length > 0) {
	_DtTermPrimPendingTextAppend(tpd->pendingWrite, buffer, length);
	_DtTermPrimStartOrStopPtyOutput(w);
	if (tpd->halfDuplex) {
	    DtTermDisplaySend(w, buffer, length);
	}
...
)
{
    ...
    XtTranslateKey(display, keyCode, modifiers, modifiersReturn,
	    keysymReturn);
    /* and reinstall the Motif translator for the next widget/event... */
    XtSetKeyTranslator(display, (XtKeyProc) XmTranslateKey);
}
...
handleKeyEvents(Widget w, XtPointer closure, XEvent *event, Boolean *cont)
{
	...
	*cont = False;
    ...
    XtSetKeyTranslator(XtDisplay(w), (XtKeyProc) KeyTranslator);
...
)
{
    ...
    /* invoke them... */
    XtCallCallbackList(w, tw->term.statusChangeCallback,
	    (XtPointer) &cb);
}
...
_DtTermPrimStartOrStopPtyOutput(Widget w)
{
    ...
    */
    ...
    else if (tpd->ptyOutputId != 0)
    {
        ...
        ** turn it off...
        */
        XtRemoveInput(tw->term.tpd->ptyOutputId);
	...
	XtPointer call_data)
{
    ...
    XtUnmanageChild(tpd->warningDialog);
    /* call the OK callback... */
    WarningDialogOkCallback(tpd->warningDialog, client_data,
	    call_data);
}
...
void _DtTermPrimWarningDialog(Widget w, char *msg)
{
	...
	XtAddCallback(tpd->warningDialog,
	...
	for (parent = tpd->warningDialog; !XtIsShell(parent);
	    ...
	    ;
	_DtTermPrimAddDeleteWindowCallback(parent,
    ...
    XtSetValues(tpd->warningDialog, arglist, i);
    XmStringFree(msgString);

    if (!XtIsRealized(tpd->warningDialog)) {
	XtRealizeWidget(tpd->warningDialog);
	XtUnmanageChild(XmMessageBoxGetChild(tpd->warningDialog,
    ...
    XtSetValues(XtParent(tpd->warningDialog), arglist, i);
...
DtTermInitialize()
{
	...
	XtWarning("DtTermInitialize was not called before toolkit initialization\n");
    }
