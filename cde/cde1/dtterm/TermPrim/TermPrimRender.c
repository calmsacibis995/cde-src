...
static char rcs_id[] = "$XConsortium: TermPrimRender.c /main/cde1_maint/2 1995/09/29 12:08:16 lehors $";
...
#include "Term/TermP.h"
#include "Term/TermBuffer.h"
#include <limits.h>
...
_DtTermPrimRefreshText(Widget w, short startColumn, short startRow,
	short endColumn, short endRow, Boolean isExposeEvent)
{
    DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget) w;
    struct termData *tpd = tw->term.tpd;
    ...
    if (tpd->mbCurMax > 1)
    {
        _DtTermPrimRefreshTextWc(w, startColumn, startRow,
                                 endColumn, endRow, isExposeEvent);
        return;
    }
    ...
    */
    ...
    (void) XFillRectangle(XtDisplay(w),         /* Display		*/
		    XtWindow(w),		/* Drawable		*/
		    gc,				/* GC			*/
		    startColumn * tpd->cellWidth + tpd->offsetX + fudge,
						/* x			*/
	...
	if (lineNum >= tpd->lastUsedRow) {
		...
		linePtr = NULL;
          ...
          int lw, mw;
          lw = _DtTermPrimBufferGetLineWidth(tBuffer, lineNum);
          mw = MIN(endColumn - startColumn + 1, lw - startColumn);
	...
	while (lineWidth > 0) {
		...
		(void) XFillRectangle(XtDisplay(w),
			...
			chunkStartColumn * tpd->cellWidth + tpd->offsetX + fudge,
			startRow * tpd->cellHeight + tpd->offsetY,
	    ...
	    linePtr += BYTES_PER_CHAR(tBuffer) * chunkWidth;
	}


	/* clear any extra space in the line.  chunkStartColumn now points to
	 * the end of the line, and lineWidth == 0...
         Make sure you clear from end of line to last column - offsetX.
         Might make smooth scrolling a bit slower. That is a rare case. */

        if (!tw->term.jumpScroll &&
            tw->term.columns > endColumn && !isExposeEvent)
          rest_of_line = tw->term.columns - 1;
        else
          rest_of_line = endColumn;

	while (rest_of_line - chunkStartColumn >= 0){
	    chunkWidth = rest_of_line - chunkStartColumn + 1;
	    if (checkSelection && 
                _DtTermPrimSelectIsInSelection(w, lineNum, chunkStartColumn,
		                         chunkWidth, &chunkWidth)) {
...
#pragma BBA_IGNORE
	    ...
	    if (startColumn == 0 && tw->term.glyphGutter) {
            	unsigned char glyph =
                    _DtTermPrimBufferGetLineGlyph(tBuffer, startRow);
            	if ( glyph ) {
                   /* render the glyph */
                   (void) _DtTermPrimRenderText(
                    	w,                          /* Widget               */
	...
	Boolean isExposeEvent)
{
    ...
    if ( tpw->term.glyphGutter ) {
	    /* extend region to whole line */
	    x = tpd->offsetX;
	    width = tpw->core.width - 2 * tpd->offsetX;
    }
	...
	if ((bothX2 > bothX1) && (bothY2 > bothY1)) {
	    (void) _DtTermPrimRefreshText(w,
		    ...
		    tpd->scrollSrcY) / tpd->cellHeight, isExposeEvent);
	}
    }
    ...
    (void) _DtTermPrimRefreshText(w, (x - tpd->offsetX) / tpd->cellWidth,
	    ...
	    (y + height - 1 - tpd->offsetY) / tpd->cellHeight, isExposeEvent);
...
_DtTermPrimFillScreenGap(Widget w)
{
    ...
    int status, j;
    unsigned char i2;
    short i1, chunkStartColumn, chunkWidth;
    enhValues enhancements;
	...
	if (tpd->topRow + tpd->cursorRow >= tpd->lastUsedRow) {
		...
		if (tpd->useHistoryBuffer &&
			...
			length = _DtTermPrimBufferGetLineWidth(tBuffer,i1);
			...
			*/
			if (length > 0) {
                            ...
                            /* get text from term buffer */
			    c1 = _DtTermPrimBufferGetCharacterPointer(
				    tBuffer, i1, 0);
			    chunkStartColumn = 0;

			    while (length > 0){
                              /* get enhancement from term buffer */
			      status =
			      ...
			      /* clip chunkWidth... */
			      if (chunkWidth > length)
				chunkWidth = length;

			      /* set enhancement to history buffer */

                              i2 = 0;
                              for (j = 0; j < NUM_ENHANCEMENT_FIELDS; j++)
                                {
                                  if (j == enhUnused)
                                    continue;
                                  ...
                                  tpd->historyBuffer,         /* TermBuffer	 */
                                  tpd->lastUsedHistoryRow,    /* row		 */
                                  chunkStartColumn,           /* col		 */
                                  i2,                         /* id            */
                                  enhancements[j]);        /* value         */

                                  i2++;
                                }
			      
			      /* insert text and enhancement to history buffer */
			      ...
			      status =
			       ...
			       chunkStartColumn += chunkWidth;
			       length -= chunkWidth;
			       c1 += BYTES_PER_CHAR(tBuffer) * chunkWidth;
...
_DtTermPrimInsertText(Widget w, unsigned char *buffer, int length)
{
    ...
    if (tpd->mbCurMax > 1)
    {
        ...
        */
        ...
        while (i < length)
        {
            switch (mbLen = mbtowc(pwc, pmb, MIN(MB_CUR_MAX, length - i)))
            {
              case -1:
                if (MB_CUR_MAX <= length - i) {
	...
	i           = 0;
        while (i < wcBufferLen)
        {
            switch (mbLen = mblen(pmb, MIN(MB_CUR_MAX, length - i)))
            {
	      case -1:
              case  0:
                /* 
	...
	if (((tpd->cursorColumn + insertCharCount) >= tw->term.columns) ||
		...
		if (needToRender) {
			...
			(void) _DtTermPrimRefreshText(w, 0, tpd->cursorRow + 1,
				renderEndX, tpd->cursorRow + 1, False);
		    }
		    wrapped = False;
		    needToRender = False;
		}
...
)
{
    ...
    *dangleBuffer = (unsigned char *) 0;
	...
	if (tpd->mbCurMax > 1) {
            switch (mbCharLen = 
                    mblen((char *) &buffer[i], MIN(MB_CUR_MAX, len - i)))
            {
              case -1:
		if (MB_CUR_MAX <= len - i)
                {
