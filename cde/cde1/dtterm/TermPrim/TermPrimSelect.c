...
static char rcs_id[] = "$XConsortium: TermPrimSelect.c /main/cde1_maint/2 1995/09/29 12:08:30 lehors $";
...
)
{
    ...
    {
	/* more than double-click "token" modifier doesn't apply, undo it */
	if (selectInfo->scanType == XmSELECT_WORD && dotoken)
		selectInfo->scanType = XmSELECT_WHITESPACE;
        i = 0;
	...
	if (selectInfo->scanType == XmSELECT_WHITESPACE && dotoken)
	    selectInfo->scanType = XmSELECT_WORD;
    } 
    else
    {	
        /* single-click event */
...
)
{
    ...
    x -= tw->term.glyphGutter;
    if ( x<0) x = 0;  
    if ( x > tw->core.width) x = tw->core.width;  
    /* 
...
)
{
	...
	int inword;
        ...
        if (  MB_CUR_MAX > 1 ) 
          {
#define	iswword(c)	(iswalnum(c) || (c) == '_')
            TermCharInfoRec charInfoRec ;
...
#define	isword(c)	(isalnum(c) || (c) == '_')
	    ...
	    inword = isword(*pbuf);
            ...
            {
              case scanLeft:
                while((isword(*pbuf) == inword) && --col >= 0)
                   _DtTermPrimBufferGetText(pb, row, col, 1, pbuf, False );
                col++;
                break;
              case scanRight:
                while(++col <= width && (isword(*pbuf) == inword))
                   _DtTermPrimBufferGetText(pb, row, col, 1, pbuf, False );
                col--;
                break;
             }
        ...
        else
          {
            ...
            switch(scanDir)
            {
              case scanLeft:
                _DtTermPrimBufferGetText(pb, row, col, 1, pbuf, False);
		inword = isspace(*pbuf);
                while((isspace(*pbuf) == inword) && --col >= 0)
                   _DtTermPrimBufferGetText(pb, row, col, 1, pbuf, False );
                col++ ;
                break;
              case scanRight:
                _DtTermPrimBufferGetText(pb, row, col, 1, pbuf, False);
		inword = isspace(*pbuf);
                while( ++col <= width && (isspace(*pbuf) == inword))
                   _DtTermPrimBufferGetText(pb, row, col, 1, pbuf, False );
                col--;
                break;
             }
    ...
    */
    if (startRow == stopRow)
    {
        _DtTermPrimRefreshText((Widget)tw, startCol, startRow, 
                               stopCol, startRow, False);
        return;
    }
    _DtTermPrimRefreshText((Widget)tw, startCol, startRow, 
                      selectInfo->columns - 1, startRow, False);
    ...
    */
    if (startRow++ < stopRow)
    {
        _DtTermPrimRefreshText((Widget)tw, 0, startRow, 
                         selectInfo->columns - 1, stopRow - 1, False);
    }

    /* 
    ** refresh the last line 
    */
    _DtTermPrimRefreshText((Widget)tw, 0, stopRow, stopCol, stopRow, False);
...
)
{
    ...
    /*
     * If we're not selecting a position (which falls between characters)
     * round down the x value to undo the rounding up that xyToPos will do.
     */
...
)
{
    ...
    */
    ...
    setScanType(w, event,
	*paramCount >= 1 && (strcmp(params[0], "token") == 0));
...
) 
{
     ...
     */
    if (selectInfo->scanType != XmSELECT_POSITION)
	selectInfo->extend.x -= tpd->cellWidth / 2;
        ...
        int *format )
{
  ...
  Atom null_atom = XmInternAtom(XtDisplay(tw), "NULL", False);
    ...
    if (transfer_rec->move)
      {
        ...
        /* ------------------------------------------------------------- */
        /*                   Fixes Bug Id 1225717                        */
        /*  This block of code fixes an infinite communication loop      */
        /*  between dtterm and OLIT and xview applications. This occurs  */
        /*  when the user drags and drop from from those apps to dtterm  */
        /*  the variable "*type" contains an atom that should have been  */
        /*  reset by the source which is xview/olit apps. But it doesnt. */
        /*  The code below ensures the infinite comm loop gets broken.   */
        /* ------------------------------------------------------------- */

        if (*type != 0)
          atom_name = XGetAtomName(XtDisplay(w), *type);

        if (*type == 0 || (atom_name && strcmp(atom_name, "DELETE") == 0))
          {
            if (value != NULL)
              {
                XtFree((char *)value);
...
{
    DeleteDropContext(w);
    if (clientData) XtFree((char *)clientData);
}
...
)
{
    ...
    if (selected_string != NULL) {
       clip_label = XmStringCreateLtoR ("XM_TERM", XmFONTLIST_DEFAULT_TAG);
...
typedef struct _SelectionCoordinates {
	short	startRow;
	short	startColumn;
	short	endRow;
	short	endColumn;
...
char *
SDtTermGetSelection(Widget wid)
{
	Atom selection = XmInternAtom(XtDisplay(wid), "PRIMARY", False);
	Atom target = XmInternAtom(XtDisplay(wid), "TEXT", False);
	Atom type = XmInternAtom(XtDisplay(wid), "STRING", False);
	unsigned long length;
	int format;
	char *string;

	_DtTermPrimSelectConvert(wid, &selection, &target, &type,
                                 (XtPointer *) &string, &length, &format);

	return string;
}

Boolean
SDtTermGetSelectionCoordinates(Widget wid, SelectionCoordinates *p)
{
	XmTextPosition begin, end;
	Boolean success = _DtTermPrimSelectGetSelection(wid, &begin, &end);
	if ( success ) {
		short row, col;
		DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)wid;
		TermBuffer pb;
		posToBufferRowCol(tw,begin,&pb,&row,&col);
		p->startRow = row;
		p->startColumn = col;
		posToBufferRowCol(tw,end,&pb,&row,&col);
		p->endRow = row;
		p->endColumn = col;
		return True;
	} else {
...
void
SDtTermSetSelectionCoordinates(Widget wid, SelectionCoordinates *p)
{
	DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)wid;
	XmTextPosition begin = rowColToPos(tw, p->startRow, p->startColumn);
	XmTextPosition end = rowColToPos(tw, p->endRow, p->endColumn);

	_DtTermPrimSelectDisown(wid);
	setSelection(tw, begin, end, CurrentTime, False);
}
