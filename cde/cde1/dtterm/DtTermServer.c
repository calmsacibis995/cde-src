...
static char rcs_id[] = "$XConsortium: DtTermServer.c /main/cde1_maint/2 1995/10/09 11:46:21 pascale $";
...
)
{
    ...
    (void) sprintf(buffer, "%s-%s-%d", DTTERM_SVC_CLASS, hostname, getuid());
...
)
{
	...
	argcnt = 0;
	(void) sprintf(buffer, "%d", serviceClientInfo->pid);
...
)
{
	...
	if (!strcmp(messageFields[0], DTTERM_SVC_START_MSG)) {
		...
		if (!strcmp(messageFields[i2], "-pid")) {
		    (void) i2++;
		    if (i2 < numFields) {
			pid = (int) strtol(messageFields[i2],
				(char **) 0, 0);
		    }
...
)
{
	...
	case SVC_NOTIFY:
		...
		if (numFields >= 2) {
		    pid = (int) strtol(messageFields[1], (char **) 0, 0);
		} else {
		    pid = -1;
		}
