/*
 * File:         main.c $XConsortium: main.c /main/cde1_maint/1 1995/07/15 01:07:50 drk $
 * Language:     C
 *
...
/*----------------------------------------------------------------------+*/
{
	...
	*/
	if(SPCD_Authentication_Dir) {
	    ...
	    */
	    tmpfile = tempnam(strdup(SPCD_Authentication_Dir), ".SPC_" );
	} else {
	    char *cp, temp_buf[MAXPATHLEN];
	    ...
	    */
	    ...
	    /* tempnam() unfortunately may not come back with the requested
	     * pathname if tmppath is  not *writeable* by the UID which runs
	     * dtspcd (i.e. root).  For NFS-mountd directories specified by
	     * the dtaction "-contextDir" command this may often be the case.
	     * So in that case we just  just tempnam() to  get the unique suffix.
	     */
	    ...
	    cp = strrchr(tmpfile, '/');
	    if  (cp == NULL) {
                SPC_Format_Log ("+++> FAILURE: pathname '%s' is bad.", tmpfile);
                SPC_Write_Protocol_Request(client_connection, NULL, LOGFILE_REPLY,
                                       FAILED_FILE_NAME, NULL, NULL);
	    ...
	    /* Construct desired path from components */
	    strcpy(temp_buf, tmppath);
	    strcat(temp_buf, cp);
	    tmpfile = temp_buf;
	}
    } else {
    ...
    */
    ...
    SPC_Format_Log("     Client protocol version is '%d'.", SPC_client_version_number);
