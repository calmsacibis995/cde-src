/* $XConsortium: props.c /main/cde1_maint/4 1995/11/14 16:59:10 drk $ */
/*
...
#pragma ident "@(#)props.c	1.17 96/03/04 Sun Microsystems, Inc."
...
extern void uname_error_popup(char *, Props_pu *);
	...
	Props_pu *p)
{
	...
	p->gap_list = NULL;
        ...
        XtSetArg(args[i], XmNrightOffset, 		RIGHT_GAP);         ++i;
	...
	XtVaSetValues(p->gap2_button_form,
        	XmNrightAttachment, 		XmATTACH_OPPOSITE_WIDGET,
        	XmNrightWidget, 		p->gap2_access_list,
        	XmNrightOffset, 		-max_button_label_width - GAP,
		NULL);
	...
	Dimension	 right_gap) 
{
	...
	xstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 2108, "View"));
	p->gap2_view_label =
	...
	XtPointer	 	 callback_struct)
{
	...
	if (!(user_name = (char *)XmTextGetString(p->gap2_user_text)) ||
		...
		XtFree(title);
		return;
	} else if (embedded_blank(user_name)) {
                uname_error_popup(user_name, p);
                XtFree(user_name);
                return;
        }
	...
	access_entry = 
		(CSA_access_rights *)XtCalloc(1, sizeof(CSA_access_rights));
	access_entry->user = 
		(CSA_calendar_user *)XtCalloc(1, sizeof(CSA_calendar_user));
