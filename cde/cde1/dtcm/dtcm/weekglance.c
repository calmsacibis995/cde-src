/* $XConsortium: weekglance.c /main/cde1_maint/3 1995/11/14 17:01:04 drk $ */
/*
...
#pragma ident "@(#)weekglance.c	1.104 96/11/12 Sun Microsystems, Inc."
...
static int week_xytoclock(), week_xytohour();
...
static int
count_week_pages (Calendar *c, Tick start_date)
{
	time_t start, stop;
	...
	Boolean first)
{
        ...
        int num_appts, day_of_week, total_pages;
        char    buf[128];
        int     i, j;
	...
	CSA_enum *ops;
        ...
        days[1] = XtNewString(catgets(c->DT_catd, 1, 596, "Monday %d"));
        days[2] = XtNewString(catgets(c->DT_catd, 1, 597, "Tuesday %d"));
        days[3] = XtNewString(catgets(c->DT_catd, 1, 598, "Wednesday %d"));
        days[4] = XtNewString(catgets(c->DT_catd, 1, 599, "Thursday %d"));
        days[5] = XtNewString(catgets(c->DT_catd, 1, 600, "Friday %d"));
        days[6] = XtNewString(catgets(c->DT_catd, 1, 601, "Saturday %d"));
        days[7] = XtNewString(catgets(c->DT_catd, 1, 602, "Sunday %d"));
		...
		num_appts = count_multi_appts(list, a_total, c);
                ...
                done = ps_print_multi_appts (fp, list, a_total, num_page, stop, weekGlance);
...
extern void
print_week_pages(Calendar * c)
{

        Props *p = (Props *)c->properties;
        register Tick first_date = c->view->date;
        int num_weeks = get_int_prop(p, CP_PRINTMONTHS);
        ...
        first_date = lowerbound(first_date - (day_of_week * daysec));
...
allocator(Calendar *c)
{
	...
	calendar->view->week_info = (caddr_t) XtCalloc(1, sizeof(Week));
	w = (Week *)c->view->week_info;

	/* navigation buttons to day view */
	w->hot_button = (Widget *) XtCalloc(7, sizeof(Widget));
	for (n=0; n<7; n++) {
		Arg al[3];
		int ac = 0;
		...
		w->hot_button[n] =
			XmCreatePushButton(c->canvas, "week2day", al, ac);
	...
	w->current_selection = XtCalloc(1, sizeof(Selection));
	...
	*boundary = okay;
	...
	char_height = regfontextents.max_logical_extent.height;
        ...
        if (w->time_array != NULL)
                free(w->time_array);
        w->time_array = (char*)XtCalloc(1, w->segs_in_array);
...
paint_entry(Calendar *c, int x, int y, int maxchars, Paint_cache *cache_entry, XRectangle *rect)
{
	...
	tick = cache_entry->start_time;
        ...
        if (cache_entry->show_time && (buf1[0] != NULL)) {
