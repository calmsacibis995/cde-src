...
**  tempbr.c
**
**  $XConsortium: tempbr.c /main/cde1_maint/3 1995/11/14 17:00:20 drk $
**
...
#pragma ident "@(#)tempbr.c	1.54 96/11/12 SMI"
...
#ifndef NOTHREADS
extern void switch_it2(Calendar *, char *, char *, WindowType);
#endif
...
tempbr_show_cb(Widget w, XtPointer data, XtPointer cbs)
{
	...
	if (cmfns_use_fns((Props *)c->properties)) {
		...
		if (cmfns_lookup_calendar(name, buf, sizeof(buf)) > 0)
			addr = buf;
	}
#endif
	if (addr == 0)
	...
	XFlush(XtDisplay(w));
#ifndef NOTHREADS
        switch_it2(tb->cal, name, addr, tempbrowser);
#else
        switch_it(tb->cal, name, addr, tempbrowser);
#endif
...
make_std_tempbr(Calendar *c)
{
	...
	if (c->tempbr == NULL) {
                c->tempbr = (caddr_t)XtCalloc(1, sizeof(Tempbr));
                tb = (Tempbr*)c->tempbr;
        }
        else
                tb = (Tempbr*)c->tempbr;
