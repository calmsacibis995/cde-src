/* $XConsortium: editor.c /main/cde1_maint/6 1995/11/14 16:11:09 barstow $ */
/*
...
#pragma ident "@(#)editor.c	1.141 96/11/12 Sun Microsystems, Inc."
...
e_insert_proc(Widget w, XtPointer client_data, XtPointer data) {
	...
	if (strcmp(c->caladdr, c->view->current_calendar) != 0) {
        ...
        /* Do not schedule yearly repeating leap year appointments. */ 
        if ((e->rfp.repeat_type == CSA_X_DT_REPEAT_YEARLY) && 
            (!strncmp(e->dssw.date_val,"2/29/2000",4))) {
               ...
               */
               if ((e->rfp.repeat_type == CSA_X_DT_REPEAT_YEARLY) &&
                   (!strncmp(e->dssw.date_val,"2/29/2000",4))) {
                      ...
                      /* 
                      e->rfp.repeat_type = 0;
                      sprintf(e->rfp.recurrence_rule,"%s","D1 #1");
                      */
                      set_editor_defaults(e, 0, 0, False);
                      add_all_appt(e, NULL);
...
e_make_editor(Calendar *c)
{
	...
	static char 	translations[] = "\
		~c ~s ~m ~a <Btn1Down>:\
			process-press(ListBeginSelect,TranslationDragStart)\n\
		c ~s ~m ~a <Btn1Down>:\
			process-press(ListBeginToggle,TranslationDragStart)";
	...
	*/
	e->frame = XtVaCreatePopupShell("frame",
		...
		XmNallowShellResize, 	True,
		NULL);
	set_editor_title(e, c->view->current_calname);
	...
	Boolean			*match_ref) 
{
	...
	backend_err_msg(e->frame, e->cal->view->current_calname, status,
			((Props_pu *)e->cal->properties_pu)->xm_error_pixmap);
	if (status != CSA_SUCCESS) {
		free_appt_struct(&appt);
	...
	/* See if the appt_ref that was passed to us matches this appt's. */
	if (appt_ref && 
	    !strncmp((const char *)
		      appt->identifier->value->item.opaque_data_value->data,
		     (const char *)
		      appt_ref->identifier->value->item.opaque_data_value->data,
		     appt_ref->identifier->value->item.opaque_data_value->size))
		*match_ref = True;

	format_appt(appt, buf, dt, DEFAULT_APPT_LEN);
	str = XmStringCreateLocalized(buf);
	/* str must be freed by calling function */
	free_appt_struct(&appt);
	...
	Dtcm_appointment	*appt_ref)
{
	...
	items = (XmString *)XtMalloc(j * sizeof(XmString *));
	for (i = 0; i < j; i++) {
		items[i] = add_to_appt_list(entry_list[i], e, appt_ref,
					    &match_ref);

		/* Did the appt_ref (reference) match one of the appts in
		 * the list?  
		 */
		if (match_ref)  {
			matched_item = i;
	...
	XmListAddItems(e->appt_list, items, j, 1);
	if (matched_item != -1) {
		XmListSelectPos(e->appt_list, matched_item + 1, False);
		XtSetSensitive(e->delete_button, True);
		XtSetSensitive(e->change_button, True);
	}
...
build_editor_list(Editor *e, Tick date, Glance glance,
	...
	setup_range(&range_attr, &ops, &range_count, start, stop,
		    CSA_TYPE_EVENT, NULL, _B_FALSE, e->cal->general->version);
	...
	*/
	...
	for (cnt = 0; cnt < entry_count; cnt++) {
		...
		*/
		...
		if (lines && lines->s) {
			buf = (char *)XtMalloc(cm_strlen(str1) + cm_strlen(str2)
					      + cm_strlen(lines->s) + 5);
			what_str = lines->s;
		} else {
			buf = (char *)XtMalloc(cm_strlen(str1)
					      + cm_strlen(str2) + 6);
			what_str = "\0";
		}
...
editor_change(Dtcm_appointment *new_a, CSA_entry_handle old_a, CSA_entry_handle *updated_a,
	...
	backend_err_msg(e->frame, c->view->current_calname, stat,
			p->xm_error_pixmap);
	if (stat != CSA_SUCCESS) {
		free_appt_struct(&appt);
		return FALSE;
	...
	/* we don't change the character set attribute */
	if (new_a->charset && new_a->charset->name) {
		free(new_a->charset->name);
		new_a->charset->name = NULL;
	}
	...
	Calendar 		*c) 
{
	...
	if (c->set_charset && appt->charset) {
		/* tag the entry with the locale of the process */
		appt->charset->value->item.string_value = strdup(c->app_locale);
	} else if (appt->charset && appt->charset->name) {
		free(appt->charset->name);
		appt->charset->name = NULL;
	}

	stat = csa_add_entry(c->cal_handle, appt->count, appt->attrs, new_a, NULL);
	backend_err_msg(c->frame, c->view->current_calname, stat,
			p->xm_error_pixmap);
	if (stat != CSA_SUCCESS)
		return FALSE;

	if (editor_showing(e)) {
		/* Get the reference id for the appt we just inserted.  
		 * We need to find this appt in the current list and select 
		 * it.
	 	 */
		appt_ref = allocate_appt_struct(appt_read, 
				        e->cal->general->version,
					CSA_ENTRY_ATTR_REFERENCE_IDENTIFIER_I, 
					NULL);
		stat = query_appt_struct(e->cal->cal_handle, *new_a, appt_ref);
		backend_err_msg(e->frame, e->cal->view->current_calname, stat,
			((Props_pu *)e->cal->properties_pu)->xm_error_pixmap);
		if (stat != CSA_SUCCESS) {
			free_appt_struct(&appt_ref);
			return FALSE;
		}
	}

	add_all_appt(e, appt_ref);
...
show_editor(Calendar *c, time_t start, time_t stop, Boolean show_notime) {
	...
	if (!editor_created(e))
		e_make_editor(c);
	else XtManageChild(e->base_form_mgr);
	if (!editor_showing(e)) {
	...
	if (build_editor_view(e, glance, False) <= 0) {
		...
		if (!editor_view_showing(e))
			ds_position_popup(c->frame, e->view_frame,
					  DS_POPUP_LOR);
		XtVaSetValues(e->view_frame, XmNmappedWhenManaged, True,
			NULL);
		XtPopup(e->view_frame, XtGrabNone);
		XtManageChild(e->view_form);
