/* $XConsortium: todo.c /main/cde1_maint/7 1995/11/14 17:00:48 drk $ */
/*
...
#pragma ident "@(#)todo.c	1.128 96/11/12 Sun Microsystems, Inc."
...
t_build_expand(ToDo *t) {
	...
	*/
	...
	XtVaSetValues(t->reminders.mail_toggle,
		XmNtopWidget, 		t->reminders.alarm_label,
		XmNleftAttachment, 	XmATTACH_FORM,
		XmNleftOffset, 		20,
		NULL);
	XtVaSetValues(t->reminders.mail_text,
		XmNleftAttachment, 	XmATTACH_WIDGET,
		XmNleftWidget, 		t->reminders.mail_toggle,
		XmNleftOffset, 		10,
		NULL);
...
t_expand_ui_proc(Widget w, XtPointer client_data, XtPointer data) {
	...
	if (expand_state_closed) {
		...
		XtVaGetValues(t->reminders.bfpm_form_mgr, XmNheight, &h, NULL);
...
t_insert_proc(Widget w, XtPointer client_data, XtPointer data) {
	...
	if (t->cal->set_charset && appt->charset) {
		/* tag the entry with the locale of the process */
		appt->charset->value->item.string_value =
			strdup(t->cal->app_locale);
	...
	if (stat != CSA_SUCCESS) {
        ...
        /* Get the reference id for the appt we just inserted.  We need to
         * find this appt in the current list and select it.
         */
        appt_ref = allocate_appt_struct(appt_read,
                                        t->cal->general->version,
                                        CSA_ENTRY_ATTR_REFERENCE_IDENTIFIER_I,
                                        NULL);
        stat = query_appt_struct(t->cal->cal_handle, new_a, appt_ref);
        backend_err_msg(t->frame, t->cal->view->current_calendar, stat,
                        ((Props_pu *)t->cal->properties_pu)->xm_error_pixmap);
...
t_execute_change(ToDo *t, CSA_entry_handle old_a, Dtcm_appointment *new_a,
	...
	stat = csa_update_entry_attributes(c->cal_handle, old_a, scope, 
			CSA_FALSE, new_a->count, new_a->attrs, 
			&updated_a, NULL);
	backend_err_msg(t->frame, c->view->current_calendar, stat,
			p->xm_error_pixmap);
	if (stat != CSA_SUCCESS) {
...
t_make_todo(Calendar *c)
{
	...
	*/
	...
	max_left_label_width = ComputeMaxWidth(t->dssw.date_label,
						t->dssw.start_label,
						t->dssw.what_label,
						NULL) + 5;
	...
	XtVaSetValues(t->dssw.what_label,
		XmNtopWidget, 	t->dssw.start_text,
		XmNrightWidget, t->dssw.what_scrollwindow,
		NULL);
	...
	t->completed_toggle = XtVaCreateManagedWidget("completed",
		...
		XmNlabelString, 	xmstr,
                XmNleftAttachment,      XmATTACH_FORM,
                XmNleftOffset,          max_left_label_width - 4,
                XmNbottomAttachment,    XmATTACH_FORM,
		XmNnavigationType, 	XmTAB_GROUP,
		NULL);
	XmStringFree(xmstr);
	...
	if (stat != CSA_SUCCESS) {
        ...
        if (appt_ref &&
            !strncmp((const char *)
                     ...
                     (const char *)
                      appt_ref->identifier->value->item.opaque_data_value->data,                     appt_ref->identifier->value->item.opaque_data_value->size))                *match_ref = True;

	lines = text_to_lines(appt->what->value->item.string_value, 1);
	if (lines && lines->s) {
		buf = (char *)XtMalloc(cm_strlen(lines->s) + 1);
		strcpy(buf, lines->s);
		destroy_lines(lines);
	} else {
		buf = (char *)XtMalloc(1);
	...
	Dtcm_appointment        *appt_ref) 
{
	...
	items = (XmString *)XtMalloc(count * sizeof(XmString *));
	for (i = 0; i < count; i++) {
		items[i] = add_to_todo_list(entry_list[i], t, appt_ref,
					   &match_ref);
	...
	XmListAddItems(t->todo_list, items, count, 1);
	if (matched_item != -1) {
		XmListSelectPos(t->todo_list, matched_item + 1, False);
		XtSetSensitive(t->delete_button, True);
		XtSetSensitive(t->change_button, True);
	}
...
build_todo_view(ToDo *t, Glance glance, Boolean redisplay) {
	...
	*/
	for (cnt = 1; cnt <= entry_count; cnt++) {
		...
		*/
		if (!step) {
			step = (TodoView *)XtCalloc(1, sizeof(TodoView));
		...
		*/
		...
		if (stat == CSA_SUCCESS) {
			...
			if (lines && lines->s) {
				...
				buf = (char *)XtMalloc(cm_strlen(str1) + 4);
				what_str = "\0";
			}
