/* $XConsortium: group_editor.c /main/cde1_maint/4 1995/11/14 16:58:12 drk $ */
/*
...
#pragma ident "@(#)group_editor.c	1.131 96/12/12 Sun Microsystems, Inc."
...
ge_add_to_gappt_list(Access_data *ad, int idx, GEditor *ge, Boolean reset) {
	...
	*/
	...
	step = (List_data *)XtCalloc(1, sizeof(List_data));
	step->entry_idx = idx;
	...
	for (i = 0, address_len = 0; i < pos_cnt; i++) {
		...
		if (ad) {
			...
			if (cmfns_use_fns((Props *)c->properties) &&
			    cmfns_lookup_calendar(ad->name, buf, sizeof(buf)) > 0)
				addrs[i] = cm_strdup(buf);
			else
#endif /* FNS */
				addrs[i] = cm_strdup(ad->name);
...
ge_insert_proc(Widget w, XtPointer client_data, XtPointer data) {
	...
	v4access = CSA_INSERT_PUBLIC_ENTRIES | CSA_INSERT_CONFIDENTIAL_ENTRIES |
		   CSA_INSERT_PRIVATE_ENTRIES | CSA_OWNER_RIGHTS;
	v3access = CSA_X_DT_INSERT_ACCESS | CSA_OWNER_RIGHTS;
	...
	for(i = 0; i < c_cnt; i++) {
		...
		if (!ad) {
		...
		} else if ((ad->version < DATAVER4 &&
		    !(ad->entry_access & v3access)) ||
		    (ad->version >= DATAVER4 &&
		    !(ad->entry_access & v4access))) {

			/* no insert access on this calendar, skip */
			continue;

		} else {

			/* it may seem a little odd to create this
			 * template appointment over and over again,
			 * but this needs to be done because each connection
			 * may support a different data model, and thus need
			 * the appointments created differently.
			 */
			...
			if (ad->set_charset && appt->charset) {
				/* tag the entry with the locale of the
				 * process
				 */
	...
	Boolean			select_val)
{
	...
	new_data = (Access_data *)XtCalloc(1, sizeof(Access_data));
	...
	new_data->set_charset = set_charset;
	CmDataListAdd(ge->access_data, (void *)new_data, 0);

	buf = (char *)XtMalloc(ACCESS_NAME_LEN + cm_strlen(catgets(c->DT_catd,
...
add_all_gappt(GEditor *ge) {
	...
	for (i = 1; i <= ge->access_data->count && stat == CSA_SUCCESS; i++) {
		...
		setup_range(&range_attr, &ops, &range_count, start, stop,
		    	CSA_TYPE_EVENT, NULL, _B_FALSE, step->version);
...
remove_all_gaccess_data(GEditor *ge) {
	...
	CmDataListDeleteAll(ge->access_data, _B_TRUE);
}
