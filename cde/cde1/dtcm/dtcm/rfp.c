/* $XConsortium: rfp.c /main/cde1_maint/4 1995/11/14 17:00:07 drk $ */
/*
...
#pragma ident "@(#)rfp.c	1.86 96/11/12 Sun Microsystems, Inc."
...
rfp_toggle_for_grey(RFP *rfp, Boolean status) {
	...
	XtVaGetValues(rfp->for_menu, DtNlist, &list, DtNtextField, &text, NULL);
...
rfp_toggle_repeat_grey(RFP *rfp, Boolean status) {
	...
	XtVaGetValues(rfp->repeat_menu, DtNlist, &list, NULL);
	XtVaSetValues(list, XmNsensitive, status, NULL);
}
...

{
	...
	if (limit == 11) {
		DtComboBoxDeletePos(rfp->repeat_menu, 11);
		if (rfp->cal->general->version == DATAVER1) {
			DtComboBoxDeletePos(rfp->repeat_menu, 10);
			DtComboBoxDeletePos(rfp->repeat_menu, 9);
			DtComboBoxDeletePos(rfp->repeat_menu, 8);
		}
		if (rfp->repeat_popup_frame)
			XtPopdown(rfp->repeat_popup_frame);
		return;
	}
	else if (limit == 10) {
        	xmstr = XmStringCreateLocalized(catgets(calendar->DT_catd, 1, 901, "Repeat Every ..."));
        	DtComboBoxAddItem(rfp->repeat_menu, xmstr, 0, False);
        	XmStringFree(xmstr);
...
rfp_set_default_for(RFP *rfp, Repeat_menu_op op, Boolean status) {
	...
	XtVaGetValues(rfp->for_menu, DtNtextField, &text, NULL);
...
rfp_apply_proc(Widget w, XtPointer client_data, XtPointer cbs) {
	...
	/*
	 * We're going to change the string to be more informative, but first
	 * make sure the combo box thinks it has the REPEAT_EVERY item selected.
	 */
	XtVaSetValues(rfp->repeat_menu, DtNselectedPosition, REPEAT_EVERY,
		      NULL);
...
rfp_repeat_every_popup(RFP *rfp) {
	...
	title = XtNewString(catgets(c->DT_catd, 1, 532,"Calendar : Repeat Every"));
	...
	label_str = XtNewString(catgets(c->DT_catd, 1, 460, "Apply"));
        rfp->repeat_apply_button = XtVaCreateWidget(
			label_str,
	...
	label_str = XtNewString(catgets(c->DT_catd, 1, 680, "Close"));
        rfp->repeat_cancel_button = XtVaCreateWidget(
			label_str,
		xmPushButtonWidgetClass, rfp->repeat_form_mgr,
		XmNleftAttachment, XmATTACH_POSITION,
		XmNleftPosition, 1,
	...
	XtPopup(rfp->repeat_popup_frame, XtGrabNone);
}
...
rfp_repeat_menu_proc(Widget w, XtPointer data, XtPointer cbs) {
	...
	XtVaGetValues(rfp->repeat_menu,
		      DtNselectedPosition, &item_no,
		      NULL);

	if (item_no == REPEAT_EVERY) {
		/*
		 * If the user cancels or quits the popup, we want the
		 * combo box to say "One Time" - so set it to that before
		 * showing the "Repeat Every ..." repeat popup (the apply
		 * proc for that popup will set the combo box accordingly if
		 * it's selected).
		 */
		XtVaSetValues(rfp->repeat_menu, DtNselectedPosition, ONE_TIME,
			      NULL);
		...
		XtVaSetValues(rfp->repeat_form_mgr, XmNdefaultButton, rfp->repeat_apply_button, NULL);
		XtVaSetValues(rfp->repeat_form_mgr, XmNcancelButton, rfp->repeat_cancel_button, NULL);
	} else
	...
	Widget 		 parent)
{
	...
	label_str = XtNewString(catgets(c->DT_catd, 1, 836, "Frequency"));
        rfp->frequency_label = XtVaCreateWidget(
		label_str,
	...
	time_t		 tick)
{
	...
	*/
	XtVaGetValues(rfp->repeat_menu, DtNselectedPosition, &op, NULL);
...
get_rfp_privacy_val(RFP *rfp) {
	...
	XtVaGetValues(rfp->privacy_menu, DtNselectedPosition, &i, NULL);
		...
		if (rfp->repeat_type == CSA_X_DT_REPEAT_MONTHLY_BY_WEEKDAY) {
			if (weekofmonth(tick, &wk) && wk == 4) {
			  	...
			  	char *ident3 = XtNewString(catgets(c->DT_catd, 1,
						541, "4th Week"));
				answer = dialog_popup(rfp->parent,
					DIALOG_TITLE, title,
					DIALOG_TEXT, text,
					BUTTON_IDENT, 1, ident1,
				...
				case 3:
					if (a->version < DATAVER4)
						a->repeat_week_num->value->
							item.sint32_value = wk;
					break;
				default:
					return False;
				}
...
set_rfp_repeat_val(RFP *rfp) {
	...
	*/
	if (rfp->for_val == CSA_X_DT_DT_REPEAT_FOREVER) {
		...
		repeat_scope = "\0";
		xmstr = XmStringCreateLocalized(buf);
		DtComboBoxSetItem(rfp->for_menu, xmstr);
		XmStringFree(xmstr);
...
set_rfp_privacy_val(RFP *rfp) {
	...
	XtVaSetValues(rfp->privacy_menu, DtNselectedPosition, i, NULL);
}
