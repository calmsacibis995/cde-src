/* $XConsortium: about.c /main/cde1_maint/4 1995/11/14 16:54:50 drk $ */
/*
...
#pragma ident "@(#)about.c	1.33 97/06/17 Sun Microsystems, Inc."
...
#ifdef CREDITS
static const char *main_credits[] = {
	"Mary Hamilton",
	"Chris Kasso",
	"Martin Knutson",
	"Terre Layton",
...
static const char *also_credits[] = {
	"Brent Ayers",
	"David Curley",
	"Betty Deluco",
	"Lin-Chuan Lee",
	...
	XtPointer	cbs)
{
...
#else /* AIX_ILS */
  	strftime(buf, 64, catgets(c->DT_catd, 1, 1109, "%B %Y"), &tm); 
#endif /* AIX_ILS */
	...
	sprintf(buf, catgets(c->DT_catd, 1, 674, "Copyright (c) %s:"),
		SDtCopyrightYears);
	...
	XtPointer	callback_struct)
{
	...
	for (i = 0; main_credits[i]; i++) {
        	xstr = XmStringCreateLocalized((char *) main_credits[i]);
		label =
	...
	for (i = 0; also_credits[i]; i++) {
        	xstr = XmStringCreateLocalized((char *) also_credits[i]);
		label =
