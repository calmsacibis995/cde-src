...
**  namesvc.c
**
**  $XConsortium: namesvc.c /main/cde1_maint/1 1995/07/14 23:12:46 drk $
**
...
#pragma ident "@(#)namesvc.c	1.8 96/03/11 Sun Microsystems, Inc."
	...
	*name2 = NULL;
	...
	while (fgets(buffer, BUFSIZ, f)) {
		...
		if (strcmp(uname, tmp) == 0) {
			...
			ptr++;
			ptr[strlen(ptr)-1] = 0;
			*name2 = XtMalloc(cm_strlen(name) + cm_strlen(ptr) + 7);
			if (domain) 
	...
	char 	*loc;
{
	...
	ec = (entry_col*)XtCalloc(1, sizeof(entry_col) * NUMTBLCOLS);
...
new_nisobj()
{
	...
	nobj = (nis_object*)XtCalloc(1, sizeof(nis_object));
...
char **target;
{
	...
	return;
        ...
        if (status == l_ok) {
                 ...
                 */
                if (host) {
                        *target = (char *)XtMalloc(cm_strlen(name) +
                                        cm_strlen(host) + 3);
                        user = get_head(name, '@');
                        domain = get_tail(name, '@');
