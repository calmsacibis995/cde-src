...
**  graphics.c
**
**  $XConsortium: graphics.c /main/cde1_maint/1 1995/07/14 23:11:00 drk $
**
...
#pragma ident "@(#)graphics.c	1.27 96/11/12 SMI"
...
gr_create_xcontext(Calendar *c, Widget widget, GR_depth depth, XtAppContext app)
{
	...
	XColor		def_color;
        ...
        /* X Drawing Stuff */
        if ((xc = (new_XContext *) XtCalloc(1, sizeof(new_XContext))) == NULL)
                return NULL;
         ...
         */
        xc->gcvals = (XGCValues *) XtCalloc(1, sizeof(XGCValues));
