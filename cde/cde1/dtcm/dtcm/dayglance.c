...
**  dayglance.c
**
**  $XConsortium: dayglance.c /main/cde1_maint/2 1995/09/06 08:03:41 lehors $
**
...
#pragma ident "@(#)dayglance.c	1.83 96/11/12 SMI"
...
*/
		...
		end_tick = cache[i].end_time;
		hr = hour(start_tick);
		if (hr >= begin_time && hr < end_time && cache[i].show_time) {
			if (last_hr != hr) curr_line = 0;
			y = c->view->topoffset + 2 + pfy;
			if (curr_line < maxlines) {
...
init_dayview(Calendar *c)
{
	...
	if (day_info->month_panels == (Widget *) NULL) {
		day_info->month_panels = (Widget *)XtCalloc(3, sizeof(Widget));
    ...
    Boolean first)
{
	...
	for (i=daybegin; i < dayend; i++) {
		...
		done = ps_print_multi_appts (fp, list, a_total, num_page, stop, dayGlance);
...
extern int 
get_lines_per_page(Props *pr)
{
	int	daybegin = get_int_prop(pr, CP_DAYBEGIN);
	int	dayend   = get_int_prop(pr, CP_DAYEND);
	int	lines, hours;

	hours = dayend - daybegin;
