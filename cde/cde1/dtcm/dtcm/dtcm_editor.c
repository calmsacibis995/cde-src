/* $XConsortium: dtcm_editor.c /main/cde1_maint/5 1995/11/14 16:57:26 drk $ */
/*
...
#pragma ident "@(#)dtcm_editor.c	1.63 97/01/08 Sun Microsystems, Inc."
...
static Boolean set_defaults(XtPointer);
        ...
        XtPointer       callData)
{
	...
	DTCM_editor	*de = (DTCM_editor *) context->editor;
	int		old_attr_count;
	...
	if (dssw_form_to_appt(de->dssw, appt, de->c->calname, now()) == False)
		return;

	if (rfp_form_to_appt(de->rfp, appt, de->c->calname) == False)
		return;
	...
	apptstr = parse_attrs_to_string(appt, de->p, attrs_to_string(appt->attrs, appt->count));
...
load_from_file(DTCM_editor *de) {
	...
	Dtcm_appointment	*appt;
	Validate_op		 op;
	Props_pu        	*pu = (Props_pu *)de->dssw->cal->properties_pu;

	list = CmDataListCreate();
	if ((op = parse_appt_from_file(de->c->DT_catd, de->file, list, de->p, 
			   query_user, de->c, DATAVER_ARCHIVE)) != VALID_APPT) {
		editor_err_msg(de->c->frame, de->c->calname, op, 
			       pu->xm_error_pixmap); 
		return op;
	}
...
de_set_defaults(DTCM_editor *de) {
	...
	if (de->file && (access(de->file, R_OK) == 0)) {
		if (load_from_file(de) != VALID_APPT) {
...
de_apply_proc(Widget w, XtPointer client_data, XtPointer data) {
	...
	appt = allocate_appt_struct(appt_write, 
					...
					NULL);
	load_appt_defaults(appt, de->p);
	if (!dssw_form_to_appt(de->dssw, appt, de->c->calname, now()))
		return;

	if (!rfp_form_to_appt(de->rfp, appt, de->c->calname))
		return;
	...
	if (w == NULL) {
		Display	*dpy = XtDisplayOfObject(de->top_level);
...

{
	...
	if ((de->c->tt_procid = ttdt_open(&ttfd, toolname, "SunSoft", "%I", 1))
								== NULL) {
		 return -1;
	}
...
main(int argc, char **argv) {
	...
	de = (DTCM_editor *)XtCalloc(1, sizeof(DTCM_editor));
	de->file = NULL;
	...
	*/
	...
	de->c = calendar;
	de->p = (Props *)XtCalloc(1, sizeof(Props));
	de->c->properties = (caddr_t)de->p;
	de->c->general = (General*) XtCalloc(1, sizeof(General));
	de->c->general->version = DATAVER_ARCHIVE;
	de->c->frame = de->top_level;
	de->pu = (Props_pu *)XtCalloc(1, sizeof(Props_pu));
	...
	*/
	...
	label_str = XtNewString(catgets(de->c->DT_catd, 1, 846, "Save"));
	de->attach_button = XtVaCreateWidget(
		label_str,
		...
		NULL);
	XtFree(label_str);
	XtAddCallback(de->attach_button, XmNactivateCallback, de_apply_proc, de);
	if (!de->file)
		XtSetSensitive(de->attach_button, False);

	label_str = XtNewString(catgets(de->c->DT_catd, 1, 691, "Reset"));
	de->reset_button = XtVaCreateWidget(
		label_str,
		xmPushButtonGadgetClass, de->form,
		XmNleftAttachment, XmATTACH_POSITION,
		XmNleftPosition, 3,
	...
	de->rfp = (RFP *)XtCalloc(1, sizeof(RFP));
	...
	de->dssw = (DSSW *)XtCalloc(1, sizeof(DSSW));
	...
	XtAddCallback(de->rfp->repeat_menu, DtNselectionCallback, de_mark_change, de);
	XtAddCallback(de->rfp->for_menu, DtNselectionCallback, de_mark_change, de);
...
*/
	...
	if (cmtt_init("AppointmentEditor", de, app, calendar->frame)
							< 1) {
