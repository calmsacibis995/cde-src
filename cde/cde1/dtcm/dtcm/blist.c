...
**  blist.c
**
**  $XConsortium: blist.c /main/cde1_maint/3 1995/11/14 16:55:20 drk $
**
...
#pragma ident "@(#)blist.c	1.73 97/05/28 Sun Microsystems, Inc."
...
blist_name_append(Calendar *c, char *name, BlistTag t) {
	...
	if (strcmp(c->calname, name) == 0) {
        ...
        bd = (BlistData *)XtCalloc(1, sizeof(BlistData));
...
blist_init_names(Calendar *c) {
...
#ifndef NOTHREADS
		/* Initialize the blist_data lock. */
		cm_mutex_init(&bl->bl_data_op_lock);
		bl->bl_timer = NULL;
#endif /* NOTHREADS */
	} 
	...
	defcal = get_char_prop(p, CP_DEFAULTCAL);
	if (strcmp(defcal, c->calname) != 0 && strcmp(defcal, c->caladdr) != 0)
		blist_name_append(c, defcal, BLIST_ACTIVE);
	...
	while (name) {
		if (strcmp(name, c->calname) != 0 && strcmp(name, c->caladdr) != 0)
	...
	Calendar *c)
{
	...
	/* If there are any outstanding threads we can't scrub the list
	 * since the threads have references to memeory that would be
	 * freed.
	 */
        while ((bd = (BlistData *)CmDataListGetData(bl->blist_data, i)) &&
               bd->bd_live) {
...
show_browselist(Calendar *c) {
	...
	if (c->browselist == NULL)
		c->browselist = (caddr_t)XtCalloc(1, sizeof(Browselist));
...
#ifndef NOTHREADS
	/* Is the Menu Editor suspended due to pending multibrowser threads? */
	if (bl->bl_suspended)
		disable_menu_editor(bl, True);
#endif /* NOTHREADS */
}
