/* $XConsortium: find.c /main/cde1_maint/5 1995/11/14 16:57:40 drk $ */
/*
...
#pragma ident "@(#)find.c	1.69 96/11/12 Sun Microsystems, Inc."

#include <stdio.h>
#include <stdlib.h>
#include <wchar.h>	/* use of wide char processing */
#include <csa.h>
...
Calendar *c;
{
	...
	if (c->find == NULL) {
                c->find = (caddr_t)XtCalloc(1, sizeof(Find));
                f = (Find*)c->find;
        }
        else
                f = (Find*)c->find;
...
XmPushButtonCallbackStruct *cbs;
{
	...
	char *location;
	Dimension w, h;
        char what_buf[100], buf[WHAT_LEN+1], buf2[WHAT_LEN+1], message[40], *astr;
	XmString buf_str;
	...
	Dtcm_appointment *appt;
	size_t comparison_length; 	/* The search string length */
	Tick	real_eot = get_eot();
	wchar_t wbuf[WHAT_LEN+1]; /* temporary wchar store for appointment */
	wchar_t w_astr[WHAT_LEN+1]; /* wide char array for search string */

#ifndef NOTHREADS
	/*
	 * If a user initiates a find operation before a browse operation
	 * is complete (i.e. the browse thread is still executing) then
	 * the browse operation is cancelled and the find op is performed
	 * on the current calendar.
	 */
	if (c->last_cal_requested) {
		char	buf[256];

		sprintf(buf, catgets(c->DT_catd, 1, 1095,
					"Browse operation cancelled: %s"), 
					c->last_cal_requested);
		set_message(c->message_text, buf);

		free(c->last_cal_requested);
		c->last_cal_requested = NULL;
	}
#endif /* NOTHREADS */
	...
	if (f->search_set == search_all) {
		...
		if (end_of_time == DATE_AEOT)
			end_of_time = real_eot;
                        ...
                        sprintf(message,  catgets(c->DT_catd, 1, 812, "Please enter an end date after 1/2/1970 and before 1/1/2038"));
                        set_message(f->find_message, message);
                        return;
                }
	...
	/* Convert string to wc string */
	comparison_length = mbstowcs(w_astr, astr, WHAT_LEN);

	/* If the search string is equal or longer than WHAT_LEN, no '\0' will
	** will be added by mbstowcs(), so add it to make it consistent */
	if ( comparison_length == WHAT_LEN )
		w_astr[WHAT_LEN] = L'\0';
	...
	_DtTurnOnHourGlass(f->frame);
		...
		for (i = 0; i < num_entries; i++) {
			...
			for (j = 0; appt->what->value->item.string_value[j] 
								!= NULL; j++)
			{
				/* Convert each apointment into wchar format*/
			  	mbstowcs(wbuf, &appt->what->value->item.string_value[j], WHAT_LEN);
				/* use wide char compare to look up the aptmnt*/
			 	if (wcsncmp(w_astr, wbuf, comparison_length) == 0) {
					new_tick = (Tick_list *) XtCalloc(1, 
							sizeof(Tick_list));
					if (new_tick == NULL) {
						free_appt_struct(&appt);
						free_range(&range_attrs, &ops, 
								  range_count);
						csa_free(entries);
						_DtTurnOffHourGlass(f->frame);
						return;
					...
					strcpy(buf2, "");
					strcpy(what_buf, "");
					strftime(buf, WHAT_LEN, catgets(
                                          c->DT_catd, 1, 1107, "%h %e, %Y"),
					    localtime(
