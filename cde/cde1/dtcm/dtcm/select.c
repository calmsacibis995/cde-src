...
**  select.c
**
**  $XConsortium: select.c /main/cde1_maint/3 1995/10/03 12:26:45 barstow $
**
...
#pragma ident "@(#)select.c	1.27 96/11/12 SMI"
...
extern const int month_row_col[12][2];
	...
	if (j >= 0) {
		...
		if (c->xcontext->screen_depth < 8) {
			...
			gr_draw_rgb_box(c->xcontext, (i * w->chart_day_width)
				...
				+ w->add_pixels, 2,  background_pixel, cms);
		...
		case daySelect:
			...
			if (j >= 0) {
				...
				if (c->xcontext->screen_depth < 8) 
					...
					dayname_height+2, boxw-4, boxh-4, NULL);
				else {
