#pragma ident "@(#)MonthPanel.c	1.22 97/06/17 Sun Microsystems, Inc."

/*********************************************************************************
**  MonthPanel.c
**
**  $XConsortium: MonthPanel.c /main/cde1_maint/3 1995/10/10 13:22:19 pascale $
**
...
#ifdef SMALL_VIEW_BUG
static Boolean clipped(Widget);
#else
#define clipped(x)	False
#endif
...
Redisplay(Widget w, XEvent *ev, Region region)
{
	...
	_XmRedisplayGadgets( w, ev, region) ;
}
 ...
 */
	...
	if (tall_header)
		y = (Position) (mw->month_panel.display_rows - 6) * row_h;
	else
		y = (Position) (mw->month_panel.display_rows - 7) * row_h;
 ...
 */
	...
	if (tall_header)
		y = (Position) (mw->month_panel.display_rows - 5) * row_h;
	else
		y = (Position) (mw->month_panel.display_rows - 6) * row_h;
...
set_header_string(XmMonthPanelWidget mw)
{
	...
	(void) strftime(buf, BUFSIZ, 
                 catgets(c->DT_catd, 1, 1110, "%B %Y"), tm_ret);
