/* $XConsortium: convert2-4.c /main/cde1_maint/1 1995/07/17 19:52:03 drk $ */
/*
...
#pragma ident "@(#)convert2-4.c	1.6 96/11/12 Sun Microsystems, Inc."
...
static void set_showtime(time_t tick, int *showtime);
...
appt2_to_appt4(Appt_2 *a2)
{
	...
	while (a2 != NULL) {
		...
		set_showtime(a2->appt_id.tick, &a4->tag->showtime);
...
/* The version 2 protocol does not have a showtime flag
 * to indicate whether to show the time of the appointment.
 * They use the magic time (3:41 am) to indicate that
 * the time should not be shown.
 */
static void set_showtime(time_t tick, int *showtime)
{
	struct tm tm;
