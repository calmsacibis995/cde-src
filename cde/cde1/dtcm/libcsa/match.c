/* $XConsortium: match.c /main/cde1_maint/3 1995/10/10 13:30:10 pascale $ */
/*
...
#pragma ident "@(#)match.c	1.18 96/11/12 Sun Microsystems, Inc."
 ...
 * match situation is detected, no_match will be set to _B_TRUE and the
	...
	CSA_enum	**hops)
{
	...
	*id = 0;
	...
	if (starti == -1)
		*no_start_time_range = _B_TRUE;
	else
		*no_start_time_range = _B_FALSE;

	if (endi == -1)
		*no_end_time_range = _B_TRUE;
	else
		*no_end_time_range = _B_FALSE;
	...
	CSA_enum *ops)
{
	...
	for (i = 0; i < num_attrs; i++) {
		if (match_this_attribute(appt, attrs[i],
		    (ops ? ops[i] : CSA_MATCH_EQUAL_TO)) == _B_FALSE)
			return (_B_FALSE);
	}
...
_DtCm_match_reminders(Reminder_4 *rems, uint num_names, char **names)
{
	...
	while (rems != NULL) {
		...
		if (match_names(rems->attr.attr, num_names, names) == _B_TRUE) {
			rems->next = head;
			head = rems;
		} else {
			rems->next = freelist;
	...
	CSA_enum op)
{
	...
	switch (op) {
	case CSA_MATCH_ANY:
		return (_B_TRUE);	/* since val1 != NULL */

	case CSA_MATCH_EQUAL_TO:
		if (val2 == NULL) {
			return (_B_FALSE); /* since val1 != NULL */
		} else {
