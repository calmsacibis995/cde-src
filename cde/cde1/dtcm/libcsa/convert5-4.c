/* $XConsortium: convert5-4.c /main/cde1_maint/3 1995/10/10 13:29:02 pascale $ */
/*
...
#pragma ident "@(#)convert5-4.c	1.25 96/11/12 Sun Microsystems, Inc."
...
_DtCm_attrs_to_apptdata(uint size, CSA_attribute *attrs, Appt_4 *appt)
{
	...
	boolean_t	set_interval = _B_FALSE, set_weeknum = _B_FALSE;
	time_t		endtick = -1;
	...
	if (stat == CSA_SUCCESS) {
		...
		if (stat == CSA_SUCCESS) {
			...
			switch (appt->period.period) {
			case nthWeekday_4:
				if (set_weeknum == _B_TRUE)
					appt->period.nth = week_num;
				break;
			case everyNthDay_4:
			case everyNthWeek_4:
			case everyNthMonth_4:
				if (set_interval == _B_TRUE)
...
{
	Attribute_4	*	attr = appt->attr;
	Attribute_4	*	prev = appt->attr;
...
_DtCmAccessRightToV4AccessType(unsigned int access)
{
	...
	if (access & ~(CSA_X_DT_BROWSE_ACCESS | CSA_X_DT_INSERT_ACCESS |
...
extern CSA_return_code
_DtCm_cmskey_to_uid4(int num, cms_key *cmskeys, Uid_4 **uids)
{
	int	i;
	Uid_4	*u, *head, *prev;
	CSA_return_code stat = CSA_SUCCESS;
	...
	i = 0;
	while (i < num) {
		if ((u = (Uid_4 *)calloc(1, sizeof(Uid_4))) == NULL) {
			stat = CSA_E_INSUFFICIENT_MEMORY;
			break;
		}

		u->appt_id.tick = cmskeys[i].time;
		u->appt_id.key = cmskeys[i].id;
	...
	if (stat == CSA_SUCCESS)
		*uids = head;
		...
		_DtCm_free_uid4(head);

	return (stat);
}

extern void
_DtCm_free_uid4(Uid_4 *uids)
{
	Uid_4	*ptr;
