/* $XConsortium: relasttick.c /main/cde1_maint/2 1995/09/06 08:30:07 lehors $ */
/*
...
#pragma ident "@(#)relasttick.c	1.13 97/06/16 Sun Microsystems, Inc."
	...
	const RepeatEvent	*re)
{
	...
	if (re->re_duration != RE_NOTSET) {
		...
		start_tm->tm_mday += (double)(re->re_duration - 1) * re->re_interval;
		start_tm->tm_isdst = -1;
	...
	return (last_time2 < start_time || last_time1 < last_time2 ? 
			last_time1 : last_time2);
}
	...
	RepeatEvent		*re)
{
	...
	free(res);
	localtime_r((const time_t *)&_start_time, start_tm);

	start_hour = start_tm->tm_hour;
	start_min = start_tm->tm_min;

	/* Go to the last day an event can happen on. */
	start_tm->tm_mday += (double)(re->re_duration - 1) * re->re_interval * 7;
	start_tm->tm_isdst = -1;
	...
	const RepeatEvent	*re)
{
	...
	localtime_r((const time_t *)&start_time, &start_tm);
	localtime_r((const time_t *)&start_time, cur_tm);
	start_tm.tm_isdst = -1;
	...
	if (md_days[0] > 28) {
		...
		start_tm.tm_mon += (double)(re->re_duration - 1) * re->re_interval;
		start_tm.tm_mday = 1;
		/* Have the year and month updated */
		cur_time = mktime(&start_tm);
		localtime_r((const time_t *)&cur_time, &start_tm);
	...
	if (brute_force) {
		cur_tm = start_tm;
	    	...
	    	while (num_intervals < re->re_duration) {
                        ...
                        localtime_r((const time_t *)&last_time2, &cur_tm);
	...
	RepeatEvent		*re)
{
	...
	/* Go to the last day an event can happen on. */
	start_tm->tm_year += (double)(re->re_duration - 1) * re->re_interval;
	start_tm->tm_isdst = -1;
	...
	if (RE_YEARLY(re)->yd_nitems) {
		...
		for (i = RE_YEARLY(re)->yd_nitems - 1; i >= 0; i--) {
			if (DayExists(start_day, RE_YEARLY(re)->yd_items[i]-1,
							start_tm->tm_year)) {
				start_tm->tm_mon = RE_YEARLY(re)->yd_items[i]-1;
				start_tm->tm_isdst = -1;
				buf_time = mktime(start_tm);
				free(start_tm);
				return (buf_time < _start_time ? 
					last_time1 : buf_time);
			}
		}
		free(start_tm);
