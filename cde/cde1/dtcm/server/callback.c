/* $XConsortium: callback.c /main/cde1_maint/3 1995/10/10 13:32:04 pascale $ */
/*
...
#pragma ident "@(#)callback.c	1.18 96/11/12 Sun Microsystems, Inc."
...
_DtCmsCheckRegistrationList(_DtCmsRegistrationInfo *rlist)
{
	...
	while (rlist != NULL) {
		...
		sourcehost = strchr(rlist->client, '@');
		sourcehost++;
		...
		if (cl == NULL || clnt_call(cl, 0, (xdrproc_t)xdr_void,
			(char *)NULL, (xdrproc_t)xdr_void, (char *)NULL,
			timeout_tv) != RPC_SUCCESS)
		{
			if (debug) {
