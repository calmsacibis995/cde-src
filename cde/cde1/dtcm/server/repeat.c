/* $XConsortium: repeat.c /main/cde1_maint/3 1995/10/10 13:35:18 pascale $ */
/*
...
#pragma ident "@(#)repeat.c	1.13 97/02/10 Sun Microsystems, Inc."
...
_DtCms_adjust_appt_startdate(Appt_4 *appt)
{
	...
	localtime_r(&(appt->appt_id.tick), tm);
...
_DtCms_closest_tick_v4(time_t target, time_t ftick, Period_4 period, int *ordinal)
{
	...
	if (period.period < monthly_4 || period.period == everyNthDay_4 ||
		...
		localtime_r(&ftick, &tm1);
		localtime_r(&target, &tm2);
	}
	switch(period.period) {
	case daily_4:
...
nthweekdayofmonth(time_t t, int *nth)
{
	...
	*nth = (12 + tm.tm_mday - tm.tm_wday)/7;
	...
	localtime_r(&firstday, &tmfirstday);
	...
	if (*nth < 4)
		...
		return((tm.tm_mon == tm2.tm_mon) ? _B_FALSE : _B_TRUE);
	}
}
...
adjust_dst(time_t start, time_t next) 
{
	...
	localtime_r(&start, &oldt);
	localtime_r(&next, &newt);
