/* $XConsortium: update.c /main/cde1_maint/3 1995/10/10 13:36:46 pascale $ */
/*
...
#pragma ident "@(#)update.c	1.22 96/11/19 Sun Microsystems, Inc."
	...
	cms_entry	**newentry)
{
	...
	if ((stat = _DtCmsUpdateAttributes(num_attrs, attrs, &newe->num_attrs,
	    &newe->attrs, &cal->entry_tbl, _B_FALSE, &cal->types))
	    != CSA_SUCCESS) {
	...
	if (scope == CSA_SCOPE_ONE) {
		...
		if (rulei == -1 ||
			...
			if (updatedold == NULL)
				...
				naptr = updatedold->attrs[\
					CSA_ENTRY_ATTR_NUMBER_RECURRENCES_I].\
					value;
				nnum = (naptr ? naptr->item.uint32_value : 1);
			}

			if (oaptr->item.uint32_value == 0)
				remain = 0;
			else
				remain = oaptr->item.uint32_value - nnum +
	...
	cms_attribute	*attrs)
{
	...
	time_t		newbod, instbod, fstbod, newfstbod, endtime, delta;
...
extern	void _DtCm_rule_parser();
	...
	if (starti >= 0 && key->time != olde->key.time) {
		...
		if (newbod == instbod) {
			...
			/* parse the rule */
		        newre = parse_rule(newaptr->item.string_value);
			if (newre == NULL)
