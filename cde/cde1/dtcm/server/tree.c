/* $XConsortium: tree.c /main/cde1_maint/2 1995/09/06 08:29:10 lehors $ */
/*
...
#pragma ident "@(#)tree.c	1.6 96/11/12 Sun Microsystems, Inc."
...
visit_subtree(Tree_node *node, Private *p, Tree_node *z, Direction dir)
{
	...
	if (link != z && visit_subtree(link, p, z, dir)) return(_B_TRUE);
	if (p->enumerate((caddr_t)node, node->data)) return(_B_TRUE);
...
{
	if (root == NULL || root->rlink == z) return (_B_FALSE);
...
check1(Tree_node *x, caddr_t max, Tree_node *z, Rec *rec, Private *private)
{
	...
	rec->bool = ((x->color == red) ? _B_TRUE : _B_FALSE);
}
