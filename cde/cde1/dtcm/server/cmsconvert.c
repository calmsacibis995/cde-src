/* $XConsortium: cmsconvert.c /main/cde1_maint/3 1995/10/10 13:32:31 pascale $ */
/*
...
#pragma ident "@(#)cmsconvert.c	1.18 97/05/21 Sun Microsystems, Inc."
...
static CSA_return_code _AttrsToAbbrAppt(CSA_uint32 size, cms_attribute *attrs,
					Abb_Appt_4 *appt);

static CSA_flags _ConvertV5Access(CSA_flags v5rights, boolean_t strictmode);

static CSA_flags _ConvertV4Access(CSA_flags v4rights);

static char * _ConvertReminderName(char *rname);

static CSA_return_code _Appt4ToCmsentryAttrs(char *calname, Appt_4 *a4,
				CSA_uint32 *num_attrs, cms_attribute **attrs_r);
/*****************************************************************************
 * extern functions
 *****************************************************************************/
...
_DtCmsCmsentriesToAppt4ForClient(cms_entry *entries, Appt_4 **appt)
{
	...
	head = prev = NULL;
	while (entries != NULL) {
		if ((newappt = _DtCm_make_appt4(_B_TRUE)) == NULL)
			break;

		if ((stat = _DtCmsAttrsToAppt4(entries->num_attrs + 1,
		    entries->attrs, newappt, _B_FALSE)) != CSA_SUCCESS) {
...
_DtCmsConvertV4AccessList(Access_Entry_4 *alist, char *owner)
{
	...
	while (alist != NULL) {
		...
		if (owner) {
			int len = strlen(owner);
		 	if (strncmp(owner, l->user, len) == 0 &&
				(l->user[len] == '\0' || l->user[len] == '@') )
				l->rights |= CSA_OWNER_RIGHTS;
 		}
	...
	cms_reminder_ref	**rems)
{
	...
	while (r4 != NULL) {
		...
		_DtCmsComposeReferenceId(ids->id, calname, buf);
	...
	boolean_t	check)
{
	...
	for (i = 0; i < size && stat == CSA_SUCCESS; i++) {
		...
		case CSA_ENTRY_ATTR_START_DATE_I:
			if (check == _B_TRUE && attrs[i].value == NULL)
				stat = CSA_E_INVALID_ATTRIBUTE_VALUE;
...
_AttrsToAbbrAppt(CSA_uint32 size, cms_attribute *attrs, Abb_Appt_4 *appt)
{
	CSA_return_code	stat = CSA_SUCCESS;
	char		*ptr;
	time_t		endtick;
...
_MakeAbbrAppt()
{
	...
	a->tag->showtime =  _B_TRUE;

	return(a);
}

static CSA_flags
_ConvertV5Access(CSA_flags v5rights, boolean_t strictmode)
{
	CSA_flags	access = 0;
...
static CSA_flags
_ConvertV4Access(CSA_flags v4rights)
{
	CSA_flags	access = 0;
	...
	if (v4rights & access_write_4)
		access |= (CSA_INSERT_PUBLIC_ENTRIES | CSA_INSERT_CONFIDENTIAL_ENTRIES 
			| CSA_INSERT_PRIVATE_ENTRIES);

        if ((v4rights & access_write_4) && (v4rights & access_delete_4))
		access |= (CSA_CHANGE_PUBLIC_ENTRIES | CSA_CHANGE_CONFIDENTIAL_ENTRIES 
	...
	cms_attribute	**attrs_r)
{
	...
	_DtCmsComposeReferenceId(a4->appt_id.key, calname, buf);
	opq.size = strlen(buf);
	opq.data = (unsigned char *)buf;

	if ((stat = _DtCm_copy_cms_attribute(&attrs[count++], &tmpattr, _B_TRUE))
	    != CSA_SUCCESS) {
		free(attrs);
		return (stat);
	}
	...
	if ((stat = _csa_tick_to_iso8601(a4->appt_id.tick, buf) ?
	    ...
	    (stat = _DtCm_copy_cms_attribute(&attrs[count++], &tmpattr, _B_TRUE))
	    != CSA_SUCCESS) {
		goto error;
	}
