/* $XConsortium: reclotick.c /main/cde1_maint/2 1995/09/06 08:27:41 lehors $ */
/*
...
#pragma ident "@(#)reclotick.c	1.18 97/06/16 Sun Microsystems, Inc."
	...
	RepeatEventState	*res)
{
	...
	if (num_intervals > re->re_duration) { 
		 ...
		 */
		closest_tick = (double)re->re_duration * re->re_interval * 60;
		/* Pop the stack */
	...
	RepeatEventState	*res)
{
	...
	localtime_r((const time_t *)&target_time, &target_tm);
	...
	RepeatEventState	*res)
{
	...
	localtime_r((const time_t *)&begin_time, &start_tm);
	...
	RepeatEventState	*res)
{
	...
	do {
		...
		if (SAME_MONTH(&target_tm, &base_tm)) {
			base_time = mktime(&base_tm);
			localtime_r((const time_t *)&base_time, &base_tm);
	...
	struct tm	*end_time)	/* Return */
{
	...
	/* By moving ahead one day we might have crossed a dst border.*/
	localtime_r(&begin_time, &next_day);
	dst_adj = DSTAdjustment(end_time, &next_day);
	if (dst_adj) {
		_begin_time -= dst_adj;
		localtime_r(&_begin_time, end_time);
	} else
		*end_time = next_day;
}
		...
		*base_tm = *start_tm;
		...
		last_time = cur_time;
				...
				if (cur_time >= target_time) {
					cur_time = last_time;
                        		localtime_r((const time_t *) &last_time, &cur_tm);
					break;
				}
