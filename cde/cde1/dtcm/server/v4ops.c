/* $XConsortium: v4ops.c /main/cde1_maint/3 1995/10/10 13:36:56 pascale $ */
/*
...
#pragma ident "@(#)v4ops.c	1.23 96/11/20 Sun Microsystems, Inc."
...
static CSA_return_code _check_access_list(Access_Entry_4 *alist, char *owner);
...
_DtCmsInsertAppt(_DtCmsCalendar *cal, Appt_4 *appt4)
{
	...
	if (debug) {
	  ...
	  ctime_r(&(appt4->appt_id.tick), cbuf);
	  fprintf(stderr, "Insert appt4: (%d)%s\n",
			appt4->appt_id.key, cbuf);
	}
	...
	Appt_4		**appt_r)
{
	...
	if ((p_node = rb_delete (APPT_TREE(cal), (caddr_t)p_key)) != NULL) {
		...
		_DtCmsObsoleteReminderV4(&cal->rm_queue, p_appt, 0, _B_FALSE);
		free (p_node);
		if (debug)
		{
		  char cbuf[26];

		  ctime_r(&(p_appt->appt_id.tick), cbuf);
		  fprintf (stderr, "Deleted (%d)%s\n",
			   APPT_KEY(p_appt), cbuf);
		}
		if (appt_r != NULL)
			*appt_r = p_appt;
		else
	...
	Appt_4		**oldappt)
{
	...
	if ((ordinal = _DtCms_in_repeater (key, p_appt, _B_TRUE)) == 0)
		return (CSA_X_DT_E_ENTRY_NOT_FOUND);
	...
	if (ordinal == 0) {
		...
		_DtCmsObsoleteReminderV4(&cal->rm_queue, p_appt, ordinal,
			(option == do_one_4 ? _B_FALSE : _B_TRUE));
		_DtCm_free_appt4(p_appt);
	...
	Appt_4		**oldappt)
{
	...
	if (is_repeater(olda) && is_repeater(newa)) {
		    ...
		    olda->period.period == newa->period.period)
		{
			...
			if ((ordinal = _DtCms_in_repeater(p_key, olda, _B_TRUE))
			    == 0) {
	...
	Abb_Appt_4	**abbr_r)
{
	...
	while (p_range != NULL)
	{
		...
		if (debug)
		{
		  ...
		  ctime_r(&lo_key.tick, cbuf);
		  fprintf(stderr,"Range lookup from %s", cbuf);
		  ctime_r(&hi_tick, cbuf);
		  fprintf(stderr, " to %s\n", cbuf);
		}
...
_DtCmsSetV4AccessListAndLog(_DtCmsCalendar *cal, Access_Entry_4 *alist)
{
	...
	/* check validity of the list first */
	if ((stat = _check_access_list(alist, cal->owner)) != CSA_SUCCESS)
		return (stat);

	/* update access list for old format calendar */
	if ((name = _DtCmGetPrefix(cal->calendar, '@')) == NULL)
		return (CSA_E_INSUFFICIENT_MEMORY);
	...
	if ((stat = _DtCmsCopyAccessList4(alist, &newlist)) != CSA_SUCCESS)
		goto done;

	if ((stat = _DtCmsAppendAccessByFN(log, newlist)) != CSA_SUCCESS) {
		/* rollback log file */
		_DtCmsTruncateFile(cal->calendar, file_size);
		goto done;
	} else {
...
extern CSA_return_code
_DtCmsCopyAccessList4(Access_Entry_4 *alist, Access_Entry_4 **newlist)
{
	Access_Entry_4	*head, *prev, *l;
	...
	while (alist != NULL) {
		...
		l->who = strdup(alist->who);
		l->access_type = alist->access_type;
...
_check_access_list(Access_Entry_4 *alist, char *owner)
{
	...
	for (; alist != NULL && stat == CSA_SUCCESS; alist = alist->next) {
		...
		/* make sure calendar owner are granted all access */

		username = _DtCmGetPrefix(alist->who, '@');
		if (checkroot == _B_TRUE) {
			if (strcmp(username, _DTCMS_SUPER_USER) == 0) {
				/* root on different machine are treated as
				 * different users
				 */
				if ((ptr = strchr(owner, '@')) &&
				    (ptr2 = strchr(alist->who, '@'))) {
					ptr++;
					if (ptr3 = strchr(ptr, '.'))
						...
						same = _DtCmIsSamePath(++ptr2,
								       ptr);
					if (same &&
			 		    !((alist->access_type & access_read_4) &&
					     (alist->access_type & access_write_4) &&
					     (alist->access_type & access_delete_4)))
					{
		...
		for (head = alist->next; head != NULL && stat == CSA_SUCCESS;
		     head = head->next) {
			if (strcmp(alist->who, head->who) == 0) {
				stat = CSA_E_INVALID_PARAMETER;
			}
		}
	} 
