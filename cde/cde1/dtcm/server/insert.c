/* $XConsortium: insert.c /main/cde1_maint/2 1995/10/10 13:33:50 pascale $ */
/*
...
#pragma ident "@(#)insert.c	1.17 97/06/11 Sun Microsystems, Inc."
...
_DtCmsInsertEntry(_DtCmsCalendar *cal, cms_entry *entry)
{
	...
	if (key == 0) {
		/* set start date */
		_csa_iso8601_to_tick(entry->attrs[CSA_ENTRY_ATTR_START_DATE_I].\
				     value->item.date_time_value,
				     &entry->key.time);

		/* set reference id */
		_DtCmsComposeReferenceId(entry->key.id, cal->calendar, buf);
		opq.size = strlen(buf);
