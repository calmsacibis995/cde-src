...
**  cm_tty.c
**
**  $XConsortium: cm_tty.c /main/cde1_maint/6 1995/11/14 17:01:17 drk $
**
...
#pragma ident "@(#)cm_tty.c	1.99 97/01/08 Sun Microsystems, Inc."
...
cm_tty_insert(nl_catd catd, CSA_session_handle target, int version, 
	...
	if (filename) {
		...
		/* tag appointment if charset info is passed in */
		if (charset && appt->charset) {
			appt->charset->value->item.string_value = charset;
...
convert_boolean_str(char *val) {
	if (strncasecmp(val, "T", 1) == 0
	 || strcasecmp(val, "_B_TRUE") == 0
	 || strcasecmp(val, "B_TRUE") == 0)
		return _B_TRUE;
	return _B_FALSE;
}
	...
	while (fgets(line, MAXNAMELEN - 1, fp)) {
		...
		/* see if the allocated buffer is large enough to 
		   ...
		   larger, and then copy the attribute. */
			...
			if (ident_name_ro(a_name, DATAVER_ARCHIVE) == _B_FALSE) {
...
generate_recurrence_rule(Dtcm_appointment *appt, int version) {
	...
	if (!appt->recurrence_rule || !appt->recurrence_rule->value) {
		...
		case CSA_X_DT_REPEAT_EVERY_NDAY : 
			sprintf(rule_buf1, "D%d ", repeat_nth);
			break;
		
		case CSA_X_DT_REPEAT_EVERY_NWEEK : 
			sprintf(rule_buf1, "W%d ", repeat_nth);
			break;
		
		case CSA_X_DT_REPEAT_EVERY_NMONTH : 
			sprintf(rule_buf1, "MD%d ", repeat_nth);
			break;
		}
        	...
        	if (repeat_for == 0)
                	...
                	sprintf(rule_buf2, "#%d", repeat_for);
...
parse_appt_from_file(nl_catd catd, char *file, CmDataList *list, Props *p,
	...
	FILE			*fp;
	boolean_t		processing_appt = _B_FALSE,
				processing_what = _B_FALSE;
	int			what_lines = 0;
	Validate_op		valid_op = VALID_APPT;
	Parse_key_op		pko = NOT_A_KEY;
	Dtcm_appointment	*appt;
	boolean_t		found_appt = _B_FALSE;
	...
	while (valid_op == VALID_APPT && fgets(line, MAXNAMELEN - 1, fp)) {
		...
		case APPOINTMENT_START:
			if (processing_appt) {
				...
				if (valid_op == VALID_APPT) {
					...
					processing_appt = _B_FALSE;
			} else
				processing_appt = _B_TRUE;
		...
		case WHAT_KEY:
			...
			if (!processing_what) {
				...
				tmp = w_buf;
				w_buf = (char *)XtCalloc(1, cm_strlen(tmp)
					+ cm_strlen(key_str) + 2);
				cm_strcpy(w_buf, tmp);
				cm_strcat(w_buf, "\n");
...
attrs_to_string(CSA_attribute * attrs, int num_attrs)
{
	...
	buffer[0] = NULL;
		...
		case CSA_VALUE_CALENDAR_USER: 
				...
				growcat(&buffer, tmp_buf);
				sprintf(tmp_buf, "caluser:%d:", attrs[i].value->item.calendar_user_value->user_type);
				growcat(&buffer, tmp_buf);
...
parse_attrs_to_string(Dtcm_appointment *appt, Props *p, char *attr_string) {
	...
	if ((nlcount = count_newlines(appt_what)) > 0) {
		whatstr = (char *)XtCalloc(1, 
					cm_strlen(appt_what) + nlcount + 1);
...
str_to_period(char *ps, CSA_sint32 *repeat_type, int *repeat_nth) {
	boolean_t	compute_times = _B_FALSE;
	...
	*repeat_nth = 0;
		...
		*repeat_type = CSA_X_DT_REPEAT_YEARLY;
        else if (strcasecmp(ps, periodstrings[6]) == 0)
...
periodstr_from_period(CSA_sint32 repeat_type, int repeat_nth) {
	...
	switch (repeat_type) {
		...
		break;
	default:
		sprintf(pstr, "Unknown repeat type (%u)", repeat_type);
		break;
	}
...
valid_time(Props *p, char *time_str) {
	...
	boolean_t	after_colon = _B_FALSE;
 ...
 * _B_TRUE if the appointment should be scheduled to the next day, _B_FALSE if
...
validate_dssw(Dtcm_appointment *a, char *s_buf, char *e_buf, char *d_buf,
	...
	a->show_time->value->item.sint32_value = _B_TRUE;
	...
	if (!blank_buf(s_buf)) {
		...
		a->show_time->value->item.sint32_value = _B_FALSE;
	}

	/*
	 * Finally, if the ending tick is before the starting tick, execute the
	 * passed function which should return _B_TRUE if we should schedule
	 * this into the next day and _B_FALSE if not.
	 *
