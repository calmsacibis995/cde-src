#pragma ident "@(#)util.c	1.79 96/11/12 Sun Microsystems, Inc."

/*******************************************************************************
**
**  util.c
**
**  $XConsortium: util.c /main/cde1_maint/6 1995/10/31 10:14:44 lehors $
**
...
str_to_cr(char *s)
{
        ...
        newstr= (char *) XtCalloc(1, (unsigned)i + 1);
        k = 0;
        for (j=0; j<i; j++) {
                if (s[j]=='\\') {
                        if (s[j+1]=='n') {
...
cr_to_str(char *s)
{
	...
	newstr = (char *) XtCalloc(1, (unsigned)((2 * i) + 1));
	k = 0;
	for (j=0; j<i; j++) {
		if (s[j]=='\n') {
			newstr[k] = '\\';
...
text_to_lines(char *s, int n)
{
	...
	do {
		if (line == NULL) break;
		l = (Lines*)XtCalloc(1, sizeof(Lines));
		if (head == NULL) head = l;
...
cm_get_credentials()
{
	...
	if (login==NULL)
	{
		...
		login = (char *) XtCalloc (1, 
					cm_strlen(name) + cm_strlen(host) + 2);
...
cm_get_local_host()
{
	static char *local_host;
        ...
        if (local_host == NULL) {
#if defined(SunOS) || defined(USL) || defined(__uxp__)
                local_host = (char *)XtMalloc(MAXHOSTNAMELEN);
...
/*
 * Check calendar's character set attribute against that of the process and
 * determine whether the entry character set attribute needs to be set when
 * inserting an entry in the calendar.
 * Note: Only calendar of data version >= 4 supports the charater set entry
 *	 attribute.
 */
extern void
mark_set_entry_charset(
	char	*app_locale,	/* locale of the process */
	char	*cal_charset,	/* character set of the calendar */
	int	version,	/* data version of the calendar */
	Boolean	*flag)		/* return flag */
{
	*flag = False;

	if (version >= DATAVER4 && app_locale) {

		if (cal_charset == NULL ||
		    (cal_charset && strcmp(cal_charset, app_locale))) {

			/* need to set the entry character set
			 * attribute when inserting an entry into the
			 * calendar if
			 * - the locale of the process is different
			 *   from the calendar's character set attribute
			 * - can't get the calendar's charset (may
			 *   due to lack of access or the calendar's
			 *   charset is not set)
			 */
			*flag = True;
		}
...
parse_date(OrderingType order, SeparatorType sep, char *datestr, char *m,
        ...
        second = strtok(NULL, str);
        third = strtok(NULL, str);
...
static const int DEF_V5_APPT_ATTR_COUNT = 23;
...
static const int default_appt_attrs[] = {CSA_ENTRY_ATTR_REFERENCE_IDENTIFIER_I,
			...
			CSA_X_DT_ENTRY_ATTR_CHARACTER_SET_I
			};
 ...
 * flag to _B_TRUE), you're setting attributes and since you can't set read-only
...
Dtcm_appointment *allocate_appt_struct (Allocation_reason reason, int version, ...) {
	...
	appt = (Dtcm_appointment *)XtCalloc(1, idx);
	appt->reason = reason;
	...
	appt->names = (CSA_attribute_reference *)XtCalloc(1, idx);

	appt->count = api_ids->count;
	if (reason == appt_write) {
		idx = sizeof(CSA_attribute) * appt->count;
		appt->attrs = (CSA_attribute *)XtCalloc(1, idx);
	}
	...
	for (idx = 0; idx < appt->count; idx++) {
		...
		appt->names[idx] = strdup(CSA_ENTRY_ATTRIBUTE_NAMES[api_idx]);
		if (reason == appt_write)
...
allocate_cal_struct(Allocation_reason reason, int version, ...) {
	...
	cal = (Dtcm_calendar *)XtCalloc(1, idx);
	cal->reason = reason;
	cal->version = version;
	...
	cal->names = (CSA_attribute_reference *)XtCalloc(1, idx);

	cal->count = api_ids->count;
	if (reason == appt_write) {
		idx = sizeof(CSA_attribute) * cal->count;
		cal->attrs = (CSA_attribute *)XtCalloc(1, idx);
	}
	...
	for (idx = 0; idx < cal->count; idx++) {
		api_idx = (int)CmDataListGetData(api_ids, idx + 1);
		cal->names[idx] = strdup(CSA_CALENDAR_ATTRIBUTE_NAMES[api_idx]);
		if (reason == appt_write)
...
entry_ident_index_tag(int id) {
	...
	case CSA_X_DT_ENTRY_ATTR_CHARACTER_SET_I:
		r_tag = CSA_VALUE_STRING;
		break;
...
initialize_cal_attr(int id, CSA_attribute *attrs, Allocation_reason reason, int version) {
	...
	attrs->name = cm_strdup(CSA_CALENDAR_ATTRIBUTE_NAMES[id]);
	if ((reason == appt_write) && !cal_ident_index_ro(id, version)) {
		size = sizeof(CSA_attribute_value);
		attrs->value = (CSA_attribute_value *)XtCalloc(1, size);
...
initialize_entry_attr(int id, CSA_attribute *attrs, Allocation_reason reason, int version) {
	...
	attrs->name = cm_strdup(CSA_ENTRY_ATTRIBUTE_NAMES[id]);
...
set_appt_links(Dtcm_appointment *appt) {
	...
	for (idx = 0; idx < appt->count; idx++) {
		...
		else if (strcmp(idx_name, CSA_X_DT_ENTRY_ATTR_CHARACTER_SET) == 0)
			appt->charset = &appt->attrs[idx];
	}
}
...
setup_range(CSA_attribute **attrs, CSA_enum **ops, int *count, time_t start,
	...
	attr_ptr = (CSA_attribute *)XtCalloc(1, a_size);

	o_size = sizeof(CSA_enum) * (*count);
	op_ptr = (CSA_enum *)XtCalloc(1, o_size);
