/* $XConsortium: helpgen.c /main/cde1_maint/5 1995/11/14 15:09:57 lehors $ */
#include <dirent.h>
#include <errno.h>
#include <locale.h>
#include <nl_types.h>
...
#include <DtI/Canvas.h>
#include <DtI/Access.h>
#include <DtI/HelpP.h>
#include <DtI/bufioI.h>
#include <DtI/AccessP.h>
#include <DtI/AccessI.h>
#include <DtI/AccessCCDFI.h>
#include <DtI/CEUtilI.h>
...
int
#ifdef _NO_PROTO
CreateVolumeLink (canvas, outTopic, volume_name)
    CanvasHandle	 canvas;
    FILE		*outTopic;
    char		*volume_name;
#else
...
#endif /* _NO_PROTO */
{
    ...
    char	*filename   = NULL;
    char        *pathName   = NULL;
    _DtHelpVolume   volume = NULL;

    pathName = FindFile (volume_name);
    if (pathName != NULL &&
	     _DtHelpCeOpenVolume(canvas, pathName, (VolumeHandle)&volume) == 0)
      {
...
int
#ifdef _NO_PROTO
CreateFamily (canvas, family_name, out_volume, out_header, out_topic)
    CanvasHandle canvas;
    char    *family_name;
    FILE    *out_volume;
    FILE    *out_header;
	...
	if (title && abstract && list && charSet)
	  {
		 ...
		 */
		fprintf (out_volume, "*.%s.filepos: %d\n",
						familyName, filepos);
...
void
#ifdef _NO_PROTO
ScanDirectory (directory, ret_time)
    char    *directory;
    long    *ret_time;
#else
ScanDirectory (
...
void
#ifdef _NO_PROTO
ExpandPaths (lang, type, env_var, default_str, list)
    char   *lang;
    char   *type;
    char   *env_var;
    char   *default_str;
