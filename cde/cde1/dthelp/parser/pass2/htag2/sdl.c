/* $XConsortium: sdl.c /main/cde1_maint/3 1995/10/09 00:11:32 pascale $ */
...
if (ptr != vptr)
    {
    fprintf(stdout, "realloc 0x%x 0x%x\n", ptr, vptr);
    fflush(stdout);
    }
#endif
...
if (stat(name, &buf) == -1)
 ...
 /* The callers should change to expect off_t rather than int */
return buf.st_size;
}
...
while (1)
    ...
    lseek(inFd, (off_t) endOfLOIDS, SEEK_SET);
...
if (!minimalTossFlag) /* use the full <toss> from the document */
	...
	lseek(inFd, (off_t) delta, SEEK_SET);
...
if (endOfIndex && haveIndex)
    lseek(inFd, (off_t) endOfIndex, SEEK_SET);
