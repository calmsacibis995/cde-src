...
$REVISION$:  $XConsortium: PrintTopics.c /main/cde1_maint/5 1995/11/13 10:48:52 lehors $
$COPYRIGHT$:
...
#include "Canvas.h"	/* from libDtHelp */
#include "FormatTermI.h" /* from libDtHelp */
...
#define wcswidth(s,n) scrwidth(s)
#define wcslen(s) wslen(s)
#endif
...
#if _NO_PROTO
static
TocEntry * TocNextEntry(toc,entry)
  Toc *      toc;
  TocEntry * entry;
#else   /*$DEF$*/
static
...
#if _NO_PROTO
static
int TocFindSortedEntry(toc, locationId, ret_parent, ret_entry)
  Toc *        toc;
  char *       locationId;
  TocEntry * * ret_parent;
  TocEntry * * ret_entry;
...
#if _NO_PROTO
static
int TocNewEntry(toc, locationId, pageNumber, level, sectNums, sectStr)
  Toc *      toc;
  char *     locationId;
  int        pageNumber;
  int        level;
...
#if _NO_PROTO
static
int IconvFile(options,helpVolumeHandle,srcFile)
			...
			void **to )
{
    ...
    *to = NULL;
     ...
     */
    iconv( CD, (const char **)NULL, 0, NULL, 0 );
    while( 1 ) {
	/*
	 * InBuf
	 *  v
	...
	iconv_ret = iconv( CD, (const char **)&InBuf, &InBytesLeft,
                               &OutBuf, &OutBytesLeft );
	if ( iconv_ret == 0 ) {
	    /* iconv done
	     *                             InBuf
...
#if _NO_PROTO
static
int IconvBuffer( helpVolumeHandle, src, dest )
      VolumeHandle helpVolumeHandle;
      char *src;
      char **dest;
#else   /*$DEF$*/
...
#if _NO_PROTO
static
FILE * OpenTmpFile(options,ret_tmpFile)
      _DtHPrOptions * options;
      char * * ret_tmpFile;
#else   /*$DEF$*/
static
...
#if _NO_PROTO
int AvailContentLines(options,state,hff);
  _DtHPrOptions * options;
  PrintState *    state;
  HeadFootFormat * hff;
#else /*$DEF$*/
...
#if _NO_PROTO
static
char * SectNumStr(sectNums, sectStr)
  int *    sectNums;
  char *   sectStr;
  char *   buf;
#else   /*$DEF$*/
...
#if _NO_PROTO
static
void OutputBlankSpaces(topicsFP, spaceCount)
  FILE *  topicsFP;
  int     spaceCount;
#else   /*$DEF$*/
static
...
#if _NO_PROTO
static
void OutputBlankLines(topicsFP, state, lineCount)
  FILE *  topicsFP;
  PrintState * state;
  int     lineCount;
#else   /*$DEF$*/
...
#if _NO_PROTO
static
int DoStrColsWidth(str,maxWidth,truncateStr)
      char *   str;
      int      maxWidth;
      Boolean  truncateStr;
#else   /*$DEF$*/
...
#if _NO_PROTO
static
void GenHeadFootFormatArgs(options,state,updateTopicTitle,sectNumStr,updatePageNum)
  _DtHPrOptions * options;
  PrintState *    state;
...
#if _NO_PROTO
static
void GenHeadFootFormatStr(specStr,io_formattedStr,io_lineCnt)
  char *          specStr;
...
#if _NO_PROTO
static
void GenAllHeadFootFormatStrs(options,state)
  _DtHPrOptions * options;
  PrintState *    state;
#else   /*$DEF$*/
static
...
#if _NO_PROTO
static
int PrintHeadFootStr(options,topicsFP,state,formattedStr,lineCnt)
  _DtHPrOptions * options;
  FILE *          topicsFP;
  PrintState *    state;
  char *          formattedStr,
...
#if _NO_PROTO
static
void PrintFooter( options, topicsFP, state, headFootInfo, headFootFormatting)
  _DtHPrOptions * options;
  FILE *          topicsFP;
  PrintState *    state;
...
#if _NO_PROTO
static
void PrintHeader( options, topicsFP, state, headFootInfo, headFootFormatting)
  _DtHPrOptions * options;
  FILE *          topicsFP;
  PrintState *    state;
  _DtHPrHeadFoot * headFootInfo;
...
#if _NO_PROTO
static
void NewPage(options,topicsFP,state,advancePage)
  _DtHPrOptions * options;
  FILE *          topicsFP;
  PrintState *    state;
  Boolean         advancePage;
...
#if _NO_PROTO
static
int ProcessOneTopic(options, topicsFP, state, printHeaderFooter)
  _DtHPrOptions * options;
  FILE *          topicsFP;
  PrintState *    state;
...
#else /* AIX_ILS */
         if (topicsFP) fprintf(topicsFP,"%s\n", *helpList); /* output title */
#endif /* AIX_ILS */
...
#if _NO_PROTO
static
int ProcessSubTopics(options,topicsFP,toc, level,state)
  _DtHPrOptions * options;
  FILE *          topicsFP;
  Toc *           toc;
  int             level;
...
#if _NO_PROTO
static
int ProcessFrontMatter(options, ret_resultsFile, state)
  _DtHPrOptions * options;
...
#if _NO_PROTO
static
int ProcessIndex(options, ret_resultsFile, state, toc)
  _DtHPrOptions * options;
  char * *        ret_resultsFile;
   PrintState *   state;
   Toc *          toc;
...
#if _NO_PROTO
static
int ProcessToc(options, ret_resultsFile, state, toc)
  _DtHPrOptions * options;
  char * *        ret_resultsFile;
   PrintState *   state;
   Toc *          toc;
...
#if _NO_PROTO
static
int ProcessTopics(options, ret_resultsFile, state, toc,
...
#if _NO_PROTO
static
void MakePageBreakFile(options,ret_resultsFile)
  _DtHPrOptions * options;
  char * *        ret_resultsFile;
#else   /*$DEF$*/
static
...
#if _NO_PROTO
static
int DoHelpTopicsProcessing(options, ret_resultsFile, 
                     processToc, processIndex, processFrontMatter,
                     processSubTopics, outputFromLocId, inhibitTopicsOutput)
  _DtHPrOptions * options;
  char * *        ret_resultsFile;
  Boolean         processToc,
...
#ifndef AIX_ILS
   IconvFile(options,state.volHandle,ret_resultsFile);
#endif /* AIX_ILS */
...
#if _NO_PROTO
static
int GenerateTopicsFile(dpy, options, ret_resultsFile)
  Display *       dpy;
  _DtHPrOptions * options;
  char * *        ret_resultsFile;
#else   /*$DEF$*/
...
#if _NO_PROTO
int _DtHPrPrintHelpTopic(dpy, options)
   Display *      dpy;
  _DtHPrOptions * options;
#else   /*$DEF$*/
...
#if _NO_PROTO
int _DtHPrPrintHelpTopicWithXvp(dpy, volHandle, locationId)
   Display *    dpy;
