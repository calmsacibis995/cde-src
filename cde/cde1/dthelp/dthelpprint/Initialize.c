...
$REVISION$:  $XConsortium: Initialize.c /main/cde1_maint/3 1995/10/08 23:57:57 pascale $
$COPYRIGHT$:
...
#include <X.h>         /* FIX: remove? */
#include <X11/Xlib.h>  /* FIX: remove? */
...
#if _NO_PROTO
static
void CalculatePageSize(appDB, options, appname, appclass)
   XrmDatabase     appDB;
...
#if _NO_PROTO
void _DtHPrBuildResourceDb(argc, argv, appDB, pDpy)
   int *         argc;
   char * *      argv;
   XrmDatabase * appDB;
   Display * *   pDpy;
...
/* what to print */
   ...
   /**	Get location of dthelprint.sh and put values in appDB as resource **/
   
   if ((tmp_string = strcat(argv[0], ".sh")) != NULL) 
   {
      i = strlen(tmp_string);
      ...
      strcpy(resource_name, name_prefix);
      strcat(resource_name, RN_shCommand);   /* i.e. dthelpprint.sh*/
      strcpy(resource_class, class_prefix);
      strcat(resource_class, RC_shCommand);  
...
#if _NO_PROTO
void _DtHPrGetResources(db, optionResources, options)
   XrmDatabase     db;
   _DtHPrOptions * options;
#else   /*$DEF$*/
...
#if _NO_PROTO
char * _DtHPrCreateTmpFile(prefix,suffix)
        char * prefix;
        char * sufffix;
#else   /*$DEF$*/
