        ...
        int fd)
{
        ...
        if (wm_state->state == IconicState)
          {
            ...
            XGetWindowAttributes(XtDisplay(pPad->app_shell),
                                 wm_state->icon, &wa);

            sprintf(bufr, "%s*mainWindow%d.iconX: %d\n", bufr, padNum, wa.x);
            sprintf(bufr, "%s*mainWindow%d.iconY: %d\n", bufr, padNum, wa.y);
          }
        else
          sprintf (bufr, "%sFalse\n", bufr);
    ...
    {
	sprintf(bufr, "*mainWindow%d.mainTitle: %s\n", padNum, 
		pPad->xrdb.mainTitle);
        write (fd, bufr, strlen(bufr));
    }
    ...
    {
	sprintf(bufr, "*mainWindow%d.idString: %s\n", padNum,
		pPad->xrdb.idString);
        write (fd, bufr, strlen(bufr));
    }
}
        ...
        caddr_t call_data )		/* data from widget class */
{
    ...
    {
	/*
	 * We don't save pads w/ confirmation strings because we assume that
	 * another program expects to look at our return status, and
	 * the process relationship can't be maintained.
	 */
	if(pPad->inUse == True && 
        ...
        Editor *pPad)
{
	 ...
	 */
        ...
        /* get window title, if one is specified */
        xrm_name[1] = XrmStringToQuark ("mainTitle");
        XrmQGetResource (db, xrm_name, xrm_name, &rep_type, &value);
        pPad->xrdb.mainTitle = strdup((char *) value.addr);

        /* get idString, if one is specified */
        xrm_name[1] = XrmStringToQuark ("idString");
        XrmQGetResource (db, xrm_name, xrm_name, &rep_type, &value);
        pPad->xrdb.idString = strdup((char *) value.addr);
   ...
   if ((iconify = (char *) value.addr) != NULL &&
       strcmp (iconify, "True") == 0)
     {
       ...
       if (XrmQGetResource (db, xrm_name, xrm_name, &rep_type, &value))
         {
           pPad->iconicX = True;
           pPad->iconX = atoi((char *)value.addr);
         }
       ...
       if (XrmQGetResource (db, xrm_name, xrm_name, &rep_type, &value))
         {
           pPad->iconicY = True;
           pPad->iconY = atoi((char *)value.addr);
         }
     }
   else
      pPad->iconic = False;
