        ...
        caddr_t call_data )
{
	...
	pStuff->openTitle = XmStringCreateLtoR(buf, XmFONTLIST_DEFAULT_TAG);
    }
    ...
    GetFileName(pPad, pStuff->openTitle);
}
        ...
        caddr_t call_data )
{
	...
	pStuff->includeTitle = XmStringCreateLtoR(buf,XmFONTLIST_DEFAULT_TAG);
    }
    pPad->fileStuff.pendingFileFunc = IncludeFile;
    pPad->fileStuff.pendingFileHelpFunc = HelpIncludeDialogCB;
    GetFileName(pPad, pStuff->includeTitle);
}
        ...
        caddr_t call_data )
{
	...
	if (!pPad->fileStuff.pendingFileFunc && pPad->xrdb.wordWrap) {
            AskIfSave(pPad);  
	...
	caddr_t call_data)
{
    ...
    actionID = DtActionInvoke(pPad->app_shell,
		...
		(DtActionCallbackProc) PrintActionCB, /* action callback */
        ...
        caddr_t call_data)
{
    ...
    if (pPad->xrdb.confirmationString != (char *)NULL) {
	/*
	 * This is a touch sleazy. ConfirmYesCB and ConfirmNoCB null out the
	 * xrdb.confirmationString pointer so we skip this the second
	 * time through.
	 */
	AskForConfirmation(pPad);
	return;
    }
        ...
        caddr_t call_data )
{
    ...
    ExtractAndStoreDir(pPad, name);	/* store pPad->fileStuff.pathDir */
...
{
    Editor *pPad = (Editor *) client_data;
    XtUnmanageChild(pPad->confirmStuff.dialog);
    pPad->xrdb.confirmationString = (char *)NULL;
    pPad->confirmStuff.confirmationStatus = 0;
        ...
        caddr_t call_data )
{
    ...
    XtUnmanageChild(pPad->confirmStuff.dialog);
    pPad->xrdb.confirmationString = (char *)NULL;
    pPad->confirmStuff.confirmationStatus = 1;
    FileExitCB(w, client_data, call_data);
}
