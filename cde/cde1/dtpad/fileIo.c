        ...
        char *include)
{
	...
	if (pPad->fileStuff.fileName && *pPad->fileStuff.fileName) {
	    char *lastSlash;
	    if (pPad->xrdb.directory != (char *)NULL)  {
		int len;
		strcpy(pPad->fileStuff.pathDir, pPad->xrdb.directory);
		len = strlen(pPad->fileStuff.pathDir);

		if (pPad->fileStuff.pathDir[len - 1] != (char)'/') {
		    pPad->fileStuff.pathDir[len] = (char)'/';
		    pPad->fileStuff.pathDir[len + 1] = (char)'\0';
		}
		XtFree(pPad->fileStuff.fileName);
...
GetTempFile(void)
{
    ...
    if ((tfp = fopen(tempname, "w")) == NULL)
    {
        ...
        pid = (int) getpid();
        sprintf(tempname, "/usr/tmp/editor%d", pid);
        if ((tfp = fopen(tempname, "w")) == NULL)
        {
            sprintf(tempname, "/tmp/editor%d", pid);
            if ((tfp = fopen(tempname, "w")) == NULL)
            {
                XtFree(tempname);
                return (char *)NULL;
...
PanicSave(signal)
{
    ...
    /* Reset the default signal handler for the signal caught and resend signal
       so that the process either dumps core or exits depending on the signal
       (bug fix 1207757)
    */
    sigfillset(&act.sa_mask);
    act.sa_flags = 0;
    act.sa_handler = SIG_DFL ; /* set the default signal handler */
    sigaction(signal,&act,(struct sigaction *)NULL);
    sigsend(P_PID,getpid(),signal); /* resend signal and it may dump core this time */
    /* exit(1); */
}
