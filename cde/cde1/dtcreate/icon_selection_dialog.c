/* $XConsortium: icon_selection_dialog.c /main/cde1_maint/4 1995/10/08 00:11:43 pascale $ */
/*******************************************************************************
...
#ifdef _NO_PROTO
int myscandir();
int get_top_b1_position ();
int get_top_b2_position ();
void load_directories_lis ();
char *get_initial_filter ();
void load_filter_text_field ();
char *update_filter ();
void apply_filter ();
char *initialize_filter ();
struct dirent **build_dirent_list ();
void update_container_contents ();
void free_container_contents ();
int new_name_select ();
/*int name_select ();*/
static void selectionCB_icon_scrolled_container ();
static void calc_bottom_attachment ();
...
/***************************************************************************/
#ifdef _NO_PROTO
int myscandir(pszDirName, NameList, Select, Compare)
      char *pszDirName;
...
#define          AINCREMENT  ((int)512)
  ...
  if (!(dirptr = opendir(pszDirName))) {
     sprintf(buffer, "%s\n   %s", "Could not open the following directory:", pszDirName);
...
/***************************************************************************/
#ifdef _NO_PROTO
struct dirent **build_dirent_list (filter, filecount)
     char *filter;
     int *filecount;
#else /* _NO_PROTO */
struct dirent **build_dirent_list (char *filter, int *filecount)
  ...
  *filecount = 0;
  ...
  if (dirptr) {
     ...
     *filecount = myscandir (dirname, &namelist, new_name_select, NULL);
     ...
     sprintf(buffer, "%s\n   %s", "Could not open the following directory:", dirname);
     display_error_message(CreateActionAppShell, buffer);
     namelist = NULL;
  }

#ifdef DEBUG
  printf ("Freeing dirname, sizeof: %i\n", sizeof(dirname));
#endif
...
/***************************************************************************/
#ifdef _NO_PROTO
int new_name_select (dir_entry, pszdir_name)
...
/***************************************************************************/
#ifdef _NO_PROTO
int get_top_b1_position (buttoncount)
     int buttoncount;
#else /* _NO_PROTO */
int get_top_b1_position (int buttoncount)
#endif /* _NO_PROTO */
...
/***************************************************************************/
#ifdef _NO_PROTO
int get_top_b2_position (buttoncount)
     int buttoncount;
#else /* _NO_PROTO */
int get_top_b2_position (int buttoncount)
#endif /* _NO_PROTO */
...
/***************************************************************************/
#ifdef _NO_PROTO
void load_directories_list (dirlist)
     char **dirlist;
#else /* _NO_PROTO */
void load_directories_list (char **dirlist)
#endif /* _NO_PROTO */
...
/***************************************************************************/
#ifdef _NO_PROTO
char *initialize_filter (filter_end)
     char *filter_end;
#else /* _NO_PROTO */
char *initialize_filter (char *filter_end)
#endif /* _NO_PROTO */
...
#endif /* _NO_PROTO */
{
  ...
  if (fe_ptr = strrchr(filter_end, EXT_DELIMITER)) {
     ...
     if (bShowPixmaps) {
        if (strlen(fe_ptr) >= strlen(PIXMAP_EXT)) {
           strcpy(fe_ptr, PIXMAP_EXT);
        }
        if ( (ffg_ptr) && (strlen(ffg_ptr) >= strlen(PIXMAP_EXT)) ) {
           strcpy(ffg_ptr, PIXMAP_EXT);
        }
     } else {
        if (strlen(fe_ptr) >= strlen(BITMAP_EXT)) {
           strcpy(fe_ptr, BITMAP_EXT);
        }
        if ( (ffg_ptr) && (strlen(ffg_ptr) >= strlen(BITMAP_EXT)) ) {
...
#ifdef DEBUG
  printf ("Freeing old_main, sizeof: %i\n", sizeof(old_main));
#endif
...
/***************************************************************************/
#ifdef _NO_PROTO
void load_filter_text_field (filter)
     char *filter;
#else /* _NO_PROTO */
void load_filter_text_field (char *filter)
#endif /* _NO_PROTO */
...
/***************************************************************************/
#ifdef _NO_PROTO
void  update_container_contents (filter)
     char *filter;
#else /* _NO_PROTO */
void  update_container_contents (char *filter)
#endif /* _NO_PROTO */
...
/***************************************************************************/
#ifdef _NO_PROTO
void load_new_filter (new_filter)
     char *new_filter;
#else /* _NO_PROTO */
...
/***************************************************************************/
#ifdef _NO_PROTO
static  void    calc_bottom_attachment( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
...
#ifdef _NO_PROTO
static  void    activateCB_bottom_button( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
        XtPointer       UxClientData, UxCallbackArg;
#else /* _NO_PROTO */
...
/******************************************************************************/
#ifdef _NO_PROTO
void    activateCB_NoFindIconDialog_ChangeNameButton( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
        XtPointer       UxClientData, UxCallbackArg;
#else /* _NO_PROTO */
...
/******************************************************************************/
#ifdef _NO_PROTO
void    activateCB_NoFindIconDialog_NameOKButton( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
        XtPointer       UxClientData, UxCallbackArg;
#else /* _NO_PROTO */
...
/******************************************************************************/
#ifdef _NO_PROTO
void display_nofindicon_message (parent)
     Widget parent;
#else /* _NO_PROTO */
...
#ifdef _NO_PROTO
static  void    activateCB_bottom_button1( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
        XtPointer       UxClientData, UxCallbackArg;
#else /* _NO_PROTO */
...
#ifdef _NO_PROTO
static  void    activateCB_bottom_button2( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
        XtPointer       UxClientData, UxCallbackArg;
#else /* _NO_PROTO */
...
#ifdef _NO_PROTO
static  void    activateCB_bottom_button3( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
        XtPointer       UxClientData, UxCallbackArg;
#else /* _NO_PROTO */
...
#endif /* _NO_PROTO */
{
  ...
  UxIcon_selection_dialogContext = UxContext =
                  (_UxCicon_selection_dialog *) UxGetContext( UxWidget );
  {
    DisplayHelpDialog(UxWidget, HELP_ICONSELECTOR, UxCallbackArg);
  }
  return;
}
...
/***************************************************************************/
#ifdef _NO_PROTO
static  void    activateCB_filter_text_field( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
        XtPointer       UxClientData, UxCallbackArg;
#else /* _NO_PROTO */
...
/***************************************************************************/
#ifdef _NO_PROTO
static  void    defaultActionCB_dir_scrolled_list( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
        XtPointer       UxClientData, UxCallbackArg;
#else /* _NO_PROTO */
...
/***************************************************************************/
#ifdef _NO_PROTO
static  void    selectionCB_icon_scrolled_container ( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
        XtPointer       UxClientData, UxCallbackArg;
#else /* _NO_PROTO */
static  void    selectionCB_icon_scrolled_container ( Widget UxWidget,
...
/***************************************************************************/
#ifdef _NO_PROTO
void    resizeCB_clipWindow( UxWidget, UxClientData, UxCallbackArg )
        Widget          UxWidget;
        XtPointer       UxClientData, UxCallbackArg;
#else /* _NO_PROTO */
...
/***************************************************************************/
#ifdef _NO_PROTO
static Widget   _Uxbuild_icon_selection_dialog()
#else /* _NO_PROTO */
...
*******************************************************************************/
#ifdef _NO_PROTO
Widget  create_icon_selection_dialog( _UxUxParent,
                                     _Uxdialog_title,
                                     _Uxfilter_field_title,
                                     _Uxuse_filter_field,
                                     _Uxfile_filter,
                                     _Uxdirectory_title,
                                     _Uxdirectories_list,
                                     _Uxcontainer_title,
                                     _UxnumberOfTopButtons,
                                     _Uxtop_button_one_label,
                                     _Uxtop_button_one_cb,
                                     _Uxtop_button_two_label,
                                     _Uxtop_button_two_cb,
                                     _Uxtop_button_three_label,
                                     _Uxtop_button_three_cb,
                                     _Uxuse_icon_name_field,
                                     _Uxname_field_title,
                                     _UxnumberOfBottomButtons,
                                     _Uxbottom_button_one_label,
                                     _Uxbottom_button_one_cb,
                                     _Uxbottom_button_two_label,
                                     _Uxbottom_button_two_cb,
                                     _Uxbottom_button_three_label,
                                     _Uxbottom_button_three_cb,
                                     _Uxbottom_button_four_label,
                                     _Uxbottom_button_four_cb )
        swidget _UxUxParent;
...

{
        ...
        load_directories_list (directories_list);
        initial_filter = initialize_filter (file_filter);
