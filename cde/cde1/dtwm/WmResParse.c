...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: WmResParse.c /main/cde1_maint/3 1995/10/09 13:45:02 pascale $"
#endif
#endif
/*
...
#endif
#ifdef WSM
static String GetNetworkFileName ();
#endif /* WSM */
...
#ifdef PANELIST
static void 	ParseIncludeSet ();
static void     ConfigStackInit ();
static FILE     *ConfigStackPush ();
static void     ConfigStackPop ();
Boolean 	ParseWmFuncActionArg ();
static void	PreprocessConfigFile ();
#endif /* PANELIST */
#else /* _NO_PROTO */
#ifdef MOTIF_ONE_DOT_ONE
...
#ifdef _NO_PROTO
void
GetFunctionTableValues (execIndex, nopIndex, actionIndex)
    int *execIndex;
    int *nopIndex;
    int *actionIndex;
#else /* _NO_PROTO */
...
#ifdef _NO_PROTO
void
GetExecIndex (tableSize, execIndex)
    int tableSize;
    int *execIndex;
#else /* _NO_PROTO */
void
...
#ifdef _NO_PROTO
void
GetActionIndex (tableSize, actionIndex)
    int tableSize;
    int *actionIndex;
#else /* _NO_PROTO */
void
...
#ifdef _NO_PROTO
void
GetNopIndex (tableSize, nopIndex)
    int tableSize;
    int *nopIndex;
#else /* _NO_PROTO */
void
...
void
#ifdef _NO_PROTO
WmDtGetHelpArgs(args, volume, topic,  argsCount)
    char *args;
...
#ifdef _NO_PROTO
Boolean
FindDtSessionMatch(commandArgc, commandArgv, pCD, pSD, 
		    pWorkSpaceList, clientMachine)
int commandArgc;
char **commandArgv;
ClientData *pCD;
WmScreenData *pSD;
...
#ifdef _NO_PROTO 
void
ParseSessionCommand (pSD, count, commandString)
    WmScreenData *pSD;
    int count;
 ...
 *************************************<->***********************************/
#ifdef _NO_PROTO
unsigned int PeekAhead(currentChar, currentLev)
...
#ifdef _NO_PROTO
void ProcessWmFile (pSD, bNested)
...
#define SLASH_MWMRC "/system.mwmrc"
	 ...
	 *
	 */
	strncat(cfileName, "/.dt/", MAXPATH-strlen(cfileName));

	if (LANG != NULL)
	{
	    strncat(cfileName, LANG, MAXPATH-strlen(cfileName));
	}
	strncat(cfileName, LANG_DT_WMRC, MAXPATH - strlen(cfileName));
    }
#else /* WSM */
    if (LANG != NULL)
    {
	...
	else
	{
	     ...
	     */
	    strncat(cfileName, HOME_DT_WMRC, MAXPATH - strlen(cfileName));
	}
#else /* WSM */
        strncat(cfileName, HOME_MWMRC, MAXPATH - strlen(cfileName));
#endif /* WSM */
...
#define SLASH_DT_WMRC "/sys.dtwmrc"
  ...
  if ((fileP == NULL) && !stackPushed)
  {
    ...
    strncat(cfileName, RC_CONFIG_SUBDIR, MAXPATH-strlen(cfileName));
    strncat(cfileName, LANG, MAXPATH-strlen(cfileName));
    strncat(cfileName, SLASH_DT_WMRC, MAXPATH - strlen(cfileName));
...
#endif 
#ifdef _NO_PROTO
Boolean ParseWmFuncMaybeStrArg (linePP, wmFunction, pArgs)
...
#endif
#ifdef _NO_PROTO
Boolean ParseWmFuncStrArg (linePP, wmFunction, pArgs)
...
#ifdef WSM
#ifdef _NO_PROTO
unsigned char *GetStringC (linePP, SmBehavior)
    unsigned char **linePP;	/* RETURNED */
    Boolean SmBehavior;
#else /* _NO_PROTO */ 
...
#ifdef _NO_PROTO
static void InitKeySubs (ppKeySubs, pNumKeySubs)
...
Boolean
#ifdef _NO_PROTO
ParseWmFunctionArg (linePP, ix, wmFunc, ppArg, sClientName, sTitle)
	unsigned char **linePP;
	int ix;
	WmFunction wmFunc;
	void **ppArg;			/* RETURNED */
...
#ifdef _NO_PROTO
static void ParseIncludeSet (pSD, lineP)
...
#ifdef _NO_PROTO
static void ConfigStackInit (pchFileName)
...
#ifdef _NO_PROTO
Boolean ParseWmFuncActionArg (linePP, wmFunction, pArgs)
