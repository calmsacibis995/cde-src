...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: WmProperty.c /main/cde1_maint/2 1995/10/09 13:44:48 pascale $"
#endif
#endif
/*
...
#ifdef _NO_PROTO
void SetMwmSaveSessionInfo (wmWindow)
    Window wmWindow;
#else /* _NO_PROTO */
...
#ifdef _NO_PROTO
Status GetWorkspaceHints(display, window, ppWsAtoms, pCount, pbAll)
    Display *display;
    Window window;
    Atom **ppWsAtoms;
    unsigned int *pCount;
...
#ifdef _NO_PROTO
void SetEmbeddedClientsProperty (propWindow, pEmbeddedClients, 
    cEmbeddedClients)
...
#ifdef _NO_PROTO
void SetWorkspaceInfo (propWindow, pWsInfo, cInfo)
    Window propWindow;
...
#ifdef _NO_PROTO
void SetWorkspacePresence (propWindow, pWsPresence, cPresence)
    Window propWindow;
    Atom *pWsPresence;
...
#ifdef _NO_PROTO
void GetDtSessionHints (pSD, sNum)
    WmScreenData *pSD;
    int sNum;
#else /* _NO_PROTO */
