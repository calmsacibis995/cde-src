...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: WmEvent.c /main/cde1_maint/2 1995/10/09 13:42:44 pascale $"
#endif
#endif
/*
...
#endif /* _NO_PROTO */
{
	...
	pSD = &(wmGD.Screens[scr]);
	    ...
	    {
		/*
		 * The desktop menu and key bindings for window manager
		 * functions use key press and key release events.  These
		 * events can appear on the root when forwarded by an FWS
		 * client which doesn't want them.
		 */
		setAttributes.event_mask |= 
		    (KeyPressMask | KeyReleaseMask);
	    }
	    /*
	     * We need to watch FWS client window ConfigureNotify events
	     * which might occur before we get to SelectInput on them.
	     */
	    setAttributes.event_mask |= SubstructureNotifyMask;
#endif /* FWS */
		 ...
		 */
		 ...
		 if (timeDiff < wmGD.doubleClickTime)
		 {
		   ...
		   _XmSetLastManagedMenuTime (
		         (Widget)XtParent(wmGD.menuActive),
		         ((XButtonEvent *)event)->time);
		   doXtDispatchEvent = True;
#endif
		 }
...
#endif /* _NO_PROTO */
{
	...
	case UnmapNotify:
	{
	  ...
	  /* END CR 5183 */
		 ...
		 */
                if (((XUnmapEvent *)event)->send_event)
		  UnManageWindow (pCD);
	    }
	    break;
	}
	...
	case DestroyNotify:
	{
	    ...
	    {
	    	if (pCD->fwsClient)
			HandleEventsOnFwsWindow (pCD, event);
	    }
	    break;
	}
#endif /* FWS */
...
#endif /* _NO_PROTO */
{
    ...
    if (!XFindContext (DISPLAY, keyEvent->subwindow, wmGD.windowContextType,
		      (XPointer *)&pCD)
        && pCD->fwsClient
        && pCD->fwsPassInput
        && !keyEvent->send_event)
...
#ifdef _NO_PROTO
void HandleDtWmClientMessage (clientEvent)
...
Boolean
#ifdef _NO_PROTO
ReplayedButtonEvent (pevB1, pevB2)
    XButtonEvent *pevB1;
    XButtonEvent *pevB2;
#else /* _NO_PROTO */
ReplayedButtonEvent (
