...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: WmManage.c /main/cde1_maint/2 1995/10/03 18:34:46 lehors $"
#endif
#endif
/*
...
static void ForceSubpanelWMState();
static void ReManageWindow ();
static void CheckPushRecallClient ();
...
#endif /* _NO_PROTO */
{
...
#ifdef FWS
    /*
     * FWS clients can not be embedded.
     */
    if (!pCD->fwsClient)
    {
#endif /* FWS */
...
#ifdef FWS
    /*
     * FWS clients provide their own initial placement.
     */
    if (!pCD->fwsClient) {
#endif
  ...
  for (i = 0; i < pCD->numInhabited; i++)
  {
...
#ifdef FWS
    /*
     * FWS clients have already been mapped and are already on the screen.
     */
    if (pCD->fwsClient)
	    pCD->clientState = NORMAL_STATE;
    else
    ...
    /*
     * In a multiple head situation send a client message to topLevel window
     * of dtsession in order to pop-up coverDialog. dtsession does not get
     * VisibilityNotify when windows pop up on locked screen
     *
     */
    if (wmGD.numScreens > 1) {
        /* check if screen is locked */
        _DtGetSmLockState(DISPLAY, wmGD.dtSmWindow, &lockState) ;

        if (lockState == True) {
            /*
             * screen is locked, has a coverDialog. this client message is
             * handled my ProcessPopup in SmLock.c in dtsession
             */
            SendClientMsg(wmGD.dtSmWindow, NULL, NULL, CurrentTime, NULL, 0);
        }
    }
...
#endif /* _NO_PROTO */
{
	...
	if (pCD->clientState & MINIMIZED_STATE)
	{
...
#ifdef FWS
			  /*
			   * The icon position just freed may be more desirable
			   * to offer FWS clients than the currently offered
			   * one.  Cancel the old reservation, offer a new one.
			   */
			  FwsUnreserveIconSpace (pWsTmp, &pWsTmp->IPData);
			  FwsReserveIconSpace (pWsTmp, &pWsTmp->IPData);
#endif /* FWS */
			}
		    }
		}
		 ...
		 * one.  Cancel the old reservation, offer a new one.
		 */
		FwsUnreserveIconSpace (pCD->pSD->pActiveWS,
				       &pCD->pSD->pActiveWS->IPData);
		FwsReserveIconSpace (pCD->pSD->pActiveWS,
				     &pCD->pSD->pActiveWS->IPData);
#endif /* FWS */
		}
	    }
...
#ifdef FWS
	    /*
	     * If this is an FWS client, then the unmap has already been done.
	     */
	    if (!pCD->fwsClient)
	    {
#endif
...
static void 
#ifdef _NO_PROTO
HandleSubstructEvents( w, ptr, event )
        Widget w ;
        caddr_t ptr ;
        XEvent *event ;
#else
