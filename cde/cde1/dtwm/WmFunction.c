...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: WmFunction.c /main/cde1_maint/6 1995/10/09 13:42:57 pascale $"
#endif
#endif
/*
...
#include <Dt/EnvControl.h>  /* for restoring **environ before an exec() */
...
#if (defined(USL) || defined(__uxp__)) && !defined(_NFILE)
...
#endif /* _NO_PROTO */
{
	...
	if (dirtyStackEntry.pCD == FindTransientOnTop(pcdLeader))
	{
	    ...
	    if (pcd_temp)
	    {
		changes.sibling = pcd_temp->clientFrameWin;
...
#ifdef FWS
    if (pCD->fwsClient && !pCD->fwsAllowStacking)
	    return;
#endif /* FWS */
...
#ifdef FWS
	if (!pcdNext->fwsClient || pcdNext->fwsAllowStacking) {
#endif /* FWS */
...
#ifndef NO_SETPGRP
#if defined(SVR4) || defined(__OSF1__) || defined(__osf__)
	setsid();
#else
#ifdef SYSV
	setpgrp();
...
#endif /* _NO_PROTO */
{
    ...
    /* DSDM - The wm is going down, so return dsdm to it's original state in case 
     * another wm is run.
     */
    SendDsdmStart();
	...
	for (scr = 0; scr < wmGD.numScreens; scr++)
	{
		...
		{
#ifdef FWS
		    if (pNextEntry->pCD->fwsClient)
		    {
			    FwsMapClient (pNextEntry->pCD);
		    }
		    else
#endif /* FWS */
		    if (pNextEntry->type == NORMAL_STATE)
...
#endif /* _NO_PROTO */
{
	...
	else if (wmGD.keyboardFocusPolicy == KEYBOARD_FOCUS_EXPLICIT)
	{
	    ...
	    {
		focusWindow = wmGD.keyboardFocus->client;
	    }
	}
	else
	{
 ...
 *  pCD = pointer to the ClientData 
 *
 *  event = X event that invoked the function (key or button)
 *
 ******************************<->***********************************/
...
#endif /* _NO_PROTO */
{
	return (F_Menu("DtPanelMenu", pCD, event));
}
...
#ifdef _NO_PROTO
Boolean F_Marquee_Selection (args, pCD, event)
    String args;
    ClientData *pCD;
    XEvent *event;
 ...
 */
        ...
        if (dirtyStackEntry.pCD)
        {
             ...
             * the transients.
             */
            if (dirtyStackEntry.pCD->transientChildren ||
                dirtyStackEntry.pCD->transientLeader)
                RestoreTransients (dirtyStackEntry.pCD, Above);
            dirtyStackEntry.pCD = NULL;
            dirtyLeader = NULL;
	}
...
#ifdef _NO_PROTO
Boolean F_Set_Context (args, pCD, event)
    String args;
    ClientData *pCD;
    XEvent *event;
...
#ifdef _NO_PROTO
Boolean F_Next_Workspace (args, pCD, event)
    String args;
    ClientData *pCD;
    XEvent *event;
...
#ifdef _NO_PROTO
Boolean F_Prev_Workspace (args, pCD, event)
    String args;
    ClientData *pCD;
    XEvent *event;
...
#ifdef _NO_PROTO
Boolean F_Workspace_Presence (args, pCD, event)
    String args;
    ClientData *pCD;
    XEvent *event;
