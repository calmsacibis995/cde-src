...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: WmResource.c /main/cde1_maint/4 1995/10/09 13:45:29 pascale $"
#endif
#endif
/*
...
#include <Xm/ScreenP.h>		/* for _XmGetMoveOpaqueByScreen */
#endif
...
#ifdef WSM
static void WriteOutXrmColors();
#endif /* WSM */
#ifdef WSM
void ProcessPresenceResources ();
void ProcessDefaultBackdropImages ();
void _WmBackdropBgDefault ();
void _WmBackdropFgDefault ();
void _WmBackdropColorSetDefault ();
void _WmIconImageMaximumDefault ();
void _WmSecondariesOnTopDefault ();
int DefaultWsColorSetId ();
#endif /* WSM */
#else /* _NO_PROTO */
...
#ifdef WSM
Const char *_Dither = DITHER;
Const char *_NoDither = NO_DITHER;
...
XtResource wmGlobalResources[] =
{
	...
	XtRBoolean,
	sizeof (Boolean),
	XtOffsetOf (WmGlobalData, fwsEnable),
	XtRImmediate,
	(caddr_t)True
    }
#endif /* FWS */
...
XtResource wmScreenResources[] =
{
    ...
    {
       WmNedgeMoveThreshold,
       WmCEdgeMoveThreshold,
       XtRInt,
       sizeof (int),
       XtOffsetOf (WmScreenData, edgeMoveThreshold),
       XtRImmediate,
       (caddr_t)0
     },
	...
	XtRCallProc,
	(caddr_t) _WmIconImageMaximumDefault
    },
#else /* WSM */
    {
	WmNiconImageMaximum,
...
XtResource wmBackdropResources[] =
{
	...
	XtRCallProc,
	(caddr_t) _WmBackdropColorSetDefault
    },
...
XtResource wmClientResources[] =
{
	...
	XtRCallProc,
	(caddr_t)_WmSecondariesOnTopDefault
    },
#endif /* WSM */
...
#ifdef _NO_PROTO
void 
_WmBackdropBgDefault (widget, offset, value)
	Widget	 widget;
	int	 offset;
	XrmValue *value;
...
#endif /* _NO_PROTO */
{
    ...
    {
	if ((colorSetId == 0) || (colorSetId > MAX_NUM_COLORS))
	{
...
#ifdef _NO_PROTO
void 
_WmBackdropFgDefault (widget, offset, value)
	Widget	 widget;
	int	 offset;
	XrmValue *value;
...
#ifdef _NO_PROTO
void 
_WmBackdropColorSetDefault (widget, offset, value)
	Widget	 widget;
	int	 offset;
	XrmValue *value;
...
#ifdef _NO_PROTO
void 
_WmIconImageMaximumDefault (widget, offset, value)
	Widget	 widget;
	int	 offset;
	XrmValue *value;
...
#define        _WS_HIGH_COLOR_COUNT	4    
	...
	switch (pSD->colorUse)
	{
	    ...
	    case LOW_COLOR:
		rval = 1 +
		  (pSD->pInactivePixelSet-pSD->pPixelData)/sizeof(PixelSet);
		break;
...
#endif /* _NO_PROTO */
{
    ...
    if ((pSD->pPixelData = (PixelSet *) 
	    XtMalloc (NUM_COLORS * sizeof(PixelSet)))) 
    {
        /*
	 *
	 *
	 ...
	 *
	 */
	if (_XmGetPixelData (pSD->screen, &pSD->colorUse,
...
#endif /* _NO_PROTO */
{
	...
	if (pSD->colorUse == B_W)
	{
	    ...
	    /* activeForeground */
	    value.addr = (caddr_t) &(BlackPixel(DISPLAY, pSD->screen));
	    ...
	    /* activeBackground */
	    value.addr = (caddr_t) &(WhitePixel(DISPLAY, pSD->screen));
	...
	else /* active colors for non-BW systems */
	{
		...
		if (DitherBottomShadow(DISPLAY, thisScreen, tpixset)) 
		{
		    XrmPutStringResource (&db,
        ...
        if (pSD->colorUse == B_W)
	{
	    ...
	    */
	    ...
	    /* Finally, deal with B_W Confirm Boxes */

	    /* B_W confirm box foreground */
	    value.addr = (caddr_t) &(spixset->fg);

	    XrmPutResource (&db, 
		ResCat (res_class, screen_name, (char *)FEEDBACK_FRAME_PART,
			WmNforeground), XtRPixel, &value);

	    /* B_W confirm box background */
	    value.addr = (caddr_t) &(spixset->bg);

	    XrmPutResource (&db, 
		ResCat (res_class, screen_name, (char *)FEEDBACK_FRAME_PART,
			WmNbackground), XtRPixel, &value);

	    /* B_W confirm box top shadow */
	...
	else /* inactive colors for non-BW systems */
	{
		...
		if (DitherTopShadow(DISPLAY, thisScreen, fpixset)) 
		{
		    XrmPutStringResource (&db, 
		...
		if (DitherBottomShadow(DISPLAY, thisScreen, fpixset)) 
		{
		    XrmPutStringResource (&db, 
    ...
    {
       value.addr = (caddr_t) &(pSD->pTextPixelSet->bg);
...
#endif /* _NO_PROTO */
{
    ...
    {
	char msg[130];
	sprintf(msg, "cannot find an appropriate font: %.100s\0", 
...
#endif /* _NO_PROTO */
{
	 ...
	 */
	pWS->title = XmStringCreateLtoR(pWS->name, 
...
#endif /* _NO_PROTO */
{
    ...
    {
	XtGetSubresources (pSD->presence.shellW, (caddr_t) &pSD->presence, 
...
#endif /* _NO_PROTO */
{
	...
	if (ok)
	{
		...
		if (pSD->pPixelData[i].bg == newBackground)
		{
		    if (type == WmFGC)
		    {
			newValue = pSD->pPixelData[i].fg;
		    }
		    else if (type == WmBGC)
		    {
			newValue = pSD->pPixelData[i].bg;
		    }
		    else if (type == WmTSC)
		    {
			newValue = pSD->pPixelData[i].ts;
		    }
		    else if (type == WmBSC)
		    {
			newValue = pSD->pPixelData[i].bs;
		    }
