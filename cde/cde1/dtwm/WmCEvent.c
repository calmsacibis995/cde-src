...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: WmCEvent.c /main/cde1_maint/2 1995/10/03 18:33:19 lehors $"
#endif
#endif
/*
...
#include <Xm/XmP.h>		/* for _XmGrabTheFocus */
...
#endif /* _NO_PROTO */
{
    ...
    if (pCD->fwsClient)
    {
	    return (HandleEventsOnFwsWindow (pCD, event));
    }
#endif /* FWS */
    ...
    {
	return (HandleEventsOnOtherWindow (pCD, event));
    }
...
#ifdef _NO_PROTO
Boolean HandleEventsOnOtherWindow (pCD, event)
    ClientData *pCD;
    XEvent *event;

#else /* _NO_PROTO */
Boolean HandleEventsOnOtherWindow (ClientData *pCD, XEvent *event)
#endif /* _NO_PROTO */
{
    Boolean dispatchEvent = False;
	...
	case UnmapNotify:
	{
	    ...
	    if (((XUnmapEvent *)event)->window == pCD->client)
	    {
	        ...
	        {
#ifdef FWS
		    /* Don't UnManage FWS clients because they unmap. */
		    if (!pCD->fwsClient)
#endif /* FWS */
		    UnManageWindow (pCD);
	        }
	...
	case ReparentNotify:
	{
		 ...
		 */
#ifdef FWS
		/* Don't UnManage FWS clients until destroyed. */
		if (!pCD->fwsClient)
#endif /* FWS */
		UnManageWindow (pCD);
	      }
...
} /* END OF FUNCTION HandleEventsOnOtherWindow */
...
#endif /* _NO_PROTO */
{
	...
	default:
	{
	    ...
	    /* DSDM - if the _SUN_DRAGDROP_INTEREST has changed, inform dsdm */
	    else if (propertyEvent->atom == wmGD.ATOM_DRAGDROP_INTEREST)
	      {
		ProcessDragdrop (pCD->client);
	      }
	    else if (propertyEvent->atom == wmGD.xa_MAIL_STATUS) {
                HandleMailUpdate(pCD);
	      }
...
#ifdef _NO_PROTO
void HandleCLeaveNotify (pCD, leaveEvent)
    ClientData *pCD;
...
/*************************************<->*************************************
 *
 *  HandleMailUpdate (pEvent)
 *
 *
 *  Description:
 *  -----------
 *  This function sets the mail icon state  to new mail icon or no new mail icon
 *
 *
 *************************************<->***********************************/

#ifdef _NO_PROTO
void HandleMailUpdate (pCD)
    ClientData *pCD;
#else /* _NO_PROTO */
void HandleMailUpdate (ClientData *pCD)
#endif /* _NO_PROTO */
{
 DtControlGadget g;
 Atom actualType;
 ...
 MailItem *tmp_mail_ptr;
 mail_data *data;


        /* if there is a mail client out there, you get here */
        if ((Success != XGetWindowProperty (DISPLAY, pCD->client,
                        wmGD.xa_MAIL_STATUS, 0L, sizeof(mail_data),
                        False, wmGD.xa_MAIL_STATUS, &actualType, &actualFormat,
                        &nitems, &leftover, (unsigned char **)&data)) &&
                        (actualType == wmGD.xa_MAIL_STATUS)) {
              XtFree((char *) data);
              return;
        }
        else
          ...
          while (tmp_mail_ptr) {
             g = (DtControlGadget)tmp_mail_ptr->mail_gadget;
            _DtControlSetFileChanged ((Widget)g, data->mail_stat);
            G_CallCallback ((DtIconGadget) g, G_Callback (g),
                                                        XmCR_MONITOR, NULL);
...
} /* END OF FUNCTION HandleMailUpdate */
