/* $XConsortium: Callback.c /main/cde1_maint/5 1995/10/09 13:40:41 pascale $ */
/*****************************************************************************
 *
 *   File:         Callback.c
 *
...
extern void SubpanelControlCreate ();
extern void WmSubpanelPosted ();
extern int PushRecallGetData ();
extern void SwitchButtonCreate ();
extern void AddControlActionList ();

void ArrowCB (); 
void HandleInputCB (); 
void MinimizeInputCB (); 
void MenuInputCB (); 
void SwitchButtonCB ();
void PushCB ();
void SubpanelUnmapCB (); 
void SubpanelTornEventHandler ();
Boolean CheckControlTypeFile ();


static void SwitchRenameCancel ();
static void SwitchRenameCB ();
static void GetValuesFromDataType();

void SwitchRenameLabel ();
void DropCB (); 
void TransferDropCB (); 
void CustomizeDropCB ();
void CustomizeTransferDropCB (); 
...
void
CallWmFunction (wm_function, args, client, event)
WmFunction wm_function;
String	   args;
Widget     client;
XEvent   * event;
...

{
   ...
   if (XFindContext (display, client_window, wm_global_data->windowContextType,
                    (XtPointer) &wm_client_data) != 0)
    return;
...
void
ArrowCB (w, client_data, call_data)
Widget    w;
...
void
MinimizeInputCB (w, client_data, call_data)
Widget    w;
XtPointer client_data;
XtPointer call_data;
...
void
MenuInputCB (w, client_data, call_data)
Widget    w;
XtPointer client_data;
XtPointer call_data;
...

{
   ...
   extern Pixmap menu_normal_pixmap;
   extern Pixmap menu_selected_pixmap;
 ...
 *	processed by this function are workspace add, delete, rename, and 
 *      switch.
 *
 ************************************************************************/
...
void
WorkspaceModifyCB (w, atom_name, type, client_data)
Widget    w;
Atom      atom_name;
int       type;
XtPointer client_data;
...

{
   ...
   /* Check to see if workspace switch exists. If not, workspaces are being
    * switched from the keyboard and there are no switch buttons to update.
    */

   if (!client_data) 
     return;

   switch_data = (SwitchData *) client_data;
   ...
   switch (type)
   {
      ...
      case DtWSM_MODIFY_WORKSPACE_TYPE_ADD:
      {
         int    switch_count;
	 Atom * atom_names;
         ...
         if (XtWidth (panel.shell) > screen_width)
	 {
      ...
      case DtWSM_MODIFY_WORKSPACE_TYPE_DELETE:
      {
	 int i;
         int j;
         int    switch_count;
	 ...
	 for (i = 0; i < switch_data->switch_count; i++)
      ...
      case DtWSM_MODIFY_WORKSPACE_TYPE_TITLE:
      {
	 ...
	 for (i = 0; i < switch_data->switch_count; i++)
      ...
      case DtWSM_MODIFY_WORKSPACE_TYPE_ACTIVE:
      {
         int i;
	 Arg al[2];
...
void
SwitchButtonCB (w, client_data, call_data)
Widget    w;
XtPointer client_data;
XtPointer call_data;
...
static void
SwitchRenameCB (w, client_data, call_data)
Widget    w;
XtPointer client_data;
XtPointer call_data;
...
void
SwitchRenameLabel (switch_button, box_data)
Widget    switch_button;
BoxData * box_data;
...

{
   ...
   if (XmStringGetLtoR (toggle_string, XmFONTLIST_DEFAULT_TAG, &edit_string))
   {
      ...
      XtSetArg (al[0], XmNcursorPosition, 
                XmTextFieldGetLastPosition (switch_edit));
      XtSetValues (switch_edit, al, 1);
...
#ifdef AIX_ILS
      XtVaSetValues( switch_edit,
		XmNwidth, toggle_width,
		XmNheight, toggle_height,
		NULL );	
#endif /* AIX_ILS */
...
Boolean
CheckControlTypeFile (control_data)
...
void
TransferDropCB (w, client_data, call_data)
Widget    w;
XtPointer client_data;
XtPointer call_data;
...
void
SubpanelUnmapCB (w, client_data, call_data)
Widget    w;
XtPointer client_data;
XtPointer call_data;
...
void StartHelpClient (widget, help_volume, help_topic)
Widget   widget;
char   * help_volume;
char   * help_topic;
...
void InvokeHelp (widget, help_topic, help_volume, help_string)
Widget    widget;
char *	  help_topic;
char *	  help_volume;
char *	  help_string;
...
void SubpanelTopicHelpCB(widget, client_data, call_data)
Widget    widget;
XtPointer client_data;
XtPointer call_data;
...
void SwitchTopicHelpCB (widget, client_data, call_data)
Widget    widget;
XtPointer client_data;
XtPointer call_data;
...
void ControlTopicHelpCB (widget, client_data, call_data)
Widget widget;
XtPointer client_data;
XtPointer call_data;
...
void GeneralTopicHelpCB (widget, client_data, call_data)
Widget    widget;
XtPointer client_data;
XtPointer call_data;
...
static void
GetValuesFromDataType (data_type, file_name, subpanel_data, element_values)
char * 		 data_type;
char *	 	 file_name;
SubpanelData   * subpanel_data;
...
void
CustomizeDropCB (w, client_data, call_data)
Widget    w;
XtPointer client_data;
XtPointer call_data;
...
void
CustomizeTransferDropCB (w, client_data, call_data)
Widget    w;
XtPointer client_data;
XtPointer call_data;
