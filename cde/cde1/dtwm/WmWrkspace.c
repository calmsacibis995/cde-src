...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: WmWrkspace.c /main/cde1_maint/4 1995/10/11 18:11:21 montyb $"
#endif
#endif
...
#ifdef FWS
    /*
     * If we had an icon slot reserved, unreserve it.
     */
    FwsUnreserveIconSpace (pSD->pLastWS, &pSD->pLastWS->IPData);
#endif /* FWS */
...
#ifdef FWS
    /*
     * Make a new icon slot available.
     */
    FwsReserveIconSpace (pNewWS, &pNewWS->IPData);
#endif /* FWS */
...
#endif /* _NO_PROTO */
{
     ...
     */
    xmstr = XmStringCreateLtoR (pchTitle, XmFONTLIST_DEFAULT_TAG);
...

{
    ...
    while (1)
    {
	/* if no windows managed for this screen*/
	if(pCLE == NULL)
	     break;
	/*
...

{
	...
	if (pWS->title)
	    XmStringFree (pWS->title);
	pWS->title = XmStringCreateLtoR ((char *)pchTitle, 
 ...
 *  Please XtFree() it when your done.
 *
 *************************************<->***********************************/

Boolean 
#ifdef _NO_PROTO
ConvertNamesToIDs( pSD, pchIn, ppAtoms, pNumAtoms )
        WmScreenData *pSD ;
        unsigned char *pchIn ;
        WorkspaceID **ppAtoms ;
...

{
   ...
   if (pchIn && (pchLocal = (unsigned char *) XtMalloc(1+strlen((char *)pchIn))))
   {
	...
	while (pchName = GetSmartString (&pch))
	{
	     ...
	     * workspace names.
	     */
            xms = XmStringCreate ((char *)pchName, XmFONTLIST_DEFAULT_TAG);
...
WorkspaceID * 
#ifdef _NO_PROTO
GetListOfOccupiedWorkspaces( pCD, numIDs )
        ClientData *pCD ;
        int *numIDs ;
#else
...
#endif /* _NO_PROTO */
{
	 ...
	 */
	for (wsCnt=0; wsCnt < pCD->numInhabited; wsCnt++)
	{
...

{
	...
	else
	{
...
#ifdef FWS
		if (pWsc->iconPlace == pWS->IPData.fwsNextIcon)
		...
		if (pWS->IPData.placeList[pWsc->iconPlace].pCD)
		{
		    ...
		    pWsc->iconPlace = 
#ifdef FWS
		    FindIconPlace (pWS, pCD, &pWS->IPData, pWsc->iconX,
				pWsc->iconY);
#else
			...
			/* Can't find a spot close by. Use the
			   next available slot */
#ifdef FWS
			pWsc->iconPlace = GetNextIconPlace (pWS, &pWS->IPData);
#else
...
#ifdef FWS
		/*
		 * Offer a new slot for FWS client use.
		 */
...
Boolean 
#ifdef _NO_PROTO
GetLeaderPresence( pCD, ppIDs, pnumIDs )
        ClientData *pCD ;
        WorkspaceID **ppIDs ;
        unsigned int *pnumIDs ;
#else
...
Boolean 
#ifdef _NO_PROTO
GetMyOwnPresence( pCD, ppIDs, pnumIDs )
        ClientData *pCD ;
        WorkspaceID **ppIDs ;
        unsigned int *pnumIDs ;
#else
...
void 
#ifdef _NO_PROTO
SaveWorkspaceResources( pWS, flags)
        WmWorkspaceData *pWS ;
        unsigned long flags ;
#else
...
#endif /* _NO_PROTO */
{
    ...
    *data = '\0';
    ...
    if (bufferLength == 0)
    {
	buffer = (char *) XtMalloc (MAXPATH+1);
	bufferLength = MAXPATH;
    }
    *buffer = '\0';
