/* $XConsortium: UI.c /main/cde1_maint/5 1995/10/20 17:00:59 lehors $ */
/*****************************************************************************
 *
 *   File:         UI.c
 *
...
extern void   PushCB ();
extern void   ArrowCB ();
extern XtTranslations HandleInputTranslations ();
extern void   HandleInputCB (); 
extern void   MinimizeInputCB (); 
extern void   MenuInputCB (); 
extern void   SwitchButtonCB ();
extern void   SubpanelUnmapCB (); 
extern void   SubpanelTornEventHandler ();
extern void   PushRecallRegister ();
extern void   EmbeddedClientRegister ();
extern void   EmbeddedClientReposition ();
extern void   DropCB ();
extern void   TransferDropCB ();
extern void   CustomizeDropCB ();
extern void   CustomizeTransferDropCB ();
extern void   FrontPanelCreate ();
extern void   EmbeddedClientReparent ();
extern void   DeleteControlActionList ();
extern Boolean CheckOtherMonitorsOn ();


static void   Initialize ();
static void   BoxCreate ();
static Widget PanelControlCreate ();
static void   MainControlCreate ();
static void   SwitchCreate ();
static void   SubpanelCreate ();
static void   ControlCreate ();
static void   ArrowCreate ();
static void   SetupPushAnimation ();
static void   SetupDropAnimation ();
static void   destroyCB ();


void SubpanelControlCreate ();
void SwitchButtonCreate ();
void DeleteSubpanelControl ();
...

{
   ...
   {
      resolution =
          _DtGetDisplayResolution (XtDisplay ((Widget) panel_shell),
	              XScreenNumberOfScreen (XtScreen ((Widget) panel_shell)));
   ...
   _XmGetIconControlInfo (screen,
			  &use_mask_return, &panel.use_color_icons,
			  &use_icon_file_cache_return);

   panel.pixel_set = (PixelSet *) XtMalloc (sizeof(PixelSet) * NUM_COLORS);

   if (_XmGetPixelData (XScreenNumberOfScreen (screen), 
   ...
   {			
      ...
      panel.pixel_set = (PixelSet *) XtMalloc (sizeof (PixelSet));
...
#ifdef _NO_PROTO
static Widget
PanelControlCreate (parent, control_name, image_name)
Widget   parent;
char   * control_name;
String   image_name;
...
#ifdef _NO_PROTO
static void
ControlSetIconData (parent, control_data, icon_label, container_type, al, ac,
		    icon_name, alternate_icon_name)
Widget        parent;
ControlData * control_data;
XmString    * icon_label;
int           container_type;
...
#ifdef _NO_PROTO
static void
ControlSetBehavior (control_data, al, ac, in_subpanel, expanded_file_name)
ControlData * control_data;
ArgList       al;
int         * ac;
Boolean       in_subpanel;
...

{
   ...
   switch ((char) control_data->element_values[CONTROL_TYPE].parsed_value)
   {
	 ...
	 if (file_name != NULL) {
            *expanded_file_name = 
		(String) _DtWmParseFilenameExpand ((unsigned char*) file_name);
...
#ifdef _NO_PROTO
static Widget
ControlCreateAndRegister (parent, control_data, main_copy, al, ac)
Widget        parent;
ControlData * control_data;
Boolean       main_copy;
ArgList       al;
...

{
   ...
   Arg al2[4];
   int ctype = 0;
   MailItem *mailitem, *tmp_mail_ptr;
   ...
   if ((int) control_data->element_values[CONTROL_TYPE].parsed_value
    ...
    icon = _DtCreateControl (parent, "icon", al, ac);
     if ((char) control_data->element_values[CONTROL_MONITOR_TYPE].parsed_value           == MONITOR_MAIL)
       {
         if ((mailitem = (MailItem *) XtMalloc (sizeof (MailItem))) == NULL)
         /* Handle insufficent memory */
         MWarning(((char *)GETMESSAGE(48, 2, 
			"Insufficient memory for mail item \n")));
         XtAddCallback (icon, XmNdestroyCallback, 
				(XtCallbackProc) destroyCB, NULL);
       mailitem->mail_gadget = icon;
       mailitem->next  = NULL;
 
       if(wmGD.mail_items == NULL) {
         wmGD.mail_items = mailitem;
       }
       else {
         tmp_mail_ptr = wmGD.mail_items;
 
         while(tmp_mail_ptr->next)  {
               printf("inside while loop \n");
               tmp_mail_ptr = tmp_mail_ptr->next;
         }
 
         if (tmp_mail_ptr->next == NULL)
               tmp_mail_ptr->next = mailitem;
       }
      }
    }
...

{
   ...
   for (i = 0; i < switch_data->control_data_count; i++)
   {
	 ...
	 else
	 {
      ...
      if (icon_label != (XmString)NULL)
	{
         XmStringFree (icon_label);
	 icon_label = (XmString)NULL;
	}

      if (icon_name != (char*)NULL)
	{
         XtFree(icon_name);
	 icon_name = (char*)NULL;
	}

      if (alt_icon_name != (char*)NULL)
	{
         XtFree(alt_icon_name);
	 alt_icon_name = (char*)NULL;
	}

/*    
      Note: Should not be freed because DtWidget does not make a copy 
      of fileName resource value. See Bug id:4018570
      if (exp_file_name != (char*)NULL)
	{
         XtFree(exp_file_name);
	 exp_file_name = (char*)NULL;
	}
*/
...
#ifdef _NO_PROTO
void
SwitchButtonCreate (switch_data, one)
...
#ifdef _NO_PROTO
static void
ControlCreate (parent, control_data, control_count)
Widget         parent;
ControlData ** control_data;
int            control_count;
...
#ifdef _NO_PROTO
static void
ArrowCreate (parent, control_data, arrow_count, right_side, first_box)
Widget         parent;
ControlData ** control_data;
int            arrow_count;
Boolean        right_side;
...
#ifdef _NO_PROTO
static void
SubpanelCreate (main_control_data, subpanel_data)
...
static void
SwapElementValue (to_value, from_value)
ElementValue * to_value;
...

{
   ...
   if (subpanel_data->torn == False)
       ArrowCB (main_control_data->arrow, main_control_data, NULL);
 ...
 *
 *  destroyCB
 *      Remove mail item from the list
 *
 ************************************************************************/

#ifdef _NO_PROTO
void
destroyCB (w, cdata, udata)
Widget       w;
XtPointer    cdata;
XtPointer    udata;
#else /* _NO_PROTO */
...
#endif /* _NO_PROTO */
{
   MailItem *tmp_mail_ptr, *prev_mail_ptr;

        tmp_mail_ptr = wmGD.mail_items;
        prev_mail_ptr = wmGD.mail_items;

        while(tmp_mail_ptr) {
                if(tmp_mail_ptr->mail_gadget == w) {

                     if(tmp_mail_ptr == prev_mail_ptr) /* first element */
                        wmGD.mail_items = tmp_mail_ptr->next;
                     else
                        prev_mail_ptr->next = tmp_mail_ptr->next;

                        XtFree((char *) tmp_mail_ptr);
                        return;
                }
                else
                        prev_mail_ptr = tmp_mail_ptr;
