...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$XConsortium: WmInitWs.c /main/cde1_maint/5 1995/10/20 17:01:23 lehors $"
#endif
#endif
/*
...
#ifdef _NO_PROTO
#ifdef WSM
static void InsureDefaultBackdropDir();
#endif /* WSM */
...
#endif /* _NO_PROTO */
{
     ...
     * 
     * Register our converters early to work around an Xt bug with
     * hash table collisions in R4
     */   
	...
	_XmGetIconControlInfo(ScreenOfDisplay(DISPLAY, sNum), &useMaskRtn,
...
#ifdef WSM
		    wmGD.xa_DT_SESSION_HINTS =
			XmInternAtom (DISPLAY, _XA_DT_SESSION_HINTS, False);

		    wmGD.xa_DT_SM_WM_PROTOCOL =
			XmInternAtom (DISPLAY, _XA_DT_SM_WM_PROTOCOL, False);

		    wmGD.xa_DT_SM_START_ACK_WINDOWS =
			XmInternAtom (DISPLAY, _XA_DT_SM_START_ACK_WINDOWS, 
				      False);

		    wmGD.xa_DT_SM_STOP_ACK_WINDOWS =
			XmInternAtom (DISPLAY, _XA_DT_SM_STOP_ACK_WINDOWS, 
				      False);

		    wmGD.xa_DT_WM_WINDOW_ACK =
			XmInternAtom (DISPLAY, _XA_DT_WM_WINDOW_ACK, False);
		    
		    wmGD.xa_DT_WM_EXIT_SESSION =
                        XmInternAtom (DISPLAY, _XA_DT_WM_EXIT_SESSION, False);
		    
                    wmGD.xa_DT_WM_LOCK_DISPLAY =
                        XmInternAtom (DISPLAY, _XA_DT_WM_LOCK_DISPLAY, False);

                    wmGD.xa_DT_WM_READY =
                        XmInternAtom (DISPLAY, _XA_DT_WM_READY, False);
#endif /* WSM */
#ifndef NO_OL_COMPAT
		    InitOLCompat();
#endif /* NO_OL_COMPAT */
    ...
    wmGD.xa_DT_WORKSPACE_HINTS = 
		XmInternAtom (DISPLAY, _XA_DT_WORKSPACE_HINTS, False);
    wmGD.xa_DT_WORKSPACE_PRESENCE = 
		XmInternAtom (DISPLAY, _XA_DT_WORKSPACE_PRESENCE, False);
    wmGD.xa_DT_WORKSPACE_INFO = 
		XmInternAtom (DISPLAY, _XA_DT_WORKSPACE_INFO, False);
    wmGD.xa_ALL_WORKSPACES = 
		XmInternAtom (DISPLAY, WmNall, False);
    wmGD.xa_DT_EMBEDDED_CLIENTS = 
		XmInternAtom (DISPLAY, _XA_DT_WORKSPACE_EMBEDDED_CLIENTS, 
				False);
    wmGD.xa_DT_WM_REQUEST = 
		XmInternAtom (DISPLAY, _XA_DT_WM_REQUEST, False);
    wmGD.xa_DT_WORKSPACE_LIST = 
		XmInternAtom (DISPLAY, _XA_DT_WORKSPACE_LIST, False);
...
# endif
     ...
     *  NEW LOCATION
     */
/* This is confusing front panel text widgets in thinking they have different
 * background color, hence text widgets use wrong foreground color and text
 * is white on white for some palettes. This is on text widgets which are
 * displayed when you edit workspace buttons. Also, action items launched
 * from the front panel, like Remote Terminal, show this behavior. See bug:
 * 1195169.
 */
...
#ifdef FWS
    /*
     * Initialize the foreign window system support.  This requires that
     * the topLevel widget's window has been created.
     */
    FwsInitializeWindow();
    /*
     * Now that the screen is initialized, reserve space for an FWS client
     * to use as an icon in the current workspace.
     */
    for (scr=0; scr<wmGD.numScreens; scr++) 
      {
	pSD = &(wmGD.Screens[scr]);
	FwsReserveIconSpace (pSD->pActiveWS, &pSD->pActiveWS->IPData);
      }
#endif /* FWS */
...
#ifdef FWS
    /*
     * Initialize the foreign window system support.
     */
...
#ifdef FWS
	/*
	 * Lower FWS client window now, so that when we go through the
	 * following loop, the backdrop windows will be placed underneath us.
	 */
...
#define LENCBUFFER 256
    ...
    */
	...
	if ((wmGD.screenNames[sNum] = 
	     (unsigned char *)realloc (wmGD.screenNames[sNum], buf_size)) == NULL)
	{
...
#ifdef _NO_PROTO
static void
InsureDefaultBackdropDir(ppchBackdropDirs)
    char **	ppchBackdropDirs;
#else /* _NO_PROTO */
static void
InsureDefaultBackdropDir(char **ppchBackdropDirs)
#endif /* _NO_PROTO */
...
#endif /* _NO_PROTO */
{
     ...
     * Initialize messages
     */
    wmGD.okLabel=XmStringCreateLtoR(_DtOkString, XmFONTLIST_DEFAULT_TAG);
    wmGD.cancelLabel=XmStringCreateLtoR(_DtCancelString, 
					XmFONTLIST_DEFAULT_TAG);
    wmGD.helpLabel=XmStringCreateLtoR(_DtHelpString, XmFONTLIST_DEFAULT_TAG);
...
#ifdef WSM
	_XmOSPutenv(wmGD.displayString);
#endif /* WSM */
    }
        ...
        Display *dsp)
{
  ...
  while(    i < num_props    )
    {
      prop_atoms[i] = XInternAtom( dsp, prop_names[i], FALSE) ;
      ++i ;
    }
