/* $XConsortium: DataBaseLoad.c /main/cde1_maint/4 1995/10/09 13:41:23 pascale $ */
/*****************************************************************************
 *
 *   File:         DataBaseLoad.c
 *
...
#ifdef _NO_PROTO
typedef Boolean (*FieldParse)();
typedef void (*FieldFree)();
#else  /* _NO_PROTO */
...
static ParseFunction control_parse_functions[] =
{
   ...
   {StringToBoolean,              NULL,   FreeString},   /* Push Recall     */
...
extern void WmFrontPanelSetBusy ();
extern char * _DtDbGetDataBaseEnv ();

static Boolean PanelParseCB ();
static Boolean BoxParseCB (); 
static Boolean SubpanelParseCB ();
static Boolean SwitchParseCB ();
static Boolean ControlParseCB ();
static Boolean AnimationParseCB ();
static Boolean ControlSingleParseCB ();
static void ProcessRecord ();
static void AllocateRecordData ();
static void ReorderByContainerType();
static void ReorderByContainerName();
static void ReorderByName();
static void ReorderByPosition();
static void OrderRecord ();
static char ** GetNameList ();
static void EliminateUnused ();
static void EliminateDeleted ();
static void ResolveDuplicates ();
static void InitializeField ();
static void ProcessBox ();
static int ProcessBoxControl ();
static void ProcessSubpanel ();
static void ProcessSwitch ();
static void ProcessControl ();
static void InitializePrimaryControlFields ();
static void InitializeSecondaryControlFields ();
static void InitializeFileControlFields ();
...
#ifdef _NO_PROTO
static Boolean
PanelParseCB (fields, path_id, host_prefix, rejection)
DtDtsDbField  * fields;
DtDbPathId   path_id;
char        * host_prefix;
Boolean       rejection;
...
#ifdef _NO_PROTO
static Boolean
BoxParseCB (fields, path_id, host_prefix, rejection)
DtDtsDbField  * fields;
DtDbPathId   path_id;
char        * host_prefix;
Boolean       rejection;
...
#ifdef _NO_PROTO
static Boolean
SubpanelParseCB (fields, path_id, host_prefix, rejection)
DtDtsDbField  * fields;
DtDbPathId   path_id;
char        * host_prefix;
Boolean       rejection;
...
#ifdef _NO_PROTO
static Boolean
SwitchParseCB (fields, path_id, host_prefix, rejection)
DtDtsDbField  * fields;
DtDbPathId   path_id;
char        * host_prefix;
Boolean       rejection;
...
#ifdef _NO_PROTO
static Boolean
ControlParseCB (fields, path_id, host_prefix, rejection)
DtDtsDbField  * fields;
DtDbPathId   path_id;
char        * host_prefix;
Boolean       rejection;
...
#ifdef _NO_PROTO
static Boolean
AnimationParseCB (fields, path_id, host_prefix, rejection)
DtDtsDbField  * fields;
DtDbPathId   path_id;
char       * host_prefix;
Boolean      rejection;
...
#ifdef _NO_PROTO
static Boolean
ControlSingleParseCB (fields, path_id, host_prefix, rejection)
DtDtsDbField  * fields;
DtDbPathId   path_id;
char        * host_prefix;
Boolean       rejection;
...
#ifdef _NO_PROTO
static void
ProcessRecord (fields, element_values)
DtDtsDbField   * fields;
...
#ifdef _NO_PROTO
static void
AllocateRecordData (record_data, record_size, record_data_count, record_count)
...
#ifdef _NO_PROTO
static void
OrderRecord (record_data, record_count, value_define)
RecordData  * record_data;
int           record_count;
int           value_define;
...
static void
#ifdef _NO_PROTO
QuickSort(rec_data, left, right, elem_index, container)
RecordData * rec_data;
int          left;
int	     right;
int 	     elem_index;
...
#ifdef _NO_PROTO
static void
BubbleSort (rec_data, start, rec_count, elem_index)
RecordData * rec_data;
int          start;
int          rec_count;
int 	     elem_index;
...
#ifdef _NO_PROTO
static void
ReversePositions (rec_data, start, end)
RecordData * rec_data;
int          start;
int          end;
...
#ifdef _NO_PROTO
static void
ReorderByContainerName (rec_data, rec_count, name, elem_type)
RecordData * rec_data;
int          rec_count;
int	     name;
int	     elem_type;
...
#ifdef _NO_PROTO
static void
ReorderByName (rec_data, rec_count, name, elem_type)
RecordData * rec_data;
int          rec_count;
int	     name;
int	     elem_type;
...
#ifdef _NO_PROTO
static void
ReorderByPosition (rec_data, rec_count, pos_hints, cont_type, cont_name)
RecordData * rec_data;
int          rec_count;
int	     pos_hints;
int	     cont_type;
...
#ifdef _NO_PROTO
static void
ReorderByContainerType (rec_data, rec_count, container_type)
RecordData * rec_data;
int          rec_count;
int	     container_type;
...
#ifdef _NO_PROTO
void
RemoveEntry (record_data, record_type)
RecordData * record_data;
int	     record_type;
...
#ifdef _NO_PROTO
static void
EliminateEntries (record_data, record_count, start, end, i, count, record_type)
RecordData * record_data;
int        * record_count;
int	     start;
int	     end;
...
#ifdef _NO_PROTO
static void
ResolveDuplicates (record_data, record_count, name_type, container_type,
		   container_name, lock_type, record_type)
RecordData * record_data;
int        * record_count;
int          name_type;
int          container_type;
...
#ifdef _NO_PROTO
static char **
GetNameList (record_data, record_count, name_type, container_type, container)
RecordData * record_data;
int          record_count;
int          name_type;
int          container_type;
...
#ifdef _NO_PROTO
static void
EliminateUnused (record_data, record_count, cont_name_list,
		 cont_name, cont_type, container, record_type)
RecordData    * record_data;
int           * record_count;
char         ** cont_name_list;
int             cont_name;
...
#ifdef _NO_PROTO
static void
EliminateDeleted (record_data, record_count, cont_name, 
		  record_name, record_type, delete_type, lock_type)
RecordData    * record_data;
int           * record_count;
int             cont_name;
int 		record_name;
...
#ifdef _NO_PROTO
static void
InitializeField (element_values, keyword, parse_functions)
ElementValue  * element_values;
int             keyword;
ParseFunction * parse_functions;
...
#ifdef _NO_PROTO
static int
CountElements (record_data, record_count, container_name, 
               name_type, container_type, container)
RecordData * record_data;
int          record_count;
char       * container_name;
int          name_type;
...
#ifdef _NO_PROTO
static void
ProcessControl (parent, parent_type, control_data_ptr, control_count, element_values)
XtPointer       parent;
char            parent_type;
...
#ifdef _NO_PROTO
static int
ProcessBoxControl (control_data)
ControlData * control_data;
...
static char *
CreateComponentFileName (record_data)
RecordData * record_data;
...
static void
WriteComponentToFile (record_data, record_type, keywords, container_name,
...
static void
RemoveComponentFile (record_data, record_type, keywords, container_name,
                     container_type)
RecordData * record_data;
int          record_type;
char      ** keywords;
...
void
WriteSubpanelComponentFile (subpanel_data)
SubpanelData * subpanel_data;
...
void
_WriteControlElementValues (element_values)
ElementValue * element_values;
...
static void
FreeFileControlField (element_values, indx)
ElementValue * element_values;
int            indx;
...

{
   if ((int)element_values[CONTROL_TYPE].parsed_value == CONTROL_FILE)
   {
...
#if DEBUG
      printf("init file control %s, %d, %s\n",
	 ...
	 if (element_values[CONTROL_LABEL].string_value == NULL) {
	    printf("string_value = NULL\n");
	 }
	 else { 
	    printf("string_value = %s\n", 
	 	   element_values[CONTROL_LABEL].string_value);
	 }
#endif
	   

         if (label == NULL && 
	     element_values[CONTROL_LABEL].string_value == NULL && 
	     file_name != NULL)
         {
...

{
   ...
   container_type = (int)control_data->element_values[CONTROL_CONTAINER_TYPE].parsed_value;
   ...
   if (container_type != SUBPANEL && ((Boolean) panel.element_values[PANEL_DISPLAY_CONTROL_LABELS].parsed_value == False))
   {
      XtSetArg(al[0], XmNstring, NULL);
   }
   else
   {
      control_label = (char *)
                                                         ...
                                                         != CONTROL_BLANK)
      {
         ...
         {
            icon_label = XmStringCreateLocalized (control_label);
            XtSetArg(al[0], XmNstring, icon_label);
	 }
    ...
    XtSetArg(al[1], XmNimageName, icon_name);
