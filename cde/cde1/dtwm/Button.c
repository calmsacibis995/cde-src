/* $XConsortium: Button.c /main/cde1_maint/2 1995/10/03 17:22:26 lehors $ */
#ifdef REV_INFO
#ifndef lint
...
if (s_t > 0)
    ...
    _XmDrawShadows (XtDisplay(bg), XtWindow (bg), gc1, gc2, 
...
else if (_XmDifferentBackground ((Widget) bg, XtParent (bg)))
   (*(xmGadgetClassRec.gadget_class.border_unhighlight))((Widget) bg);
...
#endif /* _NO_PROTO */
{
    ...
    bg -> button.timer = 0;
    if (XtIsRealized (bg) && XtIsManaged (bg)) {
...
static void 
#ifdef _NO_PROTO
CallCallback( w, cb, reason, event )
        DtButtonGadget w ;
        XtCallbackList cb ;
        int reason ;
        XEvent *event ;
...
Widget 
#ifdef _NO_PROTO
DtCreateButtonGadget( parent, name, arglist, argcount )
        Widget parent ;
        char *name ;
        ArgList arglist ;
        Cardinal argcount ;
