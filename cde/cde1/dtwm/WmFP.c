/* $XConsortium: WmFP.c /main/cde1_maint/1 1995/07/18 01:55:24 drk $ */
/*****************************************************************************
 *
 *   File:         WmFP.c
 *
...
extern void WorkspaceModifyCB ();
extern void SessionRestoreData ();
extern void UnManageWindow ();

static void PushRecallSetData ();
static void EmbeddedClientSetData ();
static void EmbeddedClientSetGeometry ();
...
void
EmbeddedClientReposition (icon, x, adjust)
Widget    icon;
Position  x;
Dimension adjust;
...
void
EmbeddedClientReparent (client_name, icon)
char   * client_name;
Widget   icon;
...
#ifdef _NO_PROTO
void
EmbeddedClientRegister (control_data, install)
...
#ifdef _NO_PROTO
static void
EmbeddedClientSetGeometry (embedded_client_data)
WmFpEmbeddedClientData * embedded_client_data;
...
#ifdef _NO_PROTO
void
PushRecallRegister (control_data, install)
ControlData * control_data;
Boolean       install;
...

{
   ...
   if ((shell_geometry == NULL) || (panel.element_values[PANEL_GEOMETRY].string_value != NULL)) {
      Position  x;
      char      geometry_buffer[32];
...
#ifdef _NO_PROTO
Widget
WmPanelistAllocate (w, global_data, screen_data)
Widget w;
...
#ifdef _NO_PROTO
void
WmSubpanelPosted (display, shell_window)
Display * display;
Window    shell_window;
