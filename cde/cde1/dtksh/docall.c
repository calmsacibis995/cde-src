/* $XConsortium: docall.c /main/cde1_maint/1 1995/07/17 23:07:18 drk $ */
...
static int
#ifdef _NO_PROTO
call_postprompt( argv0,  pargs, tbls, freeit )
        char * argv0 ;
        unsigned long *pargs ;
        memtbl_t *tbls ;
        int *freeit ;
...
#endif /* _NO_PROTO */
{
	...
	for ( ; ; ) {
		myprompt(promptStr);
                strcpy(buf, quitStr);
		if (!gets(buf) || (xk_Strncmp(buf, quitStr, 2) == 0)) {
...
int
#ifdef _NO_PROTO
asl_set( argv0, tblarray, desc, pargs )
        char *argv0 ;
        memtbl_t *tblarray ;
        char *desc ;
        unsigned char **pargs ;
