/* $XConsortium: pathpath.c /main/cde1_maint/1 1995/07/18 00:04:01 drk $ */
/***************************************************************
*                                                              *
*                      AT&T - PROPRIETARY                      *
*                                                              *
...
#line 20
{
	...
	else if (s = (char*)a)
	{
		...
		if (strchr(s, '/') || ((s = cmd) || opt_argv && (s = *opt_argv)) && strchr(s, '/') || (s = *environ) && *s++ == '_' && *s++ == '=' && strchr(s, '/') && !strneq(s, "/bin/", 5) && !strneq(s, "/usr/bin/", 9) || *x && !access(x, 0) && (s = getenv("PWD")) && *s == '/')
		{
			if (!cmd) cmd = s;
			s = strcopy(path, s);
			for (;;)
