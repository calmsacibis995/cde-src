/* $XConsortium: sfvprintf.c /main/cde1_maint/4 1995/10/16 19:41:15 montyb $ */
/***************************************************************
*                                                              *
*                      AT&T - PROPRIETARY                      *
*                                                              *
...
#ifdef __ppc
#define GETARGL(elt,arge,argf,args,type,etype,fmt,t_user,n_user) \
	{ if(!argf) \
		va_copy( elt, va_arg(args,type) ); \
	  else if((*argf)(fmt,(char*)(&arge),t_user,n_user) < 0) \
		goto pop_fa; \
	  else	va_copy( elt, arge ); \
	}
#endif
...
#define RIGHTP	')'
		...
		case ':' : /* stack a pair of format/arglist */
			...
			if(!form)
				form = "";
#ifdef __ppc
			GETARGL(alist,alist,argf,args,va_list,va_list,'2',t_user,n_user);
			va_copy( fa->args, args );
#else
			GETARG(alist,alist,argf,args,va_list,va_list,'2',t_user,n_user);
			fa->args = args;
#endif
			fa->argf.p = argf;
			fa->extf.p = extf;
...
#ifdef __ppc
				va_copy( args, savarg ); /* extf failed, treat as if unmatched */
#else
