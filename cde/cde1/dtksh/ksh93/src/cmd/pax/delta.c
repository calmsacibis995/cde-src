/* $XConsortium: delta.c /main/cde1_maint/3 1995/10/14 00:21:15 montyb $ */
/***************************************************************
*                                                              *
*                      AT&T - PROPRIETARY                      *
*                                                              *
...
getdeltaops __PARAM__((Archive_t* ap, File_t* f), (ap, f)) __OTORP__(Archive_t* ap; File_t* f;){
	...
	n = 0;
	while (f->st->st_size > 0 && bread(ap, &c, 1L, 1L, 1) > 0)
	{
		f->st->st_size--;
		if (c == '\t' || c == '\n')
		{
...
getdeltaheader __PARAM__((register Archive_t* ap, register File_t* f), (ap, f)) __OTORP__(register Archive_t* ap; register File_t* f;){
	...
	if (ap->delta && (ap->delta->format == COMPRESS || ap->delta->format == DELTA))
	{
		...
		if (ap->delta->tab && f->name && (f->delta.base = (Member_t*)hashget(ap->delta->tab, f->name)))
			f->delta.base->mark = 1;
		if (f->st->st_size <= 0 || bread(ap, &c, 1L, 1L, 1) <= 0) f->delta.op = DELTA_create;
		else
		{
			f->st->st_size--;
			f->delta.op = c;
			getdeltaops(ap, f);
			if (f->delta.op == DELTA_create || f->delta.op == DELTA_update)
			{
				sum = ap->memsum;
				s = ap->delta->hdrbuf;
				n = 12;
				if (bread(ap, s, n, n, 1) > 0)
				{
...
putdeltaheader __PARAM__((register Archive_t* ap, register File_t* f), (ap, f)) __OTORP__(register Archive_t* ap; register File_t* f;){
	...
	if (f->delta.op && ap->delta && (n = ap->delta->hdr - ap->delta->hdrbuf))
	{
		...
		{
		case IBMAR:
			if (ap->format == IBMAR) mematoe(ap->delta->hdrbuf, ap->delta->hdrbuf, n);
			/*FALLTHROUGH*/
		default:
...
void
initdelta __PARAM__((Archive_t* ap), (ap)) __OTORP__(Archive_t* ap;){
...
deltabase __PARAM__((register Archive_t* ap), (ap)) __OTORP__(register Archive_t* ap;){
	...
	bp->parent = ap;
	bp->sum++;
	if ((bp->io.fd = open(bp->name, bp->io.mode)) < 0 || fstat(bp->io.fd, &st))
	...
	if (st.st_size)
	{
		...
		{
			if (lseek(bp->io.fd, 0L, SEEK_SET) != 0)
...
deltaout __PARAM__((Archive_t* ip, Archive_t* op, register File_t* f), (ip, op, f)) __OTORP__(Archive_t* ip; Archive_t* op; register File_t* f;){
	...
	{
		if (f->type == X_IFREG && (!d || f->st->st_mtime != d->mtime))
		{
			if (f->ordered)
			{
				f->ordered = 0;
...
deltapass __PARAM__((Archive_t* ip, Archive_t* op), (ip, op)) __OTORP__(Archive_t* ip; Archive_t* op;){
			...
			case DELTA_pass:
				if (validout(op, f) && selectfile(op, f))
				{
					...
					else if (state.filter)
					{
						...
						if (!tmp) tmp = pathtemp(NiL, NiL, error_info.id);
						if ((wfd = open(tmp, O_CREAT|O_TRUNC|O_WRONLY, S_IRUSR)) < 0)
						{
	...
	if (ip->delta && ip->delta->tab)
	{
		...
		if (pos = hashscan(ip->delta->tab, 0))
		{
			...
			{
				d = (Member_t*)pos->bucket->value;
				if (!d->mark && d->info->st->st_nlink > 1 && d->info->st->st_size > 0 && selectfile(op, d->info))
				{
					d->mark = 1;
...
delread __PARAM__((__V_* buf, int n, long off, Vddisc_t* vd), (buf, n, off, vd)) __OTORP__(__V_* buf; int n; long off; Vddisc_t* vd;){
	...
	if (off != dp->offset)
	{
		...
		else
		{
			...
			{
				message((-8, "delread: fd=%d n=%d off=%lu: seek error", dp->fd, n, off));
				return(-1);
			}
		}
	}
	...
	if (n <= 0)
	{
		message((-8, "delread: fd=%d n=%d siz=%lu off=%lu: seek error", dp->fd, n, dp->vd.size, dp->offset));
		return(0);
	}
	n = (dp->op & DELTA_BIO) ? bread(dp->bp, buf, 0, n, 1) : read(dp->fd, buf, n);
	if (n > 0)
		dp->offset += n;
	return(n);
}
...
delwrite __PARAM__((__V_* buf, int n, long off, Vddisc_t* vd), (buf, n, off, vd)) __OTORP__(__V_* buf; int n; long off; Vddisc_t* vd;){
	...
	if (bp = getbuffer(dp->fd))
	{
		...
		if ((dp->fd = open(state.tmp.file, O_CREAT|O_TRUNC|O_WRONLY, S_IRUSR)) < 0)
			error(3, "%s: cannot create delta temporary file", state.tmp.file);
		off = bp->next - bp->base;
		if (off > 0 && write(dp->fd, bp->base, off) != off)
			return(-1);
	}
...
{ __OTORP__(Archive_t* ip; Archive_t* ap; File_t* f; int op; )
	...
	__VA_START__(vp, op); __OTORP__(ip = va_arg(vp, Archive_t* );ap = va_arg(vp, Archive_t* );f = va_arg(vp, File_t* );op = va_arg(vp, int );)
	if (ap && ap->delta)
	{
		if (!ap->delta->version)
	...
	while (op)
	{
		...
		if (op & DELTA_OFFSET)
		{
			dp->base = va_arg(vp, long);
#if DEBUG
			if (mp) sfprintf(mp, " offset=%ld", dp->base);
#endif
		}
		if (op & DELTA_SIZE)
		{
			dp->vd.size = va_arg(vp, long);
#if DEBUG
			if (mp) sfprintf(mp, " size=%ld", dp->vd.size);
#endif
		}
	...
	if (gen->pfd)
	{
		...
		else if ((gen->fd = open(state.tmp.file, O_CREAT|O_TRUNC|O_WRONLY, S_IRUSR)) < 0)
			error(3, "%s: cannot create delta temporary file", state.tmp.file);
	}
	switch (version)
	{
	case DELTA_88:
...

		{
			...
			if (gen == &data[DELTA_DEL])
			{
				...
				n = delta(data[DELTA_SRC].vd.data, data[DELTA_SRC].vd.size, data[DELTA_TAR].vd.data, data[DELTA_TAR].vd.size, data[DELTA_DEL].fd) ? -1L : lseek(data[DELTA_DEL].fd, 0L, SEEK_END);
			}
			else
			{
				...
				for (op = 0; op < elementsof(data); op++)
				{
					...
					case DELTA_BUFFER:
						if (!tmp) tmp = pathtemp(NiL, NiL, error_info.id);
						if ((data[op].fd = open(tmp, O_CREAT|O_TRUNC|O_WRONLY, S_IRUSR)) < 0)
						...
						close(data[op].fd);
						if ((data[op].fd = open(tmp, O_RDONLY)) < 0)
				...
				n = update(data[DELTA_SRC].fd, data[DELTA_SRC].base, data[DELTA_DEL].fd, data[DELTA_TAR].fd) ? -1L : lseek(data[DELTA_TAR].fd, 0L, SEEK_END);
			}
		}
		break;
	default:
		n = (gen == &data[DELTA_DEL]) ? vddelta((Vddisc_t*)&data[DELTA_SRC], (Vddisc_t*)&data[DELTA_TAR], (Vddisc_t*)&data[DELTA_DEL], DELTA_WINDOW) : vdupdate((Vddisc_t*)&data[DELTA_SRC], (Vddisc_t*)&data[DELTA_TAR], (Vddisc_t*)&data[DELTA_DEL]);
		break;
	}
#if DEBUG
	if (mp)
	...
	if (gen->pfd)
	{
		...
		{
			close(gen->fd);
			if ((*gen->pfd = open(state.tmp.file, O_RDONLY)) < 0)
