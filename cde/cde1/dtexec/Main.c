 ...
 *
 * File:         Main.c
 * RCS:          $XConsortium: Main.c /main/cde1_maint/2 1995/10/09 13:56:48 pascale $
...
#include <errno.h>
#include <sys/wait.h>
#if defined(SVR4) || defined(_AIX) || defined(sco) || defined (__osf__)
#include <sys/time.h>
#else
#include <time.h>
#endif /* SVR4 */

#include "osdep.h"	/* select(2) mask width and bit manipulation marcos */
#include <Tt/tt_c.h>
...
extern void _DtexecUserMsgHandler();
...
long   allactivefdsG[mskcnt];	/* all possible fildes of interest. */
...
#endif /* _NO_PROTO */
{
    char **cmdLine;
    int    success;
    long   readfds[mskcnt], exceptfds[mskcnt];
    int    nfound;
   ...
   (void) gettimeofday (&startTimeG, &zoneG);
	...
	if (nfound > 0) {
		...
		if ( GETBIT(readfds, errorpipeG[0]) ) {
		    ...
		    if (tmpBuffer) {
			_DtexecUserMsgHandler( argv[0], cmdLine, tmpBuffer );
			free( tmpBuffer );
		    }
	...
	if (nfound == 0) {
		 ...
		 */
		if (rediscoverUrgentSigG > ((1000000/SHORT_SELECT_TIMEOUT)*5) ) {
#if defined(__aix) || defined (__osf__)
		    PanicSignal(0);
#else
		    PanicSignal();
		...
		if (shutdownPhaseG == SDP_DONE_REPLY_WAIT) {
		     ...
		     */
		    rediscoverSigCldG++;
		    if (rediscoverSigCldG > ((1000000/SHORT_SELECT_TIMEOUT)*300)) {
