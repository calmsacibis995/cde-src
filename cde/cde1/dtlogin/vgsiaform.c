 ...
 * $EndLog$
 */
#pragma ident "@(#)$RCSfile: vgsiaform.c $ $Revision: /main/cde1_maint/1 $ (DEC) $Date: 1995/10/09 00:14:55 $"
...
SiaForm(SiaFormInfo *siaFormInfo)
{
    ...
    XtSetArg(argt[i], XmNbottomAttachment, 	XmATTACH_FORM		); i++;
    XtSetArg(argt[i], XmNtraversalOn, 		True			); i++;
    xmstr = ReadCatalog(MC_LABEL_SET, MC_CANCEL_LABEL, MC_DEF_CANCEL_LABEL );
    XtSetArg(argt[i], XmNlabelString,		xmstr			); i++;
