/* $XConsortium: vglang.c /main/cde1_maint/3 1995/12/01 14:02:48 rcs $ */
/*                                                                      *
 ...
 */
    ...
    /* 
     * Don't try to do translated descriptions for chooser.
     * The resources are gathered in dtlogin and don't switch
     * when language settings are updated within chooser.
     */

    if ( amChooser ) return(label);
...
static Boolean
#ifdef _NO_PROTO
_enumLangCmdNext(state, lang, desc)
  void *state;
  char **lang;
  char **desc;
#else
...
static Boolean
#ifdef _NO_PROTO
_enumLanglistNext(state, lang, desc)
  void *state;
  char **lang;
  char **desc;
#else
    ...
    */
	...
	for (k = 0; k < nlang; k++) {
	    ...
	    XtSetArg(argt[i], XmNrecomputeSize,		True		); i++;
            ...
            xmstr = XmStringCreateLtoR(list[k].desc, XmFONTLIST_DEFAULT_TAG);
	    ...
	    XtManageChild(button);
            tostr=catgets(nl_fd, MC_LABEL_SET, MC_TO_LABEL, MC_DEF_TO_LABEL);
		...
		sprintf(cblab, "%s %s %s", 
			list[first].lang, tostr, list[last].lang);

		xmstr = XmStringCreateLtoR(cblab, XmFONTLIST_DEFAULT_TAG);
		XtSetArg(argt[i], XmNlabelString,	xmstr		); i++;
