...
static void Audit();		/* construct a self-audit record	   */
static int  CheckPassword();	/* validate user's password		   */
static int  CountUsers();	/* see if user has exceeded the maximum	   */
static int  CountUsersStrict();	/* see if user has exceeded the maximum	   */
static int  PasswordAged();	/* see if password has aged		   */
static void WriteBtmp();	/* write failed login attempt to btmp file */
...
#endif /* _NO_PROTO */
{
	...
	sprintf(txtptr, "User= %s uid=%d audid=%d%s", p->pw_name,
		p->pw_uid, p->pw_audid, msg);
...
static int
#ifdef _NO_PROTO
CheckPassword(name, passwd, ppwd)
    char	*name;
    char	*passwd;
    struct passwd **ppwd;
#else
...
int 
#ifdef _NO_PROTO
Authenticate( d, name, passwd, msg )
	struct display *d;
        char *name ;
        char *passwd ;
        char **msg ;
...
#endif /* _NO_PROTO */
{
   ...
   /*
    * If "/etc/nologin" exists, lock out all user logins except for user "root".
    */

    if ( access(NO_LOGIN_FILE,R_OK) == 0 && strcmp("root", name) != 0 ) {
        Debug("File %s found.\n",NO_LOGIN_FILE);
        Debug("Login dissabled for all except user 'root'.\n");
	return(VF_NO_LOGIN);
    }
    ...
    */
    ...
    /*
     * Get Login default params
     */
        if (defopen(SOLARIS_LOGIN_DEFAULTS) == 0) {
    ...
    */
...
#ifdef PAM 
    status = PamAuthenticate("dtlogin", d->name, 
			      passwd, passreq, name, ttyLine);
    ...
    switch(status) {
	    ...
	    status = VF_PASSWD_AGED;
	    break;

	case PAM_OPEN_ERR:
	    status = (passwd ? VF_INVALID : VF_CHALLENGE);
	    break;
...
#else
    status = solaris_authenticate("dtlogin", d->name, 
				  passwd, passreq, name, ttyLine);
    ...
    /* Get user's shell, verify it is executable by user */ 
	...
	if (!p || !p->pw_shell) {
	    ...
	    seteuid(p->pw_uid);
	    if (access(p->pw_shell, X_OK) != 0) status = VF_BAD_SHELL;
	    seteuid(0);
	}
    }

    if (status == VF_OK && p->pw_uid != 0 && access("/etc/shells",R_OK) == 0) {
	char * shell;

        /* verify non-root user's shell is valid for this system  */

	while ((shell=(char*)getusershell()) != NULL) {
	    if ( strcmp(shell, p->pw_shell) == 0) break; 
	}

	if (shell == NULL) status = VF_ETC_SHELLS;

	endusershell();
    }

    /* Additional security checks for a root user id login */

    if ( status == VF_OK && p->pw_uid == 0 ) {
        char* line_dev = _sdt_create_devname(ttyLine);

        /* 
   	 * This is a root user id login.  If the login device does not match
         * the default console device, dissallow this login.
	 */

         if (line_dev) Debug("line_dev = %s\n", line_dev);
         if (console) Debug("console= %s\n", console);

         if ( console != NULL && strcmp(line_dev,console) != 0 ) {
	    audit_login_not_console();
            Debug("%s\n", MC_DEF_NOT_CONSOLE);
	    sleep(SOLARIS_LOGIN_SLEEPTIME);
	    status = VF_NOT_CONSOLE;
	 }

	 free(line_dev);
    }

    if (passreq != NULL) free(passreq);	
    if (console != NULL) free(console);	
...
void
#ifdef _NO_PROTO
GetLoginInfo(d, loginType, ttyName, hostname)
    struct display *d;
    int *loginType;
    char *ttyName;
    char **hostname;
...
void 
#ifdef _NO_PROTO
dt_failedlogin(name, ttyName, hostName)
	char *name;
	char *ttyName;
	char *hostName;
#else
