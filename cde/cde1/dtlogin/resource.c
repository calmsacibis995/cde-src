 ...
 *
 * $XConsortium: resource.c /main/cde1_maint/3 1995/10/16 13:11:43 mgreess $
 *
...
#ifdef _NO_PROTO
  void CleanUpName() ;
  static char * getFullFileName();
  extern char * qualifyWithFirst();
#else
...
#ifndef DEF_CHOOSER
#define DEF_CHOOSER CDE_INSTALLATION_TOP "/bin/dtchooser"
#endif
...
#endif /* _NO_PROTO */
{
	...
	CleanUpName (d->class ? d->class : d->name, dpyClass, sizeof (dpyClass));
        /* 
	 * Get last login language value from the system
	 */

	if (d->displayType.location == Local || strncmp(d->name,":",1) == 0) {
	...
	if ((fd = fopen(lastlangfile,"r")) != NULL)  {
	    fgets(lang_buff, NL_LANGMAX+1, fd);
	    fclose(fd);
	    lang = lang_buff; 
	}
	...
	if (lang == NULL) {
            /* 
	     * No last login language value was found, 
	     * use the system's default login language. 
	     */
            lang = getenv("LC_MONETARY");
	...
	for (i = 0; i < NUM_DISPLAY_RESOURCES; i++) {
        ...
        verify.userEnviron = (char **) setLang(d, verify.userEnviron, NULL);
...
static char *
#ifdef _NO_PROTO
getFullFileName(name, special, lang)
    char *name;
    int   special;
    char *lang;
#else
