/* $XConsortium: vgapollo.c /main/cde1_maint/2 1995/10/23 09:23:13 gtsang $ */
/*                                                                      *
...
static boolean	CheckLogin();	/* check login validity			   */
static boolean	CheckPassword();/* check just name/password		   */
...
static boolean
#ifdef _NO_PROTO
CheckLogin(user, passwd, host, status)
    char	*user;
    char	*passwd;
    char	*host;
    status_$t	*status;
