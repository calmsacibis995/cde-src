 ...
 *
 * $XConsortium: session.c /main/cde1_maint/9 1995/11/30 21:58:32 montyb $
 *
...
static int  AbortClient() ;
static void DeleteXloginResources() ;
       int  LoadXloginResources() ;
static int  ErrorHandler() ;
static int  IOErrorHandler() ;
static int  ManageGreeter() ;
static void RunGreeter() ;
static void SessionExit() ;
static void SessionPingFailed() ;
static int  StartClient() ;
static SIGVAL catchAlrm() ;
static SIGVAL catchTerm() ;
static SIGVAL waitAbort() ;
...
static void release_aix_lic();
#endif

#if defined (_AIX) && !defined (_POWER)
static int session_execve();
#else 
#define session_execve(A,B,C) execve(A,B,C)
#endif

#  ifdef __KERBEROS
      static void SetTicketFileName();
#  endif /* __KERBEROS */

char * _ExpandLang();
void LoadAltDtsResources();
...
void LoadAltDtsResources( struct display *d, char *tmpname );
...
static struct greet_info	greet={NULL, NULL, NULL};
struct verify_info	verify;
...
#endif /* _NO_PROTO */
{
   ...
   /* set LOCATION env var */
...

    {
        ...
        pwd = getpwnam("root");
        if(pwd)  {
           bzero(gid,strlen(gid));
           sprintf(gid,"%d",pwd->pw_gid);
           verify.systemEnviron = setEnv (verify.systemEnviron,
                                      "ROOT_GID",
                                      gid);
        }
...
#endif /* DEF_NETWORK_DEV */

	if ( solaris_setdevperm(ttyLine, verify.uid, verify.gid) == 0 ) {
	    SessionExit (d, OBEYSESS_DISPLAY);
	}
    }
#endif /* sun */
...
#endif /* _NO_PROTO */
{
    ...
    if (d->resources && d->resources[0]) { 
	    ...
	    Debug("LoadXloginResources - cant access %s\n", resources);
            free(resources);
	    Debug("LoadXloginResources - falling back to C.\n");
            resources = _ExpandLang(d->resources, "C");
            if (access (resources, R_OK) != 0) {
                /** can't find a resource file, so bail **/
	        Debug("LoadXloginResources - cant access %s.\n", resources);
		Debug ("LoadXloginResources - unable to find resource files\n");
                free(resources);
                return(-1);
            }
        }
	...
	LoadAltDtsResources(d, tmpname); 

	cmd[0] = '\0';
	
	strcat (cmd, "/usr/bin/cat ");
	
	strcat(cmd, resources);
	strcat(cmd, " ");
	strcat(cmd, tmpname);

        snprintf (&cmd[strlen(cmd)], sizeof(cmd)-strlen(cmd)-1, 
		" | %s%s %s -display %s -quiet -load",
			auth_key, authority, d->xrdb, d->name); 

	Debug ("Loading resource data: %s\n", cmd);
...
char *tmpname;
#else
LoadAltDtsResources(struct display *d, char *tmpname)
#endif /* _NO_PROTO */
{
    DIR         *dirp;
    struct dirent *dp;
    ...
    for(j = 0; j < 2 ; ++j) {
        ...
        if (access (resources, R_OK) == 0) {
           ...
           free (resources);
           Debug("LoadAltDtsResources() falling back to C.\n");
           resources = _ExpandLang(dirname[j], "C");
           strcpy(dirname[j],resources);
           if (access (dirname[j], R_OK) != 0) {
               Debug("LoadAltDtsResources()  unable to find resource dir\n");
	   }
	} 
	free (resources);
    }
    ...
    for(j = 0; j < 2 ; ++j) {
     if((dirp = opendir(dirname[j])) != NULL) {
		...
		&& (strcmp(dp->d_name,DOTDOT) != 0)) {
	       ...
	       if(j == 0) {
                  sprintf (res_file, "%s%s", dirname[1],dp->d_name);

	       if((access (res_file, R_OK)) == 0)
		  continue; /* skip this package, there is one in /etc/dt */
	       }
	    
               sprintf (res_file, "%s%s", dirname[j],dp->d_name);
               userDb = XrmGetFileDatabase(res_file);
               Debug ("Loading alternate desktop description in : %s\n", res_file);
 
           if ( XrmGetResource(userDb,
                   "Dtlogin*altDtsIncrement",  "Dtlogin*AltDtsIncrement",
                   &rmtype, &rmvalue ) ) {
		 ...
		 * remove the trailing spaces 
           ...
           if((strcmp(tempbuf, "True") == 0) 
            ...
            if ( XrmGetResource(userDb,
               ...
               XrmPutStringResource(&defDb, altdtres, rmvalue.addr);
            }
            else
               continue;
    ...
    sprintf(tempbuf,"%d", i);
    XrmPutStringResource(&defDb, "Dtlogin*altDts", tempbuf); 
    
    XrmPutFileDatabase(defDb, tmpname);

    /* verify.c uses XresourceDB - merge altdt(n) with Xresources info. 
       so verify.c can retrieve the info. needed */
    XrmMergeDatabases(defDb, &XresourceDB);
}
...
#endif /* _NO_PROTO */
{
  ...
  */
  ...
  if ((fp = open("/etc/security/monitord_pipe", O_RDWR, 0600)) != NULL)
  {
    write(fp, &release_me, sizeof(release_me));
    close(fp);
  }
...
#endif /* _NO_PROTO */
{
    ...
    int		firstlogin = FALSE;	/* do we run firstlogin session? */
...
#define NOPAG 0xffffffff
	...
	for (f = verify->argv; *f; f++) {
		...
		if ( strcmp(*f, FIRST_LOGIN) == 0) firstlogin= TRUE;
...
#   endif
...
#ifndef sun
	Account(d, user, NULL, getpid(), USER_PROCESS, NULL);
#endif
...
#if defined(PAM)
	if (PamSetCred( verify->argv[0], user, verify ) > 0 ) {
            Debug("Can't set User's Credentials (user=%s)\n",user);
	    return(0);
	} 
#else
	...
	if ( solaris_setcred(verify->argv[0], 
			     user, verify->uid, verify->groups[0]) > 0 ) {
            Debug("Can't set User's Credentials (user=%s)\n",user);
	    return(0);
	}
#endif /* SUNAUTH */
...
#      if (defined(__hpux) || defined(__osf__))
	initgroups(user, -1);
#      else
	setgroups (verify->ngroups, (unsigned int *)verify->groups);
#      endif
 ...
 */
	...
	if (home) {
		...
		else if(!failsafe && !firstlogin) {

			/* save user's session choice */

			_DtCreateDtDirs(NULL);

			strcpy(lastsessfile,home);
...
#  endif  /* ! NDEBUG */
	...
	if ( password ) {
		...
		Debug("Xpasswd NLSPATH=%s\n", path);
                verify->userEnviron = setEnv(verify->userEnviron, 
					     "NLSPATH", path);
	    } else {
		Debug("Xpasswd NLSPATH not found\n");
	    }

	    failsafeArgv[i++] = "/usr/dt/config/Xpasswd";
	}
	else {
...
RefreshGreeter()
#else
RefreshGreeter( int arg )
#endif /* _NO_PROTO */
{
    Debug ("Caught SIGHUP, RefreshGreeter\n");
    kill(greeterPid, SIGHUP);
#if defined(SYSV) || defined(SVR4)
    signal (SIGHUP, RefreshGreeter);
#endif
}
...
#  endif
	...
	case 0:
	    ...
	    if (d->displayType.location == Local) {
		sleep(1);   /* wait for any inprogress login to register */
                GettyRunning(d); /* refresh gettyState */
#ifdef sun
		if (daemonMode && d->gettyState == DM_GETTY_USER) {
		   /* 
		    * A user is already logged in on the console.  If in
		    * daemonMode, resuspend until user logs out of the
                    * text based console login to prevent the system from
                    * having 2 active console logins.
		    */
		   exit (NOTIFY_ABORT_DISPLAY);
		}
#endif
		if (d->gettyState != DM_GETTY_USER) {
		    /* Allow "Command Line Login" to be used in dtgreet */
		    env = setEnv(env, LOCATION, "local");
		}
	    }
	    else {
	...
	default:
	    ...
	    /*
	     * While Greeter is running, a SIGHUP will refresh the Greeter.
	     */
	    signal (SIGHUP, RefreshGreeter);
	    ...
	    /* 
	     * Enable Pam callback access to Greeter communication pipes
	     */ 

	    #if defined(PAM)
		PamUseGreetPipe(TRUE);
	    #endif

	    #if defined(SUNAUTH)
		solaris_use_greet_pipe(TRUE);
	    #endif
	    ...
	    /* 
	     * Once Greeter has ended, the reset SIGHUP signal 
	     * should longer have effect on child dtlogins.
             */
	    ...
	    /* 
	     * Dissable Pam callback access to Greeter communication pipes
	     */ 

	    #if defined(PAM)
		PamUseGreetPipe(FALSE);
	    #endif

	    #if defined(SUNAUTH)
		solaris_use_greet_pipe(FALSE);
	    #endif

	     /* 
	      *  Close Greeter communication pipes 
	      */

	    close(response[0]); /* Close read end of greet response pipe */
	    close(request[1]); /* Close write end of greet request pipe */
	  ...
	  Debug("waitVal - status is  %d\n", waitVal(status));
	   if(waitVal(status) > NOTIFY_ALT_DTS)
		...
		if (d->serverPid >= 2)
            ...
            case NOTIFY_REMOTE_CHOOSE_LOGIN:
                    exit(START_CHOOSER);
 
            case NOTIFY_REMOTE_HOST_LOGIN:
                    exit(START_REMOTE_GREET);
...
#endif /* _NO_PROTO */
{
    ...
    if (state->request->opcode != state->response->opcode)
    {
      ...
      */
      switch(state->response->opcode)
      {
        ...
        case REQUEST_OP_TERMINATE:
          {
           /*
            * Received "terminate" string from greeter 
            */
            Debug("Greeter terminated\n");
	    return(TRUE);
          }
          break;
    
        default:
          Debug("UNEXPECTED RESPONSE!, opcode=%d\n", state->response->opcode);
#ifndef SIA
          return(TRUE);
#endif
          break;
  ...
  switch(state->id)
  {
	...
	bzero(state->request, REQUEST_LIM_MAXLEN);

        state->response= (ResponseHeader *)malloc(REQUEST_LIM_MAXLEN);
	bzero(state->response, REQUEST_LIM_MAXLEN);
    ...
    case GREET_STATE_ERRORMESSAGE:
      {
        ...
        */
        ...
        switch(state->vf)
        {
          ...
          case VF_BAD_SHELL:     SETMC(msg, BAD_SHELL); break;
          case VF_NO_LOGIN:      SETMC(msg, NO_LOGIN); break;
          case VF_NOT_CONSOLE:   SETMC(msg, NOT_CONSOLE); break;
          case VF_ETC_SHELLS:    SETMC(msg, ETC_SHELLS); break;
#ifdef BLS
	...
	if ( stat(SDTLOGIN_DIR, &statbuf) != 1 ) { 
	    ...
	    if ((fp = fopen(lastlangfile,"w")) == NULL)
                Debug("Unable to open file for writing: %s\n",lastlangfile);
            else{
		Debug("Saving lang (%s) in %s\n", lang, lastlangfile);
                fputs(lang, fp);
                fclose(fp);
            }
	}
...
#endif /* _NO_PROTO */
{
    ...
    if (env = (char *)getenv("KRBTKFILE")) {
	    ...
	    (void)sprintf(krb_ticket_string, "%s%d.%s", TKT_ROOT, uid, lhost);
        } else {
