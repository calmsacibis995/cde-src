/* $XConsortium: dtchooser.c /main/cde1_maint/5 1995/11/05 11:27:30 mgreess $ */
/*                                                                      *
...
static void ErrorHandler();	/* X protocol error handler		   */
static Widget InitToolKit();	/* initialize the toolkit		   */
void MakeRootCursor();	/* make workspace cursor, if requested     */
void MakeBackground();	/* make shell and table widgets		   */
void MakeButtons();	/* make ok, clear, options, help buttons   */
void MakeLogin();	/* make login/password areas		   */
void MakeOptionsProc();	/* timeout proc to build options menu	   */
static SIGVAL Terminate();	/* signal -TERM handler			   */
extern void DoCancel (); 	/* chooser cancel callback	   */
...
static void ErrorHandler( Display *dpy, XErrorEvent *event) ;
...
Widget remote_host_dialog = NULL;
...
#endif /* _NO_PROTO */
{
    ...
    xmstr = ReadCatalog(MC_CHOOSER_SET, MC_UPDATE_LABEL, MC_DEF_UPDATE_LABEL);
    XtSetArg(argt[i], XmNlabelString,		xmstr); 		i++;
...
#endif /* _NO_PROTO */
{
    ...
    i = InitArg(LabelG);
    xmstr = ReadCatalog(MC_CHOOSER_SET, MC_CHOOSER_TITLE, MC_DEF_CHOOSER_TITLE);
    ...
    /** list head **/
    i = InitArg(LabelG);
    xmstr = ReadCatalog(MC_CHOOSER_SET, MC_CHOOSER_HEADING,
...
#endif /* _NO_PROTO */
{
    ...
    case help_chooser:
	xmstr = ReadCatalog(MC_HELP_SET, MC_HELP_CHOOSER, MC_DEF_HELP_CHOOSER);
	...
	w = XmCreateInformationDialog(table, "help_message", argt, i);
        ...
        XtSetArg(argt[i], XmNscrollingPolicy, XmAUTOMATIC); i++;
        XtSetArg(argt[i], XmNeditable, False); i++;
        XtSetArg(argt[i], XmNvalue, strdup(catgets(nl_fd, MC_HELP_SET,
				   MC_HELP_CHOOSER, MC_DEF_HELP_CHOOSER))); i++;
        text = XmCreateScrolledText(w, "help_message_text", argt, i);
...
#endif /* _NO_PROTO */
{
    ...
    if ( lang_menu != NULL ) {
      ...
      if(strcmp(getenv(LOCATION), "local") == 0) {
          i = InitArg(PushBG);
          xmstr = ReadCatalog(MC_LABEL_SET, MC_LOGIN_SCREEN_LABEL, 
                                              MC_DEF_LOGIN_SCREEN_LABEL);
          XtSetArg(argt[i], XmNlabelString,             xmstr           ); i++;
          ...
          XtAddCallback(options_item[j], XmNactivateCallback,
                  DoCancel, (XtPointer) OB_RETURN_TO_LOGIN);
          j++;
     ...
     */
    i = k = InitArg(PushBG);
    xmstr = ReadCatalog(MC_CHOOSER_SET, 
			MC_CHOOSER_RS_LABEL, 
			MC_DEF_CHOOSER_RS_LABEL);
    XtSetArg(argt[i], XmNlabelString,			xmstr		); i++;
    options_item[j] = XmCreatePushButtonGadget(options_menu,
