...
#if defined(SVR4) || defined(aix) || defined(hpux)
#if !defined(SIG_PF)
...
main(int argc, char **argv)
{
    ...
    _XmGetPixelData( DefaultScreen( display ), &colorUse, pixels,
...
#if defined(aix)
#define AIX_STRING_LIST		(const char **)
#else
#define AIX_STRING_LIST
    ...
    Tt_status status;
    snoopStream.open( snoopFile,
		      getenv( "libC_1187592_fixed" ) ? ios::app : ios::out );
    if (! snoopStream.good()) {
	    clog << "ttsnoop: " << snoopFile << ": "
		 << strerror( errno ) << endl;
    }
    ostrstream envStr;
    ...
    if (optImmediateTtOpen) {
	    ...
	    if (tt_is_err( status )) {
		    clog << "ttsnoop: ttdt_open() = " << status << endl;
		    exit( 4 );
	    }
