/* $XConsortium: fileIO.c /main/cde1_maint/4 1995/10/23 08:55:09 gtsang $ */
/*********************************************************************
...
void
#ifdef _NO_PROTO
Do_FileIO( wid, client_unused, callback_data )
        Widget wid ;
...
#endif /* _NO_PROTO */
{
  ...
  if (SaveMeNot){
  tmpSave = NULL;
  XmStringGetLtoR(callback_data->value, XmFONTLIST_DEFAULT_TAG, &tmpSave);
  }
...
#endif /* _NO_PROTO */
{
  char *base_name, *suffix, fname[256], *tmp, *tmp2;
...
#ifdef COLON_NOT_VALID_IN_FILE_NAME
  /*** put a "good" file name in fname... this means:        ***/
  /*** - convert from "<host>:/" to "/nfs/<host>/"           ***/
  /***   (probably should remove /nfs/<host> if local host)  ***/
  /*** - if we got a NULL base_name, return FALSE             ***/
  /*** - if it's not a regular file, don't use it            ***/
  /*** - if 0 length... clear icon, copy name, change title  ***/
  /***   note: 0 length files allowed so user can create new ***/
  /***         icon file then edit it directly using dtfile ***/
  /***                                                       ***/
  /*** - if none of the above, continue... (xpm/xbm file)    ***/
  /***                                                       ***/
  ...
  tmp  = strchr(fnameIn, ':');
  tmp2 = strchr(fnameIn, '/');
  if (tmp && tmp2 && (tmp2 == tmp+1))   /* form is "<foo>:/<bar>", convert it */
  {
    strcpy(fname, "/nfs/");
    tmp[0] = '\0';
    strcat(fname, fnameIn);         /* host */
    tmp[0] = ':';
    strcat(fname, tmp+1);           /* path */
  }
  else     /* wasn't in form "<host>:/<path>" so use name as is... */
#endif /* COLON_NOT_VALID_IN_FILE_NAME */
  {
  ...
  else  /* Dialog is for File-Save or File-Save_As menu item */
  {
    ...
    XSync(dpy, 0);
	 ...
	 else
	 {
         ...
         tmpXmStr = XmStringCreateLtoR (dirStr, XmNdirectory);
         n = 0;
