/* $XConsortium: utils.c /main/cde1_maint/5 1995/10/23 09:01:38 gtsang $ */
/*********************************************************************
...
extern void Repaint_Tablet();
extern void Init_Widget_List();
extern void Init_Pen_Colors();
extern void Init_Color_Table();
extern void RegisterDropSites();
extern void Abort();
extern void DoErrorDialog();
#else
...
void
#ifdef _NO_PROTO
Icon_Coords( normal_x, normal_y, fat_x, fat_y )
        int normal_x ;
        int normal_y ;
        int *fat_x ;
        int *fat_y ;
...
void
#ifdef _NO_PROTO
Tablet_Coords( fat_x, fat_y, raw_x, raw_y )
        int fat_x ;
        int fat_y ;
        int *raw_x ;
        int *raw_y ;
...
void
#ifdef _NO_PROTO
Repaint_Tablet( win, x, y, width, height )
        Window win ;
        int x ;
        int y ;
        int width ;
...
void
#ifdef _NO_PROTO
Transfer_Back_Image( x1, y1, x2, y2, tflag )
        int x1 ;
        int y1 ;
        int x2 ;
        int y2 ;
...
/*** once.                                                       ***/
  ...
  xpm_WriteAttribs.colorTable = (char ***) calloc(NUM_PENS, sizeof(char **));
  xpm_WriteAttribs.pixels = (Pixel *) calloc(NUM_PENS, sizeof(Pixel));
  if (!xpm_WriteAttribs.colorTable)
    Abort(GETSTR(10,44, "No memory(3)"));
  for (i=0; i<NUM_PENS; i++) {
    xpm_WriteAttribs.colorTable[i] = (char **) calloc(6, sizeof(char *));
    if (!xpm_WriteAttribs.colorTable[i])
  ...
  for (i=0; i<NUM_PENS; i++) {
    ...
    for (j=0; j<6; j++) {
      if (color_table[i][j] && strlen(color_table[i][j]) > 0) {
        xpm_WriteAttribs.colorTable[i][j] = (char *)
                        XtMalloc(strlen(color_table[i][j])+1);
        if (!xpm_WriteAttribs.colorTable[i][j])
          Abort(GETSTR(10,48, "No memory(5)"));
        strcpy(xpm_WriteAttribs.colorTable[i][j], color_table[i][j]);
       } /* if */
     } /* for(j...) */
   } /* for(i...) */
...
void
#ifdef _NO_PROTO
Init_Icons( width, height, saveFlag )
        Dimension width ;
        Dimension height ;
        Boolean saveFlag ;
#else
...
/*** Resizing the existing icon ***/
  ...
  i = 0;
  local_str = XmStringCreateLtoR(text, XmFONTLIST_DEFAULT_TAG);
...
static void
#ifdef _NO_PROTO
TransferCallback( w, client_data, call_data)
        Widget w ;
...
static void
#ifdef _NO_PROTO
AnimateCallback( w, client_data, call_data)
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
void
#ifdef _NO_PROTO
stat_out( msg, arg0, arg1, arg2, arg3, arg4, arg5, arg6 )
        char *msg ;
        char *arg0 ;
        char *arg1 ;
        char *arg2 ;
...
int
#ifdef _NO_PROTO
PixelTableLookup(pixelIn, allocNew)
        Pixel pixelIn;
        Boolean allocNew;
#else
PixelTableLookup(
...
#endif /* _NO_PROTO */
{
  ...
  i = 0;
  local_str = XmStringCreateLtoR(str, XmFONTLIST_DEFAULT_TAG);
...
int
#ifdef _NO_PROTO
LoadGrabbedImage(x, y, width, height)
        int x;
        int y;
        int width;
        int height;
...
XErrorEvent *error_event;
{
    ...
    _DtPrintDefaultError(disp, error_event, error_msg);
...
#endif /* _NO_PROTO */
{
    ...
    XSetIconName(dpy, XtWindow(dtIconShell), name);
