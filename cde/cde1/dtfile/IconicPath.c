/* $XConsortium: IconicPath.c /main/cde1_maint/2 1995/08/29 19:38:20 gtsang $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           IconicPath.c
...
static void
#ifdef _NO_PROTO
ButtonCallback( w, client_data, call_data )
        Widget w ;
...
# define MOVE_OBJECT(w,_x,_y) \
   ...
   {
      xm_string = XmStringCreateLtoR("...", XmFONTLIST_DEFAULT_TAG);
      n = 0;
      ...
      {
         _XmResizeObject(ip->iconic_path.dotdot_button,
      ...
      for (;;)
      {
         ...
         if (i >= ip->iconic_path.num_components)
         {
            ...
            if (fileLabel = DtDtsDataTypeToAttributeValue(
                            ...
                            NULL))
            {
               xm_string = XmStringCreateLtoR(fileLabel,XmFONTLIST_DEFAULT_TAG);
               DtDtsFreeAttributeValue(fileLabel);
            }
            else
            {
               xm_string = XmStringCreateLtoR(name, XmFONTLIST_DEFAULT_TAG);
            }
...
next_component:
   ...
   for (i = 0; i < ip->iconic_path.num_components; i++)
   {
      ...
      if (ip->iconic_path.components[i].button->core.width < (Dimension)twidth)
      {
         ...
         /* resize */
         _XmResizeObject(ip->iconic_path.components[i].button, twidth,
                         ip->iconic_path.components[i].button->core.height, 0);
      }
       ...
       strcmp(msg_buf, ip->iconic_path.msg_text) != 0)
   {
      ...
      if (ip->iconic_path.status_label)
      {
         ...
         if (XtIsRealized(ip) && ip->iconic_path.status_label->core.x > 0)
         ...
         xm_string = XmStringCreateLtoR(msg_buf, XmFONTLIST_DEFAULT_TAG);
         XtSetArg (args[0], XmNlabelString, xm_string);
   ...
   if (ip->iconic_path.status_msg)
   {
      ...
      {
         _XmMoveObject(ip->iconic_path.status_label, x, y);
      }
...
#endif /* _NO_PROTO */
{
    ...
    _XmRedisplayGadgets( (Widget) ip, event, region);
    return ;
}
...
#endif /* _NO_PROTO */
{
    ...
    _XmNavigChangeManaged((Widget) ip) ;
...
Widget
#ifdef _NO_PROTO
_DtCreateIconicPath( p, name, args, n )
        Widget p ;
        String name ;
        ArgList args ;
        Cardinal n ;
