/* $XConsortium: FileManip.c /main/cde1_maint/4 1995/10/09 14:48:23 pascale $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           FileManip.c
...
{
   int access_priv;
#if defined(SVR4) || defined(_AIX) || defined(sco) || defined(__hpux)
   int save_ruid;
   int save_rgid;
#endif /* SVR4 */
...
void
#ifdef _NO_PROTO
FileOperationError( w, message1, message2 )
        Widget w ;
        char *message1 ;
        char *message2 ;
#else
...
static int
#ifdef _NO_PROTO
Check( w, spth, dinode, mode, errorHandler )
        Widget w ;
        register char *spth ;
      ...
      if (sbuf.st_ino == dinode)
      {
         if (errorHandler)
         {
            if (mode == COPY_FILE)
              ...
              tmpStr = GETMESSAGE(11,16, "A folder cannot be moved, copied or linked into itself.\n%s");
            msg = XtNewString(tmpStr);
            if (mode == COPY_FILE)
...
static Boolean
#ifdef _NO_PROTO
MoveDir( w, source, target, sourceStatInfo, errorHandler, targetRtn,type )
        Widget w ;
...
static Boolean
#ifdef _NO_PROTO
CopyDir( w, mode, from, to, isContainer, s1, errorHandler, checkForBusyDir, type )
        Widget w ;
        int mode ;
        register char *from ;
        register char *to ;
...
Boolean
#ifdef _NO_PROTO
FileManip( w, mode, from, to, isContainer, errorHandler, checkForBusyDir, type )
        Widget w ;
        int mode ;
        register char *from ;
        register char *to ;
...
#endif /* _NO_PROTO */
{
   ...
   {
      if ((stat (to, &s3) >= 0) &&
#if defined(__hp_osf) || (__ultrix) || defined(__osf__)
   ...
   {
      /*  set for user  */
      /* Bug fix 4015290. The following call chown() needs to be
      commented because it forcibly sets (process) group id even
      if parent directory has sticky bit set. The file should have
      group id same as parent directory's group id if stick bet set. 
      Moreover, uid and gid will be by default of the creating
      process even WITHOUT this call.
      */
