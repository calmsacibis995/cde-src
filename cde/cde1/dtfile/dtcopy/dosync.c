/* $XConsortium: dosync.c /main/cde1_maint/2 1995/08/29 19:42:59 gtsang $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           dosync.c
...
#ifdef _NO_PROTO
int (*syncConfirmCallback)() = NULL;
int (*syncErrorCallback)() = NULL;
#else
int (*syncConfirmCallback)(FileOp op,
                           char *sname, int stype,
                           char *tname, int ttype,
                           char *link) = NULL;
int (*syncErrorCallback)(FileOp op, char *fname, int errno) = NULL;
#endif
...
static int
#ifdef _NO_PROTO
dirErrorCallback(fname, errno)
	char *fname;
	int errno;
#else
dirErrorCallback(char *fname, int errno)
#endif
{
  erase_rc = syncErrorCallback(op_delete, fname, errno);
  return erase_rc;
}
...
static void
#ifdef _NO_PROTO
targetLink(source, target, tlPP, msgPP)
	char *source;
	char *target;
	char **tlPP;
	char **msgPP;
...
static int
#ifdef _NO_PROTO
doCopyLink(sname, sP, tname, delP)
	char *sname;
	DirEntry *sP;
	char *tname;
	DirEntry *delP;
...
static int
#ifdef _NO_PROTO
cmpSymlink(sname, seP, tname, teP)
	char *sname;
	DirEntry *seP;
	char *tname;
	DirEntry *teP;
