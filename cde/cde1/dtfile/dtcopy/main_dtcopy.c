/* $XConsortium: main_dtcopy.c /main/cde1_maint/2 1995/08/29 19:45:34 gtsang $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           main_dtcopy.c
...
static void
#ifdef _NO_PROTO
CheckForMap(w, client_data, event, cont)
	Widget w;
...
static int
#ifdef _NO_PROTO
ErrorHandler(op, fname, errno)
	FileOp op;
	char *fname;
	int errno;
#else
ErrorHandler(FileOp op, char *fname, int errno)
#endif
{
  char msg[2048];
  ...
  if (!G_ignore_errors)
    {
       create_error_dialog(G_toplevel, LongFileOpNames[op], fname, errno);
...
static int
#ifdef _NO_PROTO
ConfirmHandler(op, sname, stype, tname, ttype, link)
	FileOp op;
	char *sname;
	int stype;
	char *tname;
...
static int
#ifdef _NO_PROTO
moveErrorCallback(fname, errno)
	char *fname;
	int errno;
#else
moveErrorCallback(char *fname, int errno)
#endif
{
  return ErrorHandler(op_delete, fname, errno);
}
...
/* XtAddEventHandler(G_toplevel, (EventMask) 0, True, _XEditResCheckMessages, NULL); */
  ...
  create_copydir_dialog(G_toplevel, app_args.source_name, app_args.target_name);
  ...
  if (G_move)
  {
    ...
    else
    {
      ...
      fsErase(app_args.sync.source, &rc);
    }
  }
