/* $XConsortium: ChangeDirP.c /main/cde1_maint/2 1995/08/29 19:31:14 gtsang $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           ChangeDirP.c
...
static Boolean
#ifdef _NO_PROTO
get_text_pieces(file_mgr_data, width, buf,
                host_len_p, host_pixels_p,
                prefix_len_p, prefix_pixels_p,
                path_len_p, path_pixels_p)
	FileMgrData *file_mgr_data;
	int width;
	char buf[];
	int *host_len_p;
...
void
#ifdef _NO_PROTO
ShowChangeDirDialog( w, client_data, callback )
        Widget w ;
        XtPointer client_data ;
        XtPointer callback ;
#else
...
#endif /* _NO_PROTO */
{
   ...
   _DtShowDialog (file_mgr_rec->shell, NULL, file_mgr_rec,
               file_mgr_data->change_dir, CurrentDirChange, file_mgr_rec,
               CurrentDirClose, file_mgr_rec,NULL, False, False, NULL, NULL,
                  False, 0, False, 0);
...
void
#ifdef _NO_PROTO
CurrentDirSelected( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
#endif /* _NO_PROTO */
{
   ...
   if (new_select != NULL && restrictMode)
   {
      ...
      if (strncmp(new_select, users_home_dir, len-1) != 0
           || new_select[len-1] != '\0' && new_select[len-1] != '/')
      {
...
void
#ifdef _NO_PROTO
CurrentDirIconCallback( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
void
#ifdef _NO_PROTO
CurrentDirectoryIconMotion( w, client_data, event )
        Widget w ;
        XtPointer client_data ;
        XEvent * event ;
#else
...
#endif /* _NO_PROTO */
{
       ...
       (B2DragPossible && (event->xmotion.state & Button2Mask)))
   {
      ...
      if ((ABS(diffX) >= dragThreshold) || (ABS(diffY) >= dragThreshold))
      {
         ...
         /* need to find the "." file_view_data */
         if (file_mgr_data->show_type != MULTIPLE_DIRECTORY) 
         { 
            for (i = 0; i < file_mgr_data->directory_set[0]->file_count; i++)
            {
               file_view_data = 
                 file_mgr_data->directory_set[0]->file_view_data[i];
               if(strcmp(file_view_data->file_data->file_name, ".") == 0)
                  break;
            }
         }
         else
            file_view_data = file_mgr_data->tree_root;
         file_mgr_data->drag_file_view_data = file_view_data;
         StartDrag(dirIcon, file_view_data, event);
      }
   }
}
...
void
#ifdef _NO_PROTO
CurrentDirExposed( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
static void
#ifdef _NO_PROTO
CurrentDirChange( client_data, old_dialog_data, new_dialog_data, call_data )
        XtPointer client_data ;
        DialogData *old_dialog_data ;
        DialogData *new_dialog_data ;
        XtPointer call_data ;
...
static void
#ifdef _NO_PROTO
CurrentDirClose( client_data, old_dialog_data, new_dialog_data )
        XtPointer client_data ;
        DialogData *old_dialog_data ;
        DialogData *new_dialog_data ;
#else
...
static void
#ifdef _NO_PROTO
draw_imagestring(display, d, fmd, gc, x, y, text, bytes)
    Display *display;
    Drawable d;
    FileMgrData *fmd;
    GC gc;
