/* $XConsortium: Trash.c /main/cde1_maint/4 1995/09/29 18:36:06 lehors $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           Trash.c
...
#if !defined(_TT_ALL) /* Don't use this var anymore */
...
static Boolean WriteTrashEntries() ;
static void Select_All() ;
static void Unselect_All() ;
void CloseTrash() ;
static String GetBasePath() ;
static Boolean ReadTrashList() ;
...
static int CheckDeletePermissionRecur(
                        char *dir);
static ino_t FileSysType(int dev);
static void RestoreVerifyOk(
                        Widget w,
                        XtPointer client_data,
                        XtPointer call_data ) ;
...
Boolean
#ifdef _NO_PROTO
InitializeTrash( enableVerifyPrompt )
...
#endif /* _NO_PROTO */
{
   ...
   _DtBuildDialog (NULL, NULL, NULL, dialog_data, NULL, NULL, NULL,
                NULL, NULL, False, True, title, NULL);
#else
   _DtBuildDialog (NULL, NULL, NULL, dialog_data, NULL, NULL, NULL,
...
#endif /* _NO_PROTO */
{
   ...
   /* Verify trash list */
   struct stat 	statbuf;
   TrashEntry* 	newTrashCan = NULL;
   int 		newNumItems = 0;
   int		newListSize = 0;

   /* I am going to check the validaty of the cached trash items here 
    * because we are finding they get out of sync with the trashinfo file
    */

   /* We want to only write out trash entries that are valid... */
   for (i = 0; i < numTrashItems; i++)
   {
      /* Create internal/trash name */
      /* Make sure the file still exists */
      if (lstat(trashCan[i].intNew, &statbuf) == 0)
      {
         /* Grow trash list */
         if (newNumItems >= newListSize)
         {
            newListSize += 10;
            newTrashCan = (TrashEntry *)XtRealloc((char *) newTrashCan,
                                                  sizeof(TrashEntry) * newListSize);
         }

         newTrashCan[newNumItems] = trashCan[i];
         newNumItems++;
      }
   }

   /* Any items missing? */
   if (newNumItems != numTrashItems)
   {
      XtFree((char*)trashCan);
      trashCan = newTrashCan;
      numTrashItems = newNumItems;
      trashListSize = newListSize;
   }
   else
   {
      /* Nothing out of sync */
      XtFree((char*)newTrashCan);
   }
...
static int 
#ifdef _NO_PROTO
WriteEntry( id, external, internal )
        FILE *id ;
        String external ;
        String internal ;
#else
...
#endif /* _NO_PROTO */
{
  return( fprintf(id, "%d %d %s %s\n", strlen(external), strlen(internal),
                                      external, internal) );
}
...
Widget
#ifdef _NO_PROTO
CreateTrashMenu( mainw, file_mgr_rec )
        Widget mainw ;
...
#endif /* _NO_PROTO */
{
   ...
   trashMenu[j].label = XtNewString(trashMenu[j].label);
   trashMenu[j].mnemonic = GETMESSAGE(20,158, "V");
   trashMenu[j].mnemonic = XtNewString(trashMenu[j].mnemonic);
   trashMenu[j].helpData = HELP_TRASH_DIALOG_STR;
...
void
#ifdef _NO_PROTO
TrashDisplayHandler( msg, workspaces, iconify_state,
...
#endif /* _NO_PROTO */
{
   ...
   if (trashFileMgrData->file_mgr_rec == 0)
   {
      ...
      TrashView = True;
      _DtShowBuiltDialog(NULL, NULL, trashDialogData, workspaces,
   ...
   trashFileMgrData->displayed = True;
}
...
static void
#ifdef _NO_PROTO
ConfirmEmptyCancel( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
static void
#ifdef _NO_PROTO
ConfirmEmptyOk( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
static void
#ifdef _NO_PROTO
Select_All( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
static void
#ifdef _NO_PROTO
Unselect_All( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
static void
#ifdef _NO_PROTO
RemoveOkCB( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
static void
#ifdef _NO_PROTO
RemoveCancelCB( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
static void
#ifdef _NO_PROTO
MessagesToFileList(msg_list, msg_cnt, file_list, file_count)
        Tt_message *msg_list;
        int msg_cnt;
	char ***file_list;
	int *file_count;
...
static String
#ifdef _NO_PROTO
CreateTrashFilename( baseName, uniqueTest )
        String baseName ;
        Boolean uniqueTest ;
#else
CreateTrashFilename(
...
static void
#ifdef _NO_PROTO
VerifyOk( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
#endif /* _NO_PROTO */
{
   VerifyCleanup (client_data, True);
}
...
static void
#ifdef _NO_PROTO
VerifyCancel( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
{
   VerifyCleanup (client_data, False);
}
...
static void
#ifdef _NO_PROTO
VerifyCleanup( mbox, completeDelete )
        Widget mbox ;
        Boolean completeDelete ;
#else
VerifyCleanup(
...
#endif /* _NO_PROTO */
{
  ...
  XtSetSensitive(*restoreBtn, True);
#ifdef _TT_MULTI_SCREEN
  XtSetSensitive(gFileMgrPopup[scr].trash_objPopup[BTN_REMOVE], True);
  XtSetSensitive(gFileMgrPopup[scr].trash_objPopup[BTN_RESTORE], True);
#else
...
static void
#ifdef _NO_PROTO
ConfirmOk( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
void
#ifdef _NO_PROTO
ConfirmRemove( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
#endif /* _NO_PROTO */
{
  ...
  XtSetSensitive(*restoreBtn, False);
#ifdef _TT_MULTI_SCREEN
  XtSetSensitive(gFileMgrPopup[scr].trash_objPopup[BTN_REMOVE], False);
  XtSetSensitive(gFileMgrPopup[scr].trash_objPopup[BTN_RESTORE], False);
#else
...
void
#ifdef _NO_PROTO
Remove( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
static void
#ifdef _NO_PROTO
AddToDeleteList(deleteList, i, filename)
   DeleteList *deleteList;
   int i;
   char *filename;
#else
...
void
#ifdef _NO_PROTO
Restore( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
void
#ifdef _NO_PROTO
CloseTrash( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
#endif /* _NO_PROTO */
{
   ...
   trashFileMgrData->displayed = False;
}
...
void
#ifdef _NO_PROTO
DropOnTrashCan(file_count, host_set, file_set, drop_parameters)
        int file_count;
        char **host_set;
        char **file_set;
...
void
#ifdef _NO_PROTO
MoveOutOfTrashCan(file_mgr_data, file_mgr_rec, w,  file_count,
                  host_set, file_set, drop_x, drop_y)
        FileMgrData *file_mgr_data;
        FileMgrRec *file_mgr_rec;
        Window w;
        int file_count;
...
static void
#ifdef _NO_PROTO
MoveToTrashProcess(pipe_fd, file_list, file_count, do_verify_checks)
	int pipe_fd;
	char **file_list;
	int file_count;
	Boolean do_verify_checks;
...
static void
#ifdef _NO_PROTO
MoveToTrashPipeCB(client_data, fd, id)
   XtPointer client_data;
   int *fd;
   XtInputId *id;
#else
...
static void
#ifdef _NO_PROTO
MoveToTrash(file_list, file_count, do_verify_checks, msg)
	char **file_list;
	int file_count;
	Boolean do_verify_checks;
	Tt_message msg;
...
static void
#ifdef _NO_PROTO
RestoreProcess(pipe_fd, file_list, file_count, target_host, target_dir,
		 rc,CheckedAlready)
	int pipe_fd;
	char **file_list;
	int file_count;
...
#endif /* _NO_PROTO */
{
   ...
   char buf[MAX_PATH];
   short pipe_msg;
   int status, count, restoreStatus = True;
   ...
   /* restore the files */
   for (i = 0,count = 0; i < file_count; i++)
   {
      ...
      if (to != NULL)
      {
         if(restoreStatus)
            status = RestoreObject((Widget)pipe_fd, MOVE_FILE, from,to,
                                   TRUE, FileOpError, False,
                                   NOT_DESKTOP,CheckedAlready);
         else
            status = RestoreObject((Widget)pipe_fd, MOVE_FILE, from,to,
                                   TRUE, NULL, False,
                                   NOT_DESKTOP,CheckedAlready);
...
static void
#ifdef _NO_PROTO
RestorePipeCB(client_data, fd, id)
   XtPointer client_data;
   int *fd;
   XtInputId *id;
#else
...
#endif /* _NO_PROTO */
{
   ...
   for (i = 0; i < cb_data->file_count; i++)
   {
      ...
      {
         ToRestoreList = (char **) XtRealloc((char *)ToRestoreList,
                                   sizeof(char *) *(++RestoreIndex));
      ...
      {
         char *restore_header,*tmpStr;


         if(buf == NULL)
         {
            count = 0;
            tmpStr = GETMESSAGE(27,101, 
   ...
   if(count >= 9)
   {
      ...
      (void) sprintf(extraFiles, AdditionalHeader, count - 9);
      AddString(&buf, &bufsize, extraFiles, NULL);
   }
...
static void
#ifdef _NO_PROTO
RestoreFromTrash(file_list, file_count, target_host, target_dir, msg,
		  CheckedAlready)
	char **file_list;
	int file_count;
	char *target_host;
...
#endif /* _NO_PROTO */
{
   ...
   {
      /* child process */
      DPRINTF(("RestoreFromTrash: child started\n"));
      close(pipe_fd[0]);  /* child won't read from the pipe */
      ...
      DPRINTF(("RestoreFromTrash: child ends\n"));
      exit(0);
   }
...
static void
#ifdef _NO_PROTO
EmptyTrashProcess(pipe_fd, del_list, del_count, rc)
	int pipe_fd;
	DeleteList *del_list;
	int del_count;
	int *rc;
...
static void
#ifdef _NO_PROTO
EmptyTrashPipeCB(client_data, fd, id)
   XtPointer client_data;
   int *fd;
   XtInputId *id;
#else
...
#endif /* _NO_PROTO */
{
   ...
   char *file_name;
#ifdef _TT_MULTI_SCREEN
  int 		scr = XScreenNumberOfScreen(XtScreen(
			((FileMgrRec*)trashFileMgrData->file_mgr_rec)->shell));
#endif
...
static void
#ifdef _NO_PROTO
EmptyTrash(del_list, del_count, removeType, msg)
	DeleteList *del_list;
	int del_count;
	int removeType;
	Tt_message msg;
...
#endif /* _NO_PROTO */
{
   ...
   {
      /* child process */
      DPRINTF(("EmptyTrash: child started\n"));
      close(pipe_fd[0]);  /* child won't read from the pipe */

      EmptyTrashProcess(pipe_fd[1], del_list, del_count, cb_data->rc);
      close(pipe_fd[1]);

      DPRINTF(("EmptyTrash: child ends\n"));
      exit(0);
   }
...
int
#ifdef _NO_PROTO
CheckDeletePermission(parentdir, destinationPath)
...
static int 
#ifdef _NO_PROTO
CheckDeletePermissionRecur(destinationPath)
  char *destinationPath;
#else
CheckDeletePermissionRecur(
  char *destinationPath)
...
static int
#ifdef _NO_PROTO
RestoreObject( w, mode, source, target, isContainer, errorHandler, 
                          checkForBusyDir, type,CheckedAlready )
        Widget w ;
        int mode ;
        register char *source ;
        register char *target ;
...
#endif /* _NO_PROTO */
{
  ...
  return ((int )FileManip((Widget)w, MOVE_FILE, source, target, TRUE,
                                  errorHandler, False, NOT_DESKTOP));
}
static void
#ifdef _NO_PROTO
CreateRestoreDialog(source,target)
   char *source;
   char *target;
#else
CreateRestoreDialog(
...
static void
#ifdef _NO_PROTO
RestoreVerifyOk( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
static void
#ifdef _NO_PROTO
RestoreVerifyCancel( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
