/* $XConsortium: FileOp.c /main/cde1_maint/3 1995/10/09 14:49:07 pascale $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           FileOp.c
...
#define PIPEMSG_RENAME_FILE	 107 /* part of bug fix 4051255 */
...
#endif /* _NO_PROTO */
{
   ...
   while (n < len)
   {
      ...
      {
         if (rc == 0)
            fprintf(stderr, "dtfile[%d]: PipeRead(): read(%d,) == 0\n",
		    getpid(), fd);
         else
...
void
#ifdef _NO_PROTO
FileOpError( w, message1, message2 )
        Widget w ;
        char *message1 ;
        char *message2 ;
#else
...
static void
#ifdef _NO_PROTO
SendModifyMsg(pipe_fd, mode, first, to, updates, i, target_file)
	int pipe_fd;
	int mode;
	Boolean first;
	char *to;
...
static int
#ifdef _NO_PROTO
FileMoveCopyProcess(pipe_s2m, pipe_m2s, to_file, directory, host,
                    updates, file_count, mode, desktopWindow)
        int pipe_s2m;
        int pipe_m2s;
        char *directory;
        char *host;
...
#endif /* _NO_PROTO */
{
   ...
   struct stat s1,s2 ; /* Bug fix 4051255 */
   short pipe_msg;
   int sameCount = 0;
   int errorCount = 0;
   ...
   for (i = 0; i < file_count; i++)
   {
     ...
     if (mode == COPY_FILE)
       ...
       */
      if ((mode != MAKE_BUFFER)
	  && (stat(target_file, &s1) == 0) && (stat(from, &s2) == 0)
		   ...
		   continue;
            ...
            switch (rc)
            {
                ...
                case PIPEMSG_RENAME_BUFFER:
                    ...
                    break;
		case PIPEMSG_RENAME_FILE:
		    if (Same) {
                    	renamed_file = PipeReadString(pipe_m2s);
                    	return_val = FileManip((Widget)pipe_s2m, mode, from,
                                                renamed_file /* toFile */, False, FileOpError,
                                                True, NOT_DESKTOP);
		    }
		    break ;
                    ...
                    else
                    {
                      ...
                      if(Same)
                      {
                         ...
                         return_val = FileManip((Widget)pipe_s2m, mode, from,
                                                ...
                                                True, NOT_DESKTOP);
                         /* XtFree(path);  part of Bug fix 4051255 */
                         XtFree(toFile);
                      }
                      else
                         return_val = FileManip((Widget)pipe_s2m, mode, from,
...
static int
#ifdef _NO_PROTO
FileMoveCopyProcessDesktop(pipe_s2m, pipe_m2s, directory, host,
                           updates, file_count, mode, desktopWindow)
        int pipe_s2m;
        int pipe_m2s;
        char *directory;
   ...
   for (i = 0; i < file_count; i++)
   {
      ...
      if (mode != MAKE_BUFFER)
      {
         ...
         else if (strcmp(to, from) == 0)
         {
            ...
            sprintf(msg,GETMESSAGE(11,16, "A folder cannot be moved, copied or linked into itself.\n%s"),to);
            PipeWriteErrmsg(pipe_s2m, -1, msg, NULL);
            XtFree ((char *) from);
             from = NULL;
         }
...
static void
#ifdef _NO_PROTO
RemoveIconFromWorkspace( iconName, targetDir )
char * iconName;
char * targetDir;
#else
...
#ifdef _TT_MULTI_SCREEN
      screen = XScreenNumberOfScreen(XtScreen(desktopWin->shell));
      currentScreen =  XtScreen(desktopWin->shell);
#else
...
static void
#ifdef _NO_PROTO
ChangeWorkspaceIconLink(fmd, iconName, targetDir, iconPdir )
FileMgrData *fmd;
char * iconName;
char * targetDir;
char * iconPdir;
 ...
 { 
  ...
  for(i = 0; i < desktop_data->numIconsUsed; i++)
  {
    ...
    if(dirp)
    {
	 ...
	 desktopWin->root_type = RootTypeOfDesktopObject( desktopWin );
...
static void
#ifdef _NO_PROTO
FileOpPipeCB(client_data, fd, id)
   ...
   switch (pipe_msg)
   {
         ...
         if(cb_data->callback_data)
	 {
      ...
      case PIPEMSG_MULTICOLLIDE_SAME:
         ...
         directory = PipeReadString(*fd);
         fileList  = (String *) XtMalloc(nCollisions * sizeof(String));   /* de-
allocated in dialog's callback functions */
      ...
      for (i = 0; i < cb_data->file_count; i++)
      {
         ...
         if(!i && cb_data->callback_data == NULL)
         {
           ...
           if(cb_data && cb_data->file_mgr_data)
           {
             ...
             if(fmd->scrollToThisDirectory != NULL)
                XtFree(fmd->scrollToThisDirectory);
             fmd->scrollToThisDirectory = XtNewString(cb_data->directory);

             if(fmd->scrollToThisFile != NULL)
                XtFree(fmd->scrollToThisFile);
...
static Boolean
#ifdef _NO_PROTO
_FileMoveCopy( data, to_file, directory, host,
               host_set, file_set, buffer_set, file_count, modifiers, desktopWindow,
...
#endif /* _NO_PROTO */
{
   ...
   {
      /* child process */
      DPRINTF(("FileMoveCopy: child started\n"));
      ...
      DPRINTF(("FileMoveCopy: child ends (rc %d)\n", rc));
      exit(rc);
   }
...
Boolean
#ifdef _NO_PROTO
FileMoveCopy( file_mgr_data, to_file, directory, host,
              host_set, file_set, file_count, modifiers,
              finish_callback, callback_data)
        FileMgrData *file_mgr_data ;
        char *to_file ;
        char *directory ;
...
FileMoveCopyDesktop (file_view_data, directory,
                     host_set, file_set, file_count, modifiers, desktopWindow,
                     finish_callback, callback_data)
      FileViewData *file_view_data;
      char * directory;
      char ** host_set;
...
static int
#ifdef _NO_PROTO
ChangeIconNameProcess(pipe_fd, host_name, directory_name, old_name, new_name)
        int pipe_fd;
        char *host_name;
        char *directory_name;
        char *old_name;
...
static void
#ifdef _NO_PROTO
ChangeIconPipeCB(client_data, fd, id)
   XtPointer client_data;
   int *fd;
   XtInputId *id;
#else
   ...
   else if (pipe_msg == PIPEMSG_DONE)
   {
      ...
      if (rc == 0)
      {
         ...
         else
         {
            ...
            label = XmStringLtoRCreate(cb_data->new_name,
...
static void
#ifdef _NO_PROTO
_ChangeIconName (w, desktop, file_view_data, desktopWindow,
...
#endif /* _NO_PROTO */
{
  ...
  for (scr = 0; scr < gNScreens ; scr++)
  {
   ...
   {
      /* child process */
      DPRINTF(("ChangeIconName: child started\n"));
      ...
      DPRINTF(("ChangeIconName: child ends (rc %d)\n", rc));
      exit(rc);
   }
...
void
#ifdef _NO_PROTO
ChangeIconName (w, client_data, call_data)
   Widget w;
   XtPointer client_data;
...
void
#ifdef _NO_PROTO
ChangeIconNameDT (w, client_data, call_data)
   Widget w;
   XtPointer client_data;
   XtPointer call_data;
#else
...
void
#ifdef _NO_PROTO
GetTarget(from_host, from_dir, new_name, to_host, to_dir, to_file)
        char *from_host;
        char *from_dir;
        char *new_name;
        char *to_host;
...
static int
#ifdef _NO_PROTO
MakeFileProcess(pipe_fd, to_host, to_dir, to_file, type)
        int pipe_fd;
        char *to_host;
        char *to_dir;
        char *to_file;
...
static void
#ifdef _NO_PROTO
MakeFilePipeCB(client_data, fd, id)
   XtPointer client_data;
   int *fd;
   XtInputId *id;
#else
...
void
#ifdef _NO_PROTO
MakeFile(w, host_name, directory_name, new_name, type,
         finish_callback, callback_data)
        Widget w ;
        char *host_name;
        char *directory_name;
...
#endif /* _NO_PROTO */
{
   ...
   {
      /* child process */
      DPRINTF(("MakeFile: child started\n"));
      close(pipe_fd[0]);  /* child won't read from the pipe */

      rc = MakeFileProcess(pipe_fd[1], to_host, to_dir, to_file, type);
      close(pipe_fd[1]);

      DPRINTF(("MakeFile: child ends (rc %d)\n", rc));
      exit(rc);
   }
...
Boolean
#ifdef _NO_PROTO
MakeFilesFromBuffers(file_mgr_data, directory, host, file_set, host_set, 
                     buffer_set, 
                     num_of_buffers, finish_callback, callback_data)
FileMgrData *file_mgr_data;
char * directory;
char * host;
char **file_set;
...
MakeFilesFromBuffersDT(file_view_data, directory, file_set, host_set,
                       ...
                       desktopWindow, finish_callback, callback_data)
FileViewData *file_view_data;
char * directory;
char **file_set;
char **host_set;
...
static void
#ifdef _NO_PROTO
DisplayDuplicateOpError(cb_data,index)
    FileOpCBData *cb_data;
    int index;
#else
DisplayDuplicateOpError(
...
static String
#ifdef _NO_PROTO
appendErrorMessage(message, new, arg)
   String message, new, arg;
#else
...
static void
#ifdef _NO_PROTO
DisplayErrorMessage(pipe_s2m,errMsg,from,tdir)
  int pipe_s2m;
  char *errMsg,*from,*tdir;
#else
DisplayErrorMessage(
  int pipe_s2m,
  char *errMsg,
...
static Boolean
#ifdef _NO_PROTO
IsInParentDir( source_path,dest_path )
        char *source_path ;
        char *dest_path ;
#else
IsInParentDir(
