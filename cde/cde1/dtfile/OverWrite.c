/* $XConsortium: OverWrite.c /main/cde1_maint/3 1995/10/09 14:52:29 pascale $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           OverWrite.c
...
#define PIPEMSG_RENAME_FILE	107 /* bug fix 4051255 */
...
void
#ifdef _NO_PROTO
replace_rename_ok_callback(w, client_data, call_data)
	Widget w;
	XtPointer client_data;
        XtPointer call_data;
#else
...
#endif
{
   ...
   if (XmToggleButtonGetState(G_rename_toggle))
   {
      ...
      else { /* bug fix 4051255 - 
		if in the same folder pass the given file name(if any) to the child process 
		thro pipe */

		rc = PIPEMSG_RENAME_FILE ;
   		write(G_filop_confirm_fd, &rc, sizeof(int));
         	PipeWriteString(G_filop_confirm_fd, renameNewName);
   ...
   else
   {
      ...
      if(IsReplaceable(destinationName))
        fsErase(destinationName,&saveError);
      else
        saveError = 1;
      if ( saveError == 0 )
      {
...
void
#ifdef _NO_PROTO
buffer_replace_rename_ok_callback(w, client_data, call_data)
        Widget w;
        XtPointer client_data;
        XtPointer call_data;
#else
...
void
#ifdef _NO_PROTO
replace_rename_cancel_callback(w, client_data, call_data)
	Widget w;
	XtPointer client_data;
        XtPointer call_data;
#else
...
static void
#ifdef _NO_PROTO
replace_rename_radio_callback(w, client_data, call_data)
	Widget w;
	XtPointer client_data;
        XtPointer call_data;
#else
...
static void
#ifdef _NO_PROTO
renameError_ok_callback(w, client_data, call_data)
	Widget w;
	XtPointer client_data;
	XtPointer call_data;
#else
...
void
#ifdef _NO_PROTO
replace_merge_ok_callback(w, client_data, call_data)
	Widget w;
	XtPointer client_data;
        XtPointer call_data;
#else
...
void
#ifdef _NO_PROTO
replace_merge_cancel_callback(w, client_data, call_data)
	Widget w;
	XtPointer client_data;
        XtPointer call_data;
#else
...
void
#ifdef _NO_PROTO
multicollide_cancel_callback(w, client_data, call_data)
	Widget w;
	XtPointer client_data;
        XtPointer call_data;
#else
...
void
#ifdef _NO_PROTO
multicollide_ok_callback(w, client_data, call_data)
	Widget w;
	XtPointer client_data;
        XtPointer call_data;
#else
      ...
      for ( i = 0; i < G_nCollisions; i++)
      {
         ...
         if(IsReplaceable(destinationPath))
         {
           if(DirectoryBusy(destinationPath))
           {
             ...
             fsErase(destinationPath,&saveError);
         }
         else
           saveError = -1;
         write(G_filop_confirm_fd, &saveError, sizeof(int));
...
void
#ifdef _NO_PROTO
buffer_multicollide_ok_callback(w, client_data, call_data)
        Widget w;
        XtPointer client_data;
        XtPointer call_data;
#else
...
static void
#ifdef _NO_PROTO
replaceError_ok_callback(w, client_data, call_data)
	Widget w;
	XtPointer client_data;
	XtPointer call_data;
#else
...
static void
#ifdef _NO_PROTO
multiReplaceError_ok_callback(w, client_data, call_data)
	Widget w;
	XtPointer client_data;
	XtPointer call_data;
#else
...
void
#ifdef _NO_PROTO
create_replace_rename_dialog(parent_widget, mode, directory, file, filop_confirm_fd, actions, Same)
...
#endif
{
  ...
  xs = XmStringCreateLtoR(GETMESSAGE(9, 20, "What do you want to do?"), XmFONTLIST_DEFAULT_TAG);
  ...
  xs = XmStringCreateLtoR(GETMESSAGE(9, 21, "Replace existing object"),XmFONTLIST_DEFAULT_TAG);
  ...
  if( mode == MAKE_BUFFER )
  {
     xs = XmStringCreateLtoR(GETMESSAGE(10,48, "New Object Name:"),XmFONTLIST_DEFAULT_TAG);
     set = TRUE;
  }
  else
  {
     if(Same)
     {
        xs = XmStringCreateLtoR(GETMESSAGE(9, 140, "Rename new object to:"), XmFONTLIST_DEFAULT_TAG);
        set = TRUE;
     }
     else
     {
        xs = XmStringCreateLtoR(GETMESSAGE(9, 22, "Rename existing object to:"),XmFONTLIST_DEFAULT_TAG);
        set = FALSE;
     }
  }
...
void
#ifdef _NO_PROTO
create_replace_merge_dialog(parent_widget, mode, directory, file, filop_confirm_fd, actions)
	Widget         parent_widget;
	int            mode;
	String         directory, file;
	int            filop_confirm_fd;
...
#endif
{
  ...
  /* create the radio buttons ... default button is replace */
  xs = XmStringCreateLtoR(GETMESSAGE(9, 25, "Replace existing folder"),XmFONTLIST_DEFAULT_TAG);
  radioButton1 = XtVaCreateManagedWidget("radio_toggle1",
                                         xmToggleButtonWidgetClass, radio,
                                         XmNalignment,   XmALIGNMENT_BEGINNING,
                                         XmNlabelString, xs,
  ...
  xs = XmStringCreateLtoR(GETMESSAGE(9, 24, "Merge contents of the two folders"),XmFONTLIST_DEFAULT_TAG);
...
void
#ifdef _NO_PROTO
create_multicollide_dialog(parent_widget, mode, nSelected, nCollisions, destination, fileList, filop_confirm_fd, actions, Same)
	Widget           parent_widget;
	int              mode;
...
#endif
{
  ...
  /* create the radio buttons ... default button is replace */
  xs = XmStringCreateLtoR(GETMESSAGE(9, 56, "Replace existing objects"),XmFONTLIST_DEFAULT_TAG);
  ...
  if(Same)
  {
     XtSetSensitive(radioButton1, False);
     set = TRUE;
     xs = XmStringCreateLtoR(GETMESSAGE(9, 141, "Rename new objects by appending a number"), XmFONTLIST_DEFAULT_TAG);
  }
  else
  {
     XtSetSensitive(radioButton1, True);
     set = FALSE;
     xs = XmStringCreateLtoR(GETMESSAGE(9, 57, "Rename existing objects by appending a number"),XmFONTLIST_DEFAULT_TAG);
  }
...
static char *
#ifdef _NO_PROTO
getVariableMessage(message, mode)
   VariableMessage message;
   int             mode;
#else
getVariableMessage(
