/* $XConsortium: File.c /main/cde1_maint/5 1995/10/17 16:25:41 gtsang $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           File.c
...
/* Need this to potentially determine which screen the drop on 
 * root CB happened on.  (You can only drop on the root of the 
 * screen you started the drag on, so this should be OK) 
 */
static Widget lDragStartWidget = NULL;
#endif
...
#ifdef _TT_MULTI_SCREEN
static void PositionDesktopIcon(int, int, int *, int *, int);
...
#ifdef _TT_MULTI_SCREEN
FileMgrPopup *gFileMgrPopup = NULL;
#else
...
void
#ifdef _NO_PROTO
GetAncestorInfo(file_mgr_data, ip, levelp, path, morep)
        FileMgrData *file_mgr_data;
        FileViewData *ip;
        int *levelp;
        char *path;
...
void
#ifdef _NO_PROTO
GetFullName(file_mgr_data, ip, path)
        FileMgrData *file_mgr_data;
        FileViewData *ip;
        char *path;
#else
...
static void
#ifdef _NO_PROTO
CopyOrderedEntries(ip, fvd_array, index)
        FileViewData *ip;
...
void
#ifdef _NO_PROTO
FlattenTree(file_mgr_data, file_view_data, file_count)
        FileMgrData *file_mgr_data;
        FileViewData ***file_view_data;
        int *file_count;
#else
...
void
#ifdef _NO_PROTO
IconCallback( w, clientData, callData )
        Widget w ;
        XtPointer clientData ;
        XtPointer callData ;
#else
...
#endif /* _NO_PROTO */
{
   ...
   else if (callback->reason == XmCR_DRAG)
   {
      ...
      B2DragPossible = True;
#ifdef _TT_MULTI_SCREEN
      memcpy((char*) &gDesktopData[XScreenNumberOfScreen(XtScreen(w))]->event,
         (char*) event, (int) sizeof(XButtonEvent));
#else
...
static int
#ifdef _NO_PROTO
GetSelectedCount(fileViewData, fileMgrData, desktopRec, dt)
        FileViewData * fileViewData;
        FileMgrData * fileMgrData;
        DesktopRec * desktopRec;
        int * dt;
...
#ifdef _TT_MULTI_SCREEN
         desktop_data = gDesktopData[XScreenNumberOfScreen(
               XtScreen(desktopRec->shell ))];
#endif
...
#endif /* _NO_PROTO */
{
       ...
       && ((FileMgrData *)initiating_view)->view == BY_ATTRIBUTES)
   {
     ...
     fileNameString = XmStringCreateLtoR( fileViewData->file_data->file_name,
...
static void
#ifdef _NO_PROTO
NewConvertDelete(w, fileViewData, fileList, numFiles)
        Widget w;
        FileViewData * fileViewData;
        char ** fileList;
        int numFiles;
...
#endif /* _NO_PROTO */
{
      ...
      DPRINTF(("NewConvertDelete: Entering function\n"));
      ...
      /* fork a background child process to honor the Move Completion */
       ...
       if (child_pid == -1)
       {
	    ...
	    DPRINTF(("NewConvertDelete: Cannot create child process\n"));

	    tmpStr = GETMESSAGE(11,36, "Cannot create child process.");
       ...
       if (child_pid == 0)
       {
	    ...
	    {
	     DPRINTF(("NewConvertDelete: Erasing file %s\n", fileList[i]));
...
static void
#ifdef _NO_PROTO
NewConvertFileName(w, fileViewData, fileList, numFiles)
        Widget w;
        FileViewData * fileViewData;
        char ** fileList;
        Cardinal *numFiles;
...
#ifdef _TT_MULTI_SCREEN /* SPR-1716 */
   int 		screenNum = 0;
   DesktopData* desktop_data = NULL;
#endif
...
#ifdef _TT_MULTI_SCREEN /* SPR-1716 */
   /* We don't want to use the passed in widget, cause it's the 
    * drag context, which is always screen 0.  So, we must come 
    * up with a more appropriate widget
    */
   if (fileViewData)
      screenNum = XScreenNumberOfScreen(XtScreen(fileViewData->widget));
   else if(fileMgrData)
      screenNum = XScreenNumberOfScreen(
                  XtScreen( ((FileMgrRec*)(fileMgrData->file_mgr_rec))->shell) );
   /* else screenNum=0.  Hope it never gets here... */

   desktop_data = gDesktopData[screenNum];
#endif
...
void
#ifdef _NO_PROTO
StartDrag( w, fileViewData, event)
        Widget w ;
        FileViewData * fileViewData ;
        XEvent * event;
#else
...
#endif /* _NO_PROTO */
{
   ...
   if ((desktopObj && desktopRec->toolbox) ||
     ...
     operations = XmDROP_COPY | XmDROP_MOVE | XmDROP_LINK;
   /* E.g. force XmDROP_COPY semantics for views restricted to floppies */
   operations &= TypeToDropOperations(
	desktopObj ? desktopRec->root_type : fileMgrData->root_type );
...
static void
#ifdef _NO_PROTO
InitiateIconDrag (fileViewData, rootX, rootY, event)
        FileViewData * fileViewData ;
        int rootX ;
        int rootY ;
        XEvent * event;
...
#endif /* _NO_PROTO */
{
   ...
   gadget = InputInGadget ((Widget)cw, x, y);
...
void
#ifdef _NO_PROTO
FileIconMotion( w, clientData, event )
        Widget w ;
        XtPointer clientData ;
        XEvent * event ;
#else
...
#ifdef _TT_MULTI_SCREEN
            DesktopData* desktop_data =
              gDesktopData[XScreenNumberOfScreen(XtScreen(w))];
...
#ifdef _NO_PROTO
PositionDesktopIcon(drop_x, drop_y, root_x, root_y, screen_num)
   int drop_x;
   int drop_y;
   int *root_x;
   int *root_y;
...
#ifdef _NO_PROTO
PositionDesktopIcon(drop_x, drop_y, root_x, root_y)
   int drop_x;
   int drop_y;
   int *root_x;
   int *root_y;
...
static void
#ifdef _NO_PROTO
RelocateDesktopIcon( desktopRec, root_x, root_y)
   DesktopRec * desktopRec;
   int root_x;
   int root_y;
#else
...
#endif /* _NO_PROTO */
{
   ...
   /*
    Corrected: Append rather than add to the front
    of the selection list. Otherwise action args etc.
    will be in reverse selection order
   */
...
#ifdef ADD_SELECTION_TO_HEAD /* Removed add to front code */
...
#else /* Changed to append to tail */

   file_mgr_data->selection_list[selection_count] = file_view_data;
   file_mgr_data->selection_list[selection_count+1] = NULL;

#endif /* ADD_SELECTION_TO_HEAD */
...
void
#ifdef _NO_PROTO
DeselectFile( file_mgr_data, file_view_data, valid )
        FileMgrData *file_mgr_data ;
        FileViewData *file_view_data ;
        Boolean valid ;
#else
...
#endif /* _NO_PROTO */
{
   ...
   if(file_mgr_data != trashFileMgrData)
   {
      if (selection_count == 0)
         ActivateNoSelect (file_mgr_data->file_mgr_rec);
      else if (selection_count == 1)
      {
         ...
         ActivateMultipleSelect (file_mgr_data->file_mgr_rec);
   }
}
...
void
#ifdef _NO_PROTO
SetToSelectColors( widget, file_window, type )
        Widget widget ;
        Widget file_window ;
        int type;
#else
...
#endif /* _NO_PROTO */
{
   ...
   XtGetValues (file_window, args, 1);
...
void
#ifdef _NO_PROTO
SetToNormalColors( widget, bg_fg_colors, top_shadow_colors, type )
        Widget widget ;
        Widget bg_fg_colors ;
...
#endif /* _NO_PROTO */
{
   ...
   XtGetValues (bg_fg_colors, args, 2);

   XtSetArg (args[0], XmNtopShadowColor, &pixmap_background);
   XtGetValues (top_shadow_colors, args, 1);

#ifdef _SHOW_LINK
   if(type == LINK_FILE)
      XtSetArg (args[0], XmNforeground, pixmap_background);
   else
#endif
      XtSetArg (args[0], XmNforeground, foreground_color);

   if (background_color == white_pixel)
      XtSetArg (args[1], XmNbackground, white_pixel);
   else if (background_color == black_pixel)
      XtSetArg (args[1], XmNbackground, black_pixel);
   else
      XtSetArg (args[1], XmNbackground, background_color);
...
void
#ifdef _NO_PROTO
DestroyIconName (w, client_data, call_data)
   Widget w;
   XtPointer client_data;
   XtPointer call_data;
#else
...
#endif
{
       ...
       ++tmp, ++i )
  {
    ...
    savedChar = *tmp;
    *tmp = 0x0;
    string = XmStringCreateLtoR( name, XmFONTLIST_DEFAULT_TAG );
...
void
#ifdef _NO_PROTO
CreateNameChangeDialog (w, file_view_data, client_data, type)
   Widget w;
   FileViewData *file_view_data;
   XtPointer client_data;
   int type;
...
#endif /* _NO_PROTO */
{
   XRectangle * textExtent = _DtIconGetTextExtent(w);
   FileMgrData * file_mgr_data;
   ...
   */
   {
     ...
     fileNameString = XmStringCreateLtoR( tmpBuf, XmFONTLIST_DEFAULT_TAG );
...
void
#ifdef _NO_PROTO
SavePositionalData (fd, file_mgr_data, name_list, name)
   int fd;
   FileMgrData * file_mgr_data;
   char ** name_list;
   char * name;
...
void
#ifdef _NO_PROTO
RestorePositionalData (db, name_list, file_mgr_data, name)
   XrmDatabase db;
   char ** name_list;
   FileMgrData * file_mgr_data;
   char * name;
...
void
#ifdef _NO_PROTO
RepaintDesktop (file_mgr_data, event, region)
   FileMgrData * file_mgr_data;
   XEvent * event;
   Region region;
#else
...
static void
#ifdef _NO_PROTO
RedisplayUsingStackingOrder (file_mgr_data, w, event, region)
   FileMgrData * file_mgr_data;
   Widget w ;
   register XEvent *event ;
   Region region ;
...
void
#ifdef _NO_PROTO
DrawingAreaRedisplay (wid, event, region)
   Widget wid;
   XEvent *event;
   Region region;
#else
...
ObjectPtr
#ifdef _NO_PROTO
GetBottomOfStack (file_mgr_data)
   FileMgrData * file_mgr_data;
#else
GetBottomOfStack (
   FileMgrData * file_mgr_data)
...
void
#ifdef _NO_PROTO
RepositionUpInStack (file_mgr_data, oldPos, newPos)
   FileMgrData * file_mgr_data;
   int oldPos;
   int newPos;
#else
...

{
   ...
   while ((ptr != 0) && (ptr->stacking_order != newPos))
      ptr = ptr->next;

   if (ptr == 0)
      return; /* fixes sporadic SEGV bug 4023488? XXX */
...
static void
#ifdef _NO_PROTO
ReorderChildrenList (file_window, manage, manageCount, unmanage, unmanageCount)
   XmManagerWidget file_window;
   Widget * manage;
   int manageCount;
   Widget * unmanage;
...
void
#ifdef _NO_PROTO
SetHotRects (file_view_data, view_type, callback, callback_data)
   FileViewData  * file_view_data;
...

{
   ...
   if (file_view_data->displayed)
   {
      ...
      if (file_view_data->file_data->physical_type == DtDIRECTORY)
          operations &= TypeToDropOperations(view_type);
...
unsigned char
#ifdef _NO_PROTO
TypeToDropOperations (file_type)
...
char *
#ifdef _NO_PROTO
TypeToAction (dropOperation, logical_type)
...
Boolean
#ifdef _NO_PROTO
IsDesktopPtr ( fileViewData, fileMgrData, desktopRec )
   FileViewData * fileViewData ;
   FileMgrData ** fileMgrData;
   DesktopRec ** desktopRec;
#else
...
#endif /* _NO_PROTO */
{
   ...
   if (XtIsManaged(file_mgr_rec->file_window))
   {
     ...
     XtGetValues ( file_mgr_rec->vertical_scroll_bar, args, 1);

     if( y < max)
     {
       /* get scroll bar values */
       (void)XmScrollBarGetValues( file_mgr_rec->vertical_scroll_bar, &value, &size, &increment, &page);

       if(max - y < size)
         y = max - size;

       /* set scroll bar values changing its position */
       (void)XmScrollBarSetValues( file_mgr_rec->vertical_scroll_bar, (int)y, size, increment, page, True);
     }
   }
   else
   {
     ...
     Arg args[2];
     int n = 0, sX = 0, sY = 0, scrollWindowWidth, scrollWindowHeight;

     scrollWindowWidth = file_mgr_rec->scroll_window->core.width;
     scrollWindowHeight = file_mgr_rec->scroll_window->core.height;

     if( file_view_data->x > scrollWindowWidth )
       sX = scrollWindowWidth
         * ((int)(file_view_data->x / scrollWindowWidth ));

     if( file_view_data->x > scrollWindowHeight )
       sY = scrollWindowHeight
         * ((int)(file_view_data->y / scrollWindowHeight ));
     else
       sY = file_view_data->y - file_mgr_data->grid_height - YSPACING(file_mgr_data);
     ...
     if( n > 0 )
     {
       XtSetValues ((Widget)file_window, args, n);
     }
   }
}
...
void
#ifdef _NO_PROTO
FmPopup (w, client_data, event, file_mgr_data)
      Widget w;
      XtPointer client_data;
      XEvent *event;
      FileMgrData *file_mgr_data;
...
/* SCW: This is a bit tricky, but the alternative is to ifdef 
 * the whole function and duplicate it.
 */
#define fileMgrPopup  gFileMgrPopup[scr]
   ...
   file_mgr_rec = (FileMgrRec *)file_mgr_data->file_mgr_rec;
#ifdef _TT_MULTI_SCREEN
   scr = XScreenNumberOfScreen(XtScreen(file_mgr_rec->shell));
#endif
...
*/
      )
   {
      ...
      if(file_mgr_data == trashFileMgrData)
      {
         ...
         label_string = XmStringCreateLtoR((GETMESSAGE(33,2, "Current Folder")),
                                            XmFONTLIST_DEFAULT_TAG);
      ...
      else
      {
         ...
         if(showFilesystem )
         {
           if (file_mgr_data->show_hid_enabled)
             XmToggleButtonGadgetSetState(fileMgrPopup.wsPopup[BTN_SHOWHIDDEN],
                                          True, False);
           else
             XmToggleButtonGadgetSetState(fileMgrPopup.wsPopup[BTN_SHOWHIDDEN],
                                          False, False);
	    ...
	    fileName = _DtGetSelectedFilePath( fileViewData );
   ...
   else
   {
      ...
      if(file_mgr_data == trashFileMgrData)
      {
             ...
             && InMultipleObjectRegion(file_mgr_data, fileViewData))
         {
           ...
           label_string = XmStringCreateLtoR ((GETMESSAGE(33,1,
                                              "Multiple Objects")),
                                              XmFONTLIST_DEFAULT_TAG);
         }
         else
         {
           ...
           label_string = XmStringCreateLtoR (label,
                                              XmFONTLIST_DEFAULT_TAG);
         }
         XtSetArg (args[0], XmNlabelString, label_string);
         XtSetValues (fileMgrPopup.title, args, 1);
         ...
         else
         {
           char *tmp_label, *fileName;
            ...
            XtAddCallback(fileMgrPopup.objPopup[BTN_PROPERTIES],
                          XmNactivateCallback, ShowModAttrDialog,
                          (XtPointer) fileViewData);
           ...
           if ( file_mgr_data->toolbox && geteuid() != root_user &&
            ...
            XtAddCallback(fileMgrPopup.objPopup[BTN_HELP],
                          XmNactivateCallback, ObjectHelp,
                          (XtPointer) fileViewData->widget);
...
static void
#ifdef _NO_PROTO
DropOnRootCB (w, client_data, call_data)
     Widget w;
     XtPointer client_data;
     XtPointer call_data;
#else
...
#endif /* _NO_PROTO */
{
   ...
   int step, dstep;
#ifdef _TT_MULTI_SCREEN
   /* Have to use the lDragStartWidget to figure out what screen.  
    * The dragContext widget (w) is always on screen zero...
    */
   int screen_num = XScreenNumberOfScreen(XtScreen(lDragStartWidget));
#endif
   ...
   if(fileList->reason != DtCR_DND_DROP_ANIMATE)
   {
	...
	start_y = effScreenWidth - step;
      ...
      {
#ifdef _TT_MULTI_SCREEN
        int EndIndex = gDesktopData[screen_num]->numIconsUsed;
#else
...
void
#ifdef _NO_PROTO
AddFileIcons( file_mgr_rec, file_mgr_data, add_dir_set )
        FileMgrRec *file_mgr_rec ;
        FileMgrData *file_mgr_data ;
        DirectorySet * add_dir_set ;
#else
...
static void
#ifdef _NO_PROTO
MakeReuseList(children, num_children, order_list, order_count,
              reuse_icons, reuse_btns)
        Widget *children;
        int num_children;
        FileViewData **order_list;
        int order_count;
...
#endif /* _NO_PROTO */
{
   ...
   icon_label = XmStringLtoRCreate(file_view_data->label,
                                                       XmFONTLIST_DEFAULT_TAG);
...
static void
#ifdef _NO_PROTO
_UpdateFileIcons( file_mgr_rec, file_mgr_data, new_directory, add_dir_set )
        FileMgrRec *file_mgr_rec ;
        FileMgrData *file_mgr_data ;
        Boolean new_directory;
        DirectorySet * add_dir_set ;
...
#endif /* _NO_PROTO */
{
   ...
   DPRINTF(("_UpdateFileIcons(\"%s\", new_dir %c, add_dir_set %x) ...\n",
...
#ifdef _NO_PROTO
Pixmap GetTreebtnPixmap(file_mgr_data, file_view_data)
        FileMgrData *file_mgr_data;
        FileViewData *file_view_data;
#else
...
static void
#ifdef _NO_PROTO
GetIconLayoutParms(file_mgr_rec, file_mgr_data, ld)
        FileMgrRec *file_mgr_rec;
        FileMgrData *file_mgr_data ;
        IconLayoutData *ld;
#else
...
static Dimension
#ifdef _NO_PROTO
GetExtraHeight(file_mgr_data, file_view_data, treebtn_size)
        FileMgrData *file_mgr_data ;
        FileViewData *file_view_data;
        int treebtn_size;
#else
...
void
#ifdef _NO_PROTO
EraseTreeLines(file_mgr_rec, file_mgr_data, file_view_data)
        FileMgrRec *file_mgr_rec ;
        FileMgrData *file_mgr_data ;
        FileViewData *file_view_data ;
#else
...
void
#ifdef _NO_PROTO
RedrawTreeLines(w, ex, ey, ewidth, eheight, ecount, file_mgr_rec, file_mgr_data)
        Widget w ;
        int ex, ey, ewidth, eheight, ecount ;
        FileMgrRec *file_mgr_rec ;
        FileMgrData *file_mgr_data ;
#else
RedrawTreeLines(
...
static Boolean
#ifdef _NO_PROTO
ToBeManaged(layout_data, file_view_data)
        IconLayoutData *layout_data;
        FileViewData *file_view_data;
#else
ToBeManaged(
...
static int
#ifdef _NO_PROTO
DisplaySomeIcons(file_mgr_rec, file_mgr_data, ex, ey, ewd, eht, workLimit, doAll)
        FileMgrRec *file_mgr_rec;
        FileMgrData *file_mgr_data;
        int ex, ey, ewd, eht;
        int workLimit;
...
*/
       ...
       file_mgr_data != trashFileMgrData)
   {
      ...
      {
         SetHotRects(change[k], file_mgr_data->root_type,
...
static void
#ifdef _NO_PROTO
CommitWorkProcUpdates(file_mgr_rec, file_mgr_data, reset)
  FileMgrRec *file_mgr_rec;
  FileMgrData * file_mgr_data;
  Boolean reset;
#else
...
void
#ifdef _NO_PROTO
LayoutFileIcons( file_mgr_rec, file_mgr_data, update_scrolling_position,
                 turn_off_hourglass )
        FileMgrRec *file_mgr_rec ;
        FileMgrData *file_mgr_data ;
...
#endif /* _NO_PROTO */
{
   ...
   /* E.g. force XmDROP_COPY semantics for views restricted to floppies */
   operations &= TypeToDropOperations(file_mgr_data->root_type);
...
layout_done:
   ...
   if( file_mgr_data->scrollToThisFile )
   {
         ...
         && file_view_data->filtered == False )
     {
       ...
       if( (Dimension)file_view_data->x > scrolled_window_width )
         sX = scrolled_window_width
           * ((int)((Dimension)file_view_data->x / scrolled_window_width ));

       if( (Dimension)file_view_data->x > scrolled_window_height )
         sY = scrolled_window_height
           * ((int)((Dimension)file_view_data->y / scrolled_window_height ));
       else
         sY = file_view_data->y - grid_height - YSPACING(file_mgr_data);
...
static void
#ifdef _NO_PROTO
TreeBtnCallback( w, clientData, callData )
        Widget w ;
        XtPointer clientData ;
        XmAnyCallbackStruct *callData ;
#else
...
static void
#ifdef _NO_PROTO
CompressObjectList (object_positions, num_objects, starting_index)
   ObjectPosition ** object_positions;
   int num_objects;
   int starting_index;
#else
...
static Boolean
#ifdef _NO_PROTO
IntersectRects( x1, y1, w1, h1, x2, y2, w2, h2 )
        Position x1 ;
        Position y1 ;
        Dimension w1 ;
        Dimension h1 ;
...
#ifdef _NO_PROTO
GetPositionalData (file_mgr_data, object, max_width, create)
   FileMgrData * file_mgr_data;
   FileViewData * object;
   Dimension max_width;
   Boolean create;
...
#endif /* _NO_PROTO */
{
   ...
   else
   {
      ...
      for (i = 0; i < file_mgr_data->num_objects; i++)
      {
         ...
         {
            if (top == 0) top = GetTopOfStack(file_mgr_data);
	    if (top != 0) { /* XXX race condition? */
		top->next = (ObjectPtr)entry;
...
void
#ifdef _NO_PROTO
RepositionIcons (file_mgr_data, file_list, file_count, drop_x, drop_y, late_binding_needed)
   FileMgrData * file_mgr_data;
   char ** file_list;
   int file_count;
   Position drop_x;
...

{
   ...
   for (i = file_count - 1; i >= 0; i--)
   {
      ...
      for (j = 0; j < file_mgr_data->num_objects; j++)
      {
         ...
         {
            /* Find the associated widget. XXX Sometimes not found. Race? */
            for (k = 0; k < file_mgr_data->directory_set[0]->file_count; k++)
	    {
		 if (strcmp(name, file_mgr_data->directory_set[0]
			         ->file_view_data[k]->file_data->file_name))
		 {
		     newEntry = False;
		     break;
		 }
   ...
   while (top)
   {
          ...
          !file_view_data->need_update)
      {
         ...
         if ((Position)(icon->core.x + icon->core.width) >= (Position)ex  &&
             ...
             icon->core.y < ey + eht)
         {
            SetHotRects(file_view_data, file_mgr_data->root_type,
                        (XtCallbackProc) DropOnObject,
...
Boolean
#ifdef _NO_PROTO
VerticalScrollbarIsVisible( vertSB, scrolledWin )
        Widget vertSB ;
        Widget scrolledWin ;
#else
VerticalScrollbarIsVisible(
...
Boolean
#ifdef _NO_PROTO
HorizontalScrollbarIsVisible( hortSB, scrolledWin )
        Widget hortSB ;
        Widget scrolledWin ;
#else
HorizontalScrollbarIsVisible(
