/* $XConsortium: IconWindow.c /main/cde1_maint/2 1995/09/29 18:35:11 lehors $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           IconWindow.c
...
#include <Xm/PrimitiveP.h>  /* For _XmHighlightBorder & _XmUnhighlightBorder */
...
#include "Filter.h"
                           ...
                           /* For _XmHightlightBorder & _XmUnhighlightBorder */
...
static void FM_XmHighlightBorder();
static void FM_XmUnhighlightBorder();
...
void
#ifdef _NO_PROTO
FileWindowExposeCallback( w, client_data, call_data )
        Widget w ;
...
void
#ifdef _NO_PROTO
FileWindowResizeCallback( w, client_data, event )
        Widget w ;
        XtPointer client_data ;
        XEvent *event ;
#else
...
void
#ifdef _NO_PROTO
FileWindowMapCallback( w, client_data, event )
        Widget w ;
        XtPointer client_data ;
        XEvent *event ;
#else
...
void
#ifdef _NO_PROTO
FileWindowInputCallback( w, client_data, call_data )
        Widget w;
        XtPointer client_data;
        XtPointer call_data;
#else
...
#endif /* _NO_PROTO */
{
   ...
   static KeySym SelectKeysym, ActivateKeysym, SpaceKeysym;
   ...
   else if (new_event->type == KeyPress || new_event->type == KeyRelease)
   {
      if(first)
      {
         ...
         _XmVirtualToActualKeysym( XtDisplay(w), osfXK_Select,
                                              &SelectKeysym, &mod );
        ...
        (keysym == PlusKeysym || keysym == MinusKeySym))
   {
      ...
      if (keybdFocusPolicy == XmEXPLICIT)
         eventIcon = (Widget)_XmGetActiveItem(w);
      else
         eventIcon = (Widget)_XmInputForGadget(w, kevent->x, kevent->y);
      if (eventIcon == NULL)
         return;
                                             ...
                                             keybdFocusPolicy == XmEXPLICIT))
   {
      ...
      if (new_event->type == KeyPress)
      {
         ...
         else if ( keysym != SelectKeysym
                   && keysym != ActivateKeysym
                   && keysym != SpaceKeysym
                   && keysym != osfXK_Delete
         ...
         if (desktopRec)
            ...
            eventIcon = (Widget)_XmGetActiveItem(file_mgr_rec->file_window);
      ...
      else 
      {
         ...
         eventIcon = (Widget)_XmInputForGadget(w, event->x, event->y);
      }
            ...
            switch( keysym )
            {
              ...
              case osfXK_Menu:
                if(file_mgr_data)  /* if not, then it is a desktop object */
                  FmPopup( NULL, fileViewData, (XEvent *)event, file_mgr_data );
                break;
              case osfXK_EndLine:
                if( !desktopRec
                    && file_mgr_data->show_type != MULTIPLE_DIRECTORY )
                                             ...
                                             keybdFocusPolicy == XmEXPLICIT))
   {
      ...
      if (((initialDragX >= 0) && (initialDragY >= 0)) &&
         (eventIcon = (Widget)_XmInputForGadget(w, initialDragX, 
                                                        initialDragY)))
      {
         /*
...
static Boolean
#ifdef _NO_PROTO
CheckForBtnUp( display, event, arg )
        Display *display ;
        XEvent *event ;
        XtPointer arg ;
#else
...
#ifdef _NO_PROTO
#ifdef _TT_MULTI_SCREEN
GetSelectionRectangle( display, widget, event, file_mgr_rec, 
                       file_mgr_data, toggleSelect )
        Display *display ;
        Widget widget;
#else
GetSelectionRectangle( display, window, event, file_mgr_rec, 
                       file_mgr_data, toggleSelect )
        Display *display ;
        Window window ;
#endif
...
#endif /* _NO_PROTO */
{
   ...
   int	  screenNum = XScreenNumberOfScreen(XtScreen(widget));
   static int	lastGCScreenNum = -1; 
#endif
   Window root, sub_win;
   int  win_x, win_y;
   unsigned int mask;
...
#ifdef _TT_MULTI_SCREEN
   /* Recreate the gc if a different screen is encountered */
   if (!draw_gc || lastGCScreenNum != screenNum)
#else
   if (!draw_gc)
#endif
   {
...
#ifdef _TT_MULTI_SCREEN
     if (draw_gc) XFreeGC(display, draw_gc);

     screen = XScreenOfDisplay(display, screenNum);
     lastGCScreenNum = screenNum;
#else
...
static void
#ifdef _NO_PROTO
DrawRectangle( display, window, gc, x1, y1, x2, y2 )
        Display *display ;
        Window window ;
        GC gc ;
        int x1 ;
...
void 
#ifdef _NO_PROTO
WidgetRectToRegion (file_mgr_data, w, region)
   FileMgrData * file_mgr_data;
   Widget w;
   Region region;
#else
...
static void 
#ifdef _NO_PROTO
ProcessSelection (file_mgr_data, icon, toggleSelect, event)
   FileMgrData * file_mgr_data;
   Widget icon;
   Boolean toggleSelect;
   XButtonEvent * event;
               ...
               if (!XEmptyRegion (intersect_region))
               {
                  ...
                  else
                  {
                     ...
                     if(file_mgr_data->view == BY_ATTRIBUTES)
                     {
                        ...
                        textExtent = (XRectangle *)_DtIconGetTextExtent(
                        ...
                        fileNameString = XmStringCreateLtoR( file_view_data->file_data->file_name,
...
void
#ifdef _NO_PROTO
DrawHighlight ( w, file_view_data, file_mgr_data, type )
      Widget w;
      FileViewData *file_view_data;
      FileMgrData *file_mgr_data;
      int type;
...
*/
   if (!s_etched)
   {
      _XmDrawShadow (XtDisplay(w), XtWindow(w), top_gc, bottom_gc,
