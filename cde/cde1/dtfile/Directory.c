/* $XConsortium: Directory.c /main/cde1_maint/4 1995/11/22 13:42:54 gtsang $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           Directory.c
...
void
#ifdef _NO_PROTO
InitializeDirectoryRead( widget )
        Widget widget;
#else
InitializeDirectoryRead(
        Widget widget )
...

{
   ...
   for ( scr = 0; scr < gNScreens; scr++ )
      if(gDesktopData[scr]->numIconsUsed > 0)
      {
         do_check = True;
         break;
      }
...
static Directory *
#ifdef _NO_PROTO
FindDirectory(host_name, directory_name)
        char *host_name;
        char *directory_name;
#else
FindDirectory(
...
void
#ifdef _NO_PROTO
FreeFileData(file_data, free_all)
        FileData *file_data;
        Boolean free_all;
#else
FreeFileData(
...
void
#ifdef _NO_PROTO
PipeWriteFileData(fd, file_data)
	int fd;
	FileData *file_data;
#else /* _NO_PROTO */
PipeWriteFileData(
...
#endif /* _NO_PROTO */
{
   ...
   {
      fprintf(stderr, "PipeReadFileData: n = %d, expected %d\n",
              n, sizeof(FileData));
   }
...
FileData *
#ifdef _NO_PROTO
FileData2toFileData(file_data2, l)
...
#endif /* _NO_PROTO */
{
   ...
   file_data->errno		= file_data2->errno;
...
#ifdef  SUN_ACLFS
   file_data->has_acl		= file_data2->has_acl;
#endif  /* SUN_ACLFS */
...
static void
#ifdef _NO_PROTO
PipeReadPositionInfo(fd, position_info)
	int fd;
	PositionInfo *position_info;
#else /* _NO_PROTO */
PipeReadPositionInfo(
...
FileData *
#ifdef _NO_PROTO
ReadFileData(full_directory_name, file_name,IsToolBox)
...
#endif /* _NO_PROTO */
{
...
#ifdef  SUN_ACLFS
   if (acl_fptr && ((acl_count =
      (*acl_fptr)( link_file_name, GETACLCNT, 0, NULL)) > 4) )
   {
      file_data->has_acl = True;
   }
#endif  /* SUN_ACLFS */
   ...
   if (stat_result == 0)
   {
	 ...
	 file_data->logical_type = (char *) DtDtsDataToDataType(
					      ...
					      NULL, NULL,
					      NULL);
         /* The problem here is there isn't a way for us to mask
         ...
         */
         ...
         {
	 /* We want to type all folders as non-writeable or locked
	    based on the user's access. However, if we change the type
	    of the CURRENT_FOLDER or the DOT_FOLDERs then they will
	    not be filtered out of the View. Need to add more logic
	    to ensure they are typed non-writeable or locked AND
	    filtered out as well. The same applies for all cases
	    that follow later on in this file.
	 */
	   if( strcmp( file_data->logical_type, LT_DIRECTORY ) == 0 )
           {
             char * fullPathName;
             ...
             if( access( fullPathName, R_OK ) != 0 ||
                 (access( fullPathName, W_OK ) != 0 &&
                 access( fullPathName, X_OK ) != 0 ))
             {
...
#ifdef _NO_PROTO
ReadFileData2(file_data2, full_directory_name, file_name,IsToolBox)
	FileData2 *file_data2;
	char *full_directory_name;
	char *file_name;
        Boolean IsToolBox;
...
static int
#ifdef _NO_PROTO
ReadDirectoryProcess(pipe_fd, directory, activity)
        int pipe_fd;
        Directory *directory;
	ActivityStatus activity;
#else /* _NO_PROTO */
...
#endif /* _NO_PROTO */
{
   ...
   for (;;)
   {
      ...
      path_logical_types[path_count] =
         (char *) DtDtsDataToDataType(full_directory_name, NULL, 0, NULL, NULL,
                                      NULL, NULL);
      {
        if(!IsToolBox && stat( full_directory_name, &stat_buf ) == 0 )
        {
	    ...
	    )
	  {
            if( access( full_directory_name, R_OK ) != 0 ||
              ( access( full_directory_name, W_OK ) != 0 &&
                access( full_directory_name, X_OK ) != 0 ) )
            {
   ...
   if (stat(full_directory_name, &stat_buf) != 0 ||
       CheckAccess(full_directory_name, R_OK | X_OK) != 0)
   {
...
static int
#ifdef _NO_PROTO
UpdateAllProcess(pipe_fd, directory, activity)
int pipe_fd;
Directory *directory;
ActivityStatus activity;
#else /* _NO_PROTO */
...
static int
#ifdef _NO_PROTO
UpdateSomeProcess(pipe_fd, directory, activity)
        int pipe_fd;
        Directory *directory;
	ActivityStatus activity;
#else /* _NO_PROTO */
...
static void
#ifdef _NO_PROTO
ReaddirPipeCallback(client_data, fd, id)
   XtPointer client_data;
   int *fd;
   XtInputId *id;
#else
...
#endif /* _NO_PROTO */
{
   ...
   switch (msg)
   {
      case PIPEMSG_PATH_LOGICAL_TYPES:
         ...
         else if (msg == PIPEMSG_FILEDATA2)
         {
           ...
           if (n < sizeof(FileData2)) {
             perror("PipeRead");
             fprintf(stderr, "PipeReadFileData2: n = %d, expected %d\n",
                     n, sizeof(FileData2));
           }
           ...
           for (i = 0; i < directory->numOfViews; i++)
           {
                 ...
                 file_mgr_data->busy_detail > 2)
             {
               if (file_mgr_data->show_status_line)
               {
                 ...
                 label_string =
                   XmStringCreateLtoR(buf, XmFONTLIST_DEFAULT_TAG);
      ...
      default:
	 if (n > 0) {
	     fprintf(stderr, "dtfile[%d]: ReaddirPipeCallback(): msg==%d\n",
		     getpid(), msg);
	 }
         directory->errno = -1;
         directory->errmsg_needed = False;
...
#endif
      DPRINTF(("ReaddirPipeCallback: done, errno %d, time %d\n",
               directory->errno, modify_time));
...
static void
#ifdef _NO_PROTO
_DtDeleteShellWindow( widget )
	Widget widget;
#else
_DtDeleteShellWindow(
	Widget widget)
#endif /* _NO_PROTO */
{
	...
	ev.data.l[1] = 0; /* XXX XtLastTimestampProcessed? */
...
static Boolean
#ifdef _NO_PROTO
ReadDirectory( w, host_name, directory_name, file_data, file_count,
               file_mgr_data)
        Widget w;
        char *host_name;
        char *directory_name;
...
#endif /* _NO_PROTO */
{
       ...
       (strcmp(directory_name, directory->directory_name) == 0))
   {
      ...
      if (directory->errmsg_needed &&
          ...
          w != NULL)
      {
	 if (1) { /* XXX filesystem got unmounted */
	    int j;
	    for (j = 0; j < directory->numOfViews; j++) {
	       FileMgrRec *rec = (FileMgrRec *)
	          directory->directoryView[j].file_mgr_data->file_mgr_rec;
	       _DtDeleteShellWindow( rec->shell );
	    }
	 } else {
...
static int
#ifdef _NO_PROTO
_ReadDir( w, file_mgr_data, host_name, directory_name, dp, level,
...
int
#ifdef _NO_PROTO
ReadDir( w, file_mgr_data, host_name, directory_name, dp, level,
         read_level, branch_list )
  Widget w;
  FileMgrData *file_mgr_data;
  char *host_name;
...
void
#ifdef _NO_PROTO
RereadDirectory( w, host_name, directory_name )
        char *w;
        char *host_name;
        char *directory_name;
#else
...
#endif /* _NO_PROTO */
{
   ...
   if (directory != NULL)
   {
      /* reset errno to make sure we'll get an error message */
      directory->errno = 0;
...
void
#ifdef _NO_PROTO
UpdateDirectory( w, host_name, directory_name )
        char *w;
        char *host_name;
        char *directory_name;
#else
...
void
#ifdef _NO_PROTO
DirectoryModifyTime(host_name, directory_name, modify_time)
        char *host_name;
        char *directory_name;
        long modify_time;
#else
...
void
#ifdef _NO_PROTO
DirectoryFileModified(host_name, directory_name, file_name)
        char *host_name;
        char *directory_name;
        char *file_name;
#else
...
void
#ifdef _NO_PROTO
DirectoryEndModify(host_name, directory_name)
        char *host_name;
        char *directory_name;
#else
DirectoryEndModify(
...
#endif /* _NO_PROTO */
{
   ...
   else
   {
      ...
      strcpy(error_msg, "(");
      strncpy(error_msg + 1, strerror(file_data->errno), msg_len - 2);
...
#ifdef  SUN_ACLFS
   if (file_data->has_acl) acl_disp = OPTION_ACL;
   else acl_disp = OPTION_BLANK;
#endif  /* SUN_ACLFS */
...
#ifdef  SUN_ACLFS
       sprintf( long_name, "%-28.28s  %s  %9lld  %c%c%c%c%c%c%c%c%c%c%c  %-9s  %-9s  %s",
#else
       sprintf( long_name, "%-28.28s  %s  %9lld  %c%c%c%c%c%c%c%c%c%c  %-9s  %-9s  %s",
#endif  /* SUN_ACLFS */
                            name,
                            time_string,
                            (long long)file_data->stat.st_size,
                            permission,
...
/*--------------------------------------------------------------------
 *  DirectoryLogicalType
 *     Returns 0 or address of private string naming the dir2type's logical CDE type. 
 *------------------------------------------------------------------*/

char *
#ifdef _NO_PROTO
DirectoryLogicalType(host, dir2lookup, dir2type)
	char *host;
	char *dir2lookup;
	char *dir2type;
#else /* _NO_PROTO */
...
#endif /* _NO_PROTO */
{
   ...
   /*  Find the directory set entry.  */
   directory = FindDirectory(host, dir2lookup);
   if (	  (directory != NULL)
       && (strcmp(dir2lookup, directory->directory_name) == 0))
   {
      /* if we don't have path_logical_types yet, we don't know */
      if (directory->path_logical_types == NULL)
         return NULL;

      /* count the number of components in dir2type */
      if (strcmp(dir2type, "/") == 0)
         n = 0;
      else
      {
         n = 1;
      ...
      DPRINTF2(("GetDirectoryLogicalType(%s): n %d, type %s\n",
                dir2type, n, directory->path_logical_types[n]));

      /* return type from path_logical_types array */
...
#endif /* _NO_PROTO */
{
   ...
   return DirectoryLogicalType(file_mgr_data->host,
			       file_mgr_data->current_directory, path);
}

/*--------------------------------------------------------------------
 *  RootTypeOfView
 *     Returns free()able name of the view's root folder.
 *------------------------------------------------------------------*/

char *
#ifdef _NO_PROTO
RootTypeOfView(file_mgr_data)
	FileMgrData *file_mgr_data;
#else /* _NO_PROTO */
RootTypeOfView(
	FileMgrData *file_mgr_data)
#endif /* _NO_PROTO */
{
	char *root = file_mgr_data->restricted_directory;
	if (root == 0) root = "/";
	return DtDtsDataToDataType( root, 0, 0, 0, 0, 0, 0);
}

/*--------------------------------------------------------------------
 *  RootTypeOfDesktopObject
 *     Returns free()able name of the object's root folder.
 *------------------------------------------------------------------*/

char *
#ifdef _NO_PROTO
RootTypeOfDesktopObject(object)
	DesktopRec *object;
#else /* _NO_PROTO */
RootTypeOfDesktopObject(
	DesktopRec *object)
#endif /* _NO_PROTO */
{
	char *root = object->restricted_directory;
	if (root == 0) root = "/";
	return DtDtsDataToDataType( root, 0, 0, 0, 0, 0, 0);
}
...
int
#ifdef _NO_PROTO
GetDirectoryPositionInfo(host_name, directory_name, position_info)
        char *host_name;
        char *directory_name;
        PositionInfo **position_info;
#else /* _NO_PROTO */
...
static int
#ifdef _NO_PROTO
WritePosInfoProcess(pipe_fd, directory, activity)
        int pipe_fd;
	Directory *directory;
	ActivityStatus activity;
#else /* _NO_PROTO */
...
static void
#ifdef _NO_PROTO
WritePosInfoPipeCallback(client_data, fd, id)
   XtPointer client_data;
   int *fd;
   XtInputId *id;
#else
...
int
#ifdef _NO_PROTO
SetDirectoryPositionInfo(host_name, directory_name,
                         position_count, position_info)
        char *host_name;
        char *directory_name;
        int position_count;
        PositionInfo *position_info;
...
static int
#ifdef _NO_PROTO
TimerEventProcess(pipe_fd, directory, activity)
        int pipe_fd;
	Directory *directory;
	ActivityStatus activity;
#else /* _NO_PROTO */
...
#endif /* _NO_PROTO */
{
    ...
    */
   if (stat(directory->path_name, &stat_buf) != 0 ||
       CheckAccess(directory->path_name, R_OK | X_OK) != 0)
   {
...
static void
#ifdef _NO_PROTO
StickyProcIdle(activity, sticky_proc, max_procs)
   ActivityStatus activity;
   StickyProcDesc *sticky_proc;
   int max_procs;
#else
...
#endif /* _NO_PROTO */
{
   ...
   for (p = *lp; p; p = *lp)
   {
      ...
      {
         DPRINTF2(("StickyProcIdle: end sticky proc %d\n", p->child));
...
static void
#ifdef _NO_PROTO
TimerPipeCallback(client_data, fd, id)
   XtPointer client_data;
   int *fd;
   XtInputId *id;
#else
...
#endif /* _NO_PROTO */
{
   ...
   DPRINTF2((
     "TimerPipeCallback: rc %d (was %d), time %d (was %d), link change %d\n",
     rc, directory->errno, modify_time, directory->modify_time, link_changed));
   ...
   if (rc == 0)
   {
      ...
      else if (modify_time != directory->modify_time || directory->errno != 0)
      {
...
static int
#ifdef _NO_PROTO
CheckDesktopProcess(pipe_fd, directory, activity)
        int pipe_fd;
	Directory *directory;
	ActivityStatus activity;
#else /* _NO_PROTO */
...
#endif /* _NO_PROTO */
{
   ...
   for (i = 0; i < desktop_data->numIconsUsed; i++)
   {
      ...
      {
         Boolean IsToolBox = desktopWindow->toolbox;
...
static void
#ifdef _NO_PROTO
CheckDesktopPipeCallback(client_data, fd, id)
   XtPointer client_data;
   int *fd;
   XtInputId *id;
#else
...
#endif /* _NO_PROTO */
{
       ...
       msg == PIPEMSG_DESKTOP_CHANGED)
   {
      ...
      found = False;
#ifdef _TT_MULTI_SCREEN
      for (scr = 0; !found && (scr < gNScreens) ; scr++)
      {
   ...
   else if (msg == PIPEMSG_DONE || (n == -1 && numFails >= 10))
   {
      /* close the pipe and cancel the callback */
      close(*fd);
      XtFree( client_data );
   ...
   {
      fprintf(stderr, "dtfile: Internal error in CheckDesktopPipeCallback\n"); 
      XtFree( client_data );
      XtRemoveInput(*id);
      directory->busy[directory->activity] = False;
      directory->activity = activity_idle;
...
void
#ifdef _NO_PROTO
TimerEventBrokenLinks( client_data, id )
        XtPointer client_data;
        XtIntervalId *id;
#else
TimerEventBrokenLinks(
...
#endif /* _NO_PROTO */
{
   ...
   if (p)
   {
      ...
      DPRINTF2(("ScheduleActivity:  use sticky proc %d\n", p->child));
	...
	d = ActivityTable[activity].sticky_procs;
        while (d && p) {
          if (d == p)
          {
   ...
   if (!p)
   {
      ...
      if (pid == 0)
      {
         /* child process */
         pid = getpid();
         DPRINTF2(("ScheduleActivity:  pid %d, sticky %d\n", pid, sticky));
         ...
         for (;;)
         {
            ...
            DPRINTF2(("StickyActivity:  pid %d, activity %d, dir %s\n",
                      pid, activity, s));
         }
         ...
         DPRINTF2(("ScheduleActivity:  end activity %d (rc %d)\n",
                   activity, rc));
         exit(rc);
      }
