/* $XConsortium: Filter.c /main/cde1_maint/3 1995/10/09 14:49:30 pascale $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           Filter.c
...
#endif /* _NO_PROTO */
{
   ...
   label_string = XmStringCreateLtoR ((GETMESSAGE(13,27, "Shown")), XmFONTLIST_DEFAULT_TAG);
   ...
   label_string = XmStringCreateLtoR ((GETMESSAGE(13,28, "Hidden")), XmFONTLIST_DEFAULT_TAG);
   XtSetArg(args[2], XmNlabelString, label_string);
   ...
   label_string = XmStringCreateLtoR ((GETMESSAGE(13,31,
                          "Select Datatypes to be:")), XmFONTLIST_DEFAULT_TAG);
   n = 0;
   ...
   XtAddCallback(fileTypeScrollWin, XmNhelpCallback,
   ...
   label_string = XmStringCreateLtoR ((GETMESSAGE(13,21, "Select All")),
   ...
   label_string = XmStringCreateLtoR ((GETMESSAGE(13,22, "Deselect All")),
                                      XmFONTLIST_DEFAULT_TAG);
   n = 0;
   XtSetArg (args[n], XmNlabelString, label_string);                    n++;
   XtSetArg (args[n], XmNleftAttachment, XmATTACH_POSITION);            n++;
   ...
   label_string = XmStringCreateLtoR (((char *)GETMESSAGE(13,26,
                    "Also hide (Optional): ")), XmFONTLIST_DEFAULT_TAG);
   n = 0;
   XtSetArg (args[n], XmNlabelString, label_string);                    n++;
   XtSetArg (args[n], XmNleftAttachment, XmATTACH_FORM);                n++;
   XtSetArg (args[n], XmNleftOffset, 5);                                n++;
   ...
   XtAddCallback(fileNameFilter, XmNhelpCallback,
                 (XtCallbackProc)HelpRequestCB,
                 HELP_FILTER_DIALOG_STR);
   ...
   label_string = XmStringCreateLtoR (((char *)GETMESSAGE(13, 11, "Defaults")), XmFONTLIST_DEFAULT_TAG);
   n = 0;
   XtSetArg (args[n], XmNlabelString, label_string);                    n++;
   XtSetArg (args[n], XmNleftAttachment, XmATTACH_POSITION);            n++;
...
static void
#ifdef _NO_PROTO
InstallChange( filter_rec, callback, client_data )
        FilterRec *filter_rec ;
        XtCallbackProc callback ;
        XtPointer client_data ;
#else
...
static void
#ifdef _NO_PROTO
InstallClose( filter_rec, callback, client_data )
        FilterRec *filter_rec ;
        XtCallbackProc callback ;
        XtPointer client_data ;
#else
...
#endif /* _NO_PROTO */
{
   ...
   int scr = XScreenNumberOfScreen(XtScreen(filter_rec->shell));
#endif
...
#ifdef _TT_MULTI_SCREEN 
   if(XmToggleButtonGadgetGetState (gMenuWidgets[scr].showHiddenMenu))
#else
   ...
   for (i = 0, x = 5, count = 0; i < filter_data->count; i++)
   {
      ...
      if (ptr = (char *)DtDtsDataTypeToAttributeValue(user_data->filetype,
                                                      ...
                                                      NULL))
      {
         icon_label = XmStringLtoRCreate(ptr, XmFONTLIST_DEFAULT_TAG);
         DtDtsFreeAttributeValue(ptr);
      }
      else
      {
         icon_label = XmStringLtoRCreate(user_data->filetype,
                                         XmFONTLIST_DEFAULT_TAG);
      }
...
static void
#ifdef _NO_PROTO
DefaultCallback( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
#endif /* _NO_PROTO */
{
   ...
   dialog_data = _DtGetInstanceData (filter_rec);
...
#ifdef _TT_MULTI_SCREEN 
   XmToggleButtonGadgetSetState(gMenuWidgets[scr].showHiddenMenu, False, False);
#else
   ...
   filterTypeChanged (w, client_data, NULL);
...
static Boolean
#ifdef _NO_PROTO
GetFilterValues( filter_rec, filter_data)
        FilterRec *filter_rec ;
        FilterData *filter_data ;
#else
GetFilterValues(
...
void
#ifdef _NO_PROTO
ShowHiddenFiles (w, client_data, callback)
   Widget  w;
   XtPointer client_data;
   XtPointer callback;
#else
...
void
#ifdef _NO_PROTO
UpdateFilterAfterDBReread (dialog_data)
   DialogData * dialog_data;
#else
UpdateFilterAfterDBReread (
   DialogData * dialog_data )
...
static void
#ifdef _NO_PROTO
filterTypeChanged( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
#endif /* _NO_PROTO */
{
   ...
   {
      filter_data->match_flag = False;
      label_string = XmStringCreateLtoR (((char *)GETMESSAGE(13,29,
                                         "Also show (Optional): ")),
                                         XmFONTLIST_DEFAULT_TAG);
   }
...
static void
#ifdef _NO_PROTO
SelectAllFilter( w, event, params, num_params )
        Widget w ;
        XEvent *event ;
        String *params ;
        Cardinal *num_params ;
...
static void
#ifdef _NO_PROTO
DeselectAllFilter( w, event, params, num_params )
        Widget w ;
        XEvent *event ;
        String *params ;
        Cardinal *num_params ;
