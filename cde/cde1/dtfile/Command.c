/* $XConsortium: Command.c /main/cde1_maint/3 1995/10/09 14:46:42 pascale $ */
/************************************<+>*************************************
 ****************************************************************************
 *
 *   FILE:           Command.c
...
static int direntsAreActions = -1;
...
void
#ifdef _NO_PROTO
UpdateActionMenuPane( client_data, file_mgr_rec, file_type, type, number, widget, physical_type, file_name )
        XtPointer *client_data;
...
#endif /* _NO_PROTO */
{
   ...
   if(command_list != NULL)
      while (command_list[verb_count] != NULL &&
                                    strlen (command_list[verb_count]) != 0)
         verb_count++;

   if (direntsAreActions < 0) {
      direntsAreActions = getenv("_SDT_DIRENTS_ARE_ACTIONS") == 0 ? 0 : 1;
   }
   if (direntsAreActions && physical_type == DtDIRECTORY) {
      DIR *dir;
      struct dirent *entry;
      int entriesUsed = 0;
      dir = opendir( file_name );
      if (dir != 0) {
	 command_list = (char **)XtRealloc( (char *)command_list,
				   sizeof(char *) * (verb_count+2));
	 command_list[verb_count] = XtNewString( "--------------------" );
	 command_list[verb_count+1] = 0;
	 verb_count++;
	 while (((entry = readdir( dir )) != 0) && (entriesUsed < 25)) {
	    if (entry->d_name[0] == '.') continue;
	    command_list = (char **)XtRealloc( (char *)command_list,
				      sizeof(char *) * (verb_count+2));
	    command_list[verb_count] = XtNewString( entry->d_name );
	    command_list[verb_count+1] = 0;
	    verb_count++;
	    entriesUsed++;
	 }
   ...
   if (type == DESKTOP && physical_type == DtDIRECTORY) {
      /* Below, we force OpenNewView in at the top */
      ++verb_count;
   }
   action_count = verb_count;
   ...
   if(type == NOT_DESKTOP)
   {
      ...
      menu_offset = number + selectedMenuMinSize;
   }
   else
   {
...
*/
   ...
   if (verb_count + menu_offset > num_children)
   {
      if(type == FM_POPUP)
         for (i = number; i < num_children; i++)
            XtManageChild (action_pane->composite.children[i]);
      for (i = num_children; i < verb_count + menu_offset; i++)
      {
   ...
   num_children =  menu_offset + verb_count;
   del_count = 0;
   limit = action_count;
   if (direntsAreActions) limit = verb_count;
   for (i = 0; i < limit; i++) {
      char *oldCommand;
      char *userData;
      Arg argsTmp[2];
      ...
      {
	 if (i < action_count) {
            userData = XtNewString(command_list[i]);
	 } else {
	    userData = _DtPath( file_name, command_list[i] );
         }
	 if (DtActionExists(command_list[i])) {
	    action_label = DtActionLabel(command_list[i]);
	 } else {
            action_label = XtNewString( command_list[i] );
         }
         string = XmStringCreateLtoR(action_label, XmFONTLIST_DEFAULT_TAG);
         if(strcmp(command_list[i], openNewView) == 0) {
	    XtFree( userData );
            userData = openNewView;
	 }
         XtSetArg (args[1], XmNuserData, userData);
         count = i + menu_offset;
      }
      else
      {
         ...
         else
         {
	       ...
	       if (i - 1 < action_count) {
		  userData = XtNewString(command_list[i - 1]);
	       } else {
		  userData = _DtPath( file_name, command_list[i - 1] );
	       }
	       if (DtActionExists(command_list[i - 1])) {
		  action_label = DtActionLabel(command_list[i - 1]);
	       } else {
		  action_label = XtNewString( command_list[i - 1] );
	       }
               string = XmStringCreateLtoR(action_label, XmFONTLIST_DEFAULT_TAG);
...
static void
#ifdef _NO_PROTO
ActionCallback( w, client_data, call_data )
        Widget w ;
        XtPointer client_data ;
        XtPointer call_data ;
#else
...
void
#ifdef _NO_PROTO
RunCommand( command, file_mgr_data, file_view_data, position, drop_parameters, drop_window )
        char *command ;
        FileMgrData *file_mgr_data ;
        FileViewData *file_view_data ;
...
#endif /* _NO_PROTO */
{
   ...
   if (strcmp (command, openInPlace) == 0)
   {
                ...
                file_mgr_data->restricted_directory) == 0)
      {
         ...
         XtSetSensitive(gMenuWidgets[scr].upBarBtn, False);
#else
...
#ifdef _TT_MULTI_SCREEN
         /* Don't use the global */
         XtSetSensitive(gMenuWidgets[scr].upBarBtn, True);
#else
   ...
   {
      /*
       * We set the 'special_reuse' global mode each time we are asked
       * (via ToolTalk, the command line, or the GUI) to open a new view.
       * XXX It would be nice if these global mode variables were replaced with
       * parameters into the interfaces that observe them.
       */
      special_reuse = False;
...
void
#ifdef _NO_PROTO
ProcessMoveCopyLink (command, file_mgr_data, file_view_data, drop_parameters,
                     drop_window)
     char *command;
     FileMgrData *file_mgr_data;
     FileViewData *file_view_data;
...
void
#ifdef _NO_PROTO
ProcessBufferDropOnFolder (command, file_mgr_data, file_view_data,
                           drop_parameters, drop_window)
     char *command;
     FileMgrData *file_mgr_data;
     FileViewData *file_view_data;
     DtDndDropCallbackStruct *drop_parameters;
...
*/
  ...
  else
  {
	...
	directory = XtNewString( file_mgr_data->current_directory );
     }
     else
	directory = _DtGetSelectedFilePath( file_view_data );
...
Boolean
#ifdef _NO_PROTO
InvalidTrashDragDrop (drag_op, trash_context, w)
     int drag_op;
     int trash_context;
     Widget w;
#else
...
void
#ifdef _NO_PROTO
ProcessAction (action, file_view_data, drop_parameters,
               cur_host, cur_dir, restricted_dir, w)
     char *action;
     FileViewData *file_view_data;
     DtDndDropCallbackStruct *drop_parameters;
     char *cur_host;
...

{
   ...
   DirectorySet *directory_set;
   FileMgrData *file_mgr_data;
   Boolean freeActionString = False;
#ifdef _TT_MULTI_SCREEN 
   char* newDisplay; 
#endif
...
#ifdef _TT_MULTI_SCREEN 
   newDisplay = getScreenDisplayString(XtDisplay(w), 
   ...
   /*
     This check misses two cases:
     XXX if the action-menu-ed folder contains a non-executable file with the
     same name as an action, or
     XXX if the action-menu-ed folder contains an action file
     */
   if (! DtActionExists(action)) {
      char *typeName;
      char *folderName = action_args[0].u.file.name;
      /*
	The 'action' is really a dirent in the action menu of the folder,
	so that dirent will be the argument of the dirent's default action.
	*/
      action_args[0].u.file.name = _DtPath( folderName, action );
      XtFree( folderName );
      typeName = DtDtsFileToDataType( action_args[0].u.file.name );
      if (DtDtsDataTypeIsAction( typeName )) {
	 action = XtNewString( typeName );
	 freeActionString = True;
	 /* The dirent is itself an action, so invoke it with no arguments */
	 arg_count = 0;
      } else {
	 action = _DtRetrieveDefaultAction( (char *)typeName );
	 freeActionString = True;
      }
      DtDtsFreeDataType( typeName );
   }
   DtActionInvoke(w, action, action_args, arg_count,
		  NULL, NULL, pwd_dir, True, NULL, NULL);
   if (freeActionString) XtFree( action );
...
void
#ifdef _NO_PROTO
MoveCopyLinkHandler( ttMsg, opType )
Tt_message ttMsg;
int opType;
#else
MoveCopyLinkHandler(
