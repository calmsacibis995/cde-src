...
static char rcsid[] = "$XConsortium: dthello.c /main/cde1_maint/2 1995/08/29 19:56:23 gtsang $";
 ...
 *  File:        dthello.c
 *
 *  dthello.c 1.13 96/05/04
 *
 *  Description:
 *  -----------
 *  This file implements the startup transition between login and 
 *  desktop environment.
 * 
 * (c) Copyright 1993, 1994 Hewlett-Packard Company
 * (c) Copyright 1993, 1994 International Business Machines Corp.
...
Boolean             solaris_star = True; /* Drawing Solaris starburst */
int		    ncolors_star = 0;	/* number colors in starturst */
...
"\t[-fnt <font>] [-string <message>] [-timeout <seconds>] [-file <name>]\n"));
}
 ...
 *           [-fnt <fontname>] [-string <message>] [-file <filename>]
 *           [-timeout <seconds>]
 * 
 *************************************<->***********************************/
#ifdef _NO_PROTO
...
#endif  /* _NO_PROTO */ 
{
    ...
    {
	/*
	 * Force call to message catalog look-up front-end.
	 * (This may change the $LANG variable.)
	 */
	char * foo = ((char *)GETMESSAGE(2, 1, ""));
    }
    ...
    ((char *)GETMESSAGE(2, 3, "Starting the\nCommon Desktop Environment\n\n")));
     ...
     * (adjust for low-color systems).  If a specific 
     * background color has been requested, use it 
     * instead of displaying the Solaris starburst 
     * and its related logos and revision info.
     */
	...
	if ((fontset = XCreateFontSet(dpy, FIXED_FONT, &missing_clist,
	    ...
	    event.xexpose.count == 0) 
	{
	    ...
	    /* Clear window and draw Solaris starburst */

	    if (solaris_star == True) {
	        Starburst(dpy, welcome, &gc, fgArg, bgArg, &ncolors_star);
	        Logo( box_line_width*2, ncolors_star );
	        Version( fontset );
	    } else {
...
#ifdef _NO_PROTO
void
ReadInTextLines (fp, fontset, pMaxWidth)
    FILE *fp;
    XFontSet fontset;
...
#ifdef _NO_PROTO
void 
VhGetResources (dpy, name, class, res, num)
    Display *dpy;
    char *name; 
    char *class; 
    VhResourceEntry *res; 
...
#endif /* _NO_PROTO */
{
	    ...
	    {
		/* draw the string */
		XmbDrawImageString (dpy, welcome, fontset, gc, x, y, 
